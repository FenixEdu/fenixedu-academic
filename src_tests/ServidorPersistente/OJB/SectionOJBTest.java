...
public class SectionOJBTest extends TestCaseOJB {
	...
	public void testReadBySiteAndSectionAndName() {
		...
		try {
			...
			persistentSupport.confirmarTransaccao();
			assertNull(
				"testReadBySiteAndSectionAndName:fail read unexisting section",
				section);
		} catch (ExcepcaoPersistencia ex) {
	...
	public void testReadBySiteAndSection() {
		...
		assertEquals(
			((ISection) inferiorSections.get(0)).getName(),
			"Seccao1deTFCI");
		assertEquals(((ISection) inferiorSections.get(0)).getSite(), site);
		assertEquals(
			((ISection) inferiorSections.get(0)).getSuperiorSection(),
			superiorSection);
		...
		assertEquals(
			((ISection) inferiorSections.get(1)).getName(),
			"SubSeccao2dePO");
		assertEquals(((ISection) inferiorSections.get(1)).getSite(), site);
		assertEquals(
			((ISection) inferiorSections.get(1)).getSuperiorSection(),
			superiorSection);
		...
		try {
			...
			inferiorSections =
				persistentSection.readBySiteAndSection(site, null);
		...
		try {
			...
			assertNotNull(supSection2);
			inferiorSections =
				persistentSection.readBySiteAndSection(site, supSection2);
			persistentSupport.confirmarTransaccao();
			assertTrue(inferiorSections.isEmpty());
		} catch (ExcepcaoPersistencia ex) {
	...
	public void testDelete() {
		...
		try {
			...
			persistentSupport.iniciarTransaccao();
			List deletedInferiorSections =
				persistentSection.readBySiteAndSection(site, section);
	...
	public void testLockWrite() {
		...
		try {
			//read section
			persistentSupport.iniciarTransaccao();
			ISection writtenSection =
				persistentSection.readBySiteAndSectionAndName(
					site,
					null,
					"xptopjhadsfkhjksaghjksa");
			assertEquals(
				"testLockWrite: written section not equal to read section",
				sectionToWrite,
				writtenSection);
			persistentSupport.confirmarTransaccao();
		...
		try {
			...
			superiorSection =
				persistentSection.readBySiteAndSectionAndName(
					...
					"xptopjhadsfkhjksaghjksa");
			assertNotNull(
				"testLockWrite:failed reading section",
				superiorSection);
			sectionToWrite =
				new Section("blablablasection", site, superiorSection);
			// write section
			persistentSection.lockWrite(sectionToWrite);
			persistentSupport.confirmarTransaccao();
		} catch (ExcepcaoPersistencia ex) {
	...
	public void testDeleteAll() {
		...
		assertEquals(
			"testDeleteAll:delete all sections",
			0,
			allSections.size());
	}
