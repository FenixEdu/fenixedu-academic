...
public class SalaOJBTest extends TestCaseOJB {
	...
	public void testReadAvailabeRoomsForExam() {
		try {
			...
			List exams =
				persistentRoom.readByCriteria(
					new Exam(beginning, beginning, null, null));
			sp.confirmarTransaccao();


			System.out.println("Executing query...");
			sp.iniciarTransaccao();
			persistentRoom.readAvailableRooms((IExam) exams.get(0));
			sp.confirmarTransaccao();
		} catch (ExcepcaoPersistencia ex) {
			fail("testDeleteSala" + ex);
		}
	}
	...
	//		}
	//		assertEquals(
	//			"testReadByNome:read existing sala",
	//			room.getNome(),
	//			"Ga1");
	//
