...
public class AulaOJBTest extends TestCaseOJB {
	...
	public void testReadByDiaSemanaAndInicioAndFimAndSala() {
		...
		assertTrue(
			lesson.getDiaSemana().getDiaSemana().equals(
				new Integer(DiaSemana.SEGUNDA_FEIRA)));
		...
		try {
			...
			lesson =
				persistentLesson.readByDiaSemanaAndInicioAndFimAndSala(
					new DiaSemana(DiaSemana.DOMINGO),
					startTime,
					endTime,
					room,
					executionCourse.getExecutionPeriod());
			persistentSupport.confirmarTransaccao();
			assertNull(
				"testReadByDiaSemanaAndInicioAndFimAndSala:fail read unexisting aula",
				lesson);
		} catch (ExcepcaoPersistencia ex) {
	...
	public void testCreateExistingAula() {
		...
		try {
			...
			fail(
				"testReadByDiaSemanaAndInicioAndFimAndSala:fail read existing aula"
					+ ex);
		}
	...
	public void testreadByDisciplinaExecucao() {
		...
		try {
			...
			assertEquals(
				"testReadByDisciplinaExecucao: Existing",
				6,
				lessons.size());
			...
			assertTrue(
				"testReadByDisciplinaExecucao: Unexisting",
				lessons.isEmpty());
			persistentSupport.confirmarTransaccao();
		} catch (ExcepcaoPersistencia ex) {
	...
	public void testReadByRoomAndExecutionPeriod() {
		...
		try {
			...
			lessons =
				persistentLesson.readByRoomAndExecutionPeriod(
					room,
					executionPeriod);
	...
	public void testReadByExecutionCourseAndLessonType() {
		...
		try {
			...
			lessons =
				persistentLesson.readByExecutionCourseAndLessonType(
					executionCourse,
					new TipoAula(TipoAula.TEORICA));
			assertNotNull(lessons);
	...
	public void testReadLessonsInBroadPeriod() {
		...
		try {
			...
			persistentSupport.iniciarTransaccao();
			List result =
				persistentLesson.readLessonsInBroadPeriod(lesson, null, executionPeriod);
			persistentSupport.confirmarTransaccao();
			assertNotNull(result);
			...
			result = persistentLesson.readLessonsInBroadPeriod(lesson, null,executionPeriod);
			persistentSupport.confirmarTransaccao();
			assertNotNull(result);
			assertEquals(result.size(), 1);
	...
	public void testReadLessonsInBroadPeriodInAnyRoom() {
		try {
			...
			// execute query
			persistentSupport.iniciarTransaccao();
			lessonsInBroadPeriodInAnyRoom =
				persistentLesson.readLessonsInBroadPeriodInAnyRoom(
					lesson,
					executionPeriod);
			persistentSupport.confirmarTransaccao();

			assertNotNull(
				"testReadLessonsInBroadPeriodInAnyRoom: result was null",
				lessonsInBroadPeriodInAnyRoom);
			...
			persistentSupport.iniciarTransaccao();
			lessonsInBroadPeriodInAnyRoom =
				persistentLesson.readLessonsInBroadPeriodInAnyRoom(lesson,executionPeriod);
			persistentSupport.confirmarTransaccao();
			...
			fail(
				"testReadLessonsInBroadPeriodInAnyRoom: Unexpected Exception :"
					+ ex);
		}
