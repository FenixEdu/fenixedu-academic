...
public class ReadStudentLessonsTest extends TestCaseServicos {
  ...
  // read student lessons by unauthorized user
  public void testUnauthorizedReadStudentLessons() {
    Object argsReadStudentLessons[] = new Object[1];
    argsReadStudentLessons[0] = infoStudent;

    Object result = null; 
      try {
        result = _gestor.executar(_userView2, "ReadStudentLessons", argsReadStudentLessons);
        fail("testUnauthorizedReadStudentLessons");
      } catch (Exception ex) {
        assertNull("testUnauthorizedReadStudentLessons", result);
      }
  }

  // read student lessons by authorized user
  public void testAuthorizedReadStudentLessons() {
	Object argsReadStudentLessons[] = new Object[1];
	argsReadStudentLessons[0] = infoStudent;
    
	Object result = null; 
	  try {
		result = _gestor.executar(_userView, "ReadStudentLessons", argsReadStudentLessons);
		assertNotNull("testAuthorizedReadStudentLessons", result);
		assertEquals("testAuthorizedReadStudentLessons", ((List)result).size(), 2);
		List infoLessons = (List) result;
	  } catch (Exception ex) {
		fail("testAuthorizedReadStudentLessons" + ex);
	  }
  }
