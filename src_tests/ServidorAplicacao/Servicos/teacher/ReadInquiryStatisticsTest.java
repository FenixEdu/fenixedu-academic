	...
	public void testSuccessfull()
	{
		try
		{
			...
			InfoSiteInquiryStatistics infoSiteInquiryStatistics =
				(InfoSiteInquiryStatistics) siteView.getComponent();
			...
			while (it.hasNext())
			{
				...
				assertEquals(
					infoInquiryStatistics.getOptionStatistics().size() - 1,
					infoInquiryStatistics
						.getInfoStudentTestQuestion()
						.getQuestion()
						.getOptionNumber()
						.intValue());
				...
				while (statisticsIt.hasNext())
				{
					...
					criteria.addEqualTo(
						"testQuestionOrder",
						infoInquiryStatistics.getInfoStudentTestQuestion().getTestQuestionOrder());
							...
							.intValue())
					{
						...
						if (statistic != 0)
							percentage =
								df.format(statistic * java.lang.Math.pow(numOfStudentResponses, -1));
