...
select concat('update ass_FUNCIONARIO_HISTORICO set dataFim = "2004-03-11" where chaveCCLocalTrabalho = 73 and chaveFuncionario =', chaveFuncionario,';') as ""
...
select concat('update ass_FUNCIONARIO_HISTORICO set dataFim = "2004-03-11" where chaveCCLocalTrabalho = 63 and chaveFuncionario =', chaveFuncionario,';') as ""
...
having count(1) >= 2;
  ...
  update DEPARTMENT set code = '2901', name = 'DEPARTAMENTO DE ENGENHARIA E GESTAO' where code = '3001';
  

-- CENTRO DE CUSTO DA CORRESPONDÊNCIA
  
select concat('update ass_FUNCIONARIO_HISTORICO set dataFim = "2004-03-11" where chaveCCCorrespondencia = 73 and chaveFuncionario =', chaveFuncionario,';') as ""
from ass_funcionario_historico fh inner join ass_funcionario f on
f.codigoInterno = fh.chaveFuncionario
where fh.chaveCCLocalTrabalho in (73,163)
group by fh.chaveFuncionario
...
select concat('update ass_FUNCIONARIO_HISTORICO set dataFim = "2004-03-11" where chaveCCCorrespondencia = 63 and chaveFuncionario =', chaveFuncionario,';') as ""
from ass_funcionario_historico fh inner join ass_funcionario f on f.codigoInterno = fh.chaveFuncionario
where chaveCCLocalTrabalho in (63,157)
group by chaveFuncionario
having count(1) >= 2;