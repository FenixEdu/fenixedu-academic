...
<h2><bean:message bundle="GEP_RESOURCES" key="title.ects.tablesConsulting" /></h2>
...
<fr:form action="/searchEctsComparabilityTables.do">
    ...
    <fr:edit id="filter" name="filter">
        <fr:schema bundle="DOMAIN_RESOURCES"
            ...
            <fr:slot name="type" layout="menu-postback" required="true" key="label.ects.EctsTableType">
                <fr:destination name="/searchEctsComparabilityTables.do?method=filterPostback" />
            </fr:slot>
...
<logic:present name="filter" property="executionInterval">
    <logic:present name="filter" property="type">
        <logic:present name="filter" property="level">
			...
			<html:link
				action="<%="/searchEctsComparabilityTables.do?method=exportTemplate" + filterArgs%>">
				<bean:message bundle="GEP_RESOURCES"
					key="link.ects.tablesManagement.exportTemplate" />
			</html:link></p>
			...
			<fr:view name="status">
				<fr:schema bundle="GEP_RESOURCES"
					...
					<logic:equal name="filter" property="type" value="ENROLMENT">
						<logic:equal name="filter" property="level"
							value="COMPETENCE_COURSE">
							<fr:slot name="targetEntity.externalId" key="label.externalId" />
							<fr:slot name="targetEntity.departmentUnit.name"
								key="label.departmentUnit.name" />
							<fr:slot name="targetEntity.name"
								key="label.competenceCourse.name" />
							<fr:slot name="targetEntity.acronym" key="label.acronym" />
							<bean:define id="sortByArg"
								value="targetEntity.departmentUnit.name,targetEntity.name" />
						</logic:equal>
						<logic:equal name="filter" property="level" value="DEGREE">
							<fr:slot name="targetEntity.externalId" key="label.externalId" />
							<fr:slot name="targetEntity.degreeType.localizedName"
								key="label.degreeType" />
