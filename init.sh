...
rm -rf .tmp.init
wget --no-check-certificate https://fenix-ashes.ist.utl.pt/FenixSetup\?action=AttachFile\&do=get\&target=content-structure.sql
mv FenixSetup\?action=AttachFile\&do=get\&target=content-structure.sql content-structure.sql
mysql -uroot -f $1 < content-structure.sql > /dev/null
