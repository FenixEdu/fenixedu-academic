...
11. Run the following SQL instructions:

    ```
    -- Photo availability extracted from CMS domain
    alter table `PARTY` add `PHOTO_AVAILABLE` tinyint(1);
    update PARTY join SITE on PARTY.OID_HOMEPAGE = SITE.OID set PARTY.PHOTO_AVAILABLE = SITE.ACTIVATED;
...
12. Run the following script to initialize Administrative Offices from organization: (InitializeAdministrativeOffices.java)[https://gist.github.com/pedrosan7os/12286574202612a66017]

13. Run the following script to import the academic authorization rules: (ImportAcademicAuthorizationRules.java)[https://gist.github.com/pedrosan7os/d7a339cd56abe0e214ce]

14. Run the following script to import the teacher authorizations: (ImportTeacherAuthorization.java)[https://gist.github.com/pedrosan7os/ab498981f31c7199e9af]
