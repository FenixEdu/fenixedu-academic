...
This metadata is used in resource search by execution course site interfaces.
  ...
  2. Grab FileContent instances OID and EXTERNAL_STORAGE_IDENTIFICATION from fenix DB
    ...
    ```
    mysql fenix_production_db -A --skip-column-names -e "select OID, EXTERNAL_STORAGE_IDENTIFICATION from FILE where OID >> 32 = (SELECT DOMAIN_CLASS_ID FROM FF\$DOMAIN_CLASS_INFO where DOMAIN_CLASS_NAME like 'org.fenixedu.academic.domain.FileContent');" > filecontents.txt
   ```
...
3. Ensure that your installation contains an implementation of `org.fenixedu.academic.util.ConnectionManager`, otherwise SQL-based lookups will not work.
...
4. Next, we will rename several of the existing infrastructural classes to their Bennu counterparts. Run the following SQL:
    ...
    update FF$DOMAIN_CLASS_INFO set DOMAIN_CLASS_NAME = 'org.fenixedu.bennu.core.domain.Bennu' where DOMAIN_CLASS_NAME = 'org.fenixedu.academic.domain.RootDomainObject';
    update FF$DOMAIN_CLASS_INFO set DOMAIN_CLASS_NAME = 'org.fenixedu.bennu.core.domain.User' where DOMAIN_CLASS_NAME = 'org.fenixedu.academic.domain.User';
    update FF$DOMAIN_CLASS_INFO set DOMAIN_CLASS_NAME = 'org.fenixedu.bennu.user.management.UserLoginPeriod' where DOMAIN_CLASS_NAME = 'org.fenixedu.academic.domain.LoginPeriod';
...
10. Go to `https://<url>/bennu-scheduler-ui/index.html#custom`, and run the following script to import User's private keys. You should change the path to the `keys.json` file being read, so that the path points to the file created in step 2), and is acessible by the Application Server.
    ...
    import org.fenixedu.academic.util.ByteArray;
