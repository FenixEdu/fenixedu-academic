...
Control.ColorPicker.prototype = {
  ...
  inputFieldValue : function() {
    if (this.field.value.substring(0, 1).match("#")) {
      return this.field.value.substring(1);
    }
  ...
  updateFromFieldValue : function(event) {
    ...
    var rgb = YAHOO.util.Color.hex2rgb( field.value.substring(1) );
    if (!YAHOO.util.Color.isValidRGB(rgb)) return;
  ...
  updateSwatch : function() {
    var rgb = YAHOO.util.Color.hex2rgb( this.inputFieldValue() );
    if (!YAHOO.util.Color.isValidRGB(rgb)) return;
  ...
  update : function(x, y) {
    ...
    this.control.input.value = this.inputFieldValue();
    this.updateSwatch();
    if (this.options.onUpdate) this.options.onUpdate.bind(this)(this.inputFieldValue());
  }
}
