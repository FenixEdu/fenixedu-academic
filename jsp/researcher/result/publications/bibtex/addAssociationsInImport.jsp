...
<logic:present role="RESEARCHER">
	...
	<bean:define id="createFile" value="<%="/result/resultDocumentFilesManagement.do?forwardTo=bibTexImport&method=create&" + parameters%>" />
	<bean:define id="backLinkFile" value="<%="/result/resultDocumentFilesManagement.do?forwardTo=bibTexImport&method=backToResult&" + parameters %>" />
	<bean:define id="prepareEditFile" value="<%="/result/resultDocumentFilesManagement.do?forwardTo=bibTexImport&method=prepareEdit&" + parameters%>" />
	<bean:define id="prepareAlterFile" value="<%="/result/resultDocumentFilesManagement.do?forwardTo=bibTexImport&method=prepareAlter&" + parameters%>"/>	
...
<%-- 			<html:link page="<%=prepareAlterFile%>"><bean:message bundle="RESEARCHER_RESOURCES" key="link.edit"/></html:link>
	...
	<fr:edit id="importBibtexBean" name="importBibtexBean" visible="false"/>
	</fr:edit>
	...
	<bean:define id="createUnit" value="<%="/resultAssociations/createUnitAssociation.do?forwardTo=bibTexImport&" + parameters%>" />
	<bean:define id="prepareEditUnit" value="<%="/resultAssociations/prepareEditUnitAssociations.do?forwardTo=bibTexImport&" + parameters%>" />
	<bean:define id="prepareAlterUnit" value="<%="/resultAssociations/prepareEditUnitRole.do?forwardTo=bibTexImport&" + parameters%>"/>	
	<bean:define id="removeUnit" value="<%="/resultAssociations/removeUnitAssociation.do?forwardTo=bibTexImport&" + parameters%>"/>
	<bean:define id="backLinkUnit" value="<%="/resultAssociations/backToResult.do?forwardTo=bibTexImport&" + parameters%>" />
	...
	<p class="mbottom0"><bean:message bundle="RESEARCHER_RESOURCES" key="researcher.ResultUnitAssociation.add"/></p>
		...
		<fr:edit id="unitBean" name="unitBean" schema="<%= unitSchema %>">
			...
			<fr:destination name="postBack" path="<%= "/resultAssociations/changeTypeOfUnit.do?forwardTo=bibTexImport&" + parameters %>"/>
		</fr:edit> 
			<fr:edit id="importBibtexBean" name="importBibtexBean" visible="false"/>
			<table class="tstyle5 mtop0">
	  			<tr>
	...
	<p>
	<fr:form action="<%= "/bibtexManagement/nextImport.do"%>">
		<fr:edit name="importBibtexBean" id="importBibtexBean" visible="false"/>
		<html:submit><bean:message key="button.next" bundle="RESEARCHER_RESOURCES"/></html:submit>
	</fr:form>
	</p>	
