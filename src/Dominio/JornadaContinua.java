...
public class JornadaContinua implements IStrategyHorarios {
	...
	public ActionErrors validateAssociarHorario(ActionForm form) {
		...
		long timeHNPeriodoMinimo = 0;
		long timeHNPeriodoMaximo = 0;
		...
		if (formHorario.getDuracaoSemanal().length() < 1) {
			...
			if (duracaoSemanal < Constants.DURACAO_MINIMA_SEMANAL_JORNADA 
			        || duracaoSemanal > Constants.DURACAO_MAXIMA_SEMANAL_JORNADA) {
			...
			if (formHorario.getRegime().length < 0) {
				...
				if ((formHorario.getInicioHN1Horas().length() < 1)
					...
					if (!(timeInicioHN1 < timeFimHN1)) {
						...
						timeHNPeriodoMinimo =
							((new Float(Constants.DURACAO_MINIMA_SEMANAL_JORNADA / Constants.SEMANA_TRABALHO_JORNADA).intValue()
								* 3600
								* 1000)
								+ new Float(
									(new Float(Constants.DURACAO_MINIMA_SEMANAL_JORNADA / Constants.SEMANA_TRABALHO_JORNADA).floatValue()
										- new Float(Constants.DURACAO_MINIMA_SEMANAL_JORNADA / Constants.SEMANA_TRABALHO_JORNADA).intValue())
										* 3600
										* 1000)
									.longValue());
						timeHNPeriodoMaximo =
							((new Float(Constants.DURACAO_MAXIMA_SEMANAL_JORNADA / Constants.SEMANA_TRABALHO_JORNADA).intValue()
								* 3600
								* 1000)
								+ new Float(
									(new Float(Constants.DURACAO_MAXIMA_SEMANAL_JORNADA / Constants.SEMANA_TRABALHO_JORNADA).floatValue()
										- new Float(Constants.DURACAO_MAXIMA_SEMANAL_JORNADA / Constants.SEMANA_TRABALHO_JORNADA).intValue())
										* 3600
										* 1000)
									.longValue());
						
						if (!(((timeFimHN1 - timeInicioHN1) >= timeHNPeriodoMinimo) 
						        &&  ((timeFimHN1 - timeInicioHN1) <= timeHNPeriodoMaximo))) {
				...
				if ((listaRegime.contains(Constants.REGIME_IPF))) {
					...
					if ((formHorario.getInicioPF1Horas().length() < 1)
						...
						if (!(timeInicioPF1 < timeFimPF1)) {
							...
							/*calendar.clear();
							calendar.set(1970, 0, 1, 2,	00, 00);
							timePFPeriodo = calendar.getTimeInMillis();
							 
							if((timeInicioPF1 - timeInicioHN1) != timePFPeriodo){
	...
	public long duracaoDiaria(Horario horario) {
        return ((new Float(horario.getDuracaoSemanal()
