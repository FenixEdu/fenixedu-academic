...
public class Flexivel implements IStrategyHorarios {
    ...
    public ActionErrors validateAssociarHorario(ActionForm form) {
        ...
        try {
            if (formHorario.getDuracaoSemanal().length() < 1) {
                errors.add("DuracaoSemanal-HFlexivel", new ActionError(
            ...
            if ((formHorario.getTrabalhoConsecutivoHoras().length() < 1)
                ...
                try {
                    trabalhoConsecutivoHoras = new Integer(formHorario.getTrabalhoConsecutivoHoras())
                            .intValue();
                    trabalhoConsecutivoMinutos = new Integer(formHorario.getTrabalhoConsecutivoMinutos())
                            .intValue();
                } catch (java.lang.NumberFormatException e) {
                    errors.add("numero", new ActionError("error.numero.naoInteiro"));
                }
            ...
            if ((formHorario.getInicioHN1Horas().length() < 1)
                ...
                if (!((timeInicioHN1 < timeFimHN1) && (timeFimHN1 < timeInicioHN2) && (timeInicioHN2 < timeFimHN2))) {
                    ...
                    // 2 periodos do horario normal que somam 7 horas diarias no
                    // total
                    timeHNPeriodo = ((new Float(Constants.DURACAO_SEMANAL_FLEXIVEL
                            / Constants.SEMANA_TRABALHO_FLEXIVEL).intValue() * 3600 * 1000) + new Float(
                            (new Float(Constants.DURACAO_SEMANAL_FLEXIVEL
                                    / Constants.SEMANA_TRABALHO_FLEXIVEL).floatValue() - new Float(
                                    Constants.DURACAO_SEMANAL_FLEXIVEL
                                            / Constants.SEMANA_TRABALHO_FLEXIVEL).intValue()) * 3600 * 1000)
                            .longValue());
            ...
            if (formHorario.getRegime().length < 0) {
                ...
                if (listaRegime.contains(Constants.REGIME_IPF)) { //Regime:new
                    // String("isencaoPeriodoFixo")
                    ...
                    /*
                     * Para horários com IPF não existe intervalo obrigatorio
                     * para almoço // Marcações de ponto para a hora do almoço
                     * calendar.clear(); calendar.set(1970, 0, 1, fimHN1Horas,
                     * fimHN1Minutos, 00); timeFimHN1 =
                     * calendar.getTimeInMillis();
                     * 
                     * calendar.clear(); calendar.set(1970, 0, 1,
                     * inicioHN2Horas, inicioHN2Minutos, 00); timeInicioHN2 =
                     * calendar.getTimeInMillis();
                     * 
                     * if ((timeFimHN1 < Constants.INICIO_REFEICAO_IPF) ||
                     * (timeInicioHN2 > Constants.FIM_REFEICAO_IPF)) {
                     * errors.add("almoco", new
                     * ActionError("error.PeriodoAlmoco.invalido")); }
                     */
                } else {
                    ...
                    if ((formHorario.getInicioPF1Horas().length() < 1)
                        ...
                        if (!((timeInicioPF1 < timeFimPF1) && (timeFimPF1 < timeInicioPF2) && (timeInicioPF2 < timeFimPF2))) {
                            ...
                            // 2 plataformas fixas nao inferiores a 4 horas no
                            // total
    ...
    public void setHorario(Horario horario, ActionForm form) {
        ...
        if (formHorario.getDiaAnteriorRefeicao().length() < 1) {
            ...
            //contagem no dia anterior
            calendar.set(1969, 11, 31, (new Integer(formHorario.getInicioRefeicaoHoras())).intValue(),
                    (new Integer(formHorario.getInicioRefeicaoMinutos())).intValue(), 00);
        }
        ...
        // Periodos Fixos
        listaRegime = Arrays.asList(formHorario.getRegime());
        if (!listaRegime.contains(Constants.REGIME_IPF)) { //Regimes: new
            // String("isencaoPeriodoFixo")
            if (formHorario.getDiaAnteriorPF1().length() < 1) {
            ...
            if (formHorario.getDiaAnteriorPF2().length() < 1) {
                ...
                //contagem no dia anterior
                calendar.set(1969, 11, 31, (new Integer(formHorario.getInicioPF2Horas())).intValue(),
                        (new Integer(formHorario.getInicioPF2Minutos())).intValue(), 00);
            }
    ...
    public void setHorarioTipoRotativo(Horario horario, ActionForm form) {
        ...
        horario.setChaveHorarioTipo(Constants.CHAVE_HTIPO); // para revelar que
        // é um horário tipo
    ...
    public void setFormAssociarHorarioConfirmar(Locale locale, ActionForm form, Pessoa pessoa,
        ...
        formHorario
                .setIntervaloMinimoHoras((new Integer(calendar.get(Calendar.HOUR_OF_DAY))).toString());
        if (calendar.get(Calendar.MINUTE) < 10) {
        ...
        if (calendar.get(Calendar.MINUTE) < 10) {
            ...
            formHorario
                    .setInicioRefeicaoMinutos((new Integer(calendar.get(Calendar.MINUTE))).toString());
        }
        if (calendar.get(Calendar.DAY_OF_MONTH) != 1) {
            //contagem no Dia Anterior
    ...
    public String descricaoHorarioTipo(Locale locale, HorarioTipo horarioTipo, List listaRegimes) {
        ...
        if (!listaRegimes.contains(Constants.REGIME_IPF)) { //Regime: new
            // String("isencaoPeriodoFixo")
            calendar.clear();
            calendar.setTimeInMillis((horarioTipo.getInicioPF1()).getTime());
    ...
    public String periodoTrabalhoHorarioTipo(HorarioTipo horarioTipo) {
        ...
        periodoTrabalho = inicioHN1Horas + ":" + inicioHN1Minutos + " - " + fimHN1Horas + ":"
                + fimHN1Minutos + " <> " + inicioHN2Horas + ":" + inicioHN2Minutos + " - " + fimHN2Horas
                + ":" + fimHN2Minutos;
    ...
    public void setSaldosHorarioVerbeteBody(Horario horario, List listaRegimes,
        ...
        if (iterador.hasNext()) {
            // o intervalo para refeicao deve ser feito no meio de dois periodos
            // de trabalho.
            ...
            if (entrada.getData().getTime() > horario.getInicioRefeicao().getTime()
                ...
                if (listaMarcacoesPonto.size() >= 3) {
                    // se no intervalo de uma saida do dia com uma entrada está
                    // contida no intervalo de
                    // refeicao é como se
                    ...
                    while (iterador.hasNext()) {
                        ...
                        // saldo que pode ser utilizado caso nao tenha havido
                        // intervalo para refeicao no
                        // periodo estipulado
                        ...
                        if (intervaloRefeicao > 0) {
                            // encontrou o intervalo de refeicao entao sai do
                            // ciclo
                            break;
                        }
                    }
                    ...
                    // marcacoes que nao estao
                    // contidas no intervalo de refeicao.
                    // neste caso nao desconta a hora de refeicao
                    if ((listaMarcacoesPonto.size() == 2)) { //apenas com duas
                        // marcaçoes
                        saida = (MarcacaoPonto) iterador.next();
                        ...
                        if (saida.getData().getTime() > horario.getInicioRefeicao().getTime()
                            ...
                            // funcionário efectuou trabalho em
                            // todo o periodo de refeição
                            if (entrada.getData().getTime() < horario.getInicioRefeicao().getTime()
                                    && saida.getData().getTime() > horario.getFimRefeicao().getTime()) {
                                saldoPenalizacaoAusenciaRefeicao = horario.getFimRefeicao().getTime()
                                        - entrada.getData().getTime()
                                        - horario.getDescontoObrigatorioRefeicao().getTime();
                            }
            ...
            if (intervaloRefeicaoJustificado > 0 && intervaloRefeicao == 0) {
                ...
                if (intervaloRefeicao < horario.getIntervaloMinimoRefeicao().getTime()
                    ...
                    // o periodo de refeicao é menor que o obrigatorio logo a
                    // penalizacao é contar o
                    // período
    ...
    public long duracaoDiaria(Horario horario) {
        ...
        return ((new Float(Constants.DURACAO_SEMANAL_FLEXIVEL / Constants.SEMANA_TRABALHO_FLEXIVEL)
                .intValue() * 3600 * 1000) + new Float(
                (new Float(Constants.DURACAO_SEMANAL_FLEXIVEL / Constants.SEMANA_TRABALHO_FLEXIVEL)
                        .floatValue() - new Float(Constants.DURACAO_SEMANAL_FLEXIVEL
                        / Constants.SEMANA_TRABALHO_FLEXIVEL).intValue()) * 3600 * 1000).longValue());
    } /* duracaoDiaria */

    public int mapeamentoFechoMes(Horario horario, List listaRegimes) {

        if (listaRegimes.contains(Constants.REGIME_TE)) {
            return Constants.MAP_FLEXIVEL_AJUST_TE;
        } else if (listaRegimes.contains(Constants.REGIME_MENORES12)) {
            return Constants.MAP_FLEXIVEL_AJUST_FILHOS12;
        } else if (listaRegimes.contains(Constants.REGIME_FILHOSDEFICIENTES)) {
            return Constants.MAP_FLEXIVEL_AJUST_FILHOSDEF;
        } else if (listaRegimes.contains(Constants.REGIME_SERVICO)) {
            return Constants.MAP_FLEXIVEL_AJUST_CONVSERV;
        } else if (listaRegimes.contains(Constants.REGIME_OUTRAS_CIRCUNSTANCIAS)) {
            return Constants.MAP_FLEXIVEL_AJUST_OUTROS;
        } else {
