...
public class MeioTempo implements IStrategyHorarios {
        ...
        try {
            ...
            if ((formHorario.getInicioHN1Horas().length() < 1)
                ...
                if (!(timeInicioHN1 < timeFimHN1)) {
                    ...
                    timeHNPeriodo = ((new Float(Constants.DURACAO_SEMANAL_MEIOTEMPO
                            / Constants.SEMANA_TRABALHO_MEIOTEMPO).intValue() * 3600 * 1000) + new Float(
                            (new Float(Constants.DURACAO_SEMANAL_MEIOTEMPO
                                    / Constants.SEMANA_TRABALHO_MEIOTEMPO).floatValue() - new Float(
                                    Constants.DURACAO_SEMANAL_MEIOTEMPO
                                            / Constants.SEMANA_TRABALHO_MEIOTEMPO).intValue()) * 3600 * 1000)
                            .longValue());
            ...
            if (formHorario.getRegime().length < 0) {
                ...
                if (listaRegime.contains(Constants.REGIME_NORMAL)) { //Regime:
                    // Normal
                    errors.add("regime", new ActionError("error.regime.naoPermitido"));
                }

                if (!listaRegime.contains(Constants.REGIME_IPF)) { //Regime:
                    // IPF
            ...
            if ((formHorario.getDiaInicio() != null) && (formHorario.getMesInicio() != null)
                ...
                if ((formHorario.getDiaInicio().length() < 1) || (formHorario.getMesInicio() == null)
                    ...
                    if (!formHorario.isExcepcaoHorario()) {
                        if (!(dataInicio.getTime() < dataFim.getTime())) {
                            ...
                            if (!(((dataFim.getTime() - dataInicio.getTime()) > (Constants.VALIDADE_MINIMA * 1000)) && ((dataFim
                                    .getTime() - dataInicio.getTime()) < (Constants.VALIDADE_MAXIMA * 1000)))) {
                                errors.add("datasValidade", new ActionError(
                                        "error.validadeHorarioMeioTempo"));
                            }
                        }
                    } else {
                        if (!(dataInicio.getTime() <= dataFim.getTime())) {
    ...
    public long duracaoDiaria(Horario horario) {
        return ((new Float(Constants.DURACAO_SEMANAL_MEIOTEMPO / Constants.SEMANA_TRABALHO_MEIOTEMPO)
                .intValue() * 3600 * 1000) + new Float(
                (new Float(Constants.DURACAO_SEMANAL_MEIOTEMPO / Constants.SEMANA_TRABALHO_MEIOTEMPO)
                        .floatValue() - new Float(Constants.DURACAO_SEMANAL_MEIOTEMPO
                        / Constants.SEMANA_TRABALHO_MEIOTEMPO).intValue()) * 3600 * 1000).longValue());
    } /* duracaoDiaria */
