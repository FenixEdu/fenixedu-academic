...
public class IsencaoHorario implements IStrategyHorarios {
	...
	public ActionErrors validateAssociarHorario(ActionForm form) {
		...
		try {
				...
				if (!((timeInicioHN1 < timeFimHN1)
					...
					timeHNPeriodo =
						((new Float(duracaoSemanal
							/ Constants.SEMANA_TRABALHO_ISENCAO)
							.intValue()
							* 3600
							* 1000)
							+ new Float(
					...
					if (((timeFimHN1 - timeInicioHN1) > Constants.MAX_TRABALHO_ISENCAO)
						|| ((timeFimHN2 - timeInicioHN2) > Constants.MAX_TRABALHO_ISENCAO)) {
			...
			if ((formHorario.getInicioPF1Horas().length() > 0)
				...
				errors.add(
					"periodoFixo",
					new ActionError("error.PF.desnecessario"));
			}
			...
			if (formHorario.getRegime().length < 0) {
				...
				if (!listaRegime.contains(Constants.IPF)) {
					errors.add(
						"periodoFixo",
						new ActionError("error.regime.IPFobrigatorio"));
				}
			}
	...
	public void setHorario(Horario horario, ActionForm form) {
		...
		if ((formHorario.getIntervaloMinimoHoras() != null)
			&& (formHorario.getDescontoObrigatorioHoras() != null)) {
	...
	public void setSaldosHorarioVerbeteBody(
		...
		if (listaMarcacoesPonto.size() == 2
			...
			if (intervaloRefeicao
				...
				saldo =
					saldo - horario.getDescontoObrigatorioRefeicao().getTime();
			}
		}
