 ...
 */
public class MaxOfAcumulatedEnrollmentsRule extends
        SelectionUponMaximumNumberEnrollmentRule implements IEnrollmentRule {
     ...
     */
    public MaxOfAcumulatedEnrollmentsRule(
        ...
        //        List enrollments =
        // studentCurricularPlan.getAllEnrollmentsInCoursesWhereStudentIsEnrolledAtTheMoment();
        //
        //        List curricularCourses = (List) CollectionUtils.collect(enrollments,
        // new Transformer() {
        //
        //            public Object transform(Object obj) {
        //                ICurricularCourse curricularCourse = ((IEnrolment)
        // obj).getCurricularCourse();
        ...
        //        });
        //
        //        acumulatedEnrollments =
        // CollectionUtils.getCardinalityMap(curricularCourses);

        this.studentCurricularPlan = studentCurricularPlan;
        enrolledEnrollments = studentCurricularPlan
                .getAllStudentEnrolledEnrollments();
        maxTotalNAC = studentCurricularPlan
                .getMaximumNumberOfAcumulatedEnrollments().intValue();
    }
    ...
    public List apply(List curricularCoursesToBeEnrolledIn) {
        ...
        for (int i = 0; i < curricularCoursesToBeEnrolledIn.size(); i++) {
            ICurricularCourse curricularCourse = (ICurricularCourse) curricularCoursesToBeEnrolledIn
                    .get(i);
            if (availableNACToEnroll < getIncrementNAC(curricularCourse)
                    .intValue())
                filtercurricularCoursesToBeEnrolledIn.add(curricularCourse);
        }

        curricularCoursesToBeEnrolledIn
                .removeAll(filtercurricularCoursesToBeEnrolledIn);
     ...
     * @return
     */
    private List filterCurricularCoursesThatExeceedAcumulatedValue(
            List curricularCoursesSortedByYear) {

        int acumulatedNAC = 0;
        int iterator = 0;
        for (; iterator < curricularCoursesSortedByYear.size(); iterator++) {
            IEnrollment enrollment = (IEnrollment) curricularCoursesSortedByYear
                    .get(iterator);
            ICurricularCourse curricularCourse = enrollment
                    .getCurricularCourse();
            ...
            if (acumulatedNAC >= availableNACToEnroll) {
                ...
                curricularCoursesSortedByYear = getCurricularCoursesFromPreviousAndCurrentYear(
                        curricularCoursesSortedByYear, iterator);
                break;
            }
        }
