...
public class ParseQuestion extends DefaultHandler
{
	...
	private InfoQuestion list2Question(InfoQuestion infoQuestion) throws ParseQuestionException
	{
		...
		while (it.hasNext())
		{
			...
			if (tag.startsWith("mat") && !tag.equals("material"))
			{
		...
		while (it.hasNext())
		{
			...
			if (tag.startsWith("render_"))
				if (!(tag.equals("render_choice")))
					throw new ParseQuestionException(tag);
			if ((tag.startsWith("mat") && !tag.equals("material")) || tag.startsWith("response_"))
			{
				if ((tag.equals("response_lid")))
				{
					if (atts.getIndex("rcardinality") != -1)
					{
						if (atts.getValue("rcardinality").equals("Single"))
							infoQuestion.setQuestionCardinality(atts.getValue("rcardinality"));
						else
							throw new ParseQuestionException(tag, "rcardinality="+atts.getValue("rcardinality"));
					}
				}
