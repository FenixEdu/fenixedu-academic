...
public class ParseQuestion extends DefaultHandler {
    ...
    public InfoStudentTestQuestion parseStudentTestQuestion(
            InfoStudentTestQuestion infoStudentTestQuestion, String path)
        ...
        infoStudentTestQuestion
                .setQuestion(list2Question(infoStudentTestQuestion
                        .getQuestion()));

        Response response = null;
        if (infoStudentTestQuestion.getQuestion().getQuestionType().getType()
                .intValue() == QuestionType.LID) {
            infoStudentTestQuestion.getQuestion().setOptions(
                    shuffleStudentTestQuestionOptions(infoStudentTestQuestion
                            .getOptionShuffle(), infoStudentTestQuestion
                            .getQuestion().getOptions()));
            infoStudentTestQuestion.getQuestion()
                    .setResponseProcessingInstructions(
                            newResponseList(infoStudentTestQuestion
                                    .getQuestion()
    ...
    public String shuffleQuestionOptions(String file, boolean shuffle,
    ...
    public void startElement(String uri, String localName, String qName,
        ...
        current = new Element(uri, localName, qName, new AttributesImpl(
                attributes));
        ...
        else if (feedback) listFeedback.add(current);
    ...
    public void endElement(String uri, String localName, String qName)
        ...
        if (qName.equals("response_lid")) {
            ...
            listResponse.add(new Element(uri, localName, "/" + qName, null));
                ...
                listFeedback
                        .add(new Element(uri, localName, "/" + qName, null));
    }
    ...
    private InfoQuestion getQuestion(InfoQuestion infoQuestion)
            throws ParseQuestionException {
            ...
            if (tag.startsWith("render_")) {
                ...
                if ((tag.equals("mattext"))) {
                    ...
                    if (atts.getIndex("label") != -1)
                            auxList.add(new LabelValueBean("image_label", atts
                                    .getValue("label")));
                    if (atts.getIndex("uri") != -1)
                            throw new ParseQuestionException(tag, "uri");
                    auxList.add(new LabelValueBean(atts.getValue("imagtype"),
                            element.getValue()));
                } else
                    throw new ParseQuestionException(tag, true);
    ...
    private InfoQuestion getOptions(InfoQuestion infoQuestion)
            throws ParseQuestionException {
            ...
            if (tag.startsWith("render_")) {
                if (tag.equals("render_fib")) {
                    ...
                    if (atts.getIndex("rows") != -1)
                            renderFIB
                                    .setRows(new Integer(atts.getValue("rows")));
                    if (atts.getIndex("columns") != -1)
                            renderFIB.setColumns(new Integer(atts
                                    .getValue("columns")));
    ...
    private InfoQuestion getResponses(InfoQuestion infoQuestion)
            throws ParseQuestionException {
            ...
            if (tag.equals("setvar")) {
                ...
                if (responseProcessing.getAction().intValue() == ResponseProcessing.SET
                    ...
                    if (infoQuestion.getQuestionValue() == null
                            || (infoQuestion.getQuestionValue()
                                    .compareTo(value) < 0))
                            infoQuestion.setQuestionValue(value);
                } else if (responseProcessing.getAction().intValue() == ResponseProcessing.SUBTRACT) {
                    if (infoQuestion.getQuestionValue() == null)
                            infoQuestion.setQuestionValue(new Double("-"
                                    + value));
                    responseProcessing
                            .setResponseValue(new Double("-" + value));
                }
                Iterator itAuxList = auxList.iterator();
                while (itAuxList.hasNext()) {
                    ResponseProcessing rp = (ResponseProcessing) itAuxList
                            .next();
                    if (rp.getResponseProcessingId() == responseProcessing
                            .getResponseProcessingId()) {
                        rp.setAction(atts.getValue("action"));
                        rp.setResponseValue(responseProcessing
                                .getResponseValue());
                ...
                if (tag.equals("varequal") || tag.equals("varlt")
                        || tag.equals("varlte") || tag.equals("vargt")
                    ...
                    if (or == 0 && and == 0) {
                        String tagName = tag;
                        if (not) tagName = new String("not").concat(tagName);
                ...
                if (and == 0 && or == 0)
                        auxList.addAll(resolveAndCondition(newResponseList, i,
                                new ArrayList(), responseProcessingId));
                and++;
            } else if (tag.equals("/and")) {
                and--;
            } else if (tag.equals("or")) {
                if (or == 0 && and == 0)
                        auxList.addAll(resolveOrCondition(newResponseList, i,
                                new ArrayList(), responseProcessingId));
                or++;
            } else if (tag.equals("/or")) {
                or--;
                ...
                f.add(new LabelValueBean("linkrefid", atts
                        .getValue("linkrefid")));
        ...
        if (responseProcessing != null
                && responseProcessing.getResponseConditions().size() != 0)
                auxList.add(responseProcessing);
        infoQuestion.setResponseProcessingInstructions(auxList);
        return infoQuestion;
    }

    private InfoQuestion getFeedback(InfoQuestion infoQuestion)
            throws ParseQuestionException {
        ...
        while (it.hasNext()) {
            ...
            if (tag.equals("itemfeedback")) {
                ...
                Iterator responsesIt = infoQuestion
                        .getResponseProcessingInstructions().iterator();
                for (int i = 0; responsesIt.hasNext(); i++) {
                    ResponseProcessing rp = (ResponseProcessing) responsesIt
                            .next();
                    if (rp.getFeedback() != null
                            && rp.getFeedback().size() > 0
                            && ((LabelValueBean) rp.getFeedback().get(0))
                                    .getValue().equals(ident))
    ...
    private List shuffleStudentTestQuestionOptions(String shuffle, List oldList) {
        ...
        for (int i = 0; i < aux.length; i++)
            newOptions = insert2NewList(oldList, newOptions,
    ...
    public List newResponseList(List responseList, List optionList) {
        ...
        while (itResponseProcessing.hasNext()) {
            ...
            while (itResponseCondition.hasNext()) {
                ResponseCondition responseCondition = (ResponseCondition) itResponseCondition
                        .next();
                ...
                while (itOption.hasNext()) {
                    ...
                    if (option.getLabel().equals("response_label"))
                            if (option.getValue().equals(response))
                                newResponseCondition = new ResponseCondition(
                                        ResponseCondition
                                                .getConditionString(responseCondition
                                                        .getCondition()),
                                        new Integer(index).toString(),
                                        responseCondition.getResponseLabelId());
                            else
                                index++;
                }
            ...
            ResponseProcessing newResponseProcessing = new ResponseProcessing(
                    newResponseConditionList, responseProcessing
                            .getResponseValue(),
                    responseProcessing.getAction(), responseProcessing
                            .getFeedback(), responseProcessing
    ...
    public InfoQuestion setFenixCorrectResponse(InfoQuestion infoQuestion) {
        if (infoQuestion.getResponseProcessingInstructions().size() != 0) {
            Iterator itResponseProcessing = infoQuestion
                    .getResponseProcessingInstructions().iterator();
            ...
            if (fenixCorrectResponseIndex != -1)
                    ((ResponseProcessing) infoQuestion
                            .getResponseProcessingInstructions().get(
    ...
    public InfoQuestion getRidOfEmptyResponseConditions(
        ...
        Iterator rpIt = infoQuestion.getResponseProcessingInstructions()
                .iterator();
        ...
        while (it.hasNext()) {
            ...
            if (tag.startsWith("var")) {
                ...
                if ((not % 2) == 0)
                    ...
                    String tagName = "or";
                    if (tag.equals("/and")) tagName = "/" + tagName;
                ...
                if ((not % 2) == 0)
                    ...
                    String tagName = "and";
                    if (tag.equals("/or")) tagName = "/" + tagName;
                    resultList.add(new Element(null, tagName, tagName, null));
                }
            } else
                resultList.add(element);
    ...
    private List resolveAndCondition(List listResponse, int index,
            List oldResponseList, int id) throws ParseQuestionException {
        ...
        for (int i = 0; it.hasNext(); i++) {
            ...
            if (tag.startsWith("var") && or == 0) {
                if (tag.equals("varequal") || tag.equals("varlt")
                    ...
                    ResponseCondition rc = new ResponseCondition(tagName,
                            element.getValue(), atts.getValue("respident"));

                    if (newResponseList.size() != 0) {
                        Iterator newResponseListIt = newResponseList.iterator();
                        while (newResponseListIt.hasNext()) {
                            ResponseProcessing rp = (ResponseProcessing) newResponseListIt
                                    .next();
                ...
                if (or == 0)
                        newResponseList.addAll(resolveOrCondition(listResponse,
                                index + i, newResponseList, id));
                or++;
    ...
    private List resolveOrCondition(List listResponse, int index,
            List oldResponseList, int id) throws ParseQuestionException {
        ListIterator it = listResponse.listIterator(index);
        boolean not = false;
        List newResponseList = new ArrayList();
        ...
        for (int i = 0; it.hasNext(); i++) {
            ...
            if (tag.startsWith("var") && and == 0) {
                if (tag.equals("varequal") || tag.equals("varlt")
                    ...
                    if (oldResponseList.size() != 0) {
                        ...
                        ResponseProcessing responseProcessing = new ResponseProcessing(
                                id);
                        responseProcessing
                                .setResponseConditions(new ArrayList());
                        responseProcessing.getResponseConditions()
                                .add(
                                        new ResponseCondition(tagName, element
                                                .getValue(), atts
                ...
                if (and == 0)
                        newResponseList = resolveAndCondition(listResponse,
                                index + i, newResponseList, id);
                and++;
