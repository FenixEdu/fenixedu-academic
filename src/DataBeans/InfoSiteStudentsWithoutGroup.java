...
public class InfoSiteStudentsWithoutGroup extends DataTranferObject implements ISiteComponent {
    ...
    public boolean equals(Object objectToCompare) {
        ...
        if (objectToCompare instanceof InfoSiteStudentsWithoutGroup) {
            ...
            result = (this.getGroupNumber().equals(((InfoSiteStudentsWithoutGroup) objectToCompare)
                    .getGroupNumber()));

        }

        if (((InfoSiteStudentsWithoutGroup) objectToCompare).getInfoStudentList() == null
                && this.getInfoStudentList() == null && result == true) {

            return true;
        }
        if (((InfoSiteStudentsWithoutGroup) objectToCompare).getInfoStudentList() == null
                || this.getInfoStudentList() == null
                || ((InfoSiteStudentsWithoutGroup) objectToCompare).getInfoStudentList().size() != this
                        .getInfoStudentList().size()) {

            return false;
        }

        ListIterator iter1 = ((InfoSiteStudentsWithoutGroup) objectToCompare).getInfoStudentList()
                .listIterator();
