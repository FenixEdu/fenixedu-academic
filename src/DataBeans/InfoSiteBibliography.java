...
public class InfoSiteBibliography extends DataTranferObject implements ISiteComponent {
    ...
    public boolean equals(Object objectToCompare) {
        ...
        if (((InfoSiteBibliography) objectToCompare).getBibliographicReferences() == null
                || this.getBibliographicReferences() == null
                || ((InfoSiteBibliography) objectToCompare).getBibliographicReferences().size() != this
                        .getBibliographicReferences().size()) {
            return false;
        }
        ListIterator iter1 = ((InfoSiteBibliography) objectToCompare).getBibliographicReferences()
                .listIterator();
        ListIterator iter2 = this.getBibliographicReferences().listIterator();
        while (result && iter1.hasNext()) {
            InfoBibliographicReference infoBibliographicReference1 = (InfoBibliographicReference) iter1
                    .next();
            InfoBibliographicReference infoBibliographicReference2 = (InfoBibliographicReference) iter2
                    .next();
