...
public class LoadExecutionCoursesAndAssociations extends LoadDataFile
{
	...
	protected void processData()
	{
		...
		List allRooms =
		/*
		 * new ArrayList(); allRooms.add("VA4"); allRooms.add("V109"); allRooms.add("VA3");
		 * allRooms.add("V004"); allRooms.add("QA02.1"); allRooms.add("V124"); allRooms.add("GA1");
		 * allRooms.add("V133"); allRooms.add("V123"); allRooms.add("V131"); allRooms.add("GA5");
		 * allRooms.add("QA02.2"); allRooms.add("C22"); allRooms.add("QA02.4"); allRooms.add("Q4.4");
		 * allRooms.add("Q4.6"); allRooms.add("Q4.1"); allRooms.add("V116"); allRooms.add("VA5");
		 * allRooms.add("V135"); allRooms.add("E3"); allRooms.add("GA3"); allRooms.add("PA1");
		 * allRooms.add("V126"); allRooms.add("GA2"); allRooms.add("C9"); allRooms.add("V138");
		 * allRooms.add("F3"); allRooms.add("C11"); allRooms.add("V111"); allRooms.add("GA4");
		 * allRooms.add("EA4"); allRooms.add("0.2.20"); allRooms.add("0.2.18"); allRooms.add("0.2.1");
		 * allRooms.add("Q4.5"); allRooms.add("V110"); allRooms.add("Q5.2"); allRooms.add("V108");
		 * allRooms.add("V112"); allRooms.add("Q5.1"); allRooms.add("P12"); allRooms.add("C12");
		 * allRooms.add("F4"); allRooms.add("C23"); allRooms.add("EA5"); allRooms.add("EA1");
		 * allRooms.add("0.2.11");
		 */
		persistentObjectOJB.readThorRooms();
		int numAulas = 0;

		IExecutionPeriod executionPeriod = persistentObjectOJB.readExecutionPeriod();
		...
		for (int s = 0; s < allRooms.size(); s++)
		{
			...
			for (int hora = 1; hora < 31; hora++)
			{
				for (int dia = 1; dia < 7; dia++)
				{
					...
					List aulas = persistentObjectOJB.readThorAulas(dia, hora, sala);
					...
					for (int a = 0; a < aulas.size(); a++)
					{
						...
						Integer degreeID = new Integer(aula.getTurma().substring(0, 2));
						Disciplinas disciplina =
							persistentObjectOJB.readDisciplina(aula.getDisciplina());

						CourseDegreesPair courseDegreesPair =
							(CourseDegreesPair) hashMapLessons.get(aula.getHashKey());
						if (courseDegreesPair == null)
						{
							courseDegreesPair =
								new CourseDegreesPair(
									...
									new ArrayList(),
									new ArrayList());
							courseDegreesPair.getNames().add(disciplina.getNome());
							courseDegreesPair.getCodes().add(disciplina.getSigla());
							courseDegreesPair.getDegrees().add(degreeID);
							hashMapLessons.put(aula.getHashKey(), courseDegreesPair);
						}
						else
						{
							if (!courseDegreesPair.getDegrees().contains(degreeID))
							{
								courseDegreesPair.getDegrees().add(degreeID);
								if (courseDegreesPair.getCourse().getNome() != disciplina.getNome())
								{
									courseDegreesPair.getNames().add(disciplina.getNome());
									courseDegreesPair.getCodes().add(disciplina.getSigla());
									System.out.println(
										"Lesson with different names: "
											+ courseDegreesPair.getCourse().getNome()
											+ " != "
											+ disciplina.getNome());
								}
							}
		...
		{
			final CourseDegreesPair courseDegreesPair = (CourseDegreesPair) iterator.next();

			List matchingExecutionCourses =
				(List) CollectionUtils.select(executionCourses, new Predicate()
			{
			...
			if (matchingExecutionCourses != null && !matchingExecutionCourses.isEmpty() /* pos > -1 */
				)
			{
				...
				for (int k = 0; k < matchingExecutionCourses.size(); k++)
				{
					...
					((CourseDegreesPair) matchingExecutionCourses.get(k)).getDegrees().removeAll(
						courseDegreesPair.getDegrees());
					courseDegreesPair.getDegrees().addAll(
						((CourseDegreesPair) matchingExecutionCourses.get(k)).getDegrees());

					((CourseDegreesPair) matchingExecutionCourses.get(k)).getNames().removeAll(
						courseDegreesPair.getNames());
					courseDegreesPair.getNames().addAll(
						((CourseDegreesPair) matchingExecutionCourses.get(k)).getNames());

					((CourseDegreesPair) matchingExecutionCourses.get(k)).getCodes().removeAll(
						courseDegreesPair.getCodes());
					courseDegreesPair.getCodes().addAll(
						((CourseDegreesPair) matchingExecutionCourses.get(k)).getCodes());
		...
		{
			CourseDegreesPair courseDegreesPair = (CourseDegreesPair) executionCourses.get(i);

			System.out.println("name = " + courseDegreesPair.getNome());
			for (int j = 0; j < courseDegreesPair.getDegrees().size(); j++)
			{
				System.out.println("degree[" + j + "] = " + courseDegreesPair.getDegrees().get(j));
			}

			//Disciplinas disciplina = courseDegreesPair.getCourse();

			Integer numElements = (Integer) hashMapCode.get(courseDegreesPair.getSigla());
			...
			for (int j = 0; j < courseDegreesPair.degrees.size(); j++)
			{
				ICurricularCourse curricularCourse =
					getAssociated((Integer) courseDegreesPair.degrees.get(j), courseDegreesPair);
				...
				{
					disciplinaExecucao =
						(IExecutionCourse) curricularCourse.getAssociatedExecutionCourses().get(0);
				}
			...
			if (disciplinaExecucao == null)
			{
				...
				disciplinaExecucao.setAssociatedCurricularCourses(new ArrayList());
			}
			else
			{
			}
			...
			for (int j = 0; j < courseDegreesPair.getDegrees().size(); j++)
			{
				Integer degree = (Integer) courseDegreesPair.getDegrees().get(j);

				ICurricularCourse curricularCourse = getAssociated(degree, courseDegreesPair);
				if (curricularCourse != null)
				{
					disciplinaExecucao.getAssociatedCurricularCourses().add(curricularCourse);
				}
			}

			if (disciplinaExecucao.getAssociatedCurricularCourses().size() > 0)
			{

				ICurricularCourse curricularCourse =
					(ICurricularCourse) disciplinaExecucao.getAssociatedCurricularCourses().get(0);
				...
				if (numElements == null)
				{
					hashMapCode.put(courseDegreesPair.getSigla(), new Integer(0));
				}
				else
				{
	 ...
	 * @return
	 */
	private ICurricularCourse getAssociated(Integer degree, CourseDegreesPair pair)
	{

		IDegreeCurricularPlan degreeCurricularPlan =
			persistentObjectOJB.readDegreeCurricularPlanNextSemester(degree.toString());
		...
		if (degreeCurricularPlan != null)
		{
			...
			while (curricularCourse == null && iterator.hasNext())
			{
				...
				ICurricularCourse curricularCourseTemp =
					persistentObjectOJB.getCurricularCourse(degreeCurricularPlan, nomeDisciplina);
	...
	protected void report()
	{
		...
		System.out.println("      Number of elements read: " + numberElementsRead);
	...
	private class CourseDegreesPair
	{
		...
		public boolean equals(Object obj)
		{
			...
			if (obj instanceof CourseDegreesPair)
			{
				...
				resultado =
					CollectionUtils.containsAny(this.getNames(), courseDegreesPair.getNames())
						&& CollectionUtils.containsAny(this.getDegrees(), courseDegreesPair.getDegrees());
			}
		 ...
		 *  
		 */
		public CourseDegreesPair(Disciplinas course, List degrees, List names, List codes)
		{
			setCourse(course);
			setDegrees(degrees);
			setNames(names);
