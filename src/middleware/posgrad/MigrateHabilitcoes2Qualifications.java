...
public class MigrateHabilitcoes2Qualifications {
	...
	public static void main(String args[]) throws Exception{
		MigrateHabilitcoes2Qualifications migrateHabilitcoes2Qualifications = new MigrateHabilitcoes2Qualifications();
	...
	private void migrateHabilitacoes2Qualifications() throws Exception{
		...
		try {
			...
			System.out.println("Migrating " + areasCientificasPG.size() + " PosGrad Qualificacoes to Fenix Qualifications ...");
			...
			while(iterator.hasNext()){
				...
				criteria.addEqualTo("numeroDocumentoIdentificacao", candidato.getNumerodocumentoidentificacao());
				...
				if (result.size() != 1) {
					System.out.println("Candidato " + candidato.getCodigointerno() + " nao foi aprovado");
					broker.commitTransaction();
					continue;
				}
			...
			System.out.println();
			throw new Exception("Error Migrating Qualificacao do Candidato " + candidato.getCodigointerno() + e);
		}
	}
	...
	private boolean checkNull(Posgrad_candidato_mestrado candidato) {
		if ( 
			((candidato.getEscolalicenciatura() == null) || (candidato.getEscolalicenciatura().length() == 0)) &&
