...
public class changeRelationBetweenCCScopeAndCCOfMathAnalysisA {
	...
	static String[] dcpName = { "LEA2003/2004", "LEBM2003/2004", "LEFT2003/2004", "LCI2003/2004", "LMAC2003/2004" };
	...
	public static void main(String[] args) {
		...
		System.out.println("Running changeRelationBetweenCCScopeAndCCOfMathAnalysisA");
		...
		for (int dcpNameIterator = 0; dcpNameIterator < dcpName.length; dcpNameIterator++) {
			...
			IDegreeCurricularPlan degreeCurricularPlan = (DegreeCurricularPlan) broker.getObjectByQuery(query);
		...
		System.out.println(
			changedEnrolments
				+ " enrolments were changed and " + deletedEnrolments+ " enrolments were deleted from "
				+ ccCodesList.size()
				+ " curricular courses");
	...
	private static LinkedList getCurricularCourseScopes(LinkedList dcpList) {
		...
		try {
			...
			while (dcpIterator.hasNext()) {
				...
				andCriteria.addEqualTo("degreeCurricularPlan.idInternal", degreeCurricularPlan.getIdInternal());
	...
	private static void changeAssociationBetweenCCScopeOfMathAnalysisA(List ccsList) {
		...
		try {
			...
			while (ccsIterator.hasNext()) {
				...
				if ((enrolmentACurricularCourseList != null) && (!enrolmentACurricularCourseList.isEmpty())) {
					...
					IDegreeCurricularPlan degreeCurricularPlan = ccScope.getCurricularCourse().getDegreeCurricularPlan();
					...
					ICurricularCourse curricularCourse = (CurricularCourse) broker.getObjectByQuery(query);
					...
					if (curricularCourse != null) {
						...
						criteria.addEqualTo("curricularCourse.idInternal", curricularCourse.getIdInternal());
						query = new QueryByCriteria(CurricularCourseScope.class, criteria);
						ICurricularCourseScope curricularCourseScope = (CurricularCourseScope) broker.getObjectByQuery(query);
			...
			System.out.println("changeAssociationBetweenCCScopeOfMathAnalysisA: A PersistenceBrokerException has occured");
			pbe.printStackTrace();
		}
