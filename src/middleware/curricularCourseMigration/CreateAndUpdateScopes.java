...
public class CreateAndUpdateScopes {
	...
	public static void main(String args[]) throws Exception {
		...
		IPersistentMWCurricularCourseScope persistentCurricularCourseScopes = mws.getIPersistentMWCurricularCourseScope();
	...
	public static void createAndUpdate(MWCurricularCourseScope mwCurricularCourseScope, SuportePersistenteOJB sp) throws Exception {
		...
		try {
			...
			IDegreeCurricularPlan degreeCurricularPlan = getDegreeCurricularPlan(mwCurricularCourseScope, sp);			 
			
			if (degreeCurricularPlan == null) {
				System.out.println("Degree Curricular Plan Not Found ! Degree Number " + mwCurricularCourseScope.getDegreecode());
				return;
			}
			...
			if (branch == null) {
				System.out.println("Branch Not Found ! Degree Number " + mwCurricularCourseScope.getDegreecode() + " Branch Number " + mwCurricularCourseScope.getBranchcode());
				return;
			}
			...
			List curricularCourses = sp.getIPersistentCurricularCourse().readbyCourseCodeAndDegreeCurricularPlan(StringUtils.trim(mwCurricularCourseScope.getCoursecode()), degreeCurricularPlan);

			ICurricularCourse curricularCourse = null;

			if (curricularCourses.size() != 1) {
				System.out.println("Error Reading Curricular Course !! Course Code : " + mwCurricularCourseScope.getCoursecode() + " DCP " + degreeCurricularPlan.getIdInternal());
			...
			ICurricularYear curricularYear = sp.getIPersistentCurricularYear().readCurricularYearByYear(mwCurricularCourseScope.getCurricularyear());
			ICurricularSemester curricularSemester = sp.getIPersistentCurricularSemester().readCurricularSemesterBySemesterAndCurricularYear(mwCurricularCourseScope.getCurricularsemester(), curricularYear);
			...
			curricularCourseScope.setCredits(new Double(mwCurricularCourseScope.getCredits().doubleValue()));
			curricularCourseScope.setCurricularCourse(curricularCourse);
			curricularCourseScope.setCurricularSemester(curricularSemester);
			curricularCourseScope.setLabHours(new Double(mwCurricularCourseScope.getLabhours().doubleValue()));
			curricularCourseScope.setTheoPratHours(new Double(mwCurricularCourseScope.getTheoprathours().doubleValue()));
			curricularCourseScope.setTheoreticalHours(new Double(mwCurricularCourseScope.getTheoreticalhours().doubleValue()));
			curricularCourseScope.setPraticalHours(new Double(mwCurricularCourseScope.getPraticahours().doubleValue()));
	...
	private static IDegreeCurricularPlan getDegreeCurricularPlan(MWCurricularCourseScope mwCurricularCourseScope, ISuportePersistente sp) throws PersistentMiddlewareSupportException, ExcepcaoPersistencia {
		...
		MWBranch mwBranch = persistentBranch.readByDegreeCodeAndBranchCode(mwCurricularCourseScope.getDegreecode(), new Integer(0));
...
//		String degreeName = StringUtils.prechomp(mwBranch.getDescription(), "DE ");
	...
	private static IBranch getBranch(MWCurricularCourseScope mwCurricularCourseScope, IDegreeCurricularPlan degreeCurricularPlan, ISuportePersistente sp) throws PersistentMiddlewareSupportException, ExcepcaoPersistencia {
		...
		sp.clearCache();
		MWBranch mwbranch = persistentBranch.readByDegreeCodeAndBranchCode(mwCurricularCourseScope.getDegreecode(), mwCurricularCourseScope.getBranchcode());
