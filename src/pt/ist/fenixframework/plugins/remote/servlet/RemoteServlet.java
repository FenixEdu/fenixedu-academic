...
public class RemoteServlet extends HttpServlet {
    ...
    protected void process(HttpServletRequest request, final HttpServletResponse response)
	...
	if (username != null && !username.isEmpty() && password != null && !password.isEmpty()) {
		...
		for (final Object o : remoteHost.getUrl().toArray()) {
		    if (url.equalsIgnoreCase((String) o)
			    && remoteHost.getAllowInvocationAccess() != null
			    && remoteHost.getAllowInvocationAccess().booleanValue()
			    && username.equalsIgnoreCase(remoteHost.getUsername())
			    && password.equals(remoteHost.getPassword())) {
			return true;
		    }
		}
	    }
	...
	if (log.isWarnEnabled()) {
	    log.warn("Denying access for: " + url + "?username=" + username + "&password=" + (password == null ? "null" : "xxxxxx"));
	}
	return false;
    }
