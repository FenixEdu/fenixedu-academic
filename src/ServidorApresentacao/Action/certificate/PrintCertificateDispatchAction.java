...
public class PrintCertificateDispatchAction extends DispatchAction
{
		...
		throws Exception
	{
		...
		if (session != null)
		{
			...
			session.removeAttribute(
				SessionConstants.FINAL_RESULT_DEGREE_SIMPLE);
			session.removeAttribute(
			...
			InfoMasterDegreeThesisDataVersion infoMasterDegreeThesisDataVersion =
				(InfoMasterDegreeThesisDataVersion) session.getAttribute(
					SessionConstants.MASTER_DEGREE_THESIS_DATA_VERSION);
			InfoMasterDegreeProofVersion infoMasterDegreeProofVersion =
				(InfoMasterDegreeProofVersion) session.getAttribute(
					SessionConstants.MASTER_DEGREE_PROOF_HISTORY);
			String certificate =
				(String) session.getAttribute(
					SessionConstants.CERTIFICATE_TYPE);
			...
			try
			{
				infoExecutionYear =
					(
						InfoExecutionYear) ServiceManagerServiceFactory
							.executeService(
						userView,
			...
			else
			{
				...
				if (certificate.equals("Inscrição"))
				{
					...
					while (iterator.hasNext())
					{
						...
						if (aux.size() > 1)
						{
							BeanComparator dateComparator =
								new BeanComparator("when");
							Collections.sort(aux, dateComparator);
							Collections.reverse(aux);
						}
						InfoEnrolmentEvaluation latestEvaluation =
							(InfoEnrolmentEvaluation) aux.get(0);
						infoEnrolment2.setInfoEnrolmentEvaluation(
							latestEvaluation);
						if (result
							instanceof InfoEnrolmentInExtraCurricularCourse)
						{
					...
					if (normalEnrolment.size() != 0)
						session.setAttribute(
							SessionConstants.ENROLMENT_LIST,
							normalEnrolment);
					if (extraEnrolment.size() != 0)
						session.setAttribute(
							SessionConstants.EXTRA_ENROLMENT_LIST,
							extraEnrolment);
					session.setAttribute(
						SessionConstants.ENROLMENT,
						certificate.toUpperCase());
				}
				else
				{
					if ((certificate.equals("Aproveitamento"))
						|| (certificate
							.equals("Aproveitamento de Disciplinas Extra Curricular")))
					{
						Object args[] =
							{
						...
						if (certificate.equals("Aproveitamento"))
						{
							...
							session.setAttribute(
								SessionConstants.APROVMENT,
								certificate.toUpperCase());
						}
						else if (
							certificate.equals(
								"Aproveitamento de Disciplinas Extra Curricular"))
						{
							if (extraEnrolment.size() == 0)
							{
								ActionErrors errors = new ActionErrors();
					...
					else
					{
						...
						{
							if ((certificate
								.equals("Fim parte escolar simples"))
								|| (certificate
									.equals("Fim parte escolar discriminada sem média"))
								|| (certificate
									.equals("Fim parte escolar discriminada com média"))
									...
									.equals("Fim de curso de Mestrado discriminada com média")))
							{
								...
								try
								{
									...
									infoFinalResult =
										(
											InfoFinalResult) ServiceManagerServiceFactory
												.executeService(
											userView,
											"FinalResult",
											args);
								...
								try
								{
									...
									endOfScholarshipDate =
										(
											Date) ServiceManagerServiceFactory
												.executeService(
											userView,
								...
								while (iterator.hasNext())
								{
									...
									InfoEnrolment infoEnrolment2 =
										(InfoEnrolment) result;
									List aux =
										infoEnrolment2.getInfoEvaluations();
									if (aux.size() > 1)
									{
										BeanComparator dateComparator =
											new BeanComparator("when");
								...
								session.setAttribute(
									...
									extraEnrolment);
								session.setAttribute(
									SessionConstants.CONCLUSION_DATE,
									conclusionDate);
								session.setAttribute(
									SessionConstants.INFO_FINAL_RESULT,
									infoFinalResult);
								session.setAttribute(
									SessionConstants
										.MASTER_DEGREE_THESIS_DATA_VERSION,
								...
								if (certificate
									...
									session.setAttribute(
										SessionConstants
											.DISCRIMINATED_WITHOUT_AVERAGE,
										certificate.toUpperCase());
								if (certificate
									.equals("Fim parte escolar discriminada com média"))
									session.setAttribute(
										SessionConstants
											.DISCRIMINATED_WITH_AVERAGE,
										certificate.toUpperCase());
								if (certificate.equals("Diploma"))
									session.setAttribute(
										SessionConstants.DIPLOMA,
										certificate.toUpperCase());
								if (certificate
									.equals("Fim de curso de Mestrado discriminada com média"))
										
								{
									...
									{
										if (infoMasterDegreeProofVersion
											.getFinalResult()
											.getValue()
											!= 3)
										{
											...
											session.setAttribute(
												SessionConstants
													.FINAL_DEGREE_DISCRIMINATED_WITH_AVERAGE,
												certificate.toUpperCase());
											String dataFinal =
												DateFormat
													.getDateInstance()
...
//													.getFinalResult()
							...
							{
								if ((certificate
									.equals("Fim de curso de Mestrado simples")))
								{
									if (infoMasterDegreeProofVersion
										.getFinalResult()
										.getValue()
										!= 3)
									{
										session.setAttribute(
											SessionConstants
												.FINAL_RESULT_DEGREE_SIMPLE,
											certificate.toUpperCase());
										session.setAttribute(
											SessionConstants
												.MASTER_DEGREE_THESIS_HISTORY,
											infoMasterDegreeProofVersion);
										String dataFinal =
											DateFormat
												.getDateInstance()
...
//												.getFinalResult()
									...
									else
									{
										...
										errors.add(
											"AlunoNãoExiste",
											new ActionError("error.invalidStudentConclusion"));
										saveErrors(request, errors);
										return new ActionForward(
											mapping.getInput());
									}
