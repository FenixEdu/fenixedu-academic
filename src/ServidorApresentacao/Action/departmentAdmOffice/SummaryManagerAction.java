 ...
 */
package ServidorApresentacao.Action.departmentAdmOffice;
...
public class SummaryManagerAction extends FenixDispatchAction {
	...
	public ActionForward prepareInsertSummary(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (!loggedIsResponsible) {
				...
				.executeService(userView, "ReadProfessorshipByTeacherIDandExecutionCourseIDDepartment",
	...
	private InfoSummary buildSummaryToInsert(HttpServletRequest request) {
		...
		//Summary's date
		if(request.getParameter("summaryDateInput") != null && request.getParameter("summaryDateInput").length() > 0){
