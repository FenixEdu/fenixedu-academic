...
public class EditReimbursementGuideSituationDispatchAction extends FenixDispatchAction
{
		...
		throws FenixActionException
	{
		...
		request.setAttribute(
			SessionConstants.REIMBURSEMENT_GUIDE_STATES_LIST,
			ReimbursementGuideState.getEnumList());
		...
		throws FenixActionException
	{
		...
		Integer officialDateMonth =
			(Integer) editReimbursementGuideSituationForm.get("officialDateMonth");
		Integer officialDateYear = (Integer) editReimbursementGuideSituationForm.get("officialDateYear");

		Date officialDate = null;

		if ((officialDateDay.intValue() > 0)
			&& (officialDateMonth.intValue() > 0)
			&& (officialDateYear.intValue() > 0))
		{
			Calendar officialDateCalendar =
				new GregorianCalendar(
					officialDateYear.intValue(),
					officialDateMonth.intValue(),
