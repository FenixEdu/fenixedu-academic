...
public class MasterDegreeThesisOperations extends DispatchAction
{
    ...
    public boolean getStudentByNumberAndDegreeType(
        ActionForm form,
        ...
        throws FenixActionException
    {
        ...
        {
            actionErrors.add(
                "error.no.student.in.database",
                new ActionError("error.no.student.in.database", studentNumber.toString()));
            result = false;
        } else
        {
        ...
        throws FenixActionException
    {
        ...
        for (int i = 0; i < teachersNumbersList.length; i++)
        {
            ...
            if (infoTeacher == null)
                actionErrors.add(
                    "error.no.teacher.in.database",
                    new ActionError("error.no.teacher.in.database", teacherNumber.toString()));
    ...
    public void getExternalPersonsByName(
        ActionForm form,
        HttpServletRequest request,
        String sessionConstant,
        ActionErrors actionErrors)
        throws FenixActionException
    {
        ...
        String externalAssistentGuiderName =
            (String) masterDegreeThesisForm.get("externalAssistentGuiderName");
        ...
        try
        {
            if (!externalAssistentGuiderName.equals(""))
                infoExternalPersonsList =
                    (ArrayList) ServiceUtils.executeService(
                        userView,
                        "SearchExternalPersonsByName",
                        args);
        } catch (FenixServiceException e)
        {
            throw new FenixActionException(e);
        ...
        request.setAttribute(
            SessionConstants.EXTERNAL_ASSISTENT_GUIDERS_SEARCH_RESULTS_FLAG,
            new Boolean(true));
        ...
        throws FenixActionException
    {
        ...
        Integer[] externalPersonsIDsList =
            (Integer[]) masterDegreeThesisForm.get(externalPersonsIDsListField);
        ...
        for (int i = 0; i < externalPersonsIDsList.length; i++)
        {
            ...
            {
                infoExternalPerson =
                    (InfoExternalPerson) ServiceUtils.executeService(
                        userView,
                        "ReadExternalPersonByID",
                        args);
            } catch (NonExistingServiceException e)
            {
                actionErrors.add(
                    e.getMessage(),
                    new ActionError(e.getMessage(), externalPersonID.toString()));
                continue;
            } catch (FenixServiceException e)
            {
                throw new FenixActionException(e);
