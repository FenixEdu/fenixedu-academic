 ...
 */
public class DownloadStudentsWithAttendsByExecutionCourseListAction
	extends FenixAction
{
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        String checkedCoursesIds[] = (String[]) request.getParameterValues("coursesIDs");
        String enrollmentType[] = (String[]) request.getParameterValues("enrollmentType");
        String checkedShiftIds[] = (String[]) request.getParameterValues("shiftIDs");
        ...
        while (attendsIterator.hasNext()){
            InfoCompositionOfAttendAndDegreeCurricularPlanAndShiftsAndStudentGroups attendacy = (InfoCompositionOfAttendAndDegreeCurricularPlanAndShiftsAndStudentGroups) attendsIterator.next();
            ...
            fileContents += (attendacy.getNumberOfEnrollments().intValue() == 0 ? NULL : attendacy.getNumberOfEnrollments().toString()) + SEPARATOR;
            
            // attendacy type (normal, improvement, not enrolled)
            String attendacyType = "";
            if (infoAttends.getInfoEnrolment() != null) {
                if(infoAttends.getInfoEnrolment().getEnrolmentEvaluationType().equals(EnrolmentEvaluationType.NORMAL_OBJ)){
                    attendacyType = ATTENDACY_TYPE_NORMAL;
                }
                else if(infoAttends.getInfoEnrolment().getEnrolmentEvaluationType().equals(EnrolmentEvaluationType.IMPROVEMENT_OBJ)){
        ...
        try {
            ...
            fileName.append(infoDTO.getInfoExecutionCourse().getSigla()).append("_").append(now.get(Calendar.DAY_OF_MONTH));
