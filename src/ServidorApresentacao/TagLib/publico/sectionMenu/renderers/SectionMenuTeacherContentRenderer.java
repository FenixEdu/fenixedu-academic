 ...
 * 
 */
public class SectionMenuTeacherContentRenderer extends TagSupport implements ISectionMenuSlotContentRenderer {
	...
	private StringBuffer renderDepthContent(InfoSection infoSection, int i, String path, boolean hasChilds) {
		...
		if (depth == 0) {
			...
			strBuffer.append(renderDepthIdent(getInfoSection()));
			strBuffer.append(
				"<a href=\"sectionManagement.do?method=viewSection&objectCode="
					+ getInfoSection().getInfoSite().getInfoExecutionCourse().getIdInternal()
					+ "&currentSectionCode="
					+ getInfoSection().getIdInternal()
					+ "\">");
