...
<p>Podem candidatar-se a um Mestrado de 2º ciclo do <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%>, ou a um 2º ciclo de um Mestrado Integrado do <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%>, os estudantes que estejam nas seguintes condições:</p>

<ul>
	<li>tenham terminado no <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%> uma Licenciatura de 1º ciclo, ou o 1º ciclo de um Mestrado Integrado, sem <a href="<%= org.fenixedu.academic.domain.Installation.getInstance().getInstituitionURL() %>files/ensino/coerencias_cientificas_0910.pdf">coerência científica</a> com o curso de 2º ciclo a que se candidatam;</li>
...
<p><a href="<%= org.fenixedu.academic.domain.Installation.getInstance().getInstituitionURL() %>files/ensino/coerencias_cientificas_0910.pdf">Tabela de Coerência Científica entre cursos do <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%></a></p>

<h2 id="vag">Vagas 2º ciclo</h2>

	<table class="tab_lay" width="70%" cellspacing="0" summary="Informações sobre os mestrados (2º ciclo), no âmbito de Bolonha, disponíveis no <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%>">
	<tr>	
		<th>Curso</th>
...
<p>Os candidatos a um Mestrado de 2º ciclo do <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%>, ou a um 2º ciclo de um Mestrado Integrado do <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%>, serão seriados pela coordenação do curso a que se candidatam tendo em conta os seguintes critérios:</p>
<ol>
...
<h3>Q1: Sou finalista do 1º ciclo de um curso de Licenciatura (ou Mestrado Integrado) no <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%>. Terei que fazer alguma coisa para prosseguir os estudos no 2º ciclo do mesmo curso?</h3>
<p>A1: Não, nestes casos a transição para o 2º ciclo é automática.</p>

<h3>Q2: Sou finalista do 1º ciclo de um curso de Licenciatura (ou Mestrado Integrado) no <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%>. Em que condições a mudança para outro 2º ciclo diferente é automática?</h3>
<p>A2: É sempre possível mudar de curso e prosseguir estudos noutra área após completar o 1º ciclo desde que esteja garantida a coerência científica entre os dois cursos do <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%>. Esta coerência verifica-se sempre que as competências de formação do 1º ciclo respeitem as necessidades de formação para ingresso no 2º ciclo. A <a href="<%= org.fenixedu.academic.domain.Installation.getInstance().getInstituitionURL() %>files/ensino/coerencias_cientificas_0910.pdf">Tabela de Coerência Científica entre cursos do <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%></a> mostra-te em que situações a mudança para um 2º ciclo diferente é automática.</p>

<h3>Q3: Sou finalista do 1º ciclo de um curso de Licenciatura (ou Mestrado Integrado) no <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%>. O que devo fazer se pretender prosseguir os estudos num 2º ciclo do <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%> sem coerência científica com o curso de 1º ciclo?</h3>
<p>A3: Deves submeter um processo de candidatura ao 2º ciclo de acordo com os prazos e regulamento em vigor, sendo o processo analisado juntamente com as candidaturas de alunos externos que pretendem ingressar num curso de 2º ciclo do <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%>.</p>

<h3>Q4: Sou titular de uma licenciatura pré-bolonha do <%=org.fenixedu.academic.domain.organizationalStructure.Unit.getInstitutionAcronym()%>. Posso candidatar-me a um mestrado de 2º ciclo, na mesma área de formação?</h3>
<p>A4: Sim. </p>
