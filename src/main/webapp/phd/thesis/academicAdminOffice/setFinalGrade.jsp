...
<%-- ### Title #### --%>
<h2><bean:message key="label.phd.thesis.set.final.grade" bundle="PHD_RESOURCES" /></h2>
<%-- ### End of Title ### --%>
...
<fr:form action="<%= "/phdThesisProcess.do?processId=" + processId.toString() %>" encoding="multipart/form-data">
	...
	<fr:edit id="thesisProcessBean.edit" name="thesisProcessBean">
		<fr:schema bundle="PHD_RESOURCES" type="<%= PhdThesisProcessBean.class.getName() %>">
			...
			<fr:slot name="finalGrade" required="true" />
		</fr:schema>
	...
	<fr:edit id="thesisProcessBean.edit.documents" name="thesisProcessBean" property="documents">
		...
		<fr:destination name="invalid" path="<%= "/phdThesisProcess.do?method=setFinalGradeInvalid&processId=" + processId.toString() %>" />

	</fr:edit>

	<html:submit bundle="HTMLALT_RESOURCES" altKey="submit.submit" onclick="this.form.method.value='setFinalGrade';"><bean:message bundle="PHD_RESOURCES" key="label.submit"/></html:submit>
