...
public class InjectCode extends Task {
    ...
    public void execute() {
        ...
        for (String file : files) {
            if (this.isClassFile(file)) {
                ...
                String nextClass = file.replaceAll(separator, ".").substring(0, file.lastIndexOf("."));
                try {
                    boolean changed =
                            injector.perform(nextClass, targetClassesDir.getAbsolutePath(), this.producerClassByName(),
                ...
                if (this.verbose) {
                    System.out.printf("Skipped %s as it is not a class file (does not end in .class)", file);
                }
            }
        }
        long after = System.currentTimeMillis();
        System.out.printf("\nProcessed %d file%s. %d class%s re-written (%s).\nDone.", processedFiles, processedFiles == 0
                || processedFiles > 1 ? "s" : "", changedFiles, changedFiles == 0 || changedFiles > 1 ? "es were" : " was",
                this.calcHMS((after - before) / 1000));
    }
    ...
    private String[] includedFiles() {
        ...
        for (int i = 0; i < filesets.size(); i++) {
            ...
            for (String fileSetFile : fileSetFiles) {
                fileNamesVector.add(fileSetFile);
            }
        }
