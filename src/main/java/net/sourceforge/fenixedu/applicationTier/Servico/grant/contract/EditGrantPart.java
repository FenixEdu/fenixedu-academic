...
public class EditGrantPart extends FenixService {
    ...
    public static void run(InfoGrantPart infoGrantPart) throws FenixServiceException {
        ...
        final GrantPaymentEntity grantPaymentEntity =
                rootDomainObject.readGrantPaymentEntityByOID(infoGrantPart.getInfoGrantPaymentEntity().getIdInternal());
        if (grantPaymentEntity == null) {
            throw new InvalidGrantPaymentEntityException();
        }
        grantPart.setGrantPaymentEntity(grantPaymentEntity);

        final GrantSubsidy grantSubsidy =
                rootDomainObject.readGrantSubsidyByOID(infoGrantPart.getInfoGrantSubsidy().getIdInternal());
