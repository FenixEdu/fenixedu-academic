...
/**
 * This exception is the top level exceptions for all exceptions related to a
 * group expression. This can include problems in the compilation of the
 * expression or problems during the execution of the expression due to it's
 * dynamic nature.
 * 
 * <p>
 * This exception allows access to the especific message about what is wrong with the expression and in some cases it gives
 * information about the line and column were is problem occured. The line and column information can even be given as a range,
 * that is, the error is reported for a region of an expression.
 * 
 * @author cfgi
 */
public class GroupExpressionException extends DomainException {
    ...
    /**
     * Indicates the the value of {@link DomainException#getKey()} should be
     * threated as the whole message and not as a key to the resources.
     * 
     * <p>
     * This is needed because some parser messages are not localized and as such are reported directly in it's original form.
     * 
    ...
    /**
     * @return <code>true</code> if there is an end line and end column point
     *         available
     */
