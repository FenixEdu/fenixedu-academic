...
public class ExamsMapRenderer implements IExamsMapRenderer {
    ...
    private void renderExecutionCourseListForYear(StringBuilder strBuffer, Integer curricularYear, PageContext pageContext) {
        if (!examsMap.hasExecutionCoursesForGivenCurricularYear(curricularYear)) {
            strBuffer.append("<i>").append(getMessageResource(pageContext, "no.execution.courses.for.curricular.year"))
    ...
    private void renderExamsForRowOfDays(StringBuilder strBuffer, int week, Integer year1, Integer year2, PageContext pageContext) {
        for (int slot = 0; slot < daysOfWeek.length; slot++) {
            ...
            strBuffer.append(examsMapSlotContentRenderer.renderDayContents(
                    (ExamsMapSlot) examsMap.getDays().get(week * daysOfWeek.length + slot), year1, year2, user, pageContext));
            strBuffer.append("</td>");
        }
    }
    ...
    private void renderHeader(StringBuilder strBuffer, PageContext pageContext) {
        String makeLocale =
                getMessageResource(pageContext, "public.degree.information.label.monday") + ","
    ...
    private void renderExamsExecutionCourseTableForYear(StringBuilder strBuffer, Integer year, PageContext pageContext) {
        ...
        if (!examsMap.getExecutionCourses().isEmpty()) {
            strBuffer.append(" - <strong>"
                    + ((InfoExecutionCourse) examsMap.getExecutionCourses().iterator().next()).getInfoExecutionPeriod()
                            .getSemester() + "&ordm; ");
