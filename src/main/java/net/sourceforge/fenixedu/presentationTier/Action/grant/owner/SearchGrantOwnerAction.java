...
@Mapping(module = "facultyAdmOffice", path = "/searchGrantOwner", input = "/searchGrantOwner.do?page=0&method=searchForm",
        attribute = "searchGrantOwnerForm", formBean = "searchGrantOwnerForm", scope = "request", parameter = "method",
        customMappingClass = net.sourceforge.fenixedu.presentationTier.mapping.framework.SearchActionMapping.class,
        customMappingProperties = { "serviceName", "SearchGrantOwner", "objectAttribute", "infoGrantOwner", "listAttribute",
                "infoGrantOwnerList", "notFoundMessageKey", "errors.person.not.found" })
@Forwards(value = {
        @Forward(name = "list-many", path = "/facultyAdmOffice/grant/owner/searchGrantOwnerResultForm.jsp",
                tileProperties = @Tile(title = "private.teachingstaffandresearcher.listings.byinsurance")),
        @Forward(name = "search-form", path = "/facultyAdmOffice/grant/owner/searchGrantOwnerForm.jsp", tileProperties = @Tile(
