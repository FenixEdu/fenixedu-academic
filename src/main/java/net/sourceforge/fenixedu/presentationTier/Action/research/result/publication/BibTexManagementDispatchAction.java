...
public class BibTexManagementDispatchAction extends FenixDispatchAction {
        ...
        for (BibtexEntry entry : entryList) {
            try {
                ...
                if (bibtexAuthors != null) {
                    List<BibtexParticipatorBean> authors =
                            getParticipators(request, bibtexAuthors, ResultParticipationRole.Author);
                ...
                if (bibtexEditors != null) {
                    List<BibtexParticipatorBean> editors =
                            getParticipators(request, bibtexEditors, ResultParticipationRole.Editor);
        ...
        if (eventBean != null) {
            ...
            if (eventBean.getEventAlreadyChosen()) {
                try {
                    ...
                    EventEdition edition = CreateResearchEventEdition.run(eventBean);
    ...
    public ActionForward editParticipation(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        ImportBibtexBean importBibtexBean =
                (ImportBibtexBean) RenderUtils.getViewState("importBibtexBean" + index).getMetaObject().getObject();
