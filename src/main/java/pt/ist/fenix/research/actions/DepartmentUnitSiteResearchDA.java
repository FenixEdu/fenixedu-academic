...
import net.sourceforge.fenixedu.presentationTier.Action.publico.department.PublicDepartmentSiteDA;
...
@Mapping(module = "publico", path = "/department/departmentSiteResearch")
@Forwards({ @Forward(name = "showPublications", path = "/publico/department/showPublications.jsp", tileProperties = @Tile(
        extend = "definition.public.department.site")) })
public class DepartmentUnitSiteResearchDA extends PublicDepartmentSiteDA {
    ...
    protected void prepareResearchersForResponse(HttpServletRequest request, Unit unit) {
        putResearchersOnRequest(request, unit, true);
    }
