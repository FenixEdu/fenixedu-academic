...
public class DomainModelInfo {
	...
	this.domainModel = FenixFramework.getDomainModel();
    }

    public boolean isDomainBaseClass(String name) {
	return ((domainModel != null) && name.endsWith("_Base") && (domainModel.findClass(name.substring(0, name.length() - 5)) != null));
    }

    public boolean isDomainNonBaseClass(String name) {
	return ((domainModel != null) && (domainModel.findClass(name) != null));
    }
