...
public class ResponseCondition extends FenixUtil {
    ...
    public static List<LabelValueBean> getConditionSignalsToStringQuestion() {
        ...
        signalsList.add(new LabelValueBean(VAREQUAL_STRING, getConditionCode(VAREQUAL_XML_STRING).toString()));
        signalsList.add(new LabelValueBean(NOTVAREQUAL_STRING, getConditionCode(NOTVAREQUAL_XML_STRING).toString()));
        signalsList.add(new LabelValueBean(VARSUBSTRING_STRING, getConditionCode(VARSUBSTRING_XML_STRING).toString()));
        signalsList.add(new LabelValueBean(NOTVARSUBSTRING_STRING, getConditionCode(NOTVARSUBSTRING_XML_STRING).toString()));
        return signalsList;
    }

    public static List<LabelValueBean> getConditionSignalsToNumericalQuestion() {
        List<LabelValueBean> signalsList = new ArrayList<LabelValueBean>();
        signalsList.add(new LabelValueBean(VAREQUAL_SIGNAL, getConditionCode(VAREQUAL_XML_STRING).toString()));
        signalsList.add(new LabelValueBean(NOTVAREQUAL_SIGNAL, getConditionCode(NOTVAREQUAL_XML_STRING).toString()));
        signalsList.add(new LabelValueBean(VARLT_SIGNAL, getConditionCode(VARLT_XML_STRING).toString()));
        signalsList.add(new LabelValueBean(VARLTE_SIGNAL, getConditionCode(VARLTE_XML_STRING).toString()));
        signalsList.add(new LabelValueBean(VARGT_SIGNAL, getConditionCode(VARGT_XML_STRING).toString()));
        signalsList.add(new LabelValueBean(VARGTE_SIGNAL, getConditionCode(VARGTE_XML_STRING).toString()));
        return signalsList;
    }
    ...
    public boolean isCorrectNUM(Double userResponse) {
        ...
        if (getCondition().intValue() == VAREQUAL) {
            ...
            return ((userResponse.compareTo(response) < 0) ? true : false);
        } else if (getCondition().intValue() == ResponseCondition.VARLTE || getCondition().intValue() == NOTVARGT) {
            return ((userResponse.compareTo(response) <= 0) ? true : false);
        } else if (getCondition().intValue() == ResponseCondition.VARGT || getCondition().intValue() == NOTVARLTE) {
            return ((userResponse.compareTo(response) > 0) ? true : false);
