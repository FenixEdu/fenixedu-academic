...
public class PermissionGroup extends CustomGroup {
    ...
    public Stream<User> getDirectMembers(){
        ...
        AccessControlPermission.findByCode(code).getProfileSet().stream().forEach(profile -> members.addAll(profile.getFenixMemberSet()));
    ...
    public PersistentPermissionGroup toPersistentGroup() {
        return PersistentPermissionGroup.getInstance(this.code)
                .orElseGet(() -> new PersistentPermissionGroup(this.code));
    }
