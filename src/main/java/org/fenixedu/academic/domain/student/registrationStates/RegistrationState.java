...
//    }
    ...
    @Deprecated
    private static RegistrationState createState(Registration registration, Person person, DateTime dateTime,
            RegistrationStateTypeEnum stateTypeEnum) {

        final RegistrationState newState = new RegistrationState();
        newState.init(registration, person, dateTime);
        final RegistrationStateType stateType = RegistrationStateType.findByCode(stateTypeEnum.getName()).orElseThrow();
        newState.setType(stateType);
...
//    public abstract RegistrationStateType getStateType();
    ...
    public RegistrationStateTypeEnum getStateTypeEnum() {
        return getType() != null ? getType().getTypeEnum() : null;
    }
