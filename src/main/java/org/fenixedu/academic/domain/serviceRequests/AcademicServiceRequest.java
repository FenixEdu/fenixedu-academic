...
abstract public class AcademicServiceRequest extends AcademicServiceRequest_Base implements IAcademicServiceRequest {
	...
	protected void init(final AcademicServiceRequestCreateBean bean, AdministrativeOffice office) {
		...
		super.setAcademicServiceRequestYear(
				AcademicServiceRequestYear.readByYear(bean.getRequestDate().year().get(), true));
		...
		if (bean.getHasCycleTypeDependency()) {
			if (bean.getRequestedCycle() == null) {
				...
				throw new DomainException(
						"error.registryDiploma.requestedCycleTypeIsNotAllowedForGivenStudentCurricularPlan");
			}
			super.setRequestedCycle(bean.getRequestedCycle());
		...
		final AcademicServiceRequestBean situationBean = new AcademicServiceRequestBean(
				AcademicServiceRequestSituationType.NEW, AccessControl.getPerson());
	...
	protected void checkRules() {
		...
		if (getRootDomainObject().getAcademicServiceRequestsSet().stream()
			...
			throw new DomainException(
					"error.serviceRequests.AcademicServiceRequest.duplicate.serviceRequestNumberYear");
		}
	}
	...
	protected boolean isPayed() {
		...
		final IAcademicTreasuryEvent event = TreasuryBridgeAPIFactory.implementation()
				.academicTreasuryEventForAcademicServiceRequest(this);
	...
	public boolean isPaymentsAccessible() {
		...
		return programs.contains(getAcademicProgram()) && TreasuryBridgeAPIFactory.implementation()
				.academicTreasuryEventForAcademicServiceRequest(this) != null;
	}
	...
	public String getPaymentURL() {
		final IAcademicServiceRequestAndAcademicTaxTreasuryEvent event = TreasuryBridgeAPIFactory.implementation()
				.academicTreasuryEventForAcademicServiceRequest(this);
	...
	public boolean isRegistrationAccessible() {
		...
		Set<AcademicProgram> programs = AcademicAccessRule
				...
				.collect(Collectors.toSet());
		programs.addAll(PermissionService.getDegrees("MANAGE_REGISTRATIONS", Authenticate.getUser()));
	...
	protected void disconnect() {
		for (; !getAcademicServiceRequestSituationsSet().isEmpty(); getAcademicServiceRequestSituationsSet().iterator()
				.next().delete()) {
			;
		}
	...
	public void setAdministrativeOffice(AdministrativeOffice administrativeOffice) {
		throw new DomainException(
				"error.serviceRequests.RegistrationAcademicServiceRequest.cannot.modify.administrativeOffice");
	}
	...
	public void edit(final AcademicServiceRequestBean academicServiceRequestBean) {
		...
		if (getAcademicServiceRequestSituationType() != academicServiceRequestBean
	...
	protected boolean hasMissingPersonalInfo() {
		return Strings.isNullOrEmpty(getPerson().getName()) || (getPerson().getDateOfBirthYearMonthDay() == null)
				|| Strings.isNullOrEmpty(getPerson().getDocumentIdNumber())
	...
	protected void internalRevertToProcessingState() {
		...
		if (activeSituation == null || activeSituation.isProcessing() || activeSituation.isNew()) {
			...
			throw new DomainException(
					"error.serviceRequests.AcademicServiceRequest.cancelled.requests.cannot.be.reverted");
		}

		if (getAcademicServiceRequestSituationsSet().size() <= 1) {
			throw new DomainException(
					"error.serviceRequests.AcademicServiceRequest.revert.is.requires.more.than.one.state");
		}
	...
	public static Set<AcademicServiceRequest> getAcademicServiceRequests(Person person, Integer year,
		...
		Set<AcademicProgram> programs = AcademicAccessRule
				...
				.collect(Collectors.toSet());
		programs.addAll(PermissionService.getDegrees("SERVICE_REQUESTS", person.getUser()));
