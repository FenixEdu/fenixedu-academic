...
public class DegreeFinalizationCertificate extends AdministrativeOfficeDocument {
    ...
    private JsonObject getToday() {
        LocalDate now = new LocalDate();
        JsonObject today = new JsonObject();
        String dateDD = now.toString("dd", getLocale());
        String dateMMMM = now.toString("MMMM", getLocale());
        String dateYYYY = now.toString("yyyy", getLocale());

        today.addProperty("day", dateDD);
        today.addProperty("month", dateMMMM);
        today.addProperty("year", dateYYYY);
        return today;
    }
    ...
    private String getDegreeFinalizationDate() {
        ...
        if (!getDocumentRequest().mustHideConclusionDate()) {
            ...
            result.append(SINGLE_SPACE).append(getDocumentRequest().getConclusionDate().toString(DD_MMMM_YYYY, getLocale()));
        }
