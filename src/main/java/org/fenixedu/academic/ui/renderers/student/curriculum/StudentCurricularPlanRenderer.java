...
public class StudentCurricularPlanRenderer extends InputRenderer {
	...
	public static class StudentCurricularPlanLayout extends Layout {
		...
		public HtmlComponent createComponent(Object object, Class type) {
			...
			if (this.studentCurricularPlan == null) {
				container.addChild(
						createHtmlTextItalic(BundleUtil.getString(Bundle.STUDENT, "message.no.curricularplan")));
		...
		protected void generateRowsForExecutionYearsOrganization(HtmlTable mainTable) {
			...
			if (renderer.isToShowEnrolments()) {
				final Set<ExecutionInterval> enrolmentExecutionPeriods = new TreeSet<>(
						ExecutionInterval.COMPARATOR_BY_BEGIN_DATE);
				...
				for (final ExecutionInterval enrolmentsExecutionPeriod : enrolmentExecutionPeriods) {
					generateGroupRowWithText(mainTable, enrolmentsExecutionPeriod.getExecutionYear().getYear() + ", "
			...
			if (renderer.isToShowDismissals()) {
				...
				if (!dismissals.isEmpty()) {
					generateGroupRowWithText(mainTable, BundleUtil.getString(Bundle.STUDENT, "label.dismissals"), true,
							0, (CurriculumGroup) null);
		...
		protected void generateRowsForGroupsOrganization(final HtmlTable mainTable,
				final CurriculumGroup curriculumGroup, final int level) {

			generateGroupRowWithText(mainTable, curriculumGroup.getName().getContent(),
		...
		protected StringBuilder createGroupName(final String text, final CurriculumGroup curriculumGroup) {
			...
			if (curriculumGroup != null && curriculumGroup.getDegreeModule() != null) {
				...
				if (creditsLimit != null) {
					...
					groupName.append(
							BundleUtil.getString(Bundle.APPLICATION, "label.curriculum.credits.legend.minCredits"));
					groupName.append(" \">m(");
				...
				groupName.append(" <span title=\"");
				groupName.append(
						BundleUtil.getString(Bundle.APPLICATION, "label.curriculum.credits.legend.creditsConcluded"));
				groupName.append(" \"> c(");
				...
				if (isViewerAllowedToViewFullStudentCurriculum(studentCurricularPlan)) {
					groupName.append(" <span title=\"");
					groupName.append(BundleUtil.getString(Bundle.APPLICATION,
							"label.curriculum.credits.legend.approvedCredits"));
					groupName.append(" \">, ca(");
				...
				if (creditsLimit != null) {
					groupName.append("<span title=\"");
					groupName.append(
							BundleUtil.getString(Bundle.APPLICATION, "label.curriculum.credits.legend.maxCredits"));
					groupName.append("\">, M(");
				...
				if (isViewerAllowedToViewFullStudentCurriculum(studentCurricularPlan)
					...
					groupName.append("<em style=\"background-color:" + getBackgroundColor(value) + "; color:"
							+ getColor(value) + "\"");
					groupName.append("> ");
					...
					if (programConclusion != null && curriculumGroup == programConclusion
						...
						if (conclusionProcess != null) {
							conclusionText.append(
									" <em style=\"background-color:" + getBackgroundColor(ConclusionValue.CONCLUDED)
											+ "; color:" + getColor(ConclusionValue.CONCLUDED) + "\"");
							conclusionText.append(">");

							String conclusionDate = conclusionProcess.getConclusionDate().toString("yyyy-MM-dd");

							conclusionText.append(BundleUtil.getString(Bundle.APPLICATION,
									"label.curriculum.group.concluded", conclusionDate));
							conclusionText.append("</em>");
						} else {
							conclusionText.append(
									" <em style=\"background-color:" + getBackgroundColor(ConclusionValue.NOT_CONCLUDED)
											+ "; color:" + getColor(ConclusionValue.NOT_CONCLUDED) + "\"");
							conclusionText.append(">");

							conclusionText.append(
									BundleUtil.getString(Bundle.APPLICATION, "label.curriculum.group.not.concluded"));
							conclusionText.append("</em>");
						}

						final HtmlComponent programConclusionLink = createProgramConclusionLink(
								conclusionText.toString(), curriculumGroup.getRegistration(), programConclusion);
						try {
		...
		protected void generateDismissalRows(HtmlTable mainTable, List<Dismissal> dismissals, int level) {
			final Set<Dismissal> sortedDismissals = new TreeSet<Dismissal>(
					Dismissal.COMPARATOR_BY_EXECUTION_PERIOD_AND_NAME_AND_ID);
		...
		protected void generateDismissalGradeCell(HtmlTableRow dismissalRow, Dismissal dismissal) {
			final String gradeValue = !StringUtils.isEmpty(dismissal.getCredits().getGivenGrade())
		...
		protected void generateDismissalLabelCell(final HtmlTable mainTable, HtmlTableRow dismissalRow,
			...
			final HtmlText text = new HtmlText(BundleUtil.getString(Bundle.STUDENT,
					"label.dismissal." + dismissal.getCredits().getClass().getSimpleName()));
			...
			if (curricularCourse != null) {
				...
				ExecutionCourse executionCourse = dismissal.getCurricularCourse()
						.getExecutionCoursesByExecutionPeriod(dismissal.getExecutionInterval()).stream().findAny()
						.orElse(null);
		...
		protected void generateEnrolmentEvaluationRows(HtmlTable mainTable, EnrolmentEvaluation evaluation, int level) {
			...
			generateCellWithText(enrolmentRow, grade.isEmpty() ? EMPTY_INFO : grade.getValue(),
			...
			if (isViewerAllowedToViewFullStudentCurriculum(studentCurricularPlan)) {
				...
				if (lastEnrolmentEvaluation != null && lastEnrolmentEvaluation.getExamDateYearMonthDay() != null) {
					...
					generateCellWithSpan(enrolmentRow,
							lastEnrolmentEvaluation.getExamDateYearMonthDay().toString(DATE_FORMAT),
							BundleUtil.getString(Bundle.APPLICATION, "label.data.avaliacao"),
							renderer.getCreationDateCellClass());
				} else {
					generateCellWithText(enrolmentRow, EMPTY_INFO, renderer.getCreationDateCellClass());
		...
		protected void generateStatisticsLinkCell(final HtmlTableRow row, final Enrolment enrolment) {
			if (enrolment.getStudent() == AccessControl.getPerson().getStudent()
					&& enrolment.getRegistration().isActive()) {
		...
		protected void generateExecutionYearCell(HtmlTableRow row, final ICurriculumEntry entry) {
			generateCellWithText(row,
					entry.getExecutionInterval() != null ? entry.getExecutionYear().getYear() : EMPTY_INFO,
					renderer.getEnrolmentExecutionYearCellClass());
		}

		protected void generateEnrolmentLastEnrolmentEvaluationTypeCell(HtmlTableRow enrolmentRow,
				Enrolment enrolment) {
			final EnrolmentEvaluation lastEnrolmentEvaluation = enrolment.getFinalEnrolmentEvaluation();
		...
		protected void generateEnrolmentEctsCell(final HtmlTableRow enrolmentRow, final Enrolment enrolment,
			...
			final String ectsCredits = String
					.valueOf(isFromDetail ? enrolment.getEctsCreditsForCurriculum() : enrolment.getEctsCredits());
			generateCellWithText(enrolmentRow, ectsCredits, renderer.getEctsCreditsCellClass());
		}

		protected void generateEnrolmentWeightCell(HtmlTableRow enrolmentRow, IEnrolment enrolment,
				boolean isFromDetail) {
		...
		protected void generateEnrolmentStateCell(HtmlTableRow enrolmentRow, Enrolment enrolment) {
			generateCellWithText(enrolmentRow,
					enrolment.isApproved() ? EMPTY_INFO
		...
		protected void generateEnrolmentTypeCell(HtmlTableRow enrolmentRow, Enrolment enrolment) {
			generateCellWithText(enrolmentRow,
					enrolment.isEnrolmentTypeNormal() ? EMPTY_INFO
							: BundleUtil.getString(Bundle.ENUMERATION, enrolment.getEnrolmentTypeName()),
					renderer.getEnrolmentTypeCellClass());
		}
		...
		protected void generateCurricularCourseCodeAndNameCell(final HtmlTableRow enrolmentRow,
				final Enrolment enrolment, final int level, boolean allowSelection) {
		...
		protected String getPresentationNameFor(final Enrolment enrolment) {
			final String code = !StringUtils.isEmpty(enrolment.getCurricularCourse().getCode())
		...
		protected HtmlLink createExecutionCourseStatisticsLink(final String text,
		...
		protected void generateHeadersForGradeWeightAndEctsCredits(final HtmlTableRow groupRow) {
			generateCellWithText(groupRow, BundleUtil.getString(Bundle.APPLICATION, "label.grade"),
					renderer.getGradeCellClass());
	...
	public static boolean canGenerate(final CurriculumGroup curriculumGroup,
	...
	public static boolean isViewerAllowedToViewFullStudentCurriculum(
			final StudentCurricularPlan studentCurricularPlan) {
		...
		programs.addAll(PermissionService.getDegrees("VIEW_FULL_STUDENT_CURRICULUM", person.getUser()));
