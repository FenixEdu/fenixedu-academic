...
public class SetEnrolmentState
{
    ...
    public static void main(String[] args)
    {
        ...
        if ((enrolmentList != null) && (!enrolmentList.isEmpty()))
        {
            ...
            SetEnrolmentStateIO.selectNewStatesForListOfTemporarilyEnrolments(enrolmentList,
                    chooseExecutionPeriodsMode);
    ...
    private static List loadPersistentData()
    {
        ...
        try
        {
            //			IPersistentEnrolmentPeriod persistentEnrolmentPeriod =
            ...
            // getEnrolmentPeriod(persistentEnrolmentPeriod,
            ...
            //			
            //			enrolmentList =
            // persistentEnrolment.readByCriteria(enrolmentCriteria);

            if (chooseExecutionPeriodsMode)
            {
                enrolmentList = readAllEnrolmentsByStudentCurricularPlanAndExecutionPeriod(
                        persistentEnrolment, studentActiveCurricularPlan);
            }
            else
            {
                enrolmentList = persistentEnrolment
                        .readEnrolmentsByStudentCurricularPlanAndEnrolmentState(
                                studentActiveCurricularPlan, EnrolmentState.TEMPORARILY_ENROLED);
            }
        }
        catch (ExcepcaoPersistencia e)
        {
    ...
    private static void writePersistentData(List enrolmentList)
    {
        ...
        System.out.println();
        System.out
                .println("UPDATING ENROLMENT STATES FOR THE FOLLOWING CURRICULAR COURSES TO DATA BASE:");
        ...
        while (iterator.hasNext())
        {
            ...
            if (enrolment instanceof IEnrolmentInOptionalCurricularCourse)
            {
                ...
                System.out.println("CURRICULAR COURSE: ["
                        + enrolmentInOptionalCurricularCourse.getCurricularCourse().getName()
                        + " -> "
                        + enrolmentInOptionalCurricularCourse.getCurricularCourseForOption().getName()
                        + "] STATE: ["
                        + SetEnrolmentStateIO.showEnrolmentState(enrolmentInOptionalCurricularCourse
                                .getEnrolmentState()) + "]");
            }
            else
            {
                System.out.println("CURRICULAR COURSE: [" + enrolment.getCurricularCourse().getName()
                        + "] STATE: ["
                        + SetEnrolmentStateIO.showEnrolmentState(enrolment.getEnrolmentState()) + "]");
            }
            try
            {
    ...
    private static void updateDataBaseForNewEnrolmentPeriod()
    {
        ...
        try
        {
            ...
            IStudentCurricularPlan studentActiveCurricularPlan = persistentStudentCurricularPlan
                    .readActiveStudentCurricularPlan(student.getNumber(), student.getDegreeType());
            IEnrolmentPeriod enrolmentPeriod = getEnrolmentPeriod(persistentEnrolmentPeriod,
                    studentActiveCurricularPlan);
            ...
            List finalExecutionPeriodList = (List) CollectionUtils.select(executionPeriodList,
                    new Predicate()
                    {
            ...
            String executionPeriodName = executionPeriod.getExecutionYear().getYear() + " - "
                    + executionPeriod.getName();
            System.out.println();
            System.out.println("SETING NEW ENROLMENT PERIOD FOR: [" + executionPeriodName + "]");
            System.out.println("START DATE: [" + enrolmentPeriod.getStartDate().toString()
                    + "] END DATE: [" + enrolmentPeriod.getEndDate().toString() + "]");
    ...
    private static IEnrolmentPeriod getEnrolmentPeriod(IPersistentEnrolmentPeriod enrolmentPeriodDAO,
    ...
    private static List readAllEnrolmentsByStudentCurricularPlanAndExecutionPeriod(
            IPersistentEnrolment persistentEnrolment, IStudentCurricularPlan studentActiveCurricularPlan)
    {
        IPersistentExecutionPeriod persistentExecutionPeriod = persistentSupport
                .getIPersistentExecutionPeriod();
        List enrolmentList = null;
        try
        {
            ...
            IExecutionPeriod executionPeriod = SetEnrolmentStateIO
                    .selectExecutionPeriod(executionPeriodList);
            enrolmentList = persistentEnrolment
                    .readAllEnrolmentsByStudentCurricularPlanAndExecutionPeriod(
                            studentActiveCurricularPlan, executionPeriod);
        }
        catch (ExcepcaoPersistencia e)
        {
            e.printStackTrace();
