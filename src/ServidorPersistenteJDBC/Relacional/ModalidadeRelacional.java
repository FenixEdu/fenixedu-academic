...
public class ModalidadeRelacional implements IModalidadePersistente {
  ...
  public boolean alterarModalidade(Modalidade modalidade) {
    ...
    try {
      ...
      UtilRelacional.prepararComando("UPDATE ass_MODALIDADE SET " +
      "codigoInterno = ? , " +
      "designacao = ? " +
      "WHERE designacao = ? ");
  ...
  public boolean apagarModalidade(String designacao) {
    ...
    try {
      PreparedStatement sql =
      UtilRelacional.prepararComando("DELETE FROM ass_MODALIDADE WHERE designacao = ?");
  ...
  public boolean escreverModalidade(Modalidade modalidade) {
    ...
    try {
      PreparedStatement sql =
      UtilRelacional.prepararComando("INSERT INTO ass_MODALIDADE VALUES (?, ?)");
  ...
  public Modalidade lerModalidade(String designacao) {
    ...
    try {
      PreparedStatement sql =
      UtilRelacional.prepararComando("SELECT * FROM ass_MODALIDADE WHERE designacao = ?");
      
      sql.setString(1, designacao);
      
      ResultSet resultado = sql.executeQuery();
      if (resultado.next()) {
        modalidade = new Modalidade(resultado.getInt("codigoInterno"),
  ...
  public Modalidade lerModalidade(int codigoInterno) {
    ...
    try {
      PreparedStatement sql =
      UtilRelacional.prepararComando("SELECT * FROM ass_MODALIDADE WHERE codigoInterno = ?");
  ...
  public ArrayList lerModalidades() {
    ArrayList modalidades = null;
    
    try {
      PreparedStatement sql =
      UtilRelacional.prepararComando("SELECT * FROM ass_MODALIDADE");
