...
public class FuncionarioRelacionalOracle implements IFuncionarioPersistente {
		...
		try {
			...
			try {
				...
				if (resultado.next()) {
					...
					//dataInicio = java.sql.Date.valueOf(resultado.getString("ASS_EMPDTINI").substring(0, resultado.getString("ASS_EMPDTINI").indexOf(" ")));
	...
	public ArrayList lerStatusAssiduidade(int numMecanografico, Timestamp dataInicio, Timestamp dataFim) {
		//ORACLE: acede à BD do teleponto para carregar os dados dos funcionarios sobre o status de Assiduidade
		//Nota: a ligação é feita fora do serviço ServicoSeguroInicializarStatusAssiduidadeFuncionarios em LeituraStatusAssiduidadeFuncionario
		...
		try {
			...
			while (resultado.next()) {
				...
				dataInicioHistorico =
					java.sql.Date.valueOf(
						resultado.getString("ASS_HISEMP_DHINI").substring(0, resultado.getString("ASS_HISEMP_DHINI").indexOf(" ")));
				dataFimHistorico =
					java.sql.Date.valueOf(
						resultado.getString("ASS_HISEMP_DHFIM").substring(0, resultado.getString("ASS_HISEMP_DHFIM").indexOf(" ")));
				historicoStatus.add(
