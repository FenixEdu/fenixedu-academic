...
public class CentroCustoRelacional implements ICentroCustoPersistente {
	...
	public CentroCusto lerCentroCusto(String sigla) {
		...
		try {
			PreparedStatement sql = UtilRelacional.prepararComando("SELECT * FROM ass_CENTRO_CUSTO WHERE sigla = ?");
	...
	public CentroCusto lerCentroCusto(int codigoInterno) {
		...
		try {
			PreparedStatement sql = UtilRelacional.prepararComando("SELECT * FROM ass_CENTRO_CUSTO WHERE codigoInterno = ?");
	...
	public ArrayList lerTodosCentrosCusto(){
		...
		try{
		PreparedStatement sql = UtilRelacional.prepararComando("SELECT * FROM ass_CENTRO_CUSTO ORDER BY sigla");
		ResultSet resultadoQuery = sql.executeQuery();
		
		listaCentrosCusto = new ArrayList();
		while(resultadoQuery.next()){
			listaCentrosCusto.add(new CentroCusto(
