...
public class PessoaRelacional implements IPessoaPersistente {
    ...
    public boolean alterarPessoa(Pessoa pessoa) {
        ...
        try {
            PreparedStatement sql = UtilRelacional.prepararComando("UPDATE PERSON SET "
                    + "ID_INTERNAL = ? , " + "DOCUMENT_ID_NUMBER = ? , "
                    + "EMISSION_LOCATION_OF_DOCUMENT_ID = ? , " + "EMISSION_DATE_OF_DOCUMENT_ID = ? , "
                    + "EXPERATION_DATE_OF_DOCUMENT_ID = ? , " + "NAME = ? , " + "DATE_OF_BIRTH = ? , "
                    + "NAME_OF_FATHER = ? , " + "NAME_OF_MOTHER = ? , " + "NATIONALITY = ? , "
                    + "PARISH_OF_BIRTH = ? , " + "DISTRICT_SUBDIVISION_OF_BIRTH = ? , "
                    + "DISTRICT_OF_BIRTH = ? , " + "ADDRESS = ? , " + "AREA = ? , " + "AREA_CODE = ? , "
                    + "AREA_OF_AREA_CODE = ? , " + "PARISH_OF_RESIDENCE = ? , "
                    + "DISTRICT_SUBDIVISION_OF_RESIDENCE = ? , " + "DISTRICT_OF_RESIDENCE = ? , "
                    + "PHONE = ? , " + "MOBILE = ? , " + "EMAIL = ? , " + "WEB_ADRDRESS = ? , "
                    + "SOCIAL_SECURITY_NUMBER = ? , " + "PROFESSION = ? , " + "USERNAME = ? , "
                    + "PASSWD = ? , " + "KEY_COUNTRY = ? , " + "FISCAL_CODE = ? , "
                    + "TYPE_ID_DOCUMENT = ? , " + "SEX = ? , " + "MARITAL_STATUS = ? "
                    + "WHERE ID_INTERNAL = ? ");
            ...
            if (pessoa.getDataValidadeDocumentoIdentificacao() != null) {
                sql.setDate(5, new java.sql.Date(pessoa.getDataValidadeDocumentoIdentificacao()
                        .getTime()));
            } else {
                sql.setDate(5, null);
            }
    ...
    public boolean apagarPessoa(int codigoInterno) {
        ...
        try {
            PreparedStatement sql = UtilRelacional
                    .prepararComando("DELETE FROM PERSON WHERE ID_INTERNAL = ?");
    ...
    public boolean escreverPapelPessoa(Pessoa pessoa, int chaveRole) {
        ...
        try {
            PreparedStatement sql = UtilRelacional.prepararComando("INSERT INTO PERSON_ROLE "
                    + "VALUES (?, ?, ?, 1)");
    ...
    public boolean escreverPessoa(Pessoa pessoa) {
        ...
        try {
            PreparedStatement sql = UtilRelacional.prepararComando("INSERT INTO PERSON "
                    + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
                    + "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, MD5(?), ?, ?, ?, ?, ?, 1)");
    ...
    public Pessoa lerPessoa(int codigoInterno) {
        ...
        try {
            PreparedStatement sql = UtilRelacional
                    .prepararComando("SELECT * FROM PERSON WHERE ID_INTERNAL = ?");
    ...
    public Pessoa lerPessoa(String username) {
        ...
        try {
            PreparedStatement sql = UtilRelacional
                    .prepararComando("SELECT * FROM PERSON WHERE USERNAME = ?");
    ...
    public Pessoa lerPessoa(String numeroDocumentoIdentificacao, int tipoDocumentoIdentificacao) {
        ...
        try {
            PreparedStatement sql = UtilRelacional
                    .prepararComando("SELECT * FROM PERSON WHERE DOCUMENT_ID_NUMBER = ? "
                            + "AND TYPE_ID_DOCUMENT = ?");
    ...
    public List lerPessoasPorCargo(String cargo) {
        List listaPessoas = null;

        try {
            PreparedStatement sql = UtilRelacional
                    .prepararComando("SELECT * FROM ass_CARGO WHERE cargo = ?");
            sql.setString(1, cargo);
            ResultSet resultadoQuery = sql.executeQuery();

            if (resultadoQuery.next()) {
                PreparedStatement sql2 = UtilRelacional
                        .prepararComando("SELECT * FROM ass_PESSOA_CARGO WHERE chaveCargo = ?");
    ...
    public int ultimoCodigoInterno() {
        ...
        try {
            PreparedStatement sql = UtilRelacional
                    .prepararComando("SELECT max(ID_INTERNAL) FROM PERSON");
    ...
    private Pessoa constroiPessoa(ResultSet resultado) {
        ...
        try {
            ...
            if (resultado.getString("EMISSION_DATE_OF_DOCUMENT_ID") != null) {
                emissionDate = converteData(java.sql.Date.valueOf(resultado
                        .getString("EMISSION_DATE_OF_DOCUMENT_ID")));
            }
            Date experationDate = null;
            if (resultado.getString("EXPERATION_DATE_OF_DOCUMENT_ID") != null) {
                experationDate = converteData(java.sql.Date.valueOf(resultado
                        .getString("EXPERATION_DATE_OF_DOCUMENT_ID")));
            }
            Date birthDate = null;
            ...
            pessoa = new Pessoa(new Integer(resultado.getInt("ID_INTERNAL")), resultado
                    ...
                    resultado.getString("DISTRICT_SUBDIVISION_OF_BIRTH"), resultado
                            .getString("DISTRICT_OF_BIRTH"), resultado.getString("ADDRESS"), resultado
                            .getString("AREA"), resultado.getString("AREA_CODE"), resultado
                            .getString("AREA_OF_AREA_CODE"), resultado.getString("PARISH_OF_RESIDENCE"),
                    resultado.getString("DISTRICT_SUBDIVISION_OF_RESIDENCE"), resultado
                            .getString("DISTRICT_OF_RESIDENCE"), resultado.getString("PHONE"), resultado
                            .getString("MOBILE"), resultado.getString("EMAIL"), resultado
                            .getString("WEB_ADRDRESS"), resultado.getString("SOCIAL_SECURITY_NUMBER"),
                    resultado.getString("PROFESSION"), resultado.getString("USERNAME"), resultado
                            .getString("PASSWD"), resultado.getString("FISCAL_CODE"));
        } catch (Exception e) {
