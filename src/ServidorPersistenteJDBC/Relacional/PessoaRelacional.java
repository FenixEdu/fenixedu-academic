...
public class PessoaRelacional implements IPessoaPersistente {
	...
	public boolean alterarPessoa(Pessoa pessoa) {
		...
		try {
			...
			if (pessoa.getDataEmissaoDocumentoIdentificacao() != null) {
				sql.setDate(4, new java.sql.Date(pessoa.getDataEmissaoDocumentoIdentificacao().getTime()));
			} else {
				sql.setDate(4, null);
			}
			if (pessoa.getDataValidadeDocumentoIdentificacao() != null) {
				sql.setDate(5, new java.sql.Date(pessoa.getDataValidadeDocumentoIdentificacao().getTime()));
			} else {
				sql.setDate(5, null);
			}
	...
	public boolean apagarPessoa(int codigoInterno) {
		...
		try {
			PreparedStatement sql = UtilRelacional.prepararComando("DELETE FROM PERSON WHERE ID_INTERNAL = ?");
	...
	public boolean escreverPapelPessoa(Pessoa pessoa, int chaveRole) {
		...
		try {
			PreparedStatement sql = UtilRelacional.prepararComando("INSERT INTO PERSON_ROLE " + "VALUES (?, ?, ?)");
	...
	public Pessoa lerPessoa(int codigoInterno) {
		...
		try {
			PreparedStatement sql = UtilRelacional.prepararComando("SELECT * FROM PERSON WHERE ID_INTERNAL = ?");
	...
	public Pessoa lerPessoa(String username) {
		...
		try {
			PreparedStatement sql = UtilRelacional.prepararComando("SELECT * FROM PERSON WHERE USERNAME = ?");
	...
	public Pessoa lerPessoa(String numeroDocumentoIdentificacao, int tipoDocumentoIdentificacao) {
		...
		try {
			PreparedStatement sql =
				UtilRelacional.prepararComando("SELECT * FROM PERSON WHERE DOCUMENT_ID_NUMBER = ? " + "AND TYPE_ID_DOCUMENT = ?");
	...
	public ArrayList lerPessoasPorCargo(String cargo) {
		...
		try {
			PreparedStatement sql = UtilRelacional.prepararComando("SELECT * FROM ass_CARGO WHERE cargo = ?");
			sql.setString(1, cargo);
			ResultSet resultadoQuery = sql.executeQuery();

			if (resultadoQuery.next()) {
				PreparedStatement sql2 = UtilRelacional.prepararComando("SELECT * FROM ass_PESSOA_CARGO WHERE chaveCargo = ?");
	...
	public int ultimoCodigoInterno() {
		...
		try {
			PreparedStatement sql = UtilRelacional.prepararComando("SELECT max(ID_INTERNAL) FROM PERSON");
	...
	/** Formata uma Date para se utilizar uma sql.Date
	 * @return sql.Date
	 */
	private java.sql.Date dataParaSql(java.util.Date data) {
	...
	/** Formata uma sql.Date para se utilizar uma Date
	 * @return Date
	 */
	private java.util.Date sqlParaData(java.sql.Date dataSql) {
		java.util.Date data = null;

		data = new java.util.Date(dataSql.getTime());
		return data;
	}
