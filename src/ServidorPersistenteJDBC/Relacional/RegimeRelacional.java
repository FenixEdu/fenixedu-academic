...
public class RegimeRelacional implements IRegimePersistente {
  ...
  public boolean alterarRegime(Regime regime) {
    ...
    try {
      ...
      UtilRelacional.prepararComando("UPDATE ass_REGIME SET " +
      "codigoInterno = ? , " +
      "designacao = ? " +
      "WHERE designacao = ? ");
  ...
  public ArrayList lerDesignacaoRegimes(ArrayList listaIdRegimes) {
    ...
    try {
      while(iterLista.hasNext()){
        ...
        sql = UtilRelacional.prepararComando("SELECT designacao FROM ass_REGIME WHERE codigoInterno = ?");
  ...
  public Regime lerRegime(String designacao) {
    ...
    try {
      ...
      if (resultado.next()) {
        regime = new Regime(resultado.getInt("codigoInterno"),
  ...
  public ArrayList lerRegimes() {
    ...
    try {
      PreparedStatement sql =
      UtilRelacional.prepararComando("SELECT * FROM ass_REGIME");
