...
public class MarcacaoPontoRelacionalOracle implements IMarcacaoPontoPersistente {
	...
	+"ASS_MARWHEN = ?, " + "ASS_MARWHO = ? " + "WHERE ASS_MARSEQ = ? ");
				...
				System.out.println("MarcacaoPontoRelacionalOracle.alterarMarcacaoPonto: " + SQLe.toString());
				return false;
			}
	...
	+"(ASS_MARDHMARCA BETWEEN ? AND ?))");
				...
				SuportePersistenteOracle.getInstance().cancelarTransaccao();
				System.out.println("MarcacaoPontoRelacionalOracle.consultarMarcacoesPonto: " + SQLe.toString());
				return null;
			}
			SuportePersistenteOracle.getInstance().confirmarTransaccao();
		} catch (Exception e) {
	...
	public ArrayList lerMarcacoesPonto() {
		...
		try {
			...
			try {
				...
				SuportePersistenteOracle.getInstance().cancelarTransaccao();
				System.out.println("MarcacaoPontoRelacionalOracle.lerMarcacoesPonto: " + SQLe.toString());
				return null;
			}
			SuportePersistenteOracle.getInstance().confirmarTransaccao();
		} catch (Exception e) {
	...
	public ArrayList obterMarcacoesPonto(int numFuncionario, int numCartao, Timestamp dataFim) {
		...
		try {
			...
			try {
				...
				SuportePersistenteOracle.getInstance().cancelarTransaccao();
				System.out.println("MarcacaoPontoRelacionalOracle.obterMarcacoesPonto: " + SQLe.toString());
				return null;
			}
			SuportePersistenteOracle.getInstance().confirmarTransaccao();
		} catch (Exception e) {
	...
	public int ultimoCodigoInterno() {
		...
		try {
			SuportePersistenteOracle.getInstance().iniciarTransaccao();
			try {
			PreparedStatement sql = UtilRelacional.prepararComando("SELECT MAX(ASS_MARSEQ) FROM ASS_MARCAS");
			...
			sql.close();
				...
				SuportePersistenteOracle.getInstance().cancelarTransaccao();
				System.out.println("MarcacaoPontoRelacionalOracle.ultimoCodigoInterno: " + SQLe.toString());
				return 0;
			}
			SuportePersistenteOracle.getInstance().confirmarTransaccao();
		} catch (Exception e) {
