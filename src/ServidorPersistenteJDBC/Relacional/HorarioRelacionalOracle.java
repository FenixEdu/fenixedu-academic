	...
	public ArrayList lerExcepcoesHorarioPorNumMecanografico(int numMecanografico) {
		...
		try {
			...
			try {
				...
				sql = UtilRelacionalOracle.prepararComando("SELECT * FROM ASS_EXCHDIA WHERE ASS_EXCHDIAPESSOA = ? ");
				sql.setInt(1, numMecanografico);
				resultadoQuery = sql.executeQuery();

				// chaves dos horarios dia das excepções de horário			
				PreparedStatement sqlChaveHorario =
					UtilRelacional.prepararComando("SELECT codigoInterno FROM ass_HORARIO_TIPO " + "WHERE sigla = ?");
			...
			try {
				...
				sql =
					UtilRelacionalOracle.prepararComando(
						"SELECT * FROM ASS_HISEMPREG WHERE EMP_NUM = ? " + "AND ASS_HISEMP_TIPO = 12 ORDER BY ASS_HISEMP_DHINI");
				sql.setInt(1, numMecanografico);
				resultadoQuery = sql.executeQuery();
				...
				while (resultadoQuery.next()) {
					siglaHorario =
						resultadoQuery.getString("ASS_HISEMP_CHOSE").substring(0, resultadoQuery.getString("ASS_HISEMP_CHOSE").indexOf("#"));
					dataInicioHorario =
						java.sql.Date.valueOf(
							resultadoQuery.getString("ASS_HISEMP_DHINI").substring(0, resultadoQuery.getString("ASS_HISEMP_DHINI").indexOf(" ")));
					dataFimHorario =
						java.sql.Date.valueOf(
							resultadoQuery.getString("ASS_HISEMP_DHFIM").substring(0, resultadoQuery.getString("ASS_HISEMP_DHFIM").indexOf(" ")));
			...
			try {
				...
				PreparedStatement sql =
					UtilRelacional.prepararComando(
						"SELECT codigoInterno FROM funcionario WHERE numeroMecanografico = ?");
				sql.setInt(1, numMecanografico);
				ResultSet resultadoQuery = sql.executeQuery();
				int chaveFuncionario = 0;
				if (resultadoQuery.next()) {
				...
				PreparedStatement sqlChaveHorario =
					UtilRelacional.prepararComando(
						"SELECT codigoInterno FROM horario_tipo " + "WHERE sigla = ?");
				ResultSet resultadoChaveHorario = null;
				...
				ListIterator iteradorExcepcoes = null;
				...
				while (resultadoQuery.next()) {
					...
					if (siglaHorario != null) {
						...
						// horários excepções deste horário
						listaExcepcoes =
							lerExcepcoesHorarioPorNumMecanografico(
								numMecanografico,
								dataInicioHorario,
								dataFimHorario);
						if (listaExcepcoes != null) {
							iteradorExcepcoes = listaExcepcoes.listIterator();
							while (iteradorExcepcoes.hasNext()) {
								excepcao = (Horario)iteradorExcepcoes.next();
	
								if (excepcao.getDataInicio() != dataInicioHorario) {
	
									iterador = listaHorarios.listIterator();
									while (iterador.hasNext()) {
										horario = (Horario) ((Horario)iterador.next()).clone();
										horario.setDataInicio(dataInicioHorario);
	
										//dia anterior ao inicio da excepaco
										calendario.clear();
										calendario.setTime(excepcao.getDataInicio());
										calendario.add(Calendar.DAY_OF_MONTH, -1);
	
										horario.setDataFim(calendario.getTime());
										historicoHorario.add(horario);
									}
								}
	
								//dia seguinte ao fim da excepaco
								calendario.clear();
	...
	public ArrayList lerHorarioActualPorNumMecanografico(int numMecanografico) {
		...
		try {
			...
			try {
				...
				sql =
					UtilRelacionalOracle.prepararComando(
						"SELECT ASS_EMPHT, ASS_EMPSTATUS, ASS_EMPDTINI FROM ASS_EMPREG " + "WHERE ASS_EMPPESSOA = ?");
				sql.setInt(1, numMecanografico);
				...
				sql =
					UtilRelacionalOracle.prepararComando(
						"SELECT MAX(ASS_HISEMP_DHFIM) FROM ASS_HISEMPREG " + "WHERE EMP_NUM = ? AND ASS_HISEMP_TIPO = 12");
				sql.setInt(1, numMecanografico);
				resultado = sql.executeQuery();
				if (resultado.next()) {
					if (resultado.getString("MAX(ASS_HISEMP_DHFIM)") != null) {
						...
						calendario.setTime(
							java.sql.Date.valueOf(
								resultado.getString("MAX(ASS_HISEMP_DHFIM)").substring(0, resultado.getString("MAX(ASS_HISEMP_DHFIM)").indexOf(" "))));
						calendario.add(Calendar.DAY_OF_MONTH, +1);
				...
				Date dataFimHorario = null;
				if ((status == 1) || (status == 3) || ((status == 4) || (status == 5) || (status == 9) || (status == 12))) {
					sql =
						UtilRelacionalOracle.prepararComando(
		...
		try {
			...
			try {
				...
				if (resultado.next()) {
					...
					//Funcionario sem horário actualmente
					sql.close();
					//return null;
				}
				sql.close();
				System.out.println("Horario Actual: ASS_EMPREG");
				...
				if (resultado.next()) {
					if (resultado.getString("MAX(ASS_HISEMP_DHFIM)") != null) {
						...
						dataInicioHorario = new java.sql.Date(calendario.getTimeInMillis());
						System.out.println("Horario Actual: dataInicio em ASS_HIS: " + dataInicioHorario);
					}
				} else {
					sql.close();
					return null;
				...
				sql.close();
				System.out.println("Horario Actual: data fim do historico");
	
				// data fim do horario uma vez que já não cumpre assiduidade
				Date dataFimHorario = null;
				if ((status == 1) || (status == 3) || ((status == 4) || (status == 5) || (status == 9))) {
					sql =
						UtilRelacionalOracle.prepararComando(
					...
					if (resultado.next()) {
						if (resultado.getString("ASS_HISEMP_DHFIM") != null) {
							...
							System.out.println("Horario Actual: dataFim em ASS_HIS: " + dataFimHorario);
						}
					}
					sql.close();
				}
				System.out.println("Horario Actual: data fim de assiduidade");
				...
				if (siglaHorario != null && dataInicioHorario != null) {
					...
					System.out.println(
						"Horario Actual: constroiu os horarios Actuais" + listaHorarios.size());
						...
						while (iteradorExcepcoes.hasNext()) {
							...
							if (excepcao.getDataInicio() != dataInicioHorario) {
								...
								while (iterador.hasNext()) {
									...
									//dia anterior ao inicio da excepcao
									calendario.clear();
									calendario.setTime(excepcao.getDataInicio());
									calendario.add(Calendar.DAY_OF_MONTH, -1);
	
									horario.setDataFim(calendario.getTime());
									listaHorariosActuais.add(horario);
								}
							}
							//dia seguinte ao fim da excepaco
							calendario.clear();
					...
					sqlChaveHorario.close();
					System.out.println(
						"Horario Actual: lista horarios actuais: " + listaHorariosActuais.size());
				}
				sql.close();
			} catch (Exception e) {
				SuportePersistenteOracle.getInstance().cancelarTransaccao();
