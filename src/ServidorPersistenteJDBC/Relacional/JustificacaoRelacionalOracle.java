...
public class JustificacaoRelacionalOracle implements IJustificacaoPersistente {
	...
	public ArrayList lerTodasJustificacoes() {
		...
		try {
			...
			try {
				PreparedStatement sql = UtilRelacionalOracle.prepararComando("SELECT * FROM ASS_PESSJUS");
				ResultSet resultado = sql.executeQuery();
				listaJustificacoes = new ArrayList();

				// chaveParamJustificacao
				PreparedStatement sql2 = UtilRelacional.prepararComando("SELECT codigoInterno FROM ass_PARAM_JUSTIFICACAO WHERE sigla = ?");
				...
				while (resultado.next()) {
					...
					if (resultado2.next()) {
						if (resultado.getInt("ASS_PESSJUSTIPO") != 14) { //não é uma justificacao de SALDO
							justificacao =
								new Justificacao(
									resultado2.getInt("codigoInterno"),
									resultado.getInt("ASS_PESSJUSPESSOA"),
									java.sql.Date.valueOf(
										resultado.getString("ASS_PESSJUSDTINI").substring(0, resultado.getString("ASS_PESSJUSDTINI").indexOf(" "))),
									new Time((resultado.getInt("ASS_PESSJUSHINI") - 3600) * 1000),
									java.sql.Date.valueOf(
										resultado.getString("ASS_PESSJUSDTFIM").substring(0, resultado.getString("ASS_PESSJUSDTFIM").indexOf(" "))),
