...
public class StatusAssiduidadeRelacional implements IStatusAssiduidadePersistente
{
	...
	public boolean escreverStatus(StatusAssiduidade status)
	{
		...
		{
			PreparedStatement sql =
			UtilRelacional.prepararComando("INSERT INTO ass_STATUS VALUES(?, ?, ? ,?, ?, ?, ?, null)");
			sql.setInt(1, status.getCodigoInterno());
			sql.setString(2, status.getSigla());
			sql.setString(3, status.getDesignacao());
			sql.setString(4, status.getEstado());
		...
		try
		{
			...
			if (resultadoQuery.next())
			{
				...
				new StatusAssiduidade(
						...
						resultadoQuery.getInt("quem"),
						resultadoQuery.getTimestamp("quando"));
			}
		}
		catch (Exception e)
		{
	...
	public StatusAssiduidade lerStatus(String designacao)
	{
		...
		{
			PreparedStatement sql =
			UtilRelacional.prepararComando("SELECT * FROM ass_STATUS WHERE designacao = ?");
	...
	public ArrayList lerTodosStatusActivos()
	{
		...
		{
			PreparedStatement sql =
			UtilRelacional.prepararComando("SELECT * FROM ass_STATUS WHERE estado='activo'");
			ResultSet resultadoQuery = sql.executeQuery();
	...
	public ArrayList lerTodosStatusInactivos()
	{
		...
		{
			PreparedStatement sql =
			UtilRelacional.prepararComando("SELECT * FROM ass_STATUS WHERE estado='inactivo'");
			ResultSet resultadoQuery = sql.executeQuery();
	...
	public ArrayList lerTodosStatusPendentes()
	{
		...
		{
			PreparedStatement sql =
			UtilRelacional.prepararComando("SELECT * FROM ass_STATUS WHERE estado='pendente'");
			ResultSet resultadoQuery = sql.executeQuery();
