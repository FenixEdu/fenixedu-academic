...
public class MarcacaoPontoRelacional implements IMarcacaoPontoPersistente {
	...
	+"ASS_MARWHO = ? " + "WHERE ASS_MARSEQ = ? ");
				...
				sql.setString(3, marcacaoPonto.getData().toString().substring(0, marcacaoPonto.getData().toString().indexOf(".")));
				...
				// data do registo
				sql.setString(11, marcacaoPonto.getData().toString().substring(0, marcacaoPonto.getData().toString().indexOf(".")));
	...
	public boolean apagarMarcacaoPonto(int chaveMarcacao) {
		...
		try {
			PreparedStatement sql = UtilRelacional.prepararComando("DELETE FROM ass_MARCACAO_PONTO WHERE codigoInterno = ?");
	...
	public boolean apagarMarcacaoPonto(Timestamp dataMarcacao) {
		...
		try {
			PreparedStatement sql = UtilRelacional.prepararComando("DELETE FROM ass_MARCACAO_PONTO WHERE dataMarcacao = ?");
	...
	public ArrayList consultarMarcacoesPonto(
		...
		try {
					...
					if (listaFuncionarios.size() > 0) {
						...
						while (iterListaFuncionarios.hasNext()) {
							...
							if (iterListaFuncionarios.hasNext()) {
								query = query.concat("numFuncionario = ? OR ");
							} else {
								query = query.concat("numFuncionario = ?)");
							}
						}
					}
				}
					...
					if (listaEstados.size() > 0) {
						...
						while (iterListaEstados.hasNext()) {
							...
							if (iterListaEstados.hasNext()) {
								query = query.concat("estado = ? OR ");
							} else {
								query = query.concat("estado = ?)");
							}
						}
					}
				}
				...
				//query = query.concat(" ORDER BY dataMarcacao");
				...
				PreparedStatement sql2 =
					UtilRelacionalOracle.prepararComando(
						"SELECT * FROM regularizacao_marcacao WHERE "
	...
	public MarcacaoPonto lerMarcacaoPonto(int codigoInterno) {
		...
		try {
			PreparedStatement sql = UtilRelacional.prepararComando("SELECT * FROM ass_MARCACAO_PONTO WHERE codigoInterno = ?");
	...
	public ArrayList lerMarcacoesPonto(int numCartao) {
		...
		try {
			SuportePersistenteOracle.getInstance().iniciarTransaccao();
			try {
				PreparedStatement sql = UtilRelacionalOracle.prepararComando("SELECT * FROM ASS_MARCAS WHERE ASS_MARCARTAO = ? ");
	...
	public ArrayList obterMarcacoesPonto(int numCartao, Timestamp dataInicio, Timestamp dataFim) {
		...
		try {
			SuportePersistenteOracle.getInstance().iniciarTransaccao();
			try {
				PreparedStatement sql = UtilRelacionalOracle.prepararComando("SELECT * FROM ASS_MARCAS " + "WHERE ASS_MARCARTAO=? AND "
					//+ "ASS_MARDHMARCA>=? AND ASS_MARDHMARCA<=?");
	...
	public int ultimoCodigoInterno() {
		...
		try {
				PreparedStatement sql = UtilRelacional.prepararComando("SELECT MAX(codigoInterno) FROM marcacao_ponto");
	...
	// ATENCAO: DESNECESSARIO PARA A APLICACAO DE ASSIDUIDADE, APENAS CONSTRUIDO PARA CONSEGUIR ESCREVER OS DADOS NA BD ORACLE
	public boolean alterarMarcacaoPontoRegularizacao(RegularizacaoMarcacaoPonto regularizacao) {
		...
		try {
			...
			try {
				...
				sql = UtilRelacional.prepararComando("SELECT sigla FROM ass_PARAM_REGULARIZACAO " + "WHERE codigoInterno = ?");
				...
				sql = UtilRelacionalOracle.prepararComando("UPDATE ASS_MARCAS SET " + "ASS_MARWHEN = TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS') , "
					//+ "ASS_MARWHEN = ?, " 
	+"ASS_MARWHO = ? " + "WHERE ASS_MARSEQ = ? ");

				// data do registo
				sql.setString(1, regularizacao.getQuando().toString().substring(0, regularizacao.getQuando().toString().indexOf(".")));
	...
	+"ASS_MARWHO = ? " + "WHERE ASS_MARSEQ = ? ");
				...
				sql.setString(2, "S");
				// data do registo
				sql.setString(3, regularizacao.getQuando().toString().substring(0, regularizacao.getQuando().toString().indexOf(".")));
				...
				sql = UtilRelacionalOracle.prepararComando("SELECT ASS_MARPESSOA FROM ASS_MARCAS " + "WHERE ASS_MARSEQ = ?");
				...
				SuportePersistenteOracle.getInstance().cancelarTransaccao();
				System.out.println("MarcacaoPontoRelacional.alterarMarcacaoPontoEscreverRegularizacao: " + e.toString());
				return false;
			}
			SuportePersistenteOracle.getInstance().confirmarTransaccao();
		} catch (Exception e) {
	...
	public int escreverMarcacaoPontoRegularizacao(MarcacaoPonto marcacaoPonto, RegularizacaoMarcacaoPonto regularizacao) {
		...
		try {
			...
			try {
				...
				sql.setString(11, regularizacao.getQuando().toString().substring(0, regularizacao.getQuando().toString().indexOf(".")));
...
//			try {
//				PreparedStatement sql =
//					UtilRelacionalOracle.prepararComando("UPDATE ASS_MARCAS SET " + "ASS_MARPESSOA = ? , " + "ASS_MARCARTAO = ? , "
//					//+ "ASS_MARDHMARCA = TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS') , "
//	+"ASS_MARDHMARCA = ?, "
//		+ "ASS_MARUNID = ? , "
//		+ "ASS_MARTIPO = ? , "
