...
public class MarcacaoPontoRelacional implements IMarcacaoPontoPersistente
{
    public boolean alterarMarcacaoPonto(MarcacaoPonto marcacaoPonto)
    {
		 ...
		 * 
		 * try { PreparedStatement sql = UtilRelacional.prepararComando("UPDATE marcacao_ponto SET " +
		 * "codigoInterno = ? , " + "unidade = ? , " + "siglaUnidade = ? , " + "dataMarcacao = ? , " +
		 * "numCartao = ? , " + "numFuncionario = ? , " + "estado = ? " + "WHERE codigoInterno = ?");
		 * 
		 * sql.setInt(1, marcacaoPonto.getCodigoInterno()); sql.setInt(2, marcacaoPonto.getUnidade());
		 * sql.setString(3, marcacaoPonto.getSiglaUnidade()); if(marcacaoPonto.getData() != null){
		 * sql.setTimestamp(4, new java.sql.Timestamp((marcacaoPonto.getData()).getTime())); }else{
		 * sql.setTimestamp(4, null); } sql.setInt(5, marcacaoPonto.getNumCartao()); sql.setInt(6,
		 * marcacaoPonto.getNumFuncionario()); sql.setString(7, marcacaoPonto.getEstado());
		 * sql.setInt(8, marcacaoPonto.getCodigoInterno());
		 * 
		 * sql.executeUpdate(); sql.close(); resultado = true; } catch(Exception e) {
        ...
        try
        {
            ...
            try
            {
                ...
                sql.setString(
                    3,
                    marcacaoPonto.getData().toString().substring(
                        0,
    ...
    public boolean apagarMarcacaoPonto(int chaveMarcacao)
    {
        ...
        {
            PreparedStatement sql =
                UtilRelacional.prepararComando("DELETE FROM ass_MARCACAO_PONTO WHERE codigoInterno = ?");
    ...
    public boolean apagarMarcacaoPonto(Timestamp dataMarcacao)
    {
        ...
        {
            PreparedStatement sql =
                UtilRelacional.prepararComando("DELETE FROM ass_MARCACAO_PONTO WHERE dataMarcacao = ?");
    ...
    public ArrayList consultarMarcacoesPonto(
    ...
    {
        /*
		 * //Acesso a BD da nossa Aplicacao ArrayList marcacoesPonto = null;
		 * 
		 * try { PreparedStatement sql = null; ResultSet resultado = null; // obtem as marcacoes no
		 * intervalo de datas StringBuffer query = new StringBuffer( FROM marcacao_ponto WHERE " +
		 * "((dataMarcacao IS NULL) OR (dataMarcacao BETWEEN ? AND ?))");
		 * 
		 * if (listaFuncionarios != null) { if (listaFuncionarios.size() > 0) { query = query.append("
		 * AND (");
		 * 
		 * ListIterator iterListaFuncionarios = listaFuncionarios.listIterator(); while
		 * (iterListaFuncionarios.hasNext()) { iterListaFuncionarios.next(); if
		 * (iterListaFuncionarios.hasNext()) { query = query.append("numFuncionario = ? OR "); } else {
		 * query = query.append("numFuncionario = ?)"); } } } }
		 * 
		 * if (listaCartoes != null) { if (listaCartoes.size() > 0) { query = query.append(" AND (");
		 * 
		 * ListIterator iterListaCartoes = listaCartoes.listIterator(); while
		 * (iterListaCartoes.hasNext()) { iterListaCartoes.next(); if (iterListaCartoes.hasNext()) {
		 * query = query.append("numCartao = ? OR "); } else { query = query.append("numCartao = ?)"); } } } }
		 * 
		 * if (listaEstados != null) { if (listaEstados.size() > 0) { query = query.append(" AND (");
		 * 
		 * ListIterator iterListaEstados = listaEstados.listIterator(); while
		 * (iterListaEstados.hasNext()) { iterListaEstados.next(); if (iterListaEstados.hasNext()) {
		 * query = query.append("estado = ? OR "); } else { query = query.append("estado = ?)"); } } } }
		 * 
		 * sql = UtilRelacional.prepararComando(query); sql.setTimestamp(1, new
		 * Timestamp(dataInicio.getTime())); sql.setTimestamp(2, new Timestamp(dataFim.getTime())); //
		 * indice do comando sql int indice = 3;
		 * 
		 ...
		 * iterListaFuncionarios.next(); sql.setInt(indice, numFuncionario.intValue()); indice++; } }
		 * 
		 * if (listaCartoes != null) { ListIterator iterListaCartoes = listaCartoes.listIterator();
		 * Integer numCartao = null;
		 * 
		 * while (iterListaCartoes.hasNext()) { numCartao = (Integer) iterListaCartoes.next();
		 * sql.setInt(indice, numCartao.intValue()); indice++; } }
		 * 
		 * if (listaEstados != null) { ListIterator iterListaEstados = listaEstados.listIterator();
		 * String estado = null;
		 * 
		 * while (iterListaEstados.hasNext()) { estado = (String) iterListaEstados.next();
		 * sql.setString(indice, estado); indice++; } }
		 * 
		 * //query = query.append(" ORDER BY dataMarcacao");
		 * 
		 * resultado = sql.executeQuery(); marcacoesPonto = new ArrayList(); Timestamp dataMarcacao =
		 * null;
		 * 
		 * PreparedStatement sql2 = UtilRelacionalOracle.prepararComando( FROM
		 * ass_REGULARIZACAO_MARCACAO WHERE " + "chaveMarcacaoPonto = ? AND chaveParamRegularizacao =
		 * 6"); ResultSet resultado2 = null;
		 * 
		 * while (resultado.next()) { // retirar as marcacoes de ponto que foram marcadas para anular
		 * sql2.setInt(1, resultado.getInt("codigoInterno")); resultado2 = sql2.executeQuery(); if
		 * (!resultado2.next()) { if (resultado.getString("dataMarcacao") != null) { dataMarcacao =
		 * Timestamp.valueOf(resultado.getString("dataMarcacao")); } marcacoesPonto.add( new
		 * MarcacaoPonto( resultado.getInt("codigoInterno"), resultado.getInt("unidade"),
		 * resultado.getString("siglaUnidade"), dataMarcacao, resultado.getInt("numCartao"),
		 * resultado.getInt("numFuncionario"), resultado.getString("estado"))); } } sql.close(); } catch
		 * (Exception e) { System.out.println( "MarcacaoPontoRelacional.consultarMarcacoesPonto: " +
		 * e.toString()); return null; } finally { return marcacoesPonto; }
		 */
        /** ********************* Acesso a BD Oracle ************************************* */
        //ATENCAO: Acede à BD do Teleponto para ler as marcacoes de ponto

        ArrayList marcacoesPonto = null;
            ...
            try
            {
                ...
                PreparedStatement sql2 =
                    UtilRelacional.prepararComando(
                        "SELECT * FROM ass_PARAM_REGULARIZACAO WHERE sigla = 'MA'");
                ...
                PreparedStatement sql3 =
                    UtilRelacional.prepararComando(
                        "SELECT * FROM ass_REGULARIZACAO_MARCACAO WHERE " + "chaveMarcacaoPonto = ?");
    ...
    public ArrayList consultarMarcacoesPontoErros(String estado)
    {
        ArrayList errosLeitura = null;

        try
        {
            /*
			 * FROM marcacao_ponto WHERE estado = '" + estado + "'"; PreparedStatement sql =
			 * UtilRelacional.prepararComando(query);
			 * 
			 * ResultSet resultado = sql.executeQuery(); errosLeitura = new ArrayList(); Timestamp
			 * dataMarcacao = null;
			 * 
			 * while (resultado.next()) { if (resultado.getString("dataMarcacao") != null) {
			 * dataMarcacao = Timestamp.valueOf(resultado.getString("dataMarcacao")); }
			 * errosLeitura.add( new MarcacaoPonto( resultado.getInt("codigoInterno"),
			 * resultado.getInt("unidade"), resultado.getString("siglaUnidade"), dataMarcacao,
    ...
    public boolean escreverMarcacaoPonto(MarcacaoPonto marcacaoPonto)
    {
        ...
        try
        {
            PreparedStatement sql =
                UtilRelacional.prepararComando(
                    "INSERT INTO ass_MARCACAO_PONTO VALUES (?, ?, ?, ?, ?, ?, ?)");
    ...
    public boolean escreverMarcacoesPonto(ArrayList listaMarcacoes)
    {
        boolean resultado = false;
    ...
    public MarcacaoPonto lerMarcacaoPonto(int codigoInterno)
    {
        ...
        try
        {
            PreparedStatement sql =
                UtilRelacional.prepararComando(
                    "SELECT * FROM ass_MARCACAO_PONTO WHERE codigoInterno = ?");
    ...
    public MarcacaoPonto lerMarcacaoPonto(Timestamp dataMarcacao, int numFuncionario)
    {
		 ...
		 * 
		 * try { PreparedStatement sql = FROM marcacao_ponto " + "WHERE dataMarcacao = ? AND
		 * numFuncionario = ?");
		 * 
		 * sql.setTimestamp(1, dataMarcacao); sql.setInt(2, numFuncionario);
		 * 
		 * ResultSet resultado = sql.executeQuery(); if (resultado.next()) { Timestamp data = null;
		 * if(resultado.getString("dataMarcacao") != null){ data =
		 * Timestamp.valueOf(resultado.getString("dataMarcacao")); } marcacaoPonto = new
		 * MarcacaoPonto(resultado.getInt("codigoInterno"), resultado.getInt("unidade"),
		 * resultado.getString("siglaUnidade"), data, resultado.getInt("numCartao"),
		 * resultado.getInt("numFuncionario"), resultado.getString("estado")); } sql.close(); }
		 * catch(Exception e) { System.out.println("MarcacaoPontoRelacional.lerMarcacaoPonto: " +
        ...
        try
        {
            ...
            try
            {
                ...
                if (marcacaoPonto != null)
                {
                    sql =
                        UtilRelacional.prepararComando(
                            "SELECT codigoInterno FROM ass_UNIDADE_MARCACAO " + "WHERE sigla = ?");
                    ...
                    sql =
                        UtilRelacionalOracle.prepararComando(
                            "SELECT * FROM ASS_MARREG " + "WHERE ASS_MARREGMARCAS = ?");
    ...
    {
        /*
		 * // Acesso a BD da nossa Aplicação ArrayList marcacoesPonto = null;
		 * 
		 * try { PreparedStatement sql = FROM marcacao_ponto");
		 * 
		 * ResultSet resultado = sql.executeQuery(); marcacoesPonto = new ArrayList(); Timestamp
		 * dataMarcacao = null; while(resultado.next()) { if(resultado.getString("dataMarcacao") !=
		 * null){ dataMarcacao = Timestamp.valueOf(resultado.getString("dataMarcacao")); }
		 * marcacoesPonto.add(new MarcacaoPonto(resultado.getInt("codigoInterno"),
		 * resultado.getInt("unidade"), resultado.getString("siglaUnidade"), dataMarcacao,
		 * resultado.getInt("numCartao"), resultado.getInt("numFuncionario"),
		 * resultado.getString("estado"))); dataMarcacao = null; } sql.close(); } catch(Exception e) {
		 * System.out.println("MarcacaoPontoRelacional.lerMarcacoesPonto: " + e.toString()); } finally {
		 * return marcacoesPonto;
		 */
    ...
    public ArrayList lerMarcacoesPonto(int numCartao)
    {
		 ...
		 * // Acesso a BD da nossa Aplicação ArrayList marcacoesPonto = null;
		 * 
		 * try { PreparedStatement sql = FROM marcacao_ponto " + "WHERE numCartao = ? ");
		 * 
        ...
        try
        {
            ...
            try
            {
                PreparedStatement sql =
                    UtilRelacionalOracle.prepararComando(
                        "SELECT * FROM ASS_MARCAS WHERE ASS_MARCARTAO = ? ");
    ...
    public ArrayList obterMarcacoesPonto(int numFuncionario, int numCartao, Timestamp dataFim)
    {
		 ...
		 * 
		 * try { PreparedStatement sql = FROM marcacao_ponto " + "WHERE dataMarcacao>=? AND
		 * numFuncionario=? AND numCartao=?");
		 * 
		 * sql.setTimestamp(1, new Timestamp(dataFim.getTime())); sql.setInt(2, numFuncionario);
		 ...
		 * resultado.getInt("numCartao"), resultado.getInt("numFuncionario"),
		 * resultado.getString("estado"))); dataMarcacao = null; } sql.close(); } catch(Exception e) {
		 * System.out.println("MarcacaoPontoRelacional.obterMarcacoesPonto: " + e.toString()); } finally {
		 * return marcacoesPonto; }
		 */
        /** ******************** Acesso a BD oracle ******************************** */
        ArrayList marcacoesPonto = null;
    ...
    public ArrayList obterMarcacoesPonto(int numCartao, Timestamp dataInicio, Timestamp dataFim)
    {
		 ...
		 * // Acesso a BD da nossa Aplicacao ArrayList marcacoesPonto = null;
		 * 
		 * try { PreparedStatement sql = FROM marcacao_ponto " + "WHERE numCartao=? AND dataMarcacao>=?
		 * AND dataMarcacao <=?");
		 * 
		 * sql.setInt(1, numCartao); sql.setTimestamp(2, new Timestamp(dataInicio.getTime()));
    ...
    public int ultimoCodigoInterno()
    {
		 ...
		 * int ultimo = 0;
		 * 
		 * try { PreparedStatement sql = UtilRelacional.prepararComando("SELECT MAX(codigoInterno) FROM
		 * marcacao_ponto");
		 * 
		 * ResultSet resultado = sql.executeQuery(); if (resultado.next()) { ultimo =
		 * resultado.getInt(1); } sql.close(); } catch (Exception e) {
		 * System.out.println("MarcacaoPontoRelacional.ultimoCodigoInterno: " + e.toString()); } finally {
		 * return ultimo; }
		 */
        int ultimo = 0;
        try
        {
            ...
            try
            {
                ...
                PreparedStatement sql =
                    UtilRelacionalOracle.prepararComando("SELECT s_ordem.NEXTVAL FROM SYS.dual");
                ResultSet resultadoQuery = sql.executeQuery();
                if (resultadoQuery.next())
                {
    ...
    // ATENCAO: DESNECESSARIO PARA A APLICACAO DE ASSIDUIDADE, APENAS CONSTRUIDO PARA CONSEGUIR ESCREVER
    ...
    public boolean alterarMarcacaoPontoRegularizacao(RegularizacaoMarcacaoPonto regularizacao)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                sql =
                    UtilRelacional.prepararComando(
                        "SELECT sigla FROM ass_PARAM_REGULARIZACAO " + "WHERE codigoInterno = ?");
                ...
                sql.setString(
                    1,
                    regularizacao.getQuando().toString().substring(
                        0,
            ...
            catch (Exception e)
            {
                SuportePersistenteOracle.getInstance().cancelarTransaccao();
                System.out.println(
                    "MarcacaoPontoRelacional.alterarMarcacaoPontoRegularizacao: " + e.toString());
                return false;
            }
            SuportePersistenteOracle.getInstance().confirmarTransaccao();
        }
    ...
    public boolean alterarMarcacaoPontoEscreverRegularizacao(RegularizacaoMarcacaoPonto regularizacao)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                sql =
                    UtilRelacionalOracle.prepararComando(
                        "SELECT ASS_MARPESSOA FROM ASS_MARCAS " + "WHERE ASS_MARSEQ = ?");
            ...
            catch (Exception e)
            {
                SuportePersistenteOracle.getInstance().cancelarTransaccao();
                System.out.println(
                    "MarcacaoPontoRelacional.alterarMarcacaoPontoEscreverRegularizacao: "
                        + e.toString());
                return false;
            }
            SuportePersistenteOracle.getInstance().confirmarTransaccao();
        ...
        {
            System.out.println(
                "MarcacaoPontoRelacional.alterarMarcacaoPontoEscreverRegularizacao: " + e.toString());
        }
        return resultado;
    ...
    // ATENCAO: DESNECESSARIO PARA A APLICACAO DE ASSIDUIDADE, APENAS CONSTRUIDO PARA CONSEGUIR ESCREVER
    // OS DADOS NA BD ORACLE
    public int escreverMarcacaoPontoRegularizacao(
        MarcacaoPonto marcacaoPonto,
        RegularizacaoMarcacaoPonto regularizacao)
    {
        ...
        try
        {
            ...
            catch (Exception e)
            {
                SuportePersistenteOracle.getInstance().cancelarTransaccao();
                System.out.println(
                    "MarcacaoPontoRelacional.escreverMarcacaoPontoRegularizacao: " + e.toString());
                return 0;
            }
            SuportePersistenteOracle.getInstance().confirmarTransaccao();
        }
...
//			sql.setString(3, marcacaoPonto.getSiglaUnidade());
//			if(marcacaoPonto.getData() != null){
//				sql.setTimestamp(4, new java.sql.Timestamp((marcacaoPonto.getData()).getTime()));
//			}else{
//				sql.setTimestamp(4, null);
//			}
//			sql.setInt(5, marcacaoPonto.getNumCartao());
...
//
//		try {
//			PreparedStatement sql = UtilRelacional.prepararComando("DELETE FROM ass_MARCACAO_PONTO WHERE
// codigoInterno = ?");
//
//			sql.setInt(1, chaveMarcacao);
//
...
//
//				// obtem o codigo das marcacoes a anular
//				PreparedStatement sql2 = UtilRelacional.prepararComando("SELECT * FROM ass_PARAM_REGULARIZACAO WHERE
// sigla = 'MA'");
//				ResultSet resultado2 = sql2.executeQuery();
//				int marcacaoAnular = 0;
//				if (resultado2.next()) {
...
//				// marcacaoes regularizadas
//				PreparedStatement sql3 =
//					UtilRelacional.prepararComando("SELECT * FROM ass_REGULARIZACAO_MARCACAO WHERE " +
// "chaveMarcacaoPonto = ?");
//				ResultSet resultado3 = null;
//				while (resultado.next()) {
//					if (resultado.getString("ASS_MARDHMARCA") != null) {
...
//	} /* consultarMarcacoesPonto */
//
//	public ArrayList consultarMarcacoesPontoErros(String estado) { //não será preciso
//		ArrayList errosLeitura = null;
//
//		try {
//			/*
...
//		boolean resultado = false;
//		try {
//			PreparedStatement sql = UtilRelacional.prepararComando("INSERT INTO ass_MARCACAO_PONTO VALUES (?, ?,
// ?, ?, ?, ?, ?)");
//
//			sql.setInt(1, marcacaoPonto.getCodigoInterno());
//			sql.setInt(2, marcacaoPonto.getUnidade());
...
//
//		try {
//			PreparedStatement sql = UtilRelacional.prepararComando("SELECT * FROM ass_MARCACAO_PONTO WHERE
// codigoInterno = ?");
//
//			sql.setInt(1, codigoInterno);
//
...
//
//				if (marcacaoPonto != null) {
//					sql = UtilRelacional.prepararComando("SELECT codigoInterno FROM ass_UNIDADE_MARCACAO " + "WHERE sigla
// = ?");
//
//					sql.setString(1, marcacaoPonto.getSiglaUnidade());
//					resultadoQuery = sql.executeQuery();
...
//					sql.close();
//
//					sql = UtilRelacionalOracle.prepararComando("SELECT * FROM ASS_MARREG " + "WHERE ASS_MARREGMARCAS =
// ?");
//
//					sql.setInt(1, marcacaoPonto.getCodigoInterno());
//					resultadoQuery = sql.executeQuery();
...
//			SuportePersistenteOracle.getInstance().iniciarTransaccao();
//			try {
//				PreparedStatement sql = UtilRelacionalOracle.prepararComando("SELECT * FROM ASS_MARCAS WHERE
// ASS_MARCARTAO = ? ");
//
//				sql.setInt(1, numCartao);
//
...
//				PreparedStatement sql =
//					UtilRelacionalOracle.prepararComando(
//						"SELECT * FROM ASS_MARCAS " + "WHERE ASS_MARCARTAO=? AND " + "ASS_MARDHMARCA>=? AND
// ASS_MARDHMARCA<=?");
//
//				sql.setInt(1, numCartao);
//
...
//		
//		try {
//			PreparedStatement sql = UtilRelacional.prepararComando("SELECT MAX(codigoInterno) FROM
// marcacao_ponto");
//		
//			ResultSet resultado = sql.executeQuery();
//			if (resultado.next()) {
...
//			try {
//				// último número de sequencia na tabela
//				PreparedStatement sql = UtilRelacionalOracle.prepararComando("SELECT MAX(ASS_MARSEQ) FROM
// ASS_MARCAS");
//				ResultSet resultadoQuery = sql.executeQuery();
//				if (resultadoQuery.next()) {
//					ultimo = resultadoQuery.getInt(1) + 1;
...
//
//				// obtem a sigla de regularizacao de marcacao de ponto
//				sql = UtilRelacional.prepararComando("SELECT sigla FROM ass_PARAM_REGULARIZACAO " + "WHERE
// codigoInterno = ?");
//
//				sql.setInt(1, regularizacao.getChaveParamRegularizacao());
//				resultadoQuery = sql.executeQuery();
...
//				sql =
//					UtilRelacionalOracle.prepararComando(
//						"UPDATE ASS_MARCAS SET " + "ASS_MARWHEN = ?, " + "ASS_MARWHO = ? " + "WHERE ASS_MARSEQ = ? ");
//
//				// data do registo
...
//			} catch (Exception e) {
//				SuportePersistenteOracle.getInstance().cancelarTransaccao();
//				System.out.println("MarcacaoPontoRelacional.alterarMarcacaoPontoRegularizacao: " + e.toString());
//				return false;
//			}
//			SuportePersistenteOracle.getInstance().confirmarTransaccao();
//		} catch (Exception e) {
...
//
//				// insere a regularizacao de marcacao de ponto na tabela ASS_MARREG
//				sql = UtilRelacionalOracle.prepararComando("SELECT ASS_MARPESSOA FROM ASS_MARCAS " + "WHERE
// ASS_MARSEQ = ?");
//				sql.setInt(1, regularizacao.getChaveMarcacaoPonto());
//				resultadoQuery = sql.executeQuery();
//				PreparedStatement sql2 = null;
//				if (resultadoQuery.next()) {
//					sql2 = UtilRelacionalOracle.prepararComando("INSERT INTO ASS_MARREG " + "VALUES (?, ?, ?, ?)");
//
//					sql2.setInt(1, resultadoQuery.getInt("ASS_MARPESSOA"));
//					sql2.setInt(2, regularizacao.getChaveMarcacaoPonto());
//					sql2.setString(3, sigla);
...
//	// ATENCAO: DESNECESSARIO PARA A APLICACAO DE ASSIDUIDADE, APENAS CONSTRUIDO PARA CONSEGUIR ESCREVER
// OS DADOS NA BD ORACLE
//	public int escreverMarcacaoPontoRegularizacao(MarcacaoPonto marcacaoPonto, RegularizacaoMarcacaoPonto
// regularizacao) {
//		int ultimo = 0;
//
//		/**************************** escrever a marcacao de ponto na BD Oracle
...
//
//				// insere a marcacao de ponto na tabela ASS_MARCAS
//				sql = UtilRelacionalOracle.prepararComando("INSERT INTO ASS_MARCAS " + "VALUES (?, ?, ?, ?, ?, ?, ?,
// ?, ?, ?, ?, ?)");
//
//				sql.setInt(1, marcacaoPonto.getNumFuncionario());
//				sql.setInt(2, -1);
...
//
//				// insere a regularizacao de marcacao de ponto na tabela ASS_MARREG
//				sql = UtilRelacionalOracle.prepararComando("INSERT INTO ASS_MARREG " + "VALUES (?, ?, ?, ?)");
//
//				sql.setInt(1, marcacaoPonto.getNumFuncionario());
//				sql.setInt(2, ultimo);
//				sql.setString(3, sigla);
...
//			} catch (Exception e) {
//				SuportePersistenteOracle.getInstance().cancelarTransaccao();
//				System.out.println("MarcacaoPontoRelacional.escreverMarcacaoPontoRegularizacao: " + e.toString());
//				return 0;
//			}
//			SuportePersistenteOracle.getInstance().confirmarTransaccao();
//		} catch (Exception e) {
