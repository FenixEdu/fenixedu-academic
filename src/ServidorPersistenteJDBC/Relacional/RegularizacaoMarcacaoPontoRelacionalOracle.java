 ...
 */
public class RegularizacaoMarcacaoPontoRelacionalOracle implements IRegularizacaoMarcacaoPontoPersistente {
	...
	public ArrayList lerTodasRegularizacoes() {
		//ORACLE: metodo utiliza a BD do teleponto para o carregamento das regularizacoes de marcacoes de ponto
		...
		try {
			...
			try {
				...
				PreparedStatement sql2 =
					UtilRelacionalOracle.prepararComando("SELECT ASS_MARWHO, ASS_MARWHEN FROM ASS_MARCAS WHERE ASS_MARSEQ = ?");
				ResultSet resultado2 = null;

				// chaveParamRegularizacao
				PreparedStatement sql3 = UtilRelacional.prepararComando("SELECT codigoInterno FROM ass_PARAM_REGULARIZACAO WHERE sigla = ?");
				...
				while (resultado.next()) {
					...
					if (resultado2.next() && resultado3.next()) {
						...
						regularizacoes.add(
							new RegularizacaoMarcacaoPonto(0,
				...
				System.out.println("RegularizacaoMarcacaoPontoRelacionalOracle.lerTodasRegularizacoes: " + SQLe.toString());
				return null;
			}
			SuportePersistenteOracle.getInstance().confirmarTransaccao();
		} catch (Exception e) {
			System.out.println("RegularizacaoMarcacaoPontoRelacionalOracle.lerTodasRegularizacoes: " + e.toString());
		} finally {
