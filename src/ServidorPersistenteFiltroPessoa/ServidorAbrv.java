...
public class ServidorAbrv {
  ...
  /*constroi hashmap em que a key é a chave de um distrito
     e o object é um arraylist com todos os concelhos
   para esse distrito
   */
  public HashMap lerMapConcelhos() {
    ...
    try {
      PreparedStatement sql =
          DB.prepararComando("SELECT * FROM Concelho");
  ...
  /*constroi hashmap em que a key é a chave de um concelho
     e o object é um arraylist com todas as freguesias
   para esse concelho
   */
  ...
  public HashMap lerMapFreguesias() {
    ...
    try {
      PreparedStatement sql =
          DB.prepararComando("SELECT * FROM Freguesia");
      resultado = sql.executeQuery();
  ...
  public ArrayList lerFreguesias() {
    ...
    try {
      ...
      while (resultado.next()) {
        listaFreguesias.add(new Freguesia(resultado.getInt(1),
                                          resultado.getInt(2),
                                          resultado.getInt(3),
  ...
  public ArrayList lerConcelhos() {
    ...
    try {
      ...
      while (resultado.next()) {
        listaConcelhos.add(new Concelho(resultado.getInt(1),
                                        resultado.getInt(2),
  ...
  public ArrayList lerDistritos() {
     ...
     try {
       PreparedStatement sql =
           DB.prepararComando("SELECT * FROM Distrito");

       resultado = sql.executeQuery();
       listaDistritos = new ArrayList();

       while (resultado.next()) {
         listaDistritos.add(new Distrito(resultado.getInt(1),
   ...
   public ArrayList lerTopWords() {
     ...
     try {
       PreparedStatement sql =
           DB.prepararComando("SELECT * FROM TopWords");
   ...
   public HashMap lerAbreviaturas() {
     ...
     try {
       PreparedStatement sql =
           DB.prepararComando("SELECT * FROM Abreviaturas");
   ...
   public HashMap lerPaisesExpansao() {
     ...
     try {
       PreparedStatement sql =
           DB.prepararComando("SELECT * FROM paisExpansao");
   ...
   public HashMap lerConversaoLocalDistrito() {
     ...
     try {
       PreparedStatement sql =
           DB.prepararComando("SELECT * FROM conversaoLocalDistrito");
   ...
   public HashMap lerConversaoCidadePais() {
     ...
     try {
       PreparedStatement sql =
           DB.prepararComando("SELECT * FROM conversaoCidadePais");
   ...
   public HashMap lerConcelhoExpansao() {
     ...
     try {
       PreparedStatement sql =
           DB.prepararComando("SELECT * FROM concelhoExpansao");
   ...
   public HashMap lerFreguesiaExpansao() {
     ...
     try {
       PreparedStatement sql =
           DB.prepararComando("SELECT * FROM freguesiaExpansao");
   ...
   public HashMap lerConversaoLocalConcelho() {
     ...
     try {
       PreparedStatement sql =
           DB.prepararComando("SELECT * FROM conversaoLocalConcelho");
   ...
   public ArrayList lerTodasMoradas() {
    ...
    try {
      PreparedStatement sql =
          DB.prepararComando("SELECT codigoInterno, nacionalidade, distritoNaturalidade, concelhoNaturalidade, freguesiaNaturalidade FROM pessoa order by codigoInterno"); //localidade, codigoPostal1, codigoPostal2

      resultado = sql.executeQuery();
      listaMoradas = new ArrayList();

      while (resultado.next()) {
        listaMoradas.add(new Morada(resultado.getInt(1),
                                    resultado.getString(2),
                                    resultado.getString(3),
                                    resultado.getString(4),
   ...
   public ArrayList lerPaises() {
     ...
     try {
       PreparedStatement sql =
           DB.prepararComando("SELECT * FROM PaisNaturalidade");

       resultado = sql.executeQuery();
       listaPaises = new ArrayList();

       while (resultado.next()) {
         listaPaises.add(new Pais(resultado.getInt(1),
                                     resultado.getString(2)));
       }
       sql.close();
     }
