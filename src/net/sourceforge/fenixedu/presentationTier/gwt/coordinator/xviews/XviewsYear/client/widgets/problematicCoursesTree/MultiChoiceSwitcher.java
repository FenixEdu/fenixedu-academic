...
public class MultiChoiceSwitcher extends Raphael {
	...
	private void drawWidget() {
		...
		sliderMechs =
				new SliderMechanics(sliderOX, sliderOY, sliderW, sliderH, sliderR, sliderStroke, slideOX + slideW - slideStroke);
		...
		for (int i = 0; i < choices.length; i++) {
			final SliderGhost ghost =
					new SliderGhost(ghostOX + (ghostW * i) + ((2 * i + 1) * ghostPadding), ghostOY, ghostW, ghostH, ghostStroke,
							ghostPadding, i);
	...
	private void determineDropZone() {
		for (SliderGhost ghost : ghosts) {
			if (slider.cx >= ghost.ghostOX && slider.cx < (ghost.ghostOX + ghost.ghostW)) {
				swapState(ghost);
			}
		}
	}
	...
	private void swapState(SliderGhost ghost) {
		for (SliderGhost ghost2 : ghosts) {
			ghost2.label.deFocus();
		}
	...
	private class SliderMechanics extends Rect implements HasMouseDownHandlers, HasMouseUpHandlers, HasMouseMoveHandlers,
			HasMouseOutHandlers {
