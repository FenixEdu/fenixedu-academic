		...
		private class BoundTableLayout extends TabularLayout {
			...
			public BoundTableLayout(List<VigilantBoundBean> bounds) {
				...
				for(VigilantBoundBean bound : bounds)  {
					metaObjects.add(MetaObjectFactory.createObject(bound, RenderKit.getInstance().findSchema("editVigilantBoundBean")));
			...
			private boolean isInMetaObjects(Person person) {
				for(MetaObject metaObject : this.personMetaObjects) {
					...
					if(onePerson.equals(person)) return true;
				}
				return false;
			}
			...
			protected int getNumberOfColumns() {
				return (this.metaObjects==null || this.metaObjects.size()==0) ? 0 : this.metaObjects.get(0).getSlots().size() + groups.size();
			}
