...
public class StudentCurricularPlanRenderer extends InputRenderer {
	...
	FONTSIZE("em"), XHEIGHT("ex"), PIXEL("px"),

	/**
	 * Absolute units
	 */

	INCHES("in"), CENTIMETERS("cm"), MILIMETERS("mm"), POINTS("pt"), PICAS("pc"), PERCENTAGE("%");
	...
	if (initialWidthValue == null) {
		...
		if (initialWidth.contains(lengthUnit.getUnitIdentifier())) {
		    initialWidthValue = Double.valueOf(initialWidth
			    .split(lengthUnit.getUnitIdentifier())[0]);
		}
	    }
	}
	...
	public HtmlComponent createComponent(Object object, Class type) {
	    ...
	    if (studentCurricularPlan.getEnrolmentsSet().isEmpty()
		    && studentCurricularPlan.getDismissals().isEmpty()) {
		scpDiv.addChild(new HtmlText(studentResources.getString("message.no.enrolments")));
		scpDiv.setClasses("italic");
		return scpDiv;
	    }

	    if (!isOrganizedByGroups() && !isOrganizedByExecutionYears()
		    && !isOrganizedByCurricularYears()) {
	    ...
	    hiddenEnrollments.bind(getInputContext().getMetaObject(), "extraCurricularEnrolments"); // slot
	    // refered
	    // by
	    // name
		...
		scpNameCell.setBody(generateDegreeCurricularPlanNameLink(studentCurricularPlan
	...
	private int calculateNumberOfCellsPerEnrolment() {
	    return MAX_CELLS_PER_ENROLMENT - (isOrganizedByGroups() ? 0 : 2)
		    - (viewLatestEnrolmentEvaluationInfo() ? 0 : 2);
	}
	...
	private HtmlComponent generateDegreeCurricularPlanNameLink(
	    ...
	    executionPeriod = (executionPeriod == null) ? ExecutionPeriod.readActualExecutionPeriod()
		    : executionPeriod;
	...
	private void generateGroup(final CurriculumGroup group, double depth) {
	    ...
	    groupTable.setClasses(getTablesClasses() + " " + getInitialWidth()
		    + (group.isRoot() ? "" : String.valueOf(Double.valueOf(depth).intValue())));
	    ...
	    groupNameCell.setBody((group.isRoot()) ? generateDegreeCurricularPlanNameLink(
		    studentCurricularPlan.getDegreeCurricularPlan(), null) : new HtmlText(group
		    .getName().getContent(LanguageUtils.getLanguage())));
	...
	private void generateGroupLines(double depth, CurriculumGroup group) {
		...
		groupLinesTable.setClasses(getTablesClasses() + " " + getInitialWidth()
			+ String.valueOf(Double.valueOf(depth).intValue()));
	...
	private void generateEnrolment(final CurriculumLine curriculumLine, final HtmlTable parentTable) {
	    ...
	    if (getEnrolmentStateSelectionType().intValue() == EnrollmentStateSelectionType.ALL_TYPE
		    || (getEnrolmentStateSelectionType().intValue() == EnrollmentStateSelectionType.APPROVED_TYPE && (enrolment
			    .isApproved() || enrolment.isEnroled()))) {
		...
		if (!StringUtils.isEmpty(enrolment.getCurricularCourse().getCode())) {
		    codeNameSpan
			    .addChild(new HtmlText(enrolment.getCurricularCourse().getCode() + " - "));
		}
		codeNameSpan.addChild(new HtmlText(enrolment.getName().getContent(
			LanguageUtils.getLanguage())));
		codeNameCell.setBody(generateEnrolmentLink(codeNameSpan, enrolment));

		// Degree Curricular Plan
		generateEnrolmentSmallInfoCell(lineRow,
			generateEnrolmentDegreeCurricularPlanLink(enrolment), getEnrolmentClasses()[1],
			null);

		// Enrolment Type
		final HtmlComponent enrolmentType = enrolment.isEnrolmentTypeNormal() ? null
			: new HtmlText(enumerationResources.getString(enrolment.getEnrolmentTypeName()));
		...
		if (enrolment.isApproved() && enableExtraCurricularEditing()) {
		    final MetaObject enrolmentMetaObject = MetaObjectFactory
			    .createObject(enrolment, RenderKit.getInstance().findSchema(
				    "Enrolment.edit-extra-curricular-status"));
		...
		// Enrolment State
		final HtmlComponent enrolmentState = enrolment.isEnrolmentStateApproved() ? null
			: new HtmlText(enumerationResources.getString(enrolment.getEnrollmentState()
				.getQualifiedName()));
		...
		final String enrolmentWeight = enrolment.getFinalGrade() == null ? "-" : enrolment
			.getWeigth().toString();
		...
		final String enrolmentEctsCredits = enrolment.isEnrolmentStateApproved() ? enrolment
		...
		final EnrolmentEvaluation latestEnrolmentEvaluation = enrolment
			.getLatestEnrolmentEvaluation();
		if (latestEnrolmentEvaluation != null
			&& latestEnrolmentEvaluation.getEnrolmentEvaluationType() != null) {
		    latestEnrolmentEvaluationType = new HtmlText(enumerationResources
			    .getString(latestEnrolmentEvaluation.getEnrolmentEvaluationType()
				    .getAcronym()));
		    latestEnrolmentEvaluationTypeQualifiedName = enumerationResources
			    .getString(latestEnrolmentEvaluation.getEnrolmentEvaluationType()
				    .getQualifiedName());
		}
		generateEnrolmentSmallInfoCell(lineRow, latestEnrolmentEvaluationType,
			getEnrolmentClasses()[8], latestEnrolmentEvaluationTypeQualifiedName);

		// Execution Period
		if (isOrganizedByGroups() || isOrganizedByCurricularYears()) {
		    // Year
		    generateEnrolmentSmallInfoCell(lineRow, new HtmlText(enrolment.getExecutionPeriod()
			    .getYear()), getEnrolmentClasses()[9], null);
		    ...
		    generateEnrolmentSmallInfoCell(lineRow, new HtmlText(semester.toString()),
			    getEnrolmentClasses()[10], null);
		}

		if (viewLatestEnrolmentEvaluationInfo()) {
		    // Latest Enrolment Evaluation Exam Date
		    generateEnrolmentSmallInfoCell(lineRow,
			    (latestEnrolmentEvaluation == null || latestEnrolmentEvaluation
				    .getExamDateYearMonthDay() == null) ? null : new HtmlText(
				    latestEnrolmentEvaluation.getExamDateYearMonthDay().toString(
					    "yyyy/MM/dd")), getEnrolmentClasses()[11],
			    "Data da Avaliação");

		    // Latest Enrolment Evaluation Person Responsible For
		    // TestsGrade
		    if (latestEnrolmentEvaluation != null
			    && latestEnrolmentEvaluation.getPersonResponsibleForGrade() != null) {
			final Person person = latestEnrolmentEvaluation.getPersonResponsibleForGrade();
			generateEnrolmentSmallInfoCell(lineRow, new HtmlText(
				person.hasEmployee() ? person.getEmployee().getRoleLoginAlias() : person
					.getIstUsername()), getEnrolmentClasses()[12],
		...
		// final HtmlTableRow enrolmentEvaluationsLine =
		// parentTable.createRow();
		// final HtmlTableCell enrolmentEvaluationsCell =
		// enrolmentEvaluationsLine.createCell();
		...
		//		
		// final SortedSet<EnrolmentEvaluation> enrolmentEvaluations =
		// new TreeSet<EnrolmentEvaluation>();
		// enrolmentEvaluations.addAll(enrolment.getEvaluations());
		// for (final EnrolmentEvaluation enrolmentEvaluation :
		// enrolmentEvaluations) {
		// final HtmlTableRow enrolmentEvaluationsRow =
		// enrolmentEvaluationsTable.createRow();
		...
		// enrolmentEvaluationsRow,
		// new
		// HtmlText(enumerationResources.getString(enrolmentEvaluation.getEnrolmentEvaluationType().getAcronym())),
		// "",
		...
		// examDate = new
		// HtmlText(enrolmentEvaluation.getExamDateYearMonthDay().toString("yyyy/MM/dd"));
		// }
		// generateEnrolmentSmallInfoCell(enrolmentEvaluationsRow,
		// examDate, "", null);
		//		    
		// // Person Responsible
		// HtmlComponent personResponsible = null;
		// if (enrolmentEvaluation.getPersonResponsibleForGrade() !=
		// null) {
		// personResponsible = new
		// HtmlText(enrolmentEvaluation.getPersonResponsibleForGrade().hasEmployee()
		// ?
		// enrolmentEvaluation.getPersonResponsibleForGrade().getEmployee().getRoleLoginAlias()
		// :
		// enrolmentEvaluation.getPersonResponsibleForGrade().getIstUsername());
		// }
		// generateEnrolmentSmallInfoCell(enrolmentEvaluationsRow,
	...
	private void generateEnrolmentSmallInfoCell(final HtmlTableRow lineRow,
		HtmlComponent htmlComponent, final String cellClasses, final String title) {
	...
	private HtmlComponent generateEnrolmentLink(final HtmlComponent htmlComponent,
		...
		result
			.setParameter("executionPeriodOID", enrolment.getExecutionPeriod()
				.getIdInternal());
		...
		if (curricularCourse.isBolonhaDegree()) {
		    ...
		    result.setParameter("executionYearID", enrolment.getExecutionPeriod()
		...
		final ExecutionCourse executionCourse = enrolment.getExecutionCourseFor(enrolment
			.getExecutionPeriod());
	...
	private HtmlComponent generateEnrolmentDegreeCurricularPlanLink(final Enrolment enrolment) {
	    ...
	    if (studentCurricularPlan.getDegreeCurricularPlan() == enrolment.getDegreeModule()
		    .getParentDegreeCurricularPlan()) {
		degreeCurricularPlan = new HtmlText("-");
	    } else {
		degreeCurricularPlan = generateDegreeCurricularPlanNameLink(enrolment.getDegreeModule()
			.getParentDegreeCurricularPlan(), enrolment.getExecutionPeriod());
	    }
	...
	private void generateEnrolmentsExecutionPeriods() {
	    final Set<ExecutionPeriod> enrolmentsExecutionPeriods = new TreeSet<ExecutionPeriod>(
	...
	private void generateEnrolmentExecutionPeriod(ExecutionPeriod executionPeriod) {
	    ...
	    executionPeriodTable.setClasses(getTablesClasses() + " " + getInitialWidth()
		    + String.valueOf(1));
	    ...
	    executionPeriodNameCell
		    .setColspan(RENDERED_CELLS_PER_ENROLMENT - RENDERED_CELLS_UNTIL_GRADE);
	    executionPeriodNameCell.setBody(new HtmlText(executionPeriod.getYear() + ", "
	    ...
	    final Set<CurriculumLine> sortedCurriculumLines = new TreeSet<CurriculumLine>(
		    CurriculumLine.COMPARATOR_BY_NAME_AND_ID);
	    sortedCurriculumLines.addAll(studentCurricularPlan
	...
	private void generateEnrolmentsCurricularPeriods() {
		...
		if (enrolment.getDegreeModule().isLeaf()) {
		    final CurricularCourse curricularCourse = (CurricularCourse) enrolment
			    .getDegreeModule();

		    List<DegreeModuleScope> degreeModuleScopes = curricularCourse
			    .getActiveDegreeModuleScopesInExecutionPeriod(enrolment.getExecutionPeriod());
		    if (degreeModuleScopes.size() == 1) {
			final DegreeModuleScope degreeModuleScope = degreeModuleScopes.iterator().next();
			final GenericPair<Integer, Integer> yearSemester = new GenericPair<Integer, Integer>(
				degreeModuleScope.getCurricularYear(), degreeModuleScope
					.getCurricularSemester());

			final Set<Enrolment> enrolmentsCurricularPeriod = enrolmentsCurricularPeriods
				.get(yearSemester);
			...
			for (final DegreeModuleScope degreeModuleScope : degreeModuleScopes) {
			    ...
			    final Set<Enrolment> enrolmentsCurricularPeriod = orphanEnrolments
				    .get(yearSemester);
			    if (enrolmentsCurricularPeriod == null) {
			...
			for (final DegreeModuleScope degreeModuleScope : degreeModuleScopes) {
			    ...
			    final Set<Enrolment> enrolmentsCurricularPeriod = undefinedEnrolments
				    .get(yearSemester);
			    if (enrolmentsCurricularPeriod == null) {
	    ...
	    List<GenericPair<Integer, Integer>> curricularPeriods = new ArrayList(
		    enrolmentsCurricularPeriods.keySet());
		...
		generateEnrolmentCurricularPeriod(yearSemester, enrolmentsCurricularPeriods
			.get(yearSemester));
	    }
	    ...
	    List<GenericPair<Integer, Integer>> orphanCurricularPeriods = new ArrayList(orphanEnrolments
		    .keySet());
	    ...
	    List<GenericPair<Integer, Integer>> undefinedCurricularPeriods = new ArrayList(
		    undefinedEnrolments.keySet());
	...
	private void generateEnrolmentCurricularPeriod(GenericPair<Integer, Integer> yearSemester,
		Set<Enrolment> enrolmentsByCurricularPeriod) {
	    final HtmlTable executionPeriodTable = new HtmlTable();
	    executionPeriodTable.setBorder("0");
	    scpDiv.addChild(executionPeriodTable);
	    executionPeriodTable.setClasses(getTablesClasses() + " " + getInitialWidth()
	    ...
	    executionPeriodNameCell.setBody(new HtmlText(yearSemester.getLeft() + " Ano, "
		    + yearSemester.getRight() + " Semestre"));
