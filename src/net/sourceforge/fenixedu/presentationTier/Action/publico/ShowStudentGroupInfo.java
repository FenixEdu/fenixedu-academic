...
public class ShowStudentGroupInfo extends Action {
	public String buildInfo(Integer executionCourseID, String username, String password, String requestURL) {
		...
		try {
			...
			for (Object element : groupings) {
				Grouping grouping = (Grouping) element;
						...
						for (Attends searchingAttendsSet : studentGroup.getAttends()) {
							if (searchingAttendsSet.getRegistration().getPerson().hasUsername(username)) {
								...
								for (Attends attends : studentGroup.getAttends()) {
									...
									result += "-";
									result +=
											attends.getRegistration().getActiveStudentCurricularPlan().getDegreeCurricularPlan()
													.getName()
													+ "\n";
									remainingStudents--;
								}
								...
								for (Lesson lesson : studentGroup.getShift().getAssociatedLessons()) {
									result +=
											lesson.getDiaSemana().toString() + " " + lesson.getInicio().get(Calendar.HOUR_OF_DAY);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
		...
		if (!request.getScheme().equalsIgnoreCase("https")) {
			...
			for (Integer coursesId : coursesIds) {
				...
				result += this.buildInfo(coursesId, studentUsername, studentPassword, requestURL);
			}
			if (result.equals("")) {
				result = "-1";
			}
