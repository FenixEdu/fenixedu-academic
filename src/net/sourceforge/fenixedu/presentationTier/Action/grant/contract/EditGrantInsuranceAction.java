...
@Mapping(
		...
		path = "/editGrantInsurance",
		input = "/editGrantInsurance.do?page=0&method=prepareEditGrantInsuranceForm",
		attribute = "editGrantInsuranceForm",
		formBean = "editGrantInsuranceForm",
		scope = "request",
		parameter = "method")
@Forwards(value = {
		@Forward(name = "manage-grant-contract", path = "/manageGrantContract.do?method=prepareManageGrantContractForm"),
		@Forward(
				name = "edit-grant-insurance",
				path = "/facultyAdmOffice/grant/contract/editGrantInsurance.jsp",
				tileProperties = @Tile(title = "private.teachingstaffandresearcher.listings.byinsurance")) })
public class EditGrantInsuranceAction extends FenixDispatchAction {

	/*
	 * Fills the form with the correspondent data
	 */
	public ActionForward prepareEditGrantInsuranceForm(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			if (infoGrantInsurance == null) { // Is a new Insurance
				...
				infoGrantContract = (InfoGrantContract) ServiceUtils.executeService("ReadGrantContract", argsContract);
		...
		try {
			...
			if (infoGrantInsurance.getInfoGrantPaymentEntity().getNumber() != null) {
				...
				InfoGrantPaymentEntity infoGrantPaymentEntity =
						ReadPaymentEntityByNumberAndClass.run(infoGrantInsurance.getInfoGrantPaymentEntity().getNumber(),
								classname);

				if (infoGrantPaymentEntity == null) {
					if (verifyStringParameterInForm(editGrantInsuranceForm, "project")) {
						return setError(request, mapping, "errors.grant.paymententity.unknownProject", null, infoGrantInsurance
						...
						return setError(request, mapping, "errors.grant.paymententity.unknownCostCenter", null,
								infoGrantInsurance.getInfoGrantPaymentEntity().getNumber());
					} else {
	 ...
	 */
	private void setFormGrantInsurance(DynaValidatorForm form, InfoGrantInsurance infoGrantInsurance) throws Exception {
	...
	private InfoGrantInsurance populateInfoFromForm(DynaValidatorForm editGrantInsuranceForm) throws Exception {
		SimpleDateFormat sdf = new SimpleDateFormat("dd-MM-yyyy");
