...
public class GlobalTeacherServiceDistributionValuationAction extends FenixDispatchAction {
    ...
    public ActionForward prepareForGlobalTeacherServiceDistributionValuation(ActionMapping mapping, ActionForm form,
    ...
    private List<TeacherServiceDistribution> selectOnlyTeacherServiceDistributionsWithPublishedPhases(
	    List<TeacherServiceDistribution> teacherServiceDistributionList) {
    ...
    public ActionForward viewGlobalTeacherServiceDistributionValuation(ActionMapping mapping, ActionForm form,
	    HttpServletRequest request, HttpServletResponse response) throws FenixFilterException, FenixServiceException {
	IUserView userView = SessionUtils.getUserView(request);
	GlobalTeacherServiceDistributionValuationForm globalForm = (GlobalTeacherServiceDistributionValuationForm) form;
	List<TeacherServiceDistribution> selectedTeacherServiceDistributionList = retrieveSelectedTeacherServiceDistributions(
		userView, globalForm);
	...
	for (TeacherServiceDistribution teacherServiceDistribution : selectedTeacherServiceDistributionList) {
	    ValuationPhase selectedValuationPhase = getSelectedValuationPhaseFromGlobalForm(userView, globalForm,
		    teacherServiceDistribution);
	    ValuationGrouping selectedValuationGrouping = getSelectedValuationGrouping(userView, globalForm,
		    teacherServiceDistribution);

	    teacherServiceDistributionMap.put(selectedValuationPhase.getIdInternal(), new Pair<Integer, Integer>(
		    selectedValuationGrouping.getIdInternal(), (selectedExecutionPeriod != null) ? selectedExecutionPeriod
			    .getIdInternal() : NOT_SELECTED_EXECUTION_PERIOD));
	}
	...
	if (viewType.equals(VIEW_TEACHER_SERVICE_DISTRIBUTION_VALUATION_BY_COURSES)
	    ...
	    courseValuationDTOEntryList = (List<CourseValuationDTOEntry>) ServiceUtils.executeService(userView,
		    "ReadCourseValuationsFromTeacherServiceDistributions", new Object[] { teacherServiceDistributionMap });
	}

	if (viewType.equals(VIEW_TEACHER_SERVICE_DISTRIBUTION_VALUATION_BY_TEACHERS)
		|| viewType.equals(VIEW_TEACHER_SERVICE_DISTRIBUTION_VALUATION_BY_TEACHERS_AND_COURSES)) {
	    valuationTeacherDTOEntryList = (List<ValuationTeacherDTOEntry>) ServiceUtils.executeService(userView,
		    "ReadValuationTeachersFromTeacherServiceDistributions", new Object[] { teacherServiceDistributionMap });
	}
    ...
    private ValuationGrouping getSelectedValuationGrouping(IUserView userView,
	    GlobalTeacherServiceDistributionValuationForm globalForm, TeacherServiceDistribution teacherServiceDistribution)
	    throws FenixFilterException, FenixServiceException {
	Integer valuationGroupingId = Integer.parseInt((String) globalForm.getValuationGrouping(teacherServiceDistribution
    ...
    private ValuationPhase getSelectedValuationPhaseFromGlobalForm(IUserView userView,
	    GlobalTeacherServiceDistributionValuationForm globalForm, TeacherServiceDistribution teacherServiceDistribution)
	    throws FenixFilterException, FenixServiceException {
	Integer valuationPhaseId = Integer.parseInt((String) globalForm.getValuationPhase(teacherServiceDistribution
		.getIdInternal().toString()));
    ...
    private List<TeacherServiceDistribution> retrieveSelectedTeacherServiceDistributions(IUserView userView,
	    GlobalTeacherServiceDistributionValuationForm globalForm) throws FenixFilterException, FenixServiceException {
    ...
    private ExecutionYear getSelectedExecutionYear(IUserView userView, GlobalTeacherServiceDistributionValuationForm globalForm)
    ...
    private ExecutionPeriod getSelectedExecutionPeriod(IUserView userView,
	    GlobalTeacherServiceDistributionValuationForm globalForm) throws FenixServiceException, FenixFilterException {
	return rootDomainObject.readExecutionPeriodByOID(globalForm.getExecutionPeriod());
    }

    private List<ExecutionPeriod> retrieveExecutionPeriodsFromSelectedTeacherServiceDistributions(
	    List<TeacherServiceDistribution> teacherServiceDistributionList) {

	final Set<ExecutionPeriod> executionPeriodSet = new HashSet<ExecutionPeriod>();
	for (final TeacherServiceDistribution teacherServiceDistribution : teacherServiceDistributionList) {
