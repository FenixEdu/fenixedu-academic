...
@Forwards({ @Forward(name = "chooseCurricularCourse", path = "/departmentAdmOffice/lists/listCourseResponsibles.jsp", tileProperties = @Tile(title = "private.administrationofcreditsofdepartmentteachers.lists.responsiblesbycourse")) })
public class ListCourseResponsibles extends FenixDispatchAction {
	...
	for (semester = 1; semester <= 2; semester++) {
		...
		for (Professorship professorship : execCourse.getProfessorships()) {
			...
			Department dept = employee.getLastDepartmentWorkingPlace(executionYear.getBeginDateYearMonthDay(),
			...
			if (dept != null && dept.equals(department)) {
				...
				if (curricCourse != null) {
					...
					SearchCourseResponsiblesParametersBean bean = new SearchCourseResponsiblesParametersBean(
						curricCourse, compCourse, person, execSemester, execDegree.getCampus(),
