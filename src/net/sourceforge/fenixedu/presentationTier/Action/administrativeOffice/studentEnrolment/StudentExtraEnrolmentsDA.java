...
public class StudentExtraEnrolmentsDA extends FenixDispatchAction {
	...
	StudentExtraEnrolmentBean extraEnrolmentBean = new StudentExtraEnrolmentBean(rootDomainObject
		.readStudentCurricularPlanByOID(scpID), rootDomainObject.readExecutionSemesterByOID(executionPeriodID), groupType);

	return showExtraEnrolments(extraEnrolmentBean, mapping, actionForm, request, response);
    }

    private ActionForward showExtraEnrolments(StudentExtraEnrolmentBean extraEnrolmentBean, ActionMapping mapping,
	    ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
	request.setAttribute("extraEnrolmentBean", extraEnrolmentBean);

	NoCourseGroupCurriculumGroup noCourseGroupCurriculumGroup = extraEnrolmentBean.getStudentCurricularPlan()
		.getNoCourseGroupCurriculumGroup(extraEnrolmentBean.getGroupType());
	...
	StudentExtraEnrolmentBean extraEnrolmentBean = (StudentExtraEnrolmentBean) RenderUtils.getViewState().getMetaObject()
		.getObject();
	...
	try {
	    ...
	    extraEnrolmentBean.setDegreeCurricularPlan(rootDomainObject.readDegreeCurricularPlanByOID(Integer.valueOf(request
		    .getParameter("dcpID"))));
