...
public class ExecutionDegreesManagementDispatchAction extends FenixDispatchAction {
	...
	if (executionDegreeID != null) {
	    ...
	    form.set("periodExamsFirstSemesterBegin", executionDegree.getPeriodExamsFirstSemester().getStartYearMonthDay()
		    .toString(dateFormat));
	    form.set("periodExamsFirstSemesterEnd", executionDegree.getPeriodExamsFirstSemester().getEndYearMonthDay().toString(
		    dateFormat));
	...
	try {
	    ...
	    final Date periodGradeSubmissionNormalSeasonFirstSemesterEnd = DateFormatUtil.parse(dateFormat, (String) form
		    .get("periodGradeSubmissionNormalSeasonFirstSemesterEnd"));
	    final Date periodGradeSubmissionNormalSeasonSecondSemesterEnd = DateFormatUtil.parse(dateFormat, (String) form
		    .get("periodGradeSubmissionNormalSeasonSecondSemesterEnd"));
	...
	final ResourceBundle enumerationResources = ResourceBundle.getBundle("resources/EnumerationResources", request
		.getLocale());
	...
	try {
		...
		for (String undeletedExecutionDegreesYear : undeletedExecutionDegreesYears) {
		    ...
		    ActionError error = new ActionError("errors.invalid.delete.not.empty.execution.degree",
