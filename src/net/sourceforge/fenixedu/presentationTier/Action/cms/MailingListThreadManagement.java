...
public class MailingListThreadManagement extends FenixDispatchAction
{
			...
			HttpServletRequest request, HttpServletResponse response) throws FenixActionException
	{
		...
		Iterator<IMailConversation> conversationsIterator = mailingListToView.getMailConversationsIterator();
		while (conversationsIterator.hasNext())
		{
			IMailConversation currentConversation = conversationsIterator.next();
			if (currentConversation.getIdInternal().equals(conversationId))
			{
				mailConversationToView = currentConversation;
				break;
			}
		}
			...
			HttpServletRequest request, HttpServletResponse response) throws FenixActionException
	{
		...
		IMailConversation mailingConversationToView = null;
		Iterator<IMailConversation> conversationsIterator = mailingListToView.getMailConversationsIterator();
		while (conversationsIterator.hasNext())
		{
			IMailConversation currentMailingConversation = conversationsIterator.next();
			if (currentMailingConversation.getIdInternal().equals(threadID))
			{
				mailingConversationToView = currentMailingConversation;
				break;
			}
		}
		...
		try
		{
			...
			for (Iterator<IMailMessage> messagesIterator = mailingConversationToView.getMailMessagesIterator(); messagesIterator.hasNext();)
			{
