...
public class OfficialPublicationBean implements Serializable {
	...
	for (SpecializationName name : names) {
	    ...
	    String enNameInEspArea = (name.getSpecializationArea().getNameEn() == null ? "" : name.getSpecializationArea()
		    .getNameEn());

	    String ptNameInEspArea = (name.getSpecializationArea().getNamePt() == null ? "" : name.getSpecializationArea()
		    .getNamePt());
	...
	try {
	    ChangeDegreeOfficialPublicationReference.run(getDegreeOfficialPublication(),
		    getNewReference());
	} catch (FenixServiceException e) {
