...
public class VigilantGroupManagement extends FenixDispatchAction {
	...
	public ActionForward generateReportForGroup(ActionMapping mapping,
	...
	protected List<WrittenEvaluationVigilancyView> getStatsViewForVigilantGroup(
			VigilantGroup group) {
		List<WrittenEvaluation> evaluations = group
	...
	public ActionForward changeDisplaySettingsByGroups(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
	...
	public ActionForward changeDisplaySettingsByVigilants(
			ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
	...
	private VigilantGroupBean prepareVigilantGroups(HttpServletRequest request) {
		...
		ExamCoordinator coordinator = person
				.getExamCoordinatorForGivenExecutionYear(bean
						.getExecutionYear());
		if (coordinator != null) {
	...
	public ActionForward createVigilantGroup(ActionMapping mapping,
		...
		CreateVigilantGroup.run(bean.getName(), bean.getUnit(), bean
				.getConvokeStrategy(), bean.getContactEmail(), bean
				.getRulesLink(), bean.getBeginFirstUnavailablePeriod(), bean
				.getEndFirstUnavailablePeriod(), bean
				.getBeginSecondUnavailablePeriod(), bean
				.getEndSecondUnavailablePeriod());
	...
	public ActionForward applyChangesToVigilantGroup(ActionMapping mapping,
		...
		UpdateVigilantGroup.run(vigilantGroup, beanWithName.getName(),
				beanWithName.getConvokeStrategy(), beanWithName
						.getContactEmail(), beanWithName.getEmailPrefix(),
				beanWithName.getRulesLink(), beanWithFirstPeriod
						.getBeginFirstUnavailablePeriod(), beanWithFirstPeriod
						.getEndFirstUnavailablePeriod(), beanWithSecondPeriod
						.getBeginSecondUnavailablePeriod(),
	...
	public ActionForward prepareGroupPointsPropertyEdition(
			ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
	...
	public ActionForward selectPreviousPointsSchema(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
	...
	public ActionForward changeYearForPoints(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
	...
	public ActionForward copySchemaPoints(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {

		String previousGroupID = request.getParameter("selectedGroupID");
		VigilantGroup previousGroup = (VigilantGroup) RootDomainObject
				.readDomainObjectByOID(VigilantGroup.class, Integer
	...
	public ActionForward prepareManageVigilantsInGroup(ActionMapping mapping,
		...
		for (Employee employee : employees) {
			...
			if (vigilant != null) {
				for (VigilantGroup group : groups) {
					bounds.add(new VigilantBoundBean(employeePerson, group,
							vigilant.hasVigilantGroup(group)));
				}
			} else {
				for (VigilantGroup group : groups) {
					bounds.add(new VigilantBoundBean(employeePerson, group,
		...
		for (Vigilant vigilant : externalVigilants) {
			...
			for (VigilantGroup group : groups) {
				externalBounds.add(new VigilantBoundBean(vigilant.getPerson(),
						group, vigilant.hasVigilantGroup(group)));
			}
		}

		VigilantGroupBean bean = new VigilantGroupBean();
		bean.setExamCoordinator(getLoggedPerson(request)
				.getCurrentExamCoordinator());
	...
	public ActionForward addVigilantsToGroup(ActionMapping mapping,
		...
		List<VigilantBoundBean> beans = (List<VigilantBoundBean>) RenderUtils
				.getViewState("bounds").getMetaObject().getObject();
		List<VigilantBoundBean> externalBeans = (List<VigilantBoundBean>) RenderUtils
				.getViewState("externalBounds").getMetaObject().getObject();
		...
		for (VigilantBoundBean bean : beans) {
			...
			if (bean.isBounded()) {
				if (vigilant == null
						|| !vigilant.hasVigilantGroup(bean.getVigilantGroup())) {
		...
		List<Vigilant> vigilantsThatCouldNotBeRemoved = (List<Vigilant>) RemoveVigilantsFromGroup
				.run(vigilantsToRemove);
