...
import net.sourceforge.fenixedu.dataTransferObject.InfoEnrolmentServiceResult;
...
public class ShiftEnrolmentAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        InfoEnrolmentServiceResult result = (InfoEnrolmentServiceResult) ServiceUtils.executeService(
                SessionUtils.getUserView(request), "StudentShiftEnrolment", args);
     ...
     * @return ActionErrors
     */
    private ActionErrors getActionErrors(InfoEnrolmentServiceResult result, InfoStudent infoStudent,
            InfoShift infoShift) {
        ...
        case InfoEnrolmentServiceResult.SHIFT_FULL:
            actionErrors.add(SHIFT_FULL_ERROR, new ActionError(SHIFT_FULL_ERROR, infoShift.getNome()));
            break;
        case InfoEnrolmentServiceResult.NON_EXISTING_SHIFT:
            actionErrors.add(SHIFT_NON_EXISTING_ERROR, new ActionError(SHIFT_NON_EXISTING_ERROR,
                    infoShift.getNome()));
            break;
        case InfoEnrolmentServiceResult.NON_EXISTING_STUDENT:
            actionErrors.add(STUDENT_NON_EXISTING_ERROR, new ActionError(STUDENT_NON_EXISTING_ERROR,
                    infoStudent.getNumber()));
            break;
        case InfoEnrolmentServiceResult.NOT_ENROLED_INTO_EXECUTION_COURSE:
            actionErrors.add(STUDENT_NOT_ENROLED_IN_EXECUTION_COURSE_ERROR, new ActionError(
                    STUDENT_NOT_ENROLED_IN_EXECUTION_COURSE_ERROR, infoShift.getNome()));
            break;
        default:
