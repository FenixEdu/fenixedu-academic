...
public class BibTexManagementDispatchAction extends FenixDispatchAction {
	...
	OpenFileBean openFileBean = (OpenFileBean) RenderUtils.getViewState("openFileBean")
	...
	try {
	    verifyPersonImportingInParticipators(getUserView(request).getPerson(), importBibtexBean
		    .getCurrentParticipators());
    ...
    private void verifyPersonImportingInParticipators(Person personImporting,
	    List<BibtexParticipatorBean> participators) throws FenixActionException {
	...
	if ((conferenceBean.getEvent() == null) && (conferenceBean.getCreateEvent() == false)) {
	    ...
	    if (conferenceBean instanceof InproceedingsBean)
		conferenceBean.setActiveSchema("result.publication.details.Inproceedings");
	    else
		conferenceBean.setActiveSchema("result.publication.details.Proceedings");
	    return true;
	}
	return false;
    }
