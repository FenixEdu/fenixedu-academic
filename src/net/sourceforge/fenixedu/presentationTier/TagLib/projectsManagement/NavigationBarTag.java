		...
		if (numberOfSpans > 1) {
			...
			for (int i = 0; i < numberOfSpans; i++) {
				if (i == span) {
					...
					navigationBar.append(" <a class=\"report-navigation-bar\" href='").append(computeUrl(i)).append("'>")
							.append(i + 1).append("</a> ");
				}
	...
	private Map filterParams(HttpServletRequest request) {
		...
		while (paramsIterator.hasNext()) {
			...
			if (param.equals("span")
					|| param.equals(pt.ist.fenixWebFramework.servlets.filters.contentRewrite.GenericChecksumRewriter.CHECKSUM_ATTRIBUTE_NAME)) {
