...
public class ExamsMapRenderer implements IExamsMapRenderer {
	...
	for (int i = 0; i < examsMap.getExecutionCourses().size(); i++) {
		...
		if (infoExecutionCourse.getComment() != null && infoExecutionCourse.getComment().length() > 0) {

		    strBuffer.append(" : ");
		    if (user.equals("sop")) {
			strBuffer.append("<a href='defineExamComment.do?executionCourseCode=");
			strBuffer.append(infoExecutionCourse.getSigla());
			strBuffer.append("&amp;executionPeriodName=" + infoExecutionCourse.getInfoExecutionPeriod().getName());
		...
		if ((season1Exam != null && season1Exam.getAssociatedRooms() != null && season1Exam.getAssociatedRooms().size() > 0)
			|| (season2Exam != null && season2Exam.getAssociatedRooms() != null && season2Exam.getAssociatedRooms()
				.size() > 0)) {
	...
	for (int slot = 0; slot < daysOfWeek.length; slot++) {
	    ...
	    strBuffer.append(examsMapSlotContentRenderer.renderDayContents((ExamsMapSlot) examsMap.getDays().get(
		    week * daysOfWeek.length + slot), year1, year2, user));
	    strBuffer.append("</td>");
	}
    }
