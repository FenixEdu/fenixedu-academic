...
public class ExamsMapContentRenderer implements ExamsMapSlotContentRenderer {
	...
	boolean isFirstDayOfSeason = ((examsMapSlot.getDay().get(Calendar.DAY_OF_MONTH) == examsMap.getFirstDayOfSeason().get(
	...
	boolean isSecondDayOfMonthAndFirstDayWasASunday = (examsMapSlot.getDay().get(Calendar.DAY_OF_MONTH) == 2 && examsMapSlot
		.getDay().get(Calendar.DAY_OF_WEEK) == 2);
	...
	for (int i = 0; i < examsMapSlot.getExams().size(); i++) {
		...
		if (typeUser.equals("sop")) {
		    strBuffer.append("<a href='viewExamsMap.do?method=edit" + "&amp;executionCourseInitials="
			    + infoExecutionCourse.getSigla() + "&amp;ePName="
			    ...
			    + infoExam.getSeason().getseason() + "&amp;" + SessionConstants.EXECUTION_PERIOD_OID + "="
