	...
	for (int hourIndex = 0; hourIndex < timeTable.getNumberOfHours().intValue(); hourIndex++) {
		...
		if (timeTableSlot != null) {
			...
			if (infoLessonWrapper != null) {
			    ...
			    Calendar start = infoLessonWrapper.getLessonSlot().getInfoLessonWrapper().getInfoShowOccupation()
				    .getInicio();
			    Calendar end = infoLessonWrapper.getLessonSlot().getInfoLessonWrapper().getInfoShowOccupation()
				    .getFim();
