...
public class ExamsMapRenderer implements IExamsMapRenderer {
    ...
    public ExamsMapRenderer(ExamsMap examsMap, ExamsMapSlotContentRenderer examsMapSlotContentRenderer, String typeUser,
	    String mapType, Locale locale) {
	...
	for (int i = 0; i < numberOfCurricularYearsToDisplay; i++) {
	    ...
	    if (i + 1 < numberOfCurricularYearsToDisplay
		    && year1.intValue() + 1 == ((Integer) this.examsMap.getCurricularYears().get(i + 1)).intValue()) {
		...
		if (user.equals("sop")) {
		    strBuffer.append("<strong>" + examsMap.getInfoExecutionDegree().getInfoDegreeCurricularPlan().getName()
			    + "</strong><br/>");
		}
    ...
    private void renderExecutionCourseListForYear(StringBuilder strBuffer, Integer curricularYear, PageContext pageContext) {
	if (!examsMap.hasExecutionCoursesForGivenCurricularYear(curricularYear)) {
	    strBuffer.append("<i>").append(getMessageResource(pageContext, "no.execution.courses.for.curricular.year")).append(
		    curricularYear).append("</i>");
	} else {
		...
		if (infoExecutionCourse.getCurricularYear().equals(curricularYear)) {
		    ...
		    boolean hasComment = infoExecutionCourse.getComment() != null
			    && infoExecutionCourse.getComment().length() > 0 && !infoExecutionCourse.getComment().equals(" ");
			...
			strBuffer.append("<i>").append(getMessageResource(pageContext, "no.exams.for.curricular.year")).append(
				"</i>");
			strBuffer.append("</td>");
		    } else {
			// Get 1st season exam
			InfoExam season1Exam = (InfoExam) CollectionUtils.find(infoExecutionCourse.getAssociatedInfoExams(),
				new Predicate() {
			...
			// Get 2nd season exam
			InfoExam season2Exam = (InfoExam) CollectionUtils.find(infoExecutionCourse.getAssociatedInfoExams(),
				new Predicate() {
				    public boolean evaluate(Object obj) {
					InfoExam infoExam = (InfoExam) obj;
			...
			// && season2Exam.getAssociatedRooms().size() > 0))
			{
				...
				if (user.equals("sop")) {
				    ...
				    strBuffer.append("<a href='" + addPublicPrefix("showExamsManagement.do")
					    + "?method=edit&amp;" + SessionConstants.EXECUTION_COURSE_OID + "="
					    ...
					    + SessionConstants.CURRICULAR_YEAR_OID + "=" + curricularYear.toString() + "&amp;"
					    + SessionConstants.EXAM_OID + "=" + season1Exam.getIdInternal() + "'>");
				}
				...
				if (user.equals("sop")) {
				    ...
				    strBuffer.append("<a href='" + addPublicPrefix("showExamsManagement.do")
					    + "?method=delete&amp;" + SessionConstants.EXECUTION_COURSE_OID + "="
					    + infoExecutionCourse.getIdInternal() + "&amp;"
					    + SessionConstants.EXECUTION_PERIOD_OID + "="
					    + infoExecutionCourse.getInfoExecutionPeriod().getIdInternal() + "&amp;"
					    + SessionConstants.EXECUTION_DEGREE_OID + "="
				...
				if (user.equals("sop")) {
				    ...
				    strBuffer.append("<a href='" + addPublicPrefix("showExamsManagement.do")
					    ...
					    + examsMap.getInfoExecutionDegree().getIdInternal() + "&amp;"
					    + SessionConstants.CURRICULAR_YEAR_OID + "=" + curricularYear.toString() + "&amp;"
					    + SessionConstants.EXAM_OID + "=" + season2Exam.getIdInternal() + "'>");
				}
	...
	for (int slot = 0; slot < daysOfWeek.length; slot++) {
	    ...
	    strBuffer.append(examsMapSlotContentRenderer.renderDayContents((ExamsMapSlot) examsMap.getDays().get(
		    week * daysOfWeek.length + slot), year1, year2, user, pageContext));
	    strBuffer.append("</td>");
	}
    }
	...
	// PRINT EXECUTION DEGREE
	strBuffer.append("<strong>" + examsMap.getInfoExecutionDegree().getInfoDegreeCurricularPlan().getInfoDegree().getNome()
		+ "</strong><br />");
	...
	for (int j = 0; j < infoExecutionCourse.getAssociatedInfoExams().size(); j++) {
		...
		for (int k = 0; k < infoExam.getWrittenEvaluationSpaceOccupations().size(); k++) {
		    ...
		    InfoRoomOccupation infoRoomOccupation = (InfoRoomOccupation) infoExam.getWrittenEvaluationSpaceOccupations()
			    .get(k);
		    strBuffer.append(infoRoomOccupation.getInfoRoom().getNome());
		}
		strBuffer.append("</td>");
