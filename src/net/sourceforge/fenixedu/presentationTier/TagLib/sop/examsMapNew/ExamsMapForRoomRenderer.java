...
public class ExamsMapForRoomRenderer implements IExamsMapRenderer {
	...
	for (int slot = 0; slot < daysOfWeek.length; slot++) {
	    ...
	    strBuffer.append(examsMapSlotContentRenderer.renderDayContents((ExamsMapSlot) examsMap.getDays().get(
		    week * daysOfWeek.length + slot), examsMap, user, pageContext));
	...
	for (int week = 0; week < numberOfWeks; week++) {
		...
		for (int i = 0; i < examsMapSlot.getExams().size(); i++) {
			...
			InfoCurricularCourseScope infoCCS = (InfoCurricularCourseScope) infoExam
				.getAssociatedCurricularCourseScope().get(iterEC);
