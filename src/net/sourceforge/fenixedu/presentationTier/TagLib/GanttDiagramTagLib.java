...
public class GanttDiagramTagLib extends TagSupport {
	...
	if (!getEvents().isEmpty()) {
		...
		builder.append("<table style=\"width:").append(tableWidth.add(BigDecimal.valueOf(FIXED_COLUMNS_SIZE_EM))).append(
			"em;\" class=\"tcalendar thlight\">");
	    } else {
		...
		String eventName = event.getGanttDiagramEventName().getContent(
		...
		if (getViewTypeEnum() == ViewType.YEAR_DAILY) {
		    builder.append("<a href=\"").append(eventUrl).append("&amp;month=").append(
			    Month.values()[((VacationsEvent) event).getMonth() - 1].toString()).append("\">").append("*").append(
			    eventName);
		} else {
		...
		for (DateTime day : getGanttDiagramObject().getDays()) {
		    ...
		    if (getViewTypeEnum() == ViewType.YEAR_DAILY) {
			monthOfYear = ((VacationsEvent) event).getMonth();
		    }
		    int year = day.getYear();
		    YearMonthDay yearMonthDay = new YearMonthDay(year, monthOfYear, 1);
		    builder.append("<td style=\"width: ").append(convertToEm(numberOfUnits)).append(
			    "em;\"><div style=\"position: relative;\">");
		...
		if (isShowPeriod()) {
		    builder.append("<td class=\"padded smalltxt\" title=\"").append(event.getGanttDiagramEventPeriod()).append(
			    "\"><div style=\"overflow:hidden;\" class=\"nowrap\">").append(event.getGanttDiagramEventPeriod())
			    .append("</div></td>");
		}
		if (isShowObservations()) {
		    builder.append("<td class=\"padded smalltxt\">").append(event.getGanttDiagramEventObservations()).append(
			    "</td>");
		}
		builder.append("</tr>");
	    }
	...
	if (!getEvents().isEmpty()) {
		...
		for (DateTime month : getGanttDiagramObject().getMonths()) {
		    ...
		    builder.append("<td style=\"width: ").append(convertToEm(monthNumberOfDays * scale)).append(
			    "em;\"><div style=\"position: relative;\">");
			...
			if (intervalStart.getMonthOfYear() == month.getMonthOfYear()
				...
				if (intervalStart.getDayOfMonth() == 1) {
					...
					entryDays = convertToEm((Days.daysBetween(intervalStart, lastDayOfMonth).getDays() + 1)
						* scale);
					...
					entryDays = convertToEm((Days.daysBetween(intervalStart, intervalEnd).getDays() + 1)
						* scale);
					startDay = convertToEm((intervalStart.getDayOfMonth() - 1) * scale);
					addSpecialDiv(builder, entryDays, startDay);
				    }
				...
				if (intervalEnd.getYear() > month.getYear()
				    ...
				    entryDays = convertToEm((Days.daysBetween(firstDayOfMonth, lastDayOfMonth).getDays() + 1)
					    * scale);
				    ...
				    if (intervalEnd.getMonthOfYear() == month.getMonthOfYear()
					    && intervalEnd.getYear() == month.getYear()) {
					entryDays = convertToEm((Days.daysBetween(firstDayOfMonth, intervalEnd).getDays() + 1)
						* scale);
					startDay = convertToEm((firstDayOfMonth.getDayOfMonth() - 1) * scale);
					addSpecialDiv(builder, entryDays, startDay);
				    }
	...
	case WEEKLY:
	    ...
	    builder.append("<tr>");
	    builder.append("<th style=\"width: 15em;\" rowspan=\"4\">").append(getMessage("label.ganttDiagram.event")).append(
		    "</th>");
	    for (Integer year : getGanttDiagramObject().getYearsView().keySet()) {
		builder.append("<th colspan=\"").append(getGanttDiagramObject().getYearsView().get(year)).append("\">").append(
			year).append("</th>");
	    }
	    if (isShowPeriod()) {
		...
		for (DateTime day : getGanttDiagramObject().getDays()) {
		    builder.append("<th>").append("<a href=\"").append(dailyViewUrl_).append(day.toString("ddMMyyyy")).append(
			    "\">").append(day.getDayOfMonth()).append("</a>").append("</th>");
		}
	    } else {
		for (DateTime day : getGanttDiagramObject().getDays()) {
	...
	case MONTHLY:
	    ...
	    builder.append("<tr>");
	    builder.append("<th style=\"width: 15em;\" rowspan=\"2\">").append(getMessage("label.ganttDiagram.event")).append(
		    "</th>");
	    for (YearMonthDay month : getGanttDiagramObject().getMonthsView().keySet()) {
		builder.append("<th colspan=\"").append(getGanttDiagramObject().getMonthsView().get(month)).append("\">").append(
			month.toString("MMM yyyy", getGanttDiagramObject().getLocale())).append("</th>");
	    }
	    if (isShowPeriod()) {
	...
	case DAILY:
	    ...
	    builder.append("<th>").append(
		    getGanttDiagramObject().getFirstInstant().toString("E", getGanttDiagramObject().getLocale())).append(", ")
		    .append(getGanttDiagramObject().getFirstInstant().getDayOfMonth()).append(" ");

	    if (!StringUtils.isEmpty(getMonthlyViewUrl())) {
		String monthlyViewUrl_ = getRequest().getContextPath() + getMonthlyViewUrl() + "&amp;" + getFirstDayParameter()
			+ "=";
		builder.append("<a href=\"").append(monthlyViewUrl_).append(
			getGanttDiagramObject().getFirstInstant().toString("ddMMyyyy")).append("\">").append(
			getGanttDiagramObject().getFirstInstant().toString("MMM", getGanttDiagramObject().getLocale())).append(
			"</a>");
	    } else {
		...
		String weeklyViewUrl_ = getRequest().getContextPath() + getWeeklyViewUrl() + "&amp;" + getFirstDayParameter()
			+ "=";
		builder.append(" (<a href=\"").append(weeklyViewUrl_).append(
			getGanttDiagramObject().getFirstInstant().toString("ddMMyyyy")).append("\">");
		builder.append(getMessage("label.ganttDiagram.week")).append(
			getGanttDiagramObject().getFirstInstant().getWeekOfWeekyear()).append(")</a>");
	    }
	...
	case MONTHLY_TOTAL:
	    ...
	    builder.append("<th>").append(
		    getGanttDiagramObject().getFirstInstant().toString("MMM", getGanttDiagramObject().getLocale())).append(" ")
		    .append(getGanttDiagramObject().getFirstInstant().getYear()).append("</th>");
	    if (isShowPeriod()) {
		builder.append("<th style=\"width: 20em;\">").append(getMessage("label.ganttDiagram.period")).append("</th>");
	    }
	    if (isShowObservations()) {
	...
	if (firstDay != null) {
		...
		if (!StringUtils.isEmpty(getWeeklyViewUrl())) {
		    ...
		    builder.append(" , ").append("<a href=\"").append(nextUrl).append("\">")
			    .append(getMessage("label.next.week")).append(" &gt;&gt;").append("</a>").append(
				    "</span></td><td class=\"tcalendarlinks\"></td><td class=\"tcalendarlinks\"></td></tr>");
		}
		break;
		...
		if (!StringUtils.isEmpty(getDailyViewUrl())) {
		    ...
		    builder
			    ...
			    .append(beforeUrl).append("\">").append("&lt;&lt; ").append(getMessage("label.previous.day")).append(
				    "</a>");
		...
		if (!StringUtils.isEmpty(getMonthlyViewUrl())) {
		    ...
		    builder.append("<tr><td class=\"tcalendarlinks\"></td><td colspan=\"").append(monthNumberOfDays).append(
			    "\" class=\"acenter tcalendarlinks\"><span class=\"smalltxt\"><a href=\"").append(beforeUrl).append(
			    "\">").append("&lt;&lt; ").append(getMessage("label.previous.month")).append("</a>");
		    builder.append(" , ").append("<a href=\"").append(nextUrl).append("\">").append(
			    getMessage("label.next.month")).append(" &gt;&gt;").append("</a>").append(
			    "</span></td><td class=\"tcalendarlinks\"></td><td class=\"tcalendarlinks\"></td></tr>");
		}
		break;
	...
	builder.append("<div style=\"height: 1.8em;\" class=\"");
