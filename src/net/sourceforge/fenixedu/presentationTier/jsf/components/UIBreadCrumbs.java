...
public class UIBreadCrumbs extends UIInput {
	...
	final ResourceBundle publicDegreeInformationResourceBundle = ResourceBundle.getBundle(
		"resources/PublicDegreeInformation", locale);
	...
	final String labelEducation = publicDegreeInformationResourceBundle
		.getObject("public.degree.information.label.education").toString();
	...
	responseWriter.write(CRUMB_SEPERATOR);
	if (degree != null) {
	    writeLink(responseWriter, context.getExternalContext().getRequestContextPath() + DEGREE_SITE_LINK
		    + degree.getIdInternal(), degree.getSigla());
	    responseWriter.write(CRUMB_SEPERATOR);
