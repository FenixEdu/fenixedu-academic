...
public class DMLRelationDescriptor {
    ...
    public DMLRelationDescriptor(final ClassDescriptor classDescriptor, final ObjectReferenceDescriptor objectReferenceDescriptor) {
	super();
    ...
    public DMLRelationDescriptor(final ClassDescriptor classDescriptor, final CollectionDescriptor collectionDescriptor) {
	super();
	this.classDescriptor1 = classDescriptor;
	...
	if (objectReferenceDescriptor2 != null) {
	    ...
	    // throw new
	    // RuntimeException("Class was not properly initialized.");
	}
	...
	for (final Iterator iterator = vector.iterator(); iterator.hasNext();) {
		...
		final FieldDescriptor fieldDescriptor = classDescriptor.getFieldDescriptorByIndex(columnIndex.intValue());
		columnName = fieldDescriptor.getColumnName();
	    } else {
		throw new RuntimeException("Unsupported foreign key field type: " + foreignKeyField.getClass().getName());
	    }

	    final CollectionDescriptor collectionDescriptor = findInverseCollectionDescriptor(objectReferenceDescriptor,
		    foreignKeyField);
	    if (collectionDescriptor != null) {
		collectionDescriptor2 = collectionDescriptor;
	    } else {
		System.out.println("No inverse reference found for: " + classDescriptor1.getFullTableName() + "."
			+ objectReferenceDescriptor.getAttributeName() + " to " + classDescriptor2.getFullTableName());
	    }
    ...
    protected CollectionDescriptor findInverseCollectionDescriptor(final ObjectReferenceDescriptor objectReferenceDescriptor,
	    final Object foreignKeyField) {
	// System.out.println(classDescriptor1.getFullTableName() + "." +
	// objectReferenceDescriptor.getAttributeName());
	final Vector collectionDescriptors = classDescriptor2.getCollectionDescriptors();
	// System.out.println("   collectionDescriptors.size= " +
	...
	for (final Iterator iterator = collectionDescriptors.iterator(); iterator.hasNext();) {
	    ...
	    //System.out.println("   collectionDescriptors.getAttributeName()= "
	    // + collectionDescriptor.getAttributeName());
		...
		if (foreignKeyField.equals(fk)
			&& collectionDescriptor.getItemClassName().equals(
				objectReferenceDescriptor.getClassDescriptor().getClassOfObject())) {
    ...
    protected String getKeyToOtherClass(final ClassDescriptor classDescriptor, final CollectionDescriptor collectionDescriptor) {
