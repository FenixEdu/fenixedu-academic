...
public class CheckOJBConversors {
	...
	for (ClassDescriptor classDescriptor : (Collection<ClassDescriptor>) ojbMetadata.values()) {
		...
		if (fieldDescriptions != null) {
			...
			Slot slot = domClass.findSlot(descriptor.getAttributeName());
			if (slot == null) {
			    // System.out.println("BITCHES here -> " +
			...
			if (slot.getType().equals("java.lang.Double")) {
			    if ((descriptor.getColumnType().equals("VARCHAR") || descriptor.getColumnType().equals("INTEGER") || descriptor
				    .getColumnType().equals("FLOAT"))) {
				alterTypesCommands.add("alter table "
					...
					+ descriptor.getColumnName()
					+ " double "
					+ DBColumnInfo.getInfoWithType(classDescriptor.getFullTableName(), descriptor
						.getColumnName(), false) + ";");
			    }
