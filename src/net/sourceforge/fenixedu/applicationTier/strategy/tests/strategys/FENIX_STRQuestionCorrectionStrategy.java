...
public class FENIX_STRQuestionCorrectionStrategy extends QuestionCorrectionStrategy {
	...
	if (studentTestQuestion.getSubQuestionByItem().getQuestionType().getType().intValue() == QuestionType.STR) {
	    ResponseProcessing responseProcessing = getSTRResponseProcessing(studentTestQuestion.getSubQuestionByItem()
		    .getResponseProcessingInstructions(), ((ResponseSTR) studentTestQuestion.getResponse()).getResponse());
