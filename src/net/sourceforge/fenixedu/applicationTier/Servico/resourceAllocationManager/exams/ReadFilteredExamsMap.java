...
public class ReadFilteredExamsMap extends Service {
	...
	List<InfoExecutionCourse> infoExecutionCourses = obtainInfoExecutionCourses(curricularYears, infoExecutionPeriod,
		executionDegree);
	...
	for (Integer curricularYear : curricularYears) {
	    ...
	    List<ExecutionCourse> executionCourses = executionDegree.getDegreeCurricularPlan()
		...
		List<InfoExam> associatedInfoExams = obtainInfoExams(executionDegree, infoExecutionPeriod.getIdInternal(),
			curricularYear, executionCourse);
	...
	for (Exam exam : executionCourse.getAssociatedExams()) {
		...
		if (!checkedCurricularCourses.contains(curricularCourse)) {
		    ...
		    int numberEnroledStudentsInCurricularCourse = curricularCourse
			    .countEnrolmentsByExecutionPeriod(executionSemester);
		...
		DegreeCurricularPlan degreeCurricularPlanFromScope = degreeModuleScope.getCurricularCourse()
