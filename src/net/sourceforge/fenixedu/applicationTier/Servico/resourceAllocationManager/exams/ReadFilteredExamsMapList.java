...
public class ReadFilteredExamsMapList extends Service {
	...
	final ExecutionDegree executionDegreeFromDB = rootDomainObject.readExecutionDegreeByOID(infoExecutionDegree
		.getIdInternal());
	...
	for (int i = 0; i < curricularYears.size(); i++) {
		...
		DegreeCurricularPlan degreeCurricularPlan = DegreeCurricularPlan.readByNameAndDegreeSigla(infoExecucaoDegree
			.getInfoDegreeCurricularPlan().getName(), infoExecucaoDegree.getInfoDegreeCurricularPlan()
		...
		if (degreeCurricularPlan != null) {
			...
			for (int k = 0; k < associatedExams.size(); k++) {
				...
				InfoCurricularYear infoCurricularYear = infoCurricularCourseScope.getInfoCurricularSemester()
