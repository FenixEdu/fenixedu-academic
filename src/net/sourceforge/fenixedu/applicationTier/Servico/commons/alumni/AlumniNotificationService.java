...
public class AlumniNotificationService extends FenixService {
	...
	final String body = MessageFormat.format(getAlumniBundle().getString("alumni.public.registration.url"),
	...
	String body;
	if (approval) {
	    body = MessageFormat.format(getManagerBundle().getString("alumni.identity.request.confirm.identity"), request
		...
		body += MessageFormat.format(getManagerBundle().getString("alumni.identity.request.password.request"), request
			.getAlumni().getLoginUsername(), String.valueOf(request.getOID()), request.getRequestToken().toString());
		break;
	    case STUDENT_NUMBER_RECOVERY:
		body += MessageFormat.format(getManagerBundle().getString("alumni.identity.request.student.number.info"), request
	    ...
	    body = MessageFormat.format(getManagerBundle().getString("alumni.identity.request.refuse.identity"), request
		    .getAlumni().getStudent().getPerson().getFirstAndLastName());
	}
	...
	if (!approval && request.getRequestType().equals(AlumniRequestType.PASSWORD_REQUEST)) {
	    body += "\n"
		    + MessageFormat.format(getManagerBundle().getString("alumni.identity.request.password.request.refuse"),
	...
	final String body = MessageFormat.format(getAlumniBundle().getString("alumni.public.username.login.url"), alumni
