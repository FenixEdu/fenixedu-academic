...
public class GenerateDegreeAdministrativeOfficeSibsOutgoingPaymentFile extends BuildInformationForDegreeAdministrativeOfficeDebts {
	...
	final SibsOutgoingPaymentFile sibsOutgoingPaymentFile = new SibsOutgoingPaymentFile(SOURCE_INSTITUTION_ID,
		DESTINATION_INSTITUTION_ID, ENTITY_CODE);

	for (final Entry<Person, List<Event>> entry : getNotPayedEventsGroupedByPerson(executionYear).entrySet()) {
	    for (final Event event : entry.getValue()) {
		for (final AccountingEventPaymentCode paymentCode : event.calculatePaymentCodes()) {
		    sibsOutgoingPaymentFile.addLine(paymentCode.getCode(), paymentCode.getMinAmount(),
			    paymentCode.getMaxAmount(), paymentCode.getStartDate(), paymentCode.getEndDate());
		}
