...
public class CreateMaterial extends Service {
	...
	if(bean != null) {	    
		...
		case EXTENSION:
		    new Extension(bean.getIdentification(), bean.getBarCodeNumber(), bean.getAcquisition(), bean.getCease(), bean.getOwner());
		    break;

		case FIRE_EXTINGUISHER:
		    new FireExtinguisher(bean.getIdentification(), bean.getBarCodeNumber(), bean.getAcquisition(), bean.getCease(), bean.getOwner(),
			    bean.getDelivererEnterprise(), bean.getLoadedDate(), bean.getToBeInspectedDate());
		    break;
