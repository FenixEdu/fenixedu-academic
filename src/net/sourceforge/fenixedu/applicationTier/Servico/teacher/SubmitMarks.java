...
public class SubmitMarks extends Service {
    ...
    public void run(Integer executionCourseID, Integer evaluationID, String[] attendsIDs, Date evaluationDate, IUserView userView)
	    throws InvalidArgumentsServiceException, OutOfPeriodException, ExcepcaoPersistencia {
	...
	Map<GenericTrio<String, Degree, CurricularCourse>, List<Attends>> attendsMap = separateAttendsByDegreeAndEnrolmentType(
		executionCourse, attendsIDs, finalEvaluation);
	...
	for (CurricularCourse curricularCourse : executionCourse.getAssociatedCurricularCourses()) {
		...
		if (evaluationDate.before(occupationPeriod.getStart()) || evaluationDate.after(occupationPeriod.getEnd())) {
		    throw new OutOfPeriodException("error.invalid.evaluationDate", occupationPeriod.getStart(), occupationPeriod
			    .getEnd());
		}
	    }
	}
    ...
    private void submitMarks(Map<GenericTrio<String, Degree, CurricularCourse>, List<Attends>> attendsMap,
	    ExecutionCourse executionCourse, FinalEvaluation finalEvaluation, Teacher teacher, Date evaluationDate)
	    throws IOException {
	...
	for (Entry<GenericTrio<String, Degree, CurricularCourse>, List<Attends>> mapEntry : attendsMap.entrySet()) {
		...
		addMarksFileToMap(improvmentFileList, finalEvaluation, mapEntry.getKey().getMiddle(), mapEntry.getKey()
			.getRight(), teacher, marksMap, executionCourse.getExecutionPeriod(), version, IMPROVMENT);
	    } else {
		addMarksFileToMap(normalFileList, finalEvaluation, mapEntry.getKey().getMiddle(), mapEntry.getKey().getRight(),
			teacher, marksMap, executionCourse.getExecutionPeriod(), version, NORMAL);
	    }
	}
    ...
    private void addMarksFileToMap(Map<String, InputStream> fileMap, FinalEvaluation finalEvaluation, Degree degree,
	    CurricularCourse curricularCourse, Teacher teacher, Map<Attends, FinalMark> marksMap,
	    ExecutionSemester executionSemester, Integer version, String season) throws IOException {
	MarksFile marksFile = new MarksFile(degree, teacher, curricularCourse.getCurricularSemesterWithLowerYearBySemester(
		executionSemester.getSemester(), executionSemester.getEndDate()), curricularCourse, version);
    ...
    private Map<Attends, FinalMark> getMarks(List<Attends> attendsList, FinalEvaluation finalEvaluation, Date evaluationDate,
	    Date when, Integer version) {
    ...
    private Map<GenericTrio<String, Degree, CurricularCourse>, List<Attends>> separateAttendsByDegreeAndEnrolmentType(
	    ExecutionCourse executionCourse, String[] attendsIDs, FinalEvaluation finalEvaluation) {
	...
	for (String attendsID : attendsIDs) {
	    ...
	    if (attends != null && attends.getEnrolment() != null
		    && !attends.getRegistration().getDegreeType().equals(DegreeType.MASTER_DEGREE)) {
		if (attends.getEnrolment().isImprovementForExecutionCourse(executionCourse)) {
		    addToMap(map, IMPROVMENT, attends.getEnrolment().getStudentCurricularPlan().getDegreeCurricularPlan()
			    .getDegree(), attends.getEnrolment().getCurricularCourse(), attends);
		} else {
		    addToMap(map, NORMAL,
			    attends.getEnrolment().getStudentCurricularPlan().getDegreeCurricularPlan().getDegree(), attends
				    .getEnrolment().getCurricularCourse(), attends);
		}
	    }
	}
    ...
    private void addToMap(Map<GenericTrio<String, Degree, CurricularCourse>, List<Attends>> map, String enrolmentType,
	    Degree degree, CurricularCourse curricularCourse, Attends attends) {
	GenericTrio<String, Degree, CurricularCourse> trio = new GenericTrio<String, Degree, CurricularCourse>(enrolmentType,
		degree, curricularCourse);
	...
	public MarksFile(Degree degree, Teacher teacher, CurricularSemester curricularSemester,
		CurricularCourse curricularCourse, Integer version) {
	...
	public void addLines(Map<Attends, FinalMark> attendsMap, String season) {
		...
		addLine(entry.getKey().getRegistration(), season, entry.getValue(), entry.getKey().getEnrolment()
	...
	public boolean equals(Object obj) {
	    ...
	    return (this.getLeft().equals(genericTrio.getLeft()) && this.getMiddle().equals(genericTrio.getMiddle()) && this
		    .getRight().equals(genericTrio.getRight()));
	}
