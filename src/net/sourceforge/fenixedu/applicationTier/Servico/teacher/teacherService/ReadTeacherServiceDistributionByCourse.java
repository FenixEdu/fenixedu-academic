...
public class ReadTeacherServiceDistributionByCourse implements IService {
		...
		for(ICompetenceCourse cc:  competenceCourseList) {
			for(ICurricularCourse curricularCourseEntry: cc.getAssociatedCurricularCourses()) {	
				...
				for(IExecutionPeriod executionPeriodEntry: executionPeriodList) {
					...
					List<ICurricularCourseScope> scopesList = curricularCourseEntry.getActiveScopesInExecutionPeriod(executionPeriodEntry);
					Set<String> curricularYearsList = new HashSet<String>();
					for(ICurricularCourseScope scopeEntry : scopesList){
						curricularYearsList.add(curricularCourseEntry.getCurricularYearByBranchAndSemester(scopeEntry.getBranch(), scopeEntry.getCurricularSemester().getSemester()).getYear().toString());
					}
					
					for(IExecutionCourse executionCourseEntry: (List<IExecutionCourse>) curricularCourseEntry.getExecutionCoursesByExecutionPeriod(executionPeriodEntry)) {
						
						if(executionCoursesMap.containsKey(executionCourseEntry.getIdInternal())){
							returnDTO.addDegreeNameToExecutionCourse(executionCourseEntry.getIdInternal(), curricularCourseEntry.getDegreeCurricularPlan().getName());
							continue;
						}
						
						int executionCourseFirstTimeEnrollementStudentNumber = executionCourseEntry.getFirstTimeEnrolmentStudentNumber();
						int executionCourseSecondTimeEnrollementStudentNumber = executionCourseEntry.getSecondOrMoreTimeEnrolmentStudentNumber();
																										
						String campus = getCampusForCurricularCourseAndExecutionPeriod(curricularCourseEntry, executionPeriodEntry);
						
						returnDTO.addExecutionCourse(executionCourseEntry.getIdInternal(), executionCourseEntry.getNome(), campus, 
								curricularCourseEntry.getDegreeCurricularPlan().getDegree().getSigla(), curricularYearsList, 
								...
								executionCourseEntry.getTotalHours(ShiftType.TEORICA), executionCourseEntry.getTotalHours(ShiftType.PRATICA), executionCourseEntry.getTotalHours(ShiftType.LABORATORIAL),
								executionCourseEntry.getTotalHours(ShiftType.TEORICO_PRATICA), executionCourseEntry.getStudentsNumberByShift(ShiftType.TEORICA), executionCourseEntry.getStudentsNumberByShift(ShiftType.PRATICA),
								executionCourseEntry.getStudentsNumberByShift(ShiftType.LABORATORIAL), executionCourseEntry.getStudentsNumberByShift(ShiftType.TEORICO_PRATICA));							
	...
	private void fillExecutionCourseDTOWithTeachers(DistributionTeacherServicesByCourseDTO dto, IExecutionCourse executionCourse, IDepartment department) {
		...
		for(IProfessorship professorShipEntry: executionCourse.getProfessorships()) {
			...
			dto.addTeacherToExecutionCourse(executionCourse.getIdInternal(), teacherIdInternal, teacherName, teacherRequiredHours.intValue(), teacherBelongsToDepartment);										
		}
		
	}

	private String getCampusForCurricularCourseAndExecutionPeriod(ICurricularCourse curricularCourse, IExecutionPeriod executionPeriod) {
		String campus = "";
		
		for(IExecutionDegree executionDegreeEntry: curricularCourse.getDegreeCurricularPlan().getExecutionDegrees()) {
