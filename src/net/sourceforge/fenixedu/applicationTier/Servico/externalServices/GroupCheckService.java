...
public class GroupCheckService extends Service {
     ...
     * 
     * Available queries: 
     * user=ISTnnnn;role=ROLE;type=DEPARTMENT;unit=IST.<DepartmentAcronym>[;year=<TheYear>]
    ...
    private Boolean checkCurricularCourseGroup(Person person, GroupCheckQuery groupCheckQuery)
        ...
        for (DegreeCurricularPlan degreeCurricularPlan : degree.getActiveDegreeCurricularPlans()) {
            ...
            if (curricularCourse != null) {
                Group group = new CurricularCourseStudentsByExecutionPeriodGroup(curricularCourse,
                        executionPeriod);
    ...
    private Unit getUnit(String[] unitAcronyms, int maxIndex) throws ExcepcaoPersistencia,
        ...
        for (int i = 1; i <= maxIndex; i++) {
            String unitAcronym = unitAcronyms[i];
            unit = unit.getChildUnitByAcronym(unitAcronym);
    ...
    private Boolean checkDepartmentGroup(Person person, GroupCheckQuery groupCheckQuery)
        ...
        Department department = getDepartment(unitAcronyms);
        ExecutionYear executionYear = getExecutionYear(groupCheckQuery.year);
        Group group;

        if (groupCheckQuery.roleType == RoleType.TEACHER) {
            group = new DepartmentTeachersByExecutionYearGroup(executionYear, department);
        } else if (groupCheckQuery.roleType == RoleType.EMPLOYEE) {
            group = new DepartmentEmployeesByExecutionYearGroup(executionYear, department);
        } else {
            group = new DepartmentStudentsByExecutionYearGroup(executionYear, department);
        }
