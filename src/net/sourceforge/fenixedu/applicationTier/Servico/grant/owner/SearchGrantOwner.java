	...
	public static List run(String name, String IdNumber, IDDocumentType IdType, Integer grantOwnerNumber, Boolean onlyGrantOwner,
		...
		if (person == null && grantOwner == null) {
			if (!StringUtils.isEmpty(name)) {
				Integer tempNumberOfElementInSpan =
						Integer.valueOf(PresentationConstants.NUMBER_OF_ELEMENTS_IN_SPAN.intValue() - 1);
				if (onlyGrantOwner.booleanValue()) {
		...
		if ((personList.size() > 0) && !onlyGrantOwner.booleanValue()) {
			...
			for (int i = 0; i < grantOwnerList.size(); i++) {
				InfoGrantOwner infoGrantOwner = InfoGrantOwnerWithPerson.newInfoFromDomain((GrantOwner) grantOwnerList.get(i));
				infoGrantOwnerList.add(infoGrantOwner);
			}
		}
