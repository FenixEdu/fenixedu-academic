...
public class ReadCurricularCoursesToEnroll extends Service {
	...
	final List<CurricularCourse> possibleStudentCurricularCoursesToEnrol = findCurricularCourses(executionDegree
		.getDegreeCurricularPlan().getCurricularCourses(), studentCurricularPlan, executionSemester);
	...
	if (filterByYearAndSemester(curricularYearsList, curricularSemestersList)) {
		...
		searchedCurricularCourses = getActiveAndFilteredCurricularCourses(possibleStudentCurricularCoursesToEnrol,
			verifyYears(curricularYearsList), verifySemesters(curricularSemestersList), executionSemester);
	    }

	} else {
	    if (studentCurricularPlan.getRegistration().getDegreeType().equals(DegreeType.BOLONHA_ADVANCED_FORMATION_DIPLOMA)) {
		searchedCurricularCourses = getActiveCurricularCoursesForBolonha(possibleStudentCurricularCoursesToEnrol,
			executionSemester);
	    } else {
		searchedCurricularCourses = getActiveCurricularCourses(possibleStudentCurricularCoursesToEnrol, executionSemester);
	    }
	}
