...
public class ReadOverheadReport extends FenixService {
	...
	public InfoOverheadReport run(String userView, String costCenter, ReportType reportType, String projectCode,
		...
		if (userNumber.equals(costCenter) || hasFullCostCenterAccess(userView, costCenter, instance)) {
			if (reportType.equals(ReportType.GENERATED_OVERHEADS)) {
				List<IGeneratedOverheadsReportLine> lines =
						new PersistentGeneratedOverheadsReport().getCompleteReport(reportType, new Integer(costCenter), instance);
				...
				List<ITransferedOverheadsReportLine> lines =
						new PersistentTransferedOverheadsReport()
								.getCompleteReport(reportType, new Integer(costCenter), instance);
				...
				List<IOverheadsSummaryReportLine> lines =
						new PersistentOverheadsSummaryReport().getCompleteReport(reportType, new Integer(costCenter), instance);
