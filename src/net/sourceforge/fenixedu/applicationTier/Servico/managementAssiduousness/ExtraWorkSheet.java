 ...
 * 
 */
public class ExtraWorkSheet extends Service {
	// »»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»»
	...
	private List buildSheet(Integer employeeNumber, Timestamp firstDay, Timestamp lastDay, Locale locale)
			throws Exception {
	...
	// Função que será chama no fim do ciclo do metodo execute do
		...
		try {
			...
			if (!_mesInjustificado) {
				...
				while (_dataConsulta.before(_dataFimEscolha)) {
					...
					// formata o verbete
					/*
					 * _listaVerbeteDiaria.add(0,
					 * FormataCalendar.data(calendario)); // data
					 * _listaVerbeteDiaria.add(1, "&nbsp;" +
					 * _horario.preencheSigla(_locale) + "&nbsp;"); // horário
					 * _listaVerbeteDiaria.add(2, new String("&nbsp;")); //
					 * saldo HN _listaVerbeteDiaria.add(3, new
					 * String("&nbsp;")); // saldo PF _listaVerbeteDiaria.add(4,
					 * formataJustificacoes()); // justificacoes
					...
					if ((_horario.getSigla() != null)
						...
						if (_listaMarcacoesPonto.size() > 0) {
							...
							if (_horario.getSigla().equals(Constants.DSC)) {
								...
								_saldoFeriado = _saldoFeriado + ((Long) _listaSaldos.get(0)).longValue();
							} else {
								// saldo de domingo e saldo de feriado ao
								// domingo
						...
						// nao efectua calculos de saldo para dias além do dia
						// de
						// hoje
						...
						if (_dataConsulta.before(agoraTimestamp)) {
							...
							// saldo global dos dias de consulta não conta com o
							// dia
						...
						/*
						 * calendario.clear();
						 * calendario.setTimeInMillis(((Long)
						 * _listaSaldos.get(0)).longValue());
						 * _listaVerbeteDiaria.set(2,
						 ...
						 * calendario.clear();
						 * calendario.setTimeInMillis(((Long)
						 * _listaSaldos.get(1)).longValue());
						 * _listaVerbeteDiaria.set(3,
						 * FormataCalendar.horasMinutosDuracao(calendario)); //
						 * saldo PF
						 */
					}
	...
	/*
	 * private void construirListaDiasInjustificados(Timestamp dataFim) throws
	 * NotExecuteException { Calendar calendario = Calendar.getInstance();
	 * calendario.setLenient(false); while (_dataConsulta.before((Timestamp)
	 * dataFim)) { calendario.clear();
	 * calendario.setTimeInMillis(_dataConsulta.getTime());
	 * _listaVerbeteDiaria.add(0, FormataCalendar.data(calendario)); // consulta
	 * do horario do funcionario numa determinada data try { lerHorario(); }
	 * catch (NotExecuteException nee) { throw new
	 * NotExecuteException(nee.getMessage()); }
	 * 
	 * _listaVerbeteDiaria.add(1, "&nbsp;" + _horario.preencheSigla(_locale) +
	 * "&nbsp;"); _listaVerbeteDiaria.add(2, new String("&nbsp;"));
	 * _listaVerbeteDiaria.add(3, new String("&nbsp;"));
	 * 
	 * _listaJustificacoes = new ArrayList(); _listaParamJustificacoes = new
	 * ArrayList(); _listaVerbeteDiaria.add(4, new String(" <b>" +
	 * Constants.INJUSTIFICADO + " </b>"));
	 * 
	 * _listaMarcacoesPonto = new ArrayList(); _listaVerbeteDiaria.add(5,
	 * formataMarcacoes()); // calcula o saldo diario de trabalho
	 * 
	 * if (!((_horario.getSigla() != null) &&
	 * (_horario.getSigla().equals(Constants.DSC) ||
	 * _horario.getSigla().equals(Constants.DS) || _horario.getSigla().equals(
	 * Constants.FERIADO)))) { IStrategyHorarios horarioStrategy =
	 * SuporteStrategyHorarios.getInstance()
	 * .callStrategy(_horario.getModalidade());
	 * horarioStrategy.setSaldosHorarioVerbeteBody(_horario, _listaRegimes,
	 * _listaParamJustificacoes, _listaMarcacoesPonto, _listaSaldos);
	 * 
	 ...
	 * FormataCalendar.horasMinutosDuracao(calendario)); calendario.clear();
	 ...
	 * _saldoHNFinal = _saldoHNFinal + ((Long) _listaSaldos.get(0)).longValue();
	 * _saldoPFFinal = _saldoPFFinal + ((Long) _listaSaldos.get(1)).longValue(); } //
	 * diminui um dia no intervalo de consulta do verbete calendario.clear();
	 * calendario.setTimeInMillis(_dataConsulta.getTime());
	 * calendario.add(Calendar.DAY_OF_MONTH, +1);
	 * _dataConsulta.setTime(calendario.getTimeInMillis()); // junta a lista
	 * diária à lista verbete _listaVerbeteBody.addAll(_listaVerbeteDiaria);
	...
	private void verificarStatusAssiduidade() throws NotExecuteException {
		...
		while (servicoSeguroLerStatusAssiduidadeFuncionario.getListaEstadosStatusAssiduidade().contains(
			 ...
			 * .getListaStatusAssiduidade().get(0), new Timestamp(calendarioFim
			 * .getTimeInMillis()));
			 */
	 ...
	 * Timestamp dataFim) throws NotExecuteException { Calendar calendario =
	 * Calendar.getInstance(); calendario.setLenient(false); while
	 * (!_dataConsulta.after((Timestamp) dataFim)) { calendario.clear();
	 * calendario.setTimeInMillis(_dataConsulta.getTime());
	 * _listaVerbeteDiaria.add(0, FormataCalendar.data(calendario)); // consulta
	 * do horario do funcionario numa determinada data try { lerHorario(); }
	 * catch (NotExecuteException nee) { throw new
	 ...
	 * 
	 * _listaMarcacoesPonto = new ArrayList(); _listaVerbeteDiaria.add(5,
	 * formataMarcacoes()); // aumenta um dia no intervalo de consulta do
	 * verbete calendario.clear();
	 * calendario.setTimeInMillis(_dataConsulta.getTime());
	 * calendario.add(Calendar.DAY_OF_MONTH, +1);
	 * _dataConsulta.setTime(calendario.getTimeInMillis()); // junta a lista
	 * diária à lista verbete _listaVerbeteBody.addAll(_listaVerbeteDiaria);
