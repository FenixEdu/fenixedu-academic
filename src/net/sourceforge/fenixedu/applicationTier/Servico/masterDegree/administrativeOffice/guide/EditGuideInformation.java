...
public class EditGuideInformation extends Service {
	...
	if (infoGuide.getInfoGuideSituation().getSituation().equals(GuideState.NON_PAYED)) {
		...
		for (InfoGuideEntry infoGuideEntry : newInfoGuideEntries) {
		    GuideEntry guideEntry = guide.getEntry(infoGuideEntry.getGraduationType(), infoGuideEntry.getDocumentType(),
		...
		for (InfoGuideEntry infoGuideEntry : (List<InfoGuideEntry>) newInfoGuideEntries) {
			...
			gratuitySituation = registration.readGratuitySituationByExecutionDegree(executionDegree);

			paymentTransaction = new GratuityTransaction(guideEntry.getPrice(), new Timestamp(Calendar.getInstance()
				.getTimeInMillis()), guideEntry.getDescription(), infoGuide.getPaymentType(),
				TransactionType.GRATUITY_ADHOC_PAYMENT, Boolean.FALSE, guide.getPerson(), personAccount,
			...
			paymentTransaction = new InsuranceTransaction(guideEntry.getPrice(), new Timestamp(Calendar.getInstance()
				.getTimeInMillis()), guideEntry.getDescription(), infoGuide.getPaymentType(),
				TransactionType.INSURANCE_PAYMENT, Boolean.FALSE, guide.getPerson(), personAccount, guideEntry,
	...
	newGuide = Guide.readByNumberAndYearAndVersion(infoGuide.getNumber(), infoGuide.getYear(), infoGuide.getVersion());
