...
public class ChangeGuideSituation extends Service {
	...
	if (newSituationOfGuide.equals(activeGuideSituation.getSituation())) {
		...
		Registration registration = studentPerson.readRegistrationByDegreeCurricularPlan(executionDegree
		...
		for (GuideEntry guideEntry : guide.getGuideEntries()) {
			...
			PaymentTransaction paymentTransaction = new GratuityTransaction(value, new Timestamp(Calendar
				.getInstance().getTimeInMillis()), guideEntry.getDescription(), guide.getPaymentType(),
				TransactionType.GRATUITY_ADHOC_PAYMENT, Boolean.FALSE, employeePerson, personAccount, guideEntry,
				gratuitySituation);
