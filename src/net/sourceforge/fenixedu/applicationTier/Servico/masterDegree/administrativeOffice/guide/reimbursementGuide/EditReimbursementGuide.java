...
public class EditReimbursementGuide extends Service {
	...
	if (newState.equals(ReimbursementGuideState.PAYED)) {
		...
		if (reimbursementGuideEntry.getGuideEntry().getDocumentType().equals(DocumentType.GRATUITY)) {
		    ...
		    reimbursementTransaction = new ReimbursementTransaction(reimbursementGuideEntry.getValue(), new Timestamp(
			    Calendar.getInstance().getTimeInMillis()), "", reimbursementGuideEntry.getGuideEntry().getGuide()
			    .getPaymentType(), TransactionType.GRATUITY_REIMBURSEMENT, Boolean.FALSE, person, personAccount,
		...
		if (reimbursementGuideEntry.getGuideEntry().getDocumentType().equals(DocumentType.INSURANCE)) {
		    ...
		    reimbursementTransaction = new ReimbursementTransaction(reimbursementGuideEntry.getValue(), new Timestamp(
			    Calendar.getInstance().getTimeInMillis()), "", reimbursementGuideEntry.getGuideEntry().getGuide()
			    .getPaymentType(), TransactionType.INSURANCE_REIMBURSEMENT, Boolean.FALSE, person, personAccount,
			    reimbursementGuideEntry);
	...
	while (it.hasNext()) {
	    ...
	    // again
	    reimbursementGuideEntryTmp = rootDomainObject.readReimbursementGuideEntryByOID(reimbursementGuideEntryTmp
		    .getIdInternal());
