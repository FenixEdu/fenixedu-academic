...
public class ReadExamsByStudent extends Service {
	...
	if (registration != null) {
		...
		if (writtenEvaluationEnrolment.getRoom() != null) {
		    InfoRoom infoRoom = InfoRoom.newInfoFromDomain(((AllocatableSpace) writtenEvaluationEnrolment.getRoom()));
		...
		examsToEnroll.addAll(((Attends) examsToEnrollIterator.next()).getExecutionCourse().getAssociatedEvaluations());
	    }
		...
		if (evaluation instanceof Exam) {
			...
			infoExam.setInfoExecutionCourses((List) CollectionUtils.collect(exam.getAssociatedExecutionCourses(),
				new Transformer() {
	...
	InfoStudentSiteExams component = new InfoStudentSiteExams(infoExamsToEnroll, infoWrittenEvaluationEnrolmentList);
	...
	return (Calendar.getInstance().getTimeInMillis() < end.getTimeInMillis() && Calendar.getInstance().getTimeInMillis() > begin
		.getTimeInMillis());
