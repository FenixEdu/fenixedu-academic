...
public class ParamJustificacaoRelacional implements IParamJustificacaoPersistente {
    public boolean alterarParamJustificacao(ParamJustificacao paramJustificacao) {
        ...
        try {
            PreparedStatement sql = UtilRelacional.prepararComando("UPDATE ass_PARAM_JUSTIFICACAO SET "
                    + "codigoInterno = ? , " + "sigla = ? , " + "descricao = ? , " + "tipo = ? , "
                    + "tipoDias = ? , " + "grupo = ? , " + "quem = ? , " + "quando = ? , "
                    + "WHERE codigoInterno = ? ");
    ...
    public boolean apagarParamJustificacao(String sigla) {
        ...
        try {
            PreparedStatement sql = UtilRelacional.prepararComando("DELETE FROM ass_PARAM_JUSTIFICACAO "
                    + "WHERE sigla = ?");
    ...
    public boolean escreverParamJustificacao(ParamJustificacao paramJustificacao) {
        ...
        try {
            PreparedStatement sql = UtilRelacional.prepararComando("INSERT INTO ass_PARAM_JUSTIFICACAO "
                    + "VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    public List lerGruposParamJustificacoes() {
        List listaGruposParamJustificacoes = null;

        try {
            PreparedStatement sql = UtilRelacional
                    .prepararComando("SELECT DISTINCT grupo FROM ass_PARAM_JUSTIFICACAO");
            ...
            System.out.println("ParamJustificacaoRelacional.lerGruposParamJustificacoes: "
                    + e.toString());
        }
    ...
    public ParamJustificacao lerParamJustificacao(int codigoInterno) {
        ...
        try {
            PreparedStatement sql = UtilRelacional
                    .prepararComando("SELECT * FROM ass_PARAM_JUSTIFICACAO " + "WHERE codigoInterno = ?");

            sql.setInt(1, codigoInterno);

            ResultSet resultado = sql.executeQuery();
            if (resultado.next()) {
                paramJustificacao = new ParamJustificacao(resultado.getInt("codigoInterno"), resultado
                        .getString("sigla"), resultado.getString("descricao"), resultado
                        .getString("tipo"), resultado.getString("tipoDias"), resultado
                        .getString("grupo"), resultado.getInt("quem"), Timestamp.valueOf(resultado
                        .getString("quando")));
            }
            sql.close();
        } catch (Exception e) {
    ...
    public ParamJustificacao lerParamJustificacao(String sigla) {
        ...
        try {
            PreparedStatement sql = UtilRelacional
                    .prepararComando("SELECT * FROM ass_PARAM_JUSTIFICACAO " + "WHERE sigla = ?");
    ...
    public List lerParamJustificacoes(List listaJustificacoes) {
        ...
        List listaParamJustificacoes = null;
        ...
        try {
            if (listaJustificacoes != null) {
                ...
                while (iterador.hasNext()) {
                    ...
                    if (resultado.next()) {
                        paramJustificacao = new ParamJustificacao(resultado.getInt("codigoInterno"),
                                resultado.getString("sigla"), resultado.getString("descricao"),
                                resultado.getString("tipo"), resultado.getString("tipoDias"), resultado
                                        .getString("grupo"), resultado.getInt("quem"), Timestamp
    ...
    public List lerSiglasJustificacao() {
        ...
        try {
            PreparedStatement sql = UtilRelacional
                    .prepararComando("SELECT sigla FROM ass_PARAM_JUSTIFICACAO");
    ...
    public List lerTipoDiasParamJustificacoes() {
        List listaTipoDiasParamJustificacoes = null;

        try {
            PreparedStatement sql = UtilRelacional
                    .prepararComando("SELECT DISTINCT tipoDias FROM ass_PARAM_JUSTIFICACAO");
            ResultSet resultado = sql.executeQuery();
            ...
            System.out.println("ParamJustificacaoRelacional.lerTipoDiasParamJustificacoes: "
                    + e.toString());
        }
    ...
    public List lerTiposParamJustificacoes() {
        List listaTiposParamJustificacoes = null;

        try {
            PreparedStatement sql = UtilRelacional
                    .prepararComando("SELECT DISTINCT tipo FROM ass_PARAM_JUSTIFICACAO");
            ResultSet resultado = sql.executeQuery();
            ...
            System.out
                    .println("ParamJustificacaoRelacional.lerTiposParamJustificacoes: " + e.toString());
        }
    ...
    public List lerTodasParamJustificacoes(String ordem) {
        ...
        try {
            ...
            listaJustificacoes = new ArrayList();
            while (resultado.next()) {
                listaJustificacoes.add(new ParamJustificacao(resultado.getInt("codigoInterno"),
                        resultado.getString("sigla"), resultado.getString("descricao"), resultado
                                .getString("tipo"), resultado.getString("tipoDias"), resultado
                                .getString("grupo"), resultado.getInt("quem"), Timestamp
                                .valueOf(resultado.getString("quando"))));
            }
            ...
            System.out
                    .println("ParamJustificacaoRelacional.lerTodasParamJustificacoes: " + e.toString());
        }
        return listaJustificacoes;
