...
// try {
// PreparedStatement sql = UtilRelacionalOracle.prepararComando("UPDATE
// ASS_MARCAS SET "
...
//
// public List consultarMarcacoesPonto(List listaFuncionarios, List
// listaCartoes, List listaEstados,
...
// + "(ASS_MARDHMARCA BETWEEN TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS') AND TO_DATE(?,
...
// MarcacaoPonto(resultado.getInt("ASS_MARSEQ"),
...
// resultado
// .getInt("unidade"), resultado.getString("siglaUnidade"), dataMarcacao,
// resultado
...
// + "WHERE ASS_MARDHMARCA = TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS') AND
// ASS_MARPESSOA = ?");
//
...
// dataMarcacao = Timestamp.valueOf(resultado.getString("ASS_MARDHMARCA"));
// }
// marcacaoPonto = new MarcacaoPonto(resultado.getInt("ASS_MARSEQ"),
// chaveUnidade,
...
// }
//
// marcacoesPonto.add(new MarcacaoPonto(resultado.getInt("ASS_MARSEQ"),
// resultado
...
// } /* lerMarcacoesPonto */
//
// public List obterMarcacoesPonto(int numFuncionario, int numCartao, Timestamp
// dataFim) {
// /*
// * // Acesso a BD da nossa Aplicacao List marcacoesPonto = null;
// *
...
// .prepararComando("SELECT * FROM ASS_MARCAS "
// + "WHERE ASS_MARDHMARCA>=TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS') AND
// ASS_MARPESSOA=? AND ASS_MARCARTAO=?");
//
...
// PreparedStatement sql = UtilRelacional
// .prepararComando("SELECT numeroMecanografico FROM ass_FUNCIONARIO WHERE
// codigoInterno = ?");
...
// // último número de sequencia na tabela
//
// sql = UtilRelacionalOracle.prepararComando("SELECT s_ordem.NEXTVAL FROM
// SYS.dual");
// resultadoQuery = sql.executeQuery();
// if (resultadoQuery.next()) {
// ultimo = resultadoQuery.getInt(1);
...
public class MarcacaoPontoRelacional implements IMarcacaoPontoPersistente {
 ...
 e.toString());
	...
	public List consultarMarcacoesPonto(
         ...
         * Timestamp dataMarcacao = null;
         * 
         * PreparedStatement sql2 = UtilRelacionalOracle.prepararComando(
         * "SELECT * FROM ass_REGULARIZACAO_MARCACAO WHERE " +
         * "chaveMarcacaoPonto = ? AND chaveParamRegularizacao = 6"); ResultSet
         * resultado2 = null;
         * 
         * while (resultado.next()) { // retirar as marcacoes de ponto que foram
         * marcadas para anular sql2.setInt(1,
 ...
 UtilRelacionalOracle.prepararComando(query.toString());
				...
				// obtem o codigo das marcacoes a anular
				PreparedStatement sql2 = UtilRelacional.prepararComando("SELECT * FROM ass_PARAM_REGULARIZACAO WHERE sigla = 'MA'");
				ResultSet resultado2 = sql2.executeQuery();
				int marcacaoAnular = 0;
				if (resultado2.next()) {
					marcacaoAnular = resultado2.getInt("codigoInterno");
				...
				PreparedStatement sql3 =
					UtilRelacional.prepararComando("SELECT * FROM ass_REGULARIZACAO_MARCACAO WHERE " + "chaveMarcacaoPonto = ?");
				ResultSet resultado3 = null;
				while (resultado.next()) {
					if (resultado.getString("ASS_MARDHMARCA") != null) {
						dataMarcacao = Timestamp.valueOf(resultado.getString("ASS_MARDHMARCA"));
 ...
 e.toString());
	...
	public List consultarMarcacoesPontoErros(String estado) { // não será preciso
		List errosLeitura = null;
 ...
 e.toString());
	...
	public MarcacaoPonto lerMarcacaoPonto(Timestamp dataMarcacao, int numFuncionario) {
         ...
         * 
         * try { PreparedStatement sql = UtilRelacional.prepararComando("SELECT *
         * FROM marcacao_ponto " + "WHERE dataMarcacao = ? AND numFuncionario =
         * ?");
         * 
         * sql.setTimestamp(1, dataMarcacao); sql.setInt(2, numFuncionario);
         * 
 ...
 e.toString());
	...
	public List lerMarcacoesPonto(int numCartao) {
         ...
         * 
         * try { PreparedStatement sql = UtilRelacional.prepararComando("SELECT *
         * FROM marcacao_ponto " + "WHERE numCartao = ? ");
         * 
         * sql.setInt(1, numCartao);
         * 
         * ResultSet resultado = sql.executeQuery(); marcacoesPonto = new
 ...
 Timestamp dataFim) {
         ...
         * 
         * try { PreparedStatement sql = UtilRelacional.prepararComando("SELECT *
         * FROM marcacao_ponto " + "WHERE numCartao=? AND dataMarcacao>=? AND
         * dataMarcacao <=?");
         * 
         * sql.setInt(1, numCartao); sql.setTimestamp(2, new
         * Timestamp(dataInicio.getTime())); sql.setTimestamp(3, new
 ...
 regularizacao) {
		...
		try {
			...
			try {
				...
				// data do registo
				// sql.setString(3,
                // regularizacao.getQuando().toString().substring(0, regularizacao.getQuando().toString().indexOf(".")));
