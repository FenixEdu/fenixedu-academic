...
public class IsencaoHorario implements IStrategyHorarios {
        ...
        try {
            ...
            if ((formHorario.getInicioHN1Horas().length() < 1)
                ...
                if (!((timeInicioHN1 < timeFimHN1) && (timeFimHN1 < timeInicioHN2) && (timeInicioHN2 < timeFimHN2))) {
                    ...
                    timeHNPeriodo = ((new Float(duracaoSemanal / Constants.SEMANA_TRABALHO_ISENCAO)
                            .intValue() * 3600 * 1000) + new Float((new Float(duracaoSemanal
                            / Constants.SEMANA_TRABALHO_ISENCAO).floatValue() - new Float(duracaoSemanal
                            / Constants.SEMANA_TRABALHO_ISENCAO).intValue()) * 3600 * 1000).longValue());
            ...
            try {
                /* intervaloMinimoRefeicaoHoras = */new Integer(formHorario.getIntervaloMinimoHoras())
                        .intValue();
                /* intervaloMinimoRefeicaoMinutos = */new Integer(formHorario
                        .getIntervaloMinimoMinutos()).intValue();
                descontoObrigatorioHoras = new Integer(formHorario.getDescontoObrigatorioHoras())
                        .intValue();
                descontoObrigatorioMinutos = new Integer(formHorario.getDescontoObrigatorioMinutos())
                        .intValue();
            } catch (java.lang.NumberFormatException e) {
                errors.add("numero", new ActionError("error.numero.naoInteiro"));
            }
            ...
            timeHNPeriodo = ((new Float(duracaoSemanal / Constants.SEMANA_TRABALHO_ISENCAO).intValue() * 3600 * 1000) + new Float(
                    (new Float(duracaoSemanal / Constants.SEMANA_TRABALHO_ISENCAO).floatValue() - new Float(
                            duracaoSemanal / Constants.SEMANA_TRABALHO_ISENCAO).intValue()) * 3600 * 1000)
                    .longValue())
    ...
    public void setHorario(Horario horario, ActionForm form) {
        ...
        if (formHorario.getDiaAnteriorExpediente().length() < 1) {
            ...
            //contagem no dia anterior
            calendar.set(1936, 11, 31, (new Integer(formHorario.getInicioExpedienteHoras())).intValue(),
                    (new Integer(formHorario.getInicioExpedienteMinutos())).intValue(), 00);
            horario.setInicioExpediente(new Timestamp(calendar.getTimeInMillis()));
        }
        ...
        if (formHorario.getIntervaloMinimoHoras() != null
                ...
                && formHorario.getIntervaloMinimoMinutos() != null
        ...
        if (formHorario.getTrabalhoConsecutivoHoras() != null
                && formHorario.getTrabalhoConsecutivoMinutos() != null) {
            calendar.clear();
        ...
        if (iterador.hasNext()) {
            ...
            if (entrada.getData().getTime() > horario.getInicioRefeicao().getTime()
                ...
                if (listaMarcacoesPonto.size() > 3) {
                    ...
                    // contida no intervalo de refeicao é como se
    ...
    public long duracaoDiaria(Horario horario) {
        long duracaoDiaria = ((new Float(horario.getDuracaoSemanal() / Constants.SEMANA_TRABALHO_ISENCAO)
                .intValue() * 3600 * 1000) + new Float((new Float(horario.getDuracaoSemanal()
                / Constants.SEMANA_TRABALHO_ISENCAO).floatValue() - new Float(horario
                .getDuracaoSemanal()
                / Constants.SEMANA_TRABALHO_ISENCAO).intValue()) * 3600 * 1000).longValue());

        if (!((horario.getSigla() != null) && ((horario.getSigla().equals(Constants.DS))
                || (horario.getSigla().equals(Constants.DSC)) || (horario.getSigla()
                .equals(Constants.FERIADO))))) {
            //é um horário de descanso, nao se desconta hora de almoco
            //ao saldo do dia de descanso é descontado o desconto obrigatorio
            // de refeicao
