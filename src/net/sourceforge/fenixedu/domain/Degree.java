...
public class Degree extends Degree_Base implements Comparable<Degree> {
	...
	static final private Comparator<Degree> COMPARATOR_BY_DEGREE_TYPE = new Comparator<Degree>() {
		public int compare(final Degree o1, final Degree o2) {
			return Collator.getInstance().compare(
					o1.getDegreeType().getLocalizedName(),
	...
	public Degree(String name, String nameEn, String code,
			DegreeType degreeType, GradeScale gradeScale,
	...
	public Degree(String name, String nameEn, String acronym,
			DegreeType degreeType, Double ectsCredits, GradeScale gradeScale,
			String prevailingScientificArea) {
		this();
		commonFieldsChange(name, nameEn, acronym, gradeScale);
		newStructureFieldsChange(degreeType, ectsCredits,
				prevailingScientificArea);
	}

	private void commonFieldsChange(String name, String nameEn, String code,
	...
	private void newStructureFieldsChange(DegreeType degreeType,
		...
		this
				.setPrevailingScientificArea(prevailingScientificArea == null ? null
	...
	public void edit(String name, String nameEn, String acronym,
			DegreeType degreeType, Double ectsCredits, GradeScale gradeScale,
			String prevailingScientificArea) {
	...
	private void checkIfCanEdit(final DegreeType degreeType) {
		if (!this.getDegreeType().equals(degreeType)
				&& hasAnyDegreeCurricularPlans()) {
	...
	public DegreeCurricularPlan getNewDegreeCurricularPlan() {
		...
		try {
			Class classDefinition = Class
					.forName(getConcreteClassForDegreeCurricularPlans());
			degreeCurricularPlan = (DegreeCurricularPlan) classDefinition
					.newInstance();
	...
	public GradeScale getGradeScaleChain() {
		return super.getGradeScale() != null ? super.getGradeScale()
				: getDegreeType().getGradeScale();
	}

	public DegreeCurricularPlan createPreBolonhaDegreeCurricularPlan(
			String name, DegreeCurricularPlanState state, Date initialDate,
			Date endDate, Integer degreeDuration,
			Integer minimalYearForOptionalCourses, Double neededCredits,
			MarkType markType, Integer numerusClausus, String anotation,
			GradeScale gradeScale) {
			...
			for (DegreeCurricularPlan dcp : this.getDegreeCurricularPlansSet()) {
				if (dcp.getName().equalsIgnoreCase(name)) {
					throw new DomainException(
							"DEGREE.degreeCurricularPlan.existing.name.and.degree");
				}
			}

			return new DegreeCurricularPlan(this, name, state, initialDate,
					endDate, degreeDuration, minimalYearForOptionalCourses,
					neededCredits, markType, numerusClausus, anotation,
					gradeScale);
		} else {
			throw new DomainException(
					"DEGREE.calling.pre.bolonha.method.to.bolonha.degree");
		}
	}

	public DegreeCurricularPlan createBolonhaDegreeCurricularPlan(String name,
			GradeScale gradeScale, Person creator) {
		if (this.isBolonhaDegree()) {
			if (name == null) {
				throw new DomainException(
						"DEGREE.degree.curricular.plan.name.cannot.be.null");
			}
			for (DegreeCurricularPlan dcp : this.getDegreeCurricularPlansSet()) {
				if (dcp.getName().equalsIgnoreCase(name)) {
					throw new DomainException(
			...
			if (creator == null) {
				throw new DomainException(
						"DEGREE.degree.curricular.plan.creator.cannot.be.null");
			}
			final Role bolonhaRole = Role
					.getRoleByRoleType(RoleType.BOLONHA_MANAGER);
			if (!creator.hasPersonRoles(bolonhaRole)) {
				creator.addPersonRoles(bolonhaRole);
			}

			CurricularPeriod curricularPeriod = new CurricularPeriod(this
					.getDegreeType().getCurricularPeriodType());

			return new DegreeCurricularPlan(this, name, gradeScale, creator,
					curricularPeriod);
		} else {
			throw new DomainException(
					"DEGREE.calling.bolonha.method.to.non.bolonha.degree");
		}
	}
	...
	public boolean isDegreeOrBolonhaDegreeOrBolonhaIntegratedMasterDegree() {
		return getDegreeType()
				.isDegreeOrBolonhaDegreeOrBolonhaIntegratedMasterDegree();
	}

	public List<DegreeCurricularPlan> findDegreeCurricularPlansByState(
	...
	public List<DegreeCurricularPlan> getDegreeCurricularPlansForYear(
			ExecutionYear year) {
		List<DegreeCurricularPlan> result = new ArrayList<DegreeCurricularPlan>();
	...
	public List<ExecutionYear> getDegreeCurricularPlansExecutionYears() {
		...
		for (final DegreeCurricularPlan degreeCurricularPlan : getDegreeCurricularPlansSet()) {
			for (final ExecutionDegree executionDegree : degreeCurricularPlan
	...
	final public String getFilteredName() {
		...
		for (final net.sourceforge.fenixedu.domain.space.Campus campus : Space
				.getAllCampus()) {
			final String toRemove = " - " + campus.getName();
			if (result.toString().contains(toRemove)) {
				result.replace(result.indexOf(toRemove), result
						.indexOf(toRemove)
						+ toRemove.length(), "");
			}
		}
	...
	final public String getSeniorTitle() {
		...
		seniorTitle.append(" ");
		seniorTitle.append(ResourceBundle.getBundle(
	...
	public OldInquiriesCoursesRes getOldInquiriesCoursesResByCourseCodeAndExecutionPeriod(
			String code, ExecutionPeriod executionPeriod) {
		for (OldInquiriesCoursesRes oldInquiriesCoursesRes : this
				.getAssociatedOldInquiriesCoursesRes()) {
			if (oldInquiriesCoursesRes.getCourseCode().equalsIgnoreCase(code)
					&& oldInquiriesCoursesRes.getExecutionPeriod().equals(
							executionPeriod)) {
	...
	public List<OldInquiriesSummary> getOldInquiriesSummariesByExecutionPeriod(
		...
		for (OldInquiriesSummary oldInquiriesSummary : this
				.getAssociatedOldInquiriesSummaries()) {
			if (oldInquiriesSummary.getExecutionPeriod()
	...
	public List<OldInquiriesTeachersRes> getOldInquiriesTeachersResByExecutionPeriodAndCurricularYearAndCourseCode(
			ExecutionPeriod executionPeriod, Integer curricularYear,
			String courseCode) {
		...
		for (OldInquiriesTeachersRes oldInquiriesTeachersRes : this
			...
			if (oldInquiriesTeachersRes.getExecutionPeriod().equals(
					executionPeriod)
					&& oldInquiriesTeachersRes.getCurricularYear().equals(
							curricularYear)
					&& oldInquiriesTeachersRes.getCourseCode()
	...
	public List<OldInquiriesTeachersRes> getOldInquiriesTeachersResByExecutionPeriodAndCurricularYearAndCourseCodeAndTeacher(
			ExecutionPeriod executionPeriod, Integer curricularYear,
			String courseCode, Teacher teacher) {
		List<OldInquiriesTeachersRes> result = new ArrayList<OldInquiriesTeachersRes>();
		for (OldInquiriesTeachersRes oldInquiriesTeachersRes : this
				.getAssociatedOldInquiriesTeachersRes()) {
			if (oldInquiriesTeachersRes.getExecutionPeriod().equals(
	...
	public DegreeCurricularPlan getMostRecentDegreeCurricularPlan() {
		...
		for (final DegreeCurricularPlan degreeCurricularPlan : this
	...
	private DegreeCurricularPlan getMostRecentDegreeCurricularPlanByInitialDate() {
		...
		for (final DegreeCurricularPlan degreeCurricularPlan : this
			...
			if (mostRecentDegreeCurricularPlan == null
					|| degreeCurricularPlan.getInitialDateYearMonthDay()
							.isAfter(
	...
	private static void loadCache() {
		synchronized (degrees) {
			degrees.clear();
			for (final Degree degree : RootDomainObject.getInstance()
					.getDegrees()) {
				degrees.put(degree.getSigla().toLowerCase(),
						new SoftReference<Degree>(degree));
			}
		}
	}

	private static void updateCache(final Degree degree,
			final String newLowerCaseSigla) {
		final String currentLowerCaseSigla = (degree.getSigla() != null) ? degree
	...
	public static Degree readBySigla(final String sigla) {
		...
		final SoftReference<Degree> degreeReference = degrees
				.get(lowerCaseString);
		if (degreeReference != null) {
			final Degree degree = degreeReference.get();
			if (degree != null
					&& degree.getRootDomainObject() == RootDomainObject
							.getInstance()
					&& degree.getSigla().equalsIgnoreCase(lowerCaseString)) {
				return degree;
			} else {
				loadCache();
				final SoftReference<Degree> otherDegreeReference = degrees
						.get(lowerCaseString);
				if (otherDegreeReference != null) {
					final Degree otherDegree = otherDegreeReference.get();
					if (otherDegree != null
							&& otherDegree.getRootDomainObject() == RootDomainObject
									.getInstance()
							&& otherDegree.getSigla().equalsIgnoreCase(
									lowerCaseString)) {
						return otherDegree;
					}
				}
	...
	public static List<Degree> readAllByDegreeType(final DegreeType degreeType) {
		List<Degree> result = new ArrayList<Degree>();
		for (final Degree degree : RootDomainObject.getInstance().getDegrees()) {
			if (degree.getDegreeType() != null
					&& degree.getDegreeType() == degreeType) {
				result.add(degree);
			}
		}
		return result;
	...
	public DegreeInfo getMostRecentDegreeInfo(ExecutionYear executionYear) {
		...
		for (; executionYear != null; executionYear = executionYear
	...
	public DegreeInfo createCurrentDegreeInfo() {
		...
		ExecutionYear currentExecutionYear = ExecutionYear
	...
	public Collection<ScientificCommission> getScientificCommissionMembers(
	...
	public Collection<Coordinator> getResponsibleCoordinators(
			ExecutionYear executionYear) {
	...
	public Collection<Teacher> getResponsibleCoordinatorsTeachers(
			ExecutionYear executionYear) {
		Set<Teacher> result = new TreeSet<Teacher>(
				Teacher.TEACHER_COMPARATOR_BY_CATEGORY_AND_NUMBER);
		for (final DegreeCurricularPlan degreeCurricularPlan : getDegreeCurricularPlansSet()) {
			final ExecutionDegree executionDegree = degreeCurricularPlan
					.getExecutionDegreeByYear(executionYear);
			if (executionDegree != null) {
				for (final Coordinator coordinator : executionDegree
						.getResponsibleCoordinators()) {
	...
	public Collection<Teacher> getCurrentResponsibleCoordinatorsTeachers() {
		...
		Collection<Teacher> result = this
				.getResponsibleCoordinatorsTeachers(executionYear);
	...
	public String constructSchoolClassPrefix(final Integer curricularYear) {
		return isBolonhaDegree() ? StringAppender.append(getSigla(), "0",
