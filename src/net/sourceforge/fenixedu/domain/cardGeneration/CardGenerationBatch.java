...
public class CardGenerationBatch extends CardGenerationBatch_Base {
	...
	if (!person.hasCardGenerationEntryLine(record.getLine())) {
	    ...
	    final CardGenerationEntry cardGenerationEntry = createCardGenerationEntries(record.getLine(), record
		    .getIdentificationId());
	    cardGenerationEntry.setPerson(person);
	...
	if (peopleForId.size() == 1) {
	    ...
	    report.addIdDuplication(record.getIdentificationId(), new java.util.ArrayList<Person>(peopleForId), record
		    .getNumbers());
	}
