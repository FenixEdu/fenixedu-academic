...
public class CardGenerationProblem extends CardGenerationProblem_Base {
	...
	final CardGenerationBatch cardGenerationBatch = findBatchUnderConstruction(cardGenerationBatchWithProblems.getExecutionYear());
	cardGenerationEntry.setCardGenerationBatch(cardGenerationBatch);
	crossReference(cardGenerationEntry);
	delete();
	if (!cardGenerationBatchWithProblems.hasAnyCardGenerationProblems() && !cardGenerationBatchWithProblems.hasAnyCardGenerationEntries()) {
	...
	if (category == Category.CODE_73 || category == Category.CODE_82 || category == Category.CODE_96) {
		...
		if (category != Category.CODE_96) {
		    ...
		    new CardGenerationProblem(cardGenerationBatchWithProblems, "multiple.user.information.type.not.crossed", cardGenerationEntry.getDocumentID(), person);
		}
	    } else {
		final Category categoryForStudentLine = CardGenerationEntry.readCategory(studentLine);
		final String newLine = ImportIdentificationCardDataFromFile.merge(line, categoryForLine, studentLine, categoryForStudentLine);
