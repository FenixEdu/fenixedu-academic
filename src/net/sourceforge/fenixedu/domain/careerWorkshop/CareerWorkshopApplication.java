...
public class CareerWorkshopApplication extends CareerWorkshopApplication_Base {
	...
	if (getSessions() != null) {
		...
		final CareerWorkshopSessions careerWorkshopSessions = CareerWorkshopSessions.valueOf(parts[i].substring(0, seperatorPos));
	...
	if (getThemes() != null) {
	    ...
	    for (; i < parts.length; i++) {
		final int seperatorPos = parts[i].indexOf(':');
		final CareerWorkshopThemes careerWorkshopThemes = CareerWorkshopThemes.valueOf(parts[i].substring(0, seperatorPos));
