 ...
 * create and all the arguments to pass and tries to create the group from those
 * arguments.
 * 
 * <p>
 * Example: <code>custom($C('accessControl.RoleGroup'), 'MANAGER')</code>
 * 
 * @author cfgi
 * 
...
class CustomGroupBuilder implements GroupBuilder {
	...
	try {
		...
		throw new GroupDynamicExpressionException("accessControl.group.builder.custom.class.notGroup", groupClass
			.getName());
	    }
	    ...
	    throw new GroupDynamicExpressionException(e, "accessControl.group.builder.custom.constructor.failed", Arrays.asList(
		    arguments).toString());
	}
    }
