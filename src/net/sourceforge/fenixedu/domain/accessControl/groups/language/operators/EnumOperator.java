...
/**
 * The <code>$E</code> operator converts a parameter value into a enum. You must
 * also specify the type of the enum. You can also type the enum name directly
 * when the value is known in advance.
 * 
 * <p>
 * If the request contains <code>role=MANAGER&gender=FEMALE</code> then
 * 
 * <code>$E(MANAGER, 'person.RoleType') = MANAGER</code> and <code>$E($P(role), 'person.RoleType') = MANAGER</code> and <code>$E($P(gender), 'person.Gender') = FEMALE</code>
 * 
 * @author cfgi
 */
public class EnumOperator extends OperatorArgument {
	...
	this(new StaticArgument(value.name()), new StaticArgument(ClassOperator.simplify(value.getDeclaringClass().getName())));
    }
