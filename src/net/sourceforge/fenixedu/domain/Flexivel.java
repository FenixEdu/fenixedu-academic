...
public class Flexivel implements IStrategyHorarios
{
		...
		try
		{
				...
				|| (formHorario.getTrabalhoConsecutivoMinutos().length() < 1))
			{
				errors.add(
					...
					new ActionError("error.trabalhoConsecutivo.obrigatorio"));
			}
			else
			{
				try
				{
					trabalhoConsecutivoHoras =
						new Integer(formHorario.getTrabalhoConsecutivoHoras()).intValue();
					trabalhoConsecutivoMinutos =
				...
				else
				{
					...
					timeHNPeriodo =
						((new Float(Constants.DURACAO_SEMANAL_FLEXIVEL
							/ Constants.SEMANA_TRABALHO_FLEXIVEL)
							.intValue()
							* 3600
							* 1000)
							+ new Float(
		...
		if (iterador.hasNext())
		{
			...
			else
			{
				if (listaMarcacoesPonto.size() >= 3)
				{
					...
					while (iterador.hasNext())
					{
						...
						long saldoEntreMarcacoes =
							saida.getData().getTime() - entrada.getData().getTime();
						...
						if (saida.getData().getTime() < horario.getFimRefeicao().getTime())
						{
							saldoPenalizacaoAusenciaRefeicao =
								saldoPenalizacaoAusenciaRefeicao + saldoEntreMarcacoes;
						}
						else if (entrada.getData().getTime() < horario.getFimRefeicao().getTime())
						{
							saldoPenalizacaoAusenciaRefeicao =
						...
						// para refeicao
						saldoPenalizacaoMinimoRefeicao =
							saldoPenalizacaoMinimoRefeicao + saldoEntreMarcacoes;

						if (entrada.getNumFuncionario() == 0 && intervaloRefeicaoJustificado == 0)
						{
							// se for uma justificacao
							intervaloRefeicaoJustificado =
								encontrarIntervaloRefeicaoJustificado(horario, entrada, saida);
						}
						...
						if (iterador.hasNext())
						{
							...
							{
								intervaloRefeicao =
									horario.getFimRefeicao().getTime() - saida.getData().getTime();
								...
								&& entrada.getData().getTime() <= horario.getFimRefeicao().getTime())
							{
								...
								entrada.getData().getTime() <= horario.getInicioRefeicao().getTime())
							{
								intervaloRefeicao =
									intervaloRefeicao
										- (horario.getFimRefeicao().getTime()
			...
			else
			{
					...
					&& intervaloRefeicao != 0)
				{
					...
					saldo =
						saldo - (horario.getDescontoObrigatorioRefeicao().getTime() - intervaloRefeicao);
				}
			}
		}
		...
		if (horario.getInicioPF1() != null)
		{
			...
			while (iterador.hasNext())
			{
				...
				if (iterador.hasNext())
				{
					...
					{
						trabalhouPF =
							trabalhouPF + (saida.getData().getTime() - horario.getFimPF1().getTime());
					}
					...
					{
						trabalhouPF =
							trabalhouPF + (saida.getData().getTime() - horario.getFimPF2().getTime());
					}
	...
	private long encontrarIntervaloRefeicaoJustificado(
		Horario horario,
		...
		ArrayList listaSaldos)
	{
		if (listaMarcacoesPonto.size() > 0)
		{
			...
			else
			{
				...
				{
					saldoSegEscalao =
						saldoSegEscalao + (saldoExtraordinario - Constants.PRIMEIRO_ESCALAO);
				}
				else
				{
					saldoSegEscalao =
