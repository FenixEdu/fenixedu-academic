...
public abstract class AcademicCalendarEntry extends AcademicCalendarEntry_Base implements GanttDiagramEvent {
	...
	parentEntry = !parentEntry.getRootEntry().equals(rootEntry) ? createVirtualPathUntil(parentEntry, rootEntry)
		: parentEntry;
	...
	if (!parentEntry.isRoot()) {
		...
		if (parentEntry.isPossibleToChangeTimeInterval()) {
			...
			parentEntry.edit(parentEntry.getTitle(), parentEntry.getDescription(), beginDate, endDate, parentEntry
				.getRootEntry(), parentEntry.getTemplateEntry());
	...
	List<AcademicCalendarEntry> childEntries = getTemplateEntry() != null ? getChildEntriesWithTemplateEntries()
		: getChildEntries();
	...
	int index = getBegin().withChronology(academicChronology).get(AcademicSemesterDateTimeFieldType.academicSemester());
	AcademicSemesterCE academicSemester = academicChronology.getAcademicSemesterIn(index);
	return academicSemester != null ? academicSemester.getTeacherCreditsFillingForDepartmentAdmOffice(academicChronology)
		: null;
    }
