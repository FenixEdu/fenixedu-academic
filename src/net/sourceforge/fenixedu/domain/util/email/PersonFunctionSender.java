...
public class PersonFunctionSender extends PersonFunctionSender_Base {
	...
	public PersonFunctionSender(PersonFunction personFunction) {
		super();
		String fromName;
		if (!personFunction.getFunction().isOfFunctionType(
				FunctionType.DELEGATE_OF_YEAR)) {
			fromName = String.format("%s", personFunction.getFunction()
					.getTypeName());
		} else {
		...
		getRecipients().addAll(
				Recipient.createNewRecipient(getPossibleReceivers(person)));
	}

	public static List<IGroup> getPossibleReceivers(Person person) {
		...
		if (person.hasStudent()) {
			...
			final Registration lastRegistration = delegate
			...
			delegateFunction = degree
					.getMostSignificantDelegateFunctionForStudent(delegate,
							null);
			...
			if (delegateFunction != null
				...
				for (PersonFunction function : delegate
						.getAllActiveDelegateFunctions()) {
					groups.add(new DelegatesGroup(function.getFunction()
							.getFunctionType()));
				}
			}
			...
			if (person.getActiveGGAEDelegatePersonFunction() != null) {
				groups.add(new DelegatesGroup(person
						.getActiveGGAEDelegatePersonFunction().getFunction()
						.getFunctionType()));
			}
		} else {
			delegateFunction = person.getActiveGGAEDelegatePersonFunction();
			groups.add(new DelegatesGroup(delegateFunction.getFunction()
					.getFunctionType()));
		}
	...
	public static PersonFunctionSender newInstance(PersonFunction personFunction) {
		...
		return sender == null ? new PersonFunctionSender(personFunction)
				: sender;
	}
}
