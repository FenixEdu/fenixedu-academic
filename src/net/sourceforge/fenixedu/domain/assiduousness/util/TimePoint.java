...
public class TimePoint {
	...
	return new String(time.toString() + ", " + getPointAttributes().toString() + ", " + getIntervalAttributes().toString());
    }

    // Checks is point has both attributes
    // has both attributes if the point attribute contains a1 and interval
    // attributes contains a2 (point
    // attribute is a1 but an a2 interval is open) or
    // if the point attribute contains a2 and interval attributes contains a1
    // (point attribute is a2 but
    // an attribute a1 is open) or
    // the point attribute contains both a1 and a2 (the point is both attribute
    // types)
	...
	return ((getPointAttributes().contains(attribute1) && getIntervalAttributes().contains(attributes))
		|| (getPointAttributes().contains(attributes) && getIntervalAttributes().contains(attribute1)) || (getPointAttributes()
		.contains(attribute1) && getPointAttributes().contains(attributes)));
    }
	...
	for (AttributeType attributeType : DomainConstants.WORKED_ATTRIBUTES.getAttributes()) {
	    if ((getPointAttributes().contains(attribute) && getIntervalAttributes().contains(attributeType))
		    || (getPointAttributes().contains(attributeType) && getIntervalAttributes().contains(attribute))
		    || (getPointAttributes().contains(attribute) && getPointAttributes().contains(attributeType))) {
