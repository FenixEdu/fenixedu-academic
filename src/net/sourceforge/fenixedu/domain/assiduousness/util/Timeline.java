...
public class Timeline {
	...
	// startPosition. the timeline's last point is excluded 'cause it's the end
	...
	// As the point opens the interval, this method adds the point's attribute
	// to the open intervals set and to all subsequent point's attribute sets
	// part of plotPoint
	...
	// Finds the start point of a worked interval before a given TimePoint. The
	// start point must have worked as point attributes and worked must be in
	...
	private TimePoint findWorkedStartPointBetweenPoints(TimePoint startPoint, TimePoint endPoint) {
		...
		for (int i = startPosition; i < timelineSize; i++) {
			...
			// worked is never overlapped to another worked, so if it's a worked
			// point it's unique
	...
	// Finds the end point of the interval. The end point must have attribute as
	...
	// point must have attribute as point attributes and attribute must not be
	...
	// calculateAttributesDuration since we need to build and interval to get
	// its duration
	...
	public Attributes findAttributesIntervalThatOverlapFromAttributes(AttributeType attribute,
		...
		for (TimePoint point : getTimePoints()) {
			// ou o ponto tem o atributo ou o intervalo tem o atributo (caso em
	...
	// Calcula a duracao do periodo em q o funcionario saiu para almoco durante
	// o periodo de almoco.
	...
	public TimeInterval calculateBreakPeriod() {
		...
		if (mealStart.getIntervalAttributes().contains(DomainConstants.WORKED_ATTRIBUTES)) {
			...
			AttributeType workedAttribute = mealStart.getIntervalAttributes().intersects(
					DomainConstants.WORKED_ATTRIBUTES);
			// saber qual o atributo de worked encontrar o ponto dentro de meal
		...
		} else { // mealStart nao esta' dentro dum worked procurar inicio de
			// periodo de trabalho
			TimePoint workedStartPoint = findWorkedStartPointBetweenPoints(mealStart, mealEnd);
	...
	// time interval. The clockings should be done during the Normal Work Period
	// 1 or 2.
	// TODO verify this!
	...
	public boolean hasWorkingPointBeforeLeave(Leave leave) {
		TimePoint[] result = findIntervalByAttribute(AttributeType.JUSTIFICATION, leave.getDate()
				.toTimeOfDay());
