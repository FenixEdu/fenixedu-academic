...
public class Assiduousness extends Assiduousness_Base {
	...
	YearMonthDay beginDate = new YearMonthDay(partial.get(DateTimeFieldType.year()), partial
		.get(DateTimeFieldType.monthOfYear()), 1);
	YearMonthDay endDate = new YearMonthDay(partial.get(DateTimeFieldType.year()), partial
		.get(DateTimeFieldType.monthOfYear()), beginDate.dayOfMonth().getMaximumValue());
	...
	if (workDaySheet.getWorkSchedule() != null
		...
		if (dayOfWeek.equals(WeekDay.SATURDAY) || dayOfWeek.equals(WeekDay.SUNDAY)
			|| isDayHoliday) {
		    timeline.plotListInTimeline(workDaySheet.getAssiduousnessRecords(),
			    new ArrayList<Leave>(), attributesIt, workDaySheet.getDate());
		} else {
    ...
    public List<Leave> getLeavesByType(YearMonthDay beginDate, YearMonthDay endDate,
	...
	for (AssiduousnessStatusHistory assiduousnessStatusHistory : getAssiduousnessStatusHistories()) {
	    ...
	    } else if ((assiduousnessStatusHistory.getBeginDate().isBefore(date) || assiduousnessStatusHistory
		    .getBeginDate().isEqual(date))
		    && assiduousnessStatusHistory.getAssiduousnessStatus().getState() == AssiduousnessState.ACTIVE) {
		return assiduousnessStatusHistory.getAssiduousnessStatus();
	    }
	}
    ...
    public AssiduousnessStatus getLastAssiduousnessStatusBetween(YearMonthDay beginDate,
	    YearMonthDay endDate) {
	...
	for (AssiduousnessStatusHistory assiduousnessStatusHistory : getAssiduousnessStatusHistories()) {
		...
		if (interval.overlaps(statusInterval)
			&& assiduousnessStatusHistory.getAssiduousnessStatus().getState() == AssiduousnessState.ACTIVE) {
		    if (lastActiveStatus == null
			    || !lastActiveStatus.getEndDate().isAfter(
	...
	for (AssiduousnessStatusHistory assiduousnessStatusHistory : getAssiduousnessStatusHistories()) {
		...
		DateInterval statusInterval = new DateInterval(
			assiduousnessStatusHistory.getBeginDate(), assiduousnessStatusHistory
				.getEndDate());
		DateInterval interval = new DateInterval(beginDate, endDate);
		if (interval.containsInterval(statusInterval)
			|| statusInterval.containsInterval(interval)) {
	...
	for (AssiduousnessClosedMonth assiduousnessClosedMonth : getAssiduousnessClosedMonths()) {
	    if (assiduousnessClosedMonth.getClosedMonth().getClosedYearMonth().get(
		    ...
		    && assiduousnessClosedMonth.getClosedMonth().getClosedYearMonth().get(
			    DateTimeFieldType.monthOfYear()) == closedMonth.getClosedYearMonth().get(
	...
	return getAssiduousnessCampusHistories().isEmpty() && getAssiduousnessClosedMonths().isEmpty()
		...
		&& getAssiduousnessVacations().isEmpty() && getSchedules().isEmpty()
