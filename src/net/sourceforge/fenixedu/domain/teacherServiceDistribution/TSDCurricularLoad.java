...
public class TSDCurricularLoad extends TSDCurricularLoad_Base {
	...
	double numberOfShifts = StrictMath.ceil(((new Double(course.getTotalNumberOfStudents(type)) / course
		.getStudentsPerShift(type))));
	...
	return new Double(Math
		.ceil((SchoolClassCalculationMethod.SHIFT_BASED.equals(getSchoolClassCalculationMethod())) ? getNumberOfShifts()
			: getNumberOfShifts() * getFrequency())).intValue();
    }
}
