...
public class ValuationTeacher extends ValuationTeacher_Base {
	...
	public Department getDepartment() {
		if(isRealTeacher()) {
			...
			List<ExecutionPeriod> executionPeriods = getValuationGroupings().get(0).getValuationPhase().	getTeacherServiceDistribution().getExecutionPeriods();
			for(ExecutionPeriod period : executionPeriods) {
				 Department department = getTeacher().getLastWorkingDepartment(period.getBeginDateYearMonthDay(), period.getEndDateYearMonthDay());
				 if (department != null) {
					 return department;
				 }
			}
    ...
    private Double getRealHoursByShiftTypeAndExecutionCourses(ShiftType shiftType, List<ExecutionCourse> executionCourseList) {
    	Teacher teacher = getTeacher();
    	Double theoreticalHoursLastYear = 0.0;
    	
    	if(teacher != null) {
    		for (ExecutionCourse executionCourse : executionCourseList) {
    			theoreticalHoursLastYear += teacher.getHoursLecturedOnExecutionCourseByShiftType(executionCourse, shiftType);
    		}
    	}
    ...
    private Double getTotalHoursLecturedByShiftType(ShiftType shiftType, List<ExecutionPeriod> executionPeriodList) {
    	...
    	for (ExecutionPeriod executionPeriod : executionPeriodList) {
        	for (ProfessorshipValuation professorshipValuation : getActiveProfessorshipValuations()) {
    			if(shiftType == ShiftType.TEORICA) {
    				totalHours += (professorshipValuation.getExecutionPeriod() == executionPeriod) ? professorshipValuation.getTheoreticalHours() : 0d;
    			} else if(shiftType == ShiftType.PRATICA) {
    				totalHours += (professorshipValuation.getExecutionPeriod() == executionPeriod) ? professorshipValuation.getPraticalHours() : 0d;
    			} else if(shiftType == ShiftType.TEORICO_PRATICA) {
    				totalHours += (professorshipValuation.getExecutionPeriod() == executionPeriod) ? professorshipValuation.getTheoPratHours() : 0d;
    			} else if(shiftType == ShiftType.LABORATORIAL) {
    				totalHours += (professorshipValuation.getExecutionPeriod() == executionPeriod) ? professorshipValuation.getLaboratorialHours() : 0d;
    			}
    		}			
		}
    ...
    public Integer getRequiredHours(final List<ExecutionPeriod> executionPeriodList) {
    	...
    	if(isRealTeacher()) {
    		for (ExecutionPeriod executionPeriod : executionPeriodList) {
    			requiredHours += getTeacher().getHoursByCategory(executionPeriod);
			}
    	} else {
    ...
    public Double getAvailability(List<ExecutionPeriod> executionPeriodList) {	
    	return getRequiredHours(executionPeriodList) - getTotalHoursLectured(executionPeriodList) - (getUsingExtraCredits() ? getExtraCreditsValue(executionPeriodList) : 0.0);
    }
	...
	public Double getServiceExemptionCredits(List<ExecutionPeriod> executionPeriodList) {
		...
		if(isRealTeacher()) {
			for (ExecutionPeriod executionPeriod : executionPeriodList) {
				serviceExemptionCredits += new Double(getTeacher().getServiceExemptionCredits(executionPeriod));
			}
		}
	...
	public Double getManagementFunctionsCredits(List<ExecutionPeriod> executionPeriodList) {
		...
		if(isRealTeacher()) {
			for (ExecutionPeriod executionPeriod : executionPeriodList) {
				managementFunctionsCredits += getTeacher().getManagementFunctionsCredits(executionPeriod);
			}
		}
	...
	public List<TeacherServiceExemption> getServiceExemptions(List<ExecutionPeriod> executionPeriodList) {
		...
		if(getIsRealTeacher()) {
			for(ExecutionPeriod executionPeriod : executionPeriodList) {
				TeacherServiceExemption teacherServiceExemption = getTeacher().getDominantServiceExemption(executionPeriod);
				if(teacherServiceExemption != null) teacherServiceExemptionList.add(teacherServiceExemption);
			}
		}
	...
	public Double getExtraCreditsValue(List<ExecutionPeriod> executionPeriodList) {
		return getExtraCreditsValue() * (executionPeriodList.size() / getValuationGroupings().get(0).getValuationPhase().getTeacherServiceDistribution().getExecutionPeriodsCount());
	}
	...
	public List<ProfessorshipValuation> getActiveProfessorshipValuations() {
		return (List<ProfessorshipValuation>) CollectionUtils.select(getProfessorshipValuations(), new Predicate() {
