...
public class ValuationPhase extends ValuationPhase_Base {
	...
	public ValuationPhase(
			TeacherServiceDistribution teacherServiceDistribution,
			String name,
			ValuationPhase previousValuationPhase,
			ValuationPhase nextValuationPhase,
			ValuationPhaseStatus status,
			Map<String, Object> omissionValues) {
		this();
		...
		this.setStudentsPerLaboratorialShift((Integer) omissionValues.get("studentsPerLaboratorialShift"));
		this.setWeightFirstTimeEnrolledStudentsPerTheoShift((Double) omissionValues.get("weightFirstTimeEnrolledStudentsPerTheoShift"));
		this.setWeightFirstTimeEnrolledStudentsPerPratShift((Double) omissionValues.get("weightFirstTimeEnrolledStudentsPerPratShift"));
		this.setWeightFirstTimeEnrolledStudentsPerTheoPratShift((Double) omissionValues.get("weightFirstTimeEnrolledStudentsPerTheoPratShift"));
		this.setWeightFirstTimeEnrolledStudentsPerLabShift((Double) omissionValues.get("weightFirstTimeEnrolledStudentsPerLabShift"));
		this.setWeightSecondTimeEnrolledStudentsPerTheoShift((Double) omissionValues.get("weightSecondTimeEnrolledStudentsPerTheoShift"));
		this.setWeightSecondTimeEnrolledStudentsPerPratShift((Double) omissionValues.get("weightSecondTimeEnrolledStudentsPerPratShift"));
		this.setWeightSecondTimeEnrolledStudentsPerTheoPratShift((Double) omissionValues.get("weightSecondTimeEnrolledStudentsPerTheoPratShift"));
		this.setWeightSecondTimeEnrolledStudentsPerLabShift((Double) omissionValues.get("weightSecondTimeEnrolledStudentsPerLabShift"));

		createRootValuationGroupingWithTeachersAndCourses(teacherServiceDistribution);
		createSubGroupings();
	}

	private void createRootValuationGroupingWithTeachersAndCourses(TeacherServiceDistribution teacherServiceDistribution) {
		List<ValuationTeacher> valuationTeacherList = createValuationTeacher(
				teacherServiceDistribution.getDepartment(),
				teacherServiceDistribution.getExecutionPeriods());

		new ValuationGrouping(
				this,
				teacherServiceDistribution.getDepartment().getAcronym(),
				null,
				valuationTeacherList,
				createValuationCompetenceCourses(teacherServiceDistribution),
				null,
				null);
	}
	...
	private void createSubGroupings() {
		...
		for (ValuationTeacher valuationTeacher : rootGrouping.getValuationTeachers()) {
			...
			if (teacher.getCurrentWorkingUnit() != null) {
				createChildUnitsGroupingsForTeachers(valuationTeacher, teacher.getCurrentWorkingUnit(), null,
						rootGrouping, unitTeachersNewGrouping);
			}
		}

		HashMap<Unit, ValuationGrouping> unitCoursesNewGrouping = new HashMap<Unit, ValuationGrouping>();
		
		for (ValuationCompetenceCourse valuationCompetenceCourse : rootGrouping.getValuationCompetenceCourses()) {
			...
			if (competenceCourse != null && competenceCourse.getCompetenceCourseGroupUnit() != null) {
				createChildCompetenceCourseGroupUnits(valuationCompetenceCourse, rootGrouping, unitCoursesNewGrouping);
			}
		}
		...
		for (Unit courseUnit : unitCoursesNewGrouping.keySet()) {
			...
			ValuationGrouping scientificAreaGrouping = unitScientificAreaGrouping.get(scientificAreaUnit);
			...
			if(scientificAreaGrouping == null){
				if(teachersNewGrouping == null){
					scientificAreaGrouping = new ValuationGrouping(this, scientificAreaUnit.getName(), rootGrouping, 
							new ArrayList<ValuationTeacher>(), new ArrayList<ValuationCompetenceCourse>(),
							rootGrouping.getCoursesAndTeachersValuationManagers(),
							rootGrouping.getCoursesAndTeachersManagementGroup());
				} else {
			...
			scientificAreaGrouping.getValuationCompetenceCoursesSet().addAll(coursesNewGrouping.getValuationCompetenceCourses());
			coursesNewGrouping.setParent(scientificAreaGrouping);
		}
	}
	
	private void createChildUnitsGroupingsForTeachers(
			ValuationTeacher valuationTeacher,
			Unit unit,
			ValuationGrouping sonGrouping,
			ValuationGrouping rootGrouping,
		...
		if (newGrouping == null) {
			...
			newGrouping = new ValuationGrouping(
					this,
					unit.getName(),
					null,
					teachersList,
					emptyValuationCompetenceCourseList,
					null,
					null);
			...
			for (Unit parentUnit : unit.getParentByOrganizationalStructureAccountabilityType()) {
				for (Unit grandParentUnit : parentUnit.getParentByOrganizationalStructureAccountabilityType()) {
					if (!grandParentUnit.getParentByOrganizationalStructureAccountabilityType().isEmpty()) {
						createChildUnitsGroupingsForTeachers(
								valuationTeacher,
								parentUnit,
								newGrouping,
								rootGrouping,
	...
	private void createChildCompetenceCourseGroupUnits(
		...
		CompetenceCourse competenceCourse = valuationCompetenceCourse.getCompetenceCourse();
		ValuationGrouping newGrouping = unitNewGrouping.get(competenceCourse.getCompetenceCourseGroupUnit());
		...
		if (newGrouping == null) {
			...
			newGrouping = new ValuationGrouping(
					this,
					competenceCourse.getCompetenceCourseGroupUnit().getName(),
					rootGrouping,
					emptyValuationTeacherList,
					valuationCompetenceCourseList,
					null,
					null);
	...
	private List<ValuationCompetenceCourse> createValuationCompetenceCourses(
		...
		for (CompetenceCourse competenceCourse : teacherServiceDistribution.getAllCompetenceCourses()) {
			ValuationCompetenceCourse valuationCompetenceCourse = competenceCourse.getValuationCompetenceCourse();
	...
	private List<ValuationTeacher> createValuationTeacher(
	...
	public ValuationPhase getCurrentValuationPhase() {
		for (ValuationPhase valuationPhase = getFirstValuationPhase(); valuationPhase != null; valuationPhase = valuationPhase.getNextValuationPhase()) {
	...
	private void copyDataFromValuationPhase(
		...
		Map<ValuationTeacher, ValuationTeacher> oldAndNewValuationTeacherMap = createAndCopyFromValuationTeacherList(oldValuationPhase.getRootValuationGrouping().getValuationTeachers());

		ValuationGrouping.createAndCopyFromValuationGrouping(
				this,
				oldValuationPhase.getRootValuationGrouping(),
				null,
				oldAndNewValuationTeacherMap,
				oldAndNewValuationCompetenceCourseMap);

		createAndCopyCompetenceCourseValuationsAndProfessorshipValuations(
				oldValuationPhase,
				oldAndNewValuationCompetenceCourseMap,
				oldAndNewValuationTeacherMap,
		...
		Map<CurricularCourseValuation, CurricularCourseValuation> oldAndNewCurricularCourseValuationMap = createAndCopyCurricularCourseValuationsAndProfessorshipValuations(
				oldValuationPhase,
				oldAndNewValuationCurricularCourseMap,
				oldAndNewValuationTeacherMap,
				oldAndNewExecutionPeriodMap);

		createAndCopyCurricularCourseValuationGroupsAndProfessorshipValuations(
				oldValuationPhase,
				oldAndNewValuationCompetenceCourseMap,
				oldAndNewValuationTeacherMap,
				oldAndNewExecutionPeriodMap,
		...
		this.setWeightFirstTimeEnrolledStudentsPerTheoShift(oldValuationPhase.getWeightFirstTimeEnrolledStudentsPerTheoShift());
		this.setWeightFirstTimeEnrolledStudentsPerPratShift(oldValuationPhase.getWeightFirstTimeEnrolledStudentsPerPratShift());
		this.setWeightFirstTimeEnrolledStudentsPerTheoPratShift(oldValuationPhase.getWeightFirstTimeEnrolledStudentsPerTheoPratShift());
		this.setWeightFirstTimeEnrolledStudentsPerLabShift(oldValuationPhase.getWeightFirstTimeEnrolledStudentsPerLabShift());
		this.setWeightSecondTimeEnrolledStudentsPerTheoShift(oldValuationPhase.getWeightSecondTimeEnrolledStudentsPerTheoShift());
		this.setWeightSecondTimeEnrolledStudentsPerPratShift(oldValuationPhase.getWeightSecondTimeEnrolledStudentsPerPratShift());
		this.setWeightSecondTimeEnrolledStudentsPerTheoPratShift(oldValuationPhase.getWeightSecondTimeEnrolledStudentsPerTheoPratShift());
		this.setWeightSecondTimeEnrolledStudentsPerLabShift(oldValuationPhase.getWeightSecondTimeEnrolledStudentsPerLabShift());

	}

	public void copyDataFromValuationPhase(ValuationPhase oldValuationPhase) {
		Map<ExecutionPeriod, ExecutionPeriod> oldAndNewExecutionPeriodMap = TeacherServiceDistribution.getExecutionPeriodTranslationMap(
				oldValuationPhase.getTeacherServiceDistribution().getExecutionPeriods(),
				this.getTeacherServiceDistribution().getExecutionPeriods());
	...
	private Map<ValuationCurricularCourse, ValuationCurricularCourse> createAndCopyFromValuationCurricularCourses(
		...
		for (ValuationCompetenceCourse valuationCompetenceCourse : oldAndNewValuationCompetenceCourseMap.keySet()) {
			for (ValuationCurricularCourse valuationCurricularCourse : valuationCompetenceCourse.getValuationCurricularCourses(getTeacherServiceDistribution().getExecutionPeriods())) {
				if (valuationCurricularCourse.getIsRealCurricularCourse()) {
					oldAndNewValuationCurricularCourseMap.put(valuationCurricularCourse, valuationCurricularCourse);
				} else {
					ValuationCurricularCourse newValuationCurricularCourse = new ValuationCurricularCourse(
							...
							valuationCurricularCourse.getCurricularYears(),
							valuationCurricularCourse.getDegreeCurricularPlan(),
							valuationCurricularCourse.getExecutionPeriod(),
							valuationCurricularCourse.getAcronym(),
							valuationCurricularCourse.getTheoreticalHours(),
							valuationCurricularCourse.getPraticalHours(),
							valuationCurricularCourse.getTheoPratHours(),
	...
	private Map<ExecutionPeriod, ExecutionPeriod> getSimpleExecutionPeriodsTranslationMap(
		...
		for (ExecutionPeriod executionPeriod : oldValuationPhase.getTeacherServiceDistribution().getExecutionPeriods()) {
	...
	private void createAndCopyCompetenceCourseValuationsAndProfessorshipValuations(
			...
			Map<ExecutionPeriod, ExecutionPeriod> oldAndNewExecutionPeriodMap) {
		for (ValuationCompetenceCourse oldValuationCompetenceCourse : oldAndNewValuationCompetenceCourseMap.keySet()) {
			for (ExecutionPeriod oldExecutionPeriod : oldAndNewExecutionPeriodMap.keySet()) {
				CompetenceCourseValuation oldCompetenceCourseValuation = oldValuationCompetenceCourse.getCompetenceCourseValuationByValuationPhaseAndExecutionPeriod(
						oldValuationPhase,
						oldExecutionPeriod);
				ExecutionPeriod newExecutionPeriod = oldAndNewExecutionPeriodMap.get(oldExecutionPeriod);
				ValuationCompetenceCourse newValuationCompetenceCourse = oldAndNewValuationCompetenceCourseMap.get(oldValuationCompetenceCourse);

				if (oldCompetenceCourseValuation != null && newExecutionPeriod != null
						&& newValuationCompetenceCourse != null) {
					CompetenceCourseValuation newCompetenceCourseValuation = CompetenceCourseValuation.createAndCopyFromCompetenceCourseValuation(
							this,
							newValuationCompetenceCourse,
							newExecutionPeriod,
							oldCompetenceCourseValuation);

					createAndCopyProfessorshipValuations(
							oldAndNewValuationTeacherMap,
							oldCompetenceCourseValuation,
							newCompetenceCourseValuation);
				}
			}
		}
	}
	...
	private Map<CurricularCourseValuation, CurricularCourseValuation> createAndCopyCurricularCourseValuationsAndProfessorshipValuations(
		...
		for (ValuationCurricularCourse oldValuationCurricularCourse : oldAndNewValuationCurricularCourseMap.keySet()) {
			for (ExecutionPeriod oldExecutionPeriod : oldAndNewExecutionPeriodMap.keySet()) {
				CurricularCourseValuation oldCurricularCourseValuation = oldValuationCurricularCourse.getCurricularCourseValuationByValuationPhaseAndExecutionPeriod(
						oldValuationPhase,
						oldExecutionPeriod);
				ExecutionPeriod newExecutionPeriod = oldAndNewExecutionPeriodMap.get(oldExecutionPeriod);
				ValuationCurricularCourse newValuationCurricularCourse = oldAndNewValuationCurricularCourseMap.get(oldValuationCurricularCourse);
				...
				if (oldCurricularCourseValuation != null) {
					...
					if (newExecutionPeriod != null && newValuationCurricularCourse != null) {
						newCurricularCourseValuation = CurricularCourseValuation.createAndCopyFromCurricularCourseValuation(
								this,
								newValuationCurricularCourse,
								newExecutionPeriod,
								oldCurricularCourseValuation);
						oldAndNewCurricularCourseValuationMap.put(
								oldCurricularCourseValuation,
	...
	private void createAndCopyCurricularCourseValuationGroupsAndProfessorshipValuations(
		...
		for (ValuationCompetenceCourse oldValuationCompetenceCourse : oldAndNewValuationCompetenceCourseMap.keySet()) {
			for (ExecutionPeriod oldExecutionPeriod : oldAndNewExecutionPeriodMap.keySet()) {
				for (CurricularCourseValuationGroup oldCurricularCourseValuationGroup : oldValuationCompetenceCourse.getCurricularCourseValuationGroupsByValuationPhaseAndExecutionPeriod(
						oldValuationPhase,
						oldExecutionPeriod)) {
					...
					if (!newAssociatedCurricularCourseValuations.isEmpty()
						...
						CurricularCourseValuationGroup newCurricularCourseValuationGroup = CurricularCourseValuationGroup.createAndCopyFromCurricularCoursevValuationGroup(
								newAssociatedCurricularCourseValuations,
								oldCurricularCourseValuationGroup);
						createAndCopyProfessorshipValuations(
								oldAndNewValuationTeacherMap,
								oldCurricularCourseValuationGroup,
								newCurricularCourseValuationGroup);
					}
				}
			}
		}
	...
	private void createAndCopyProfessorshipValuations(
			Map<ValuationTeacher, ValuationTeacher> oldAndNewValuationTeacherMap,
			CourseValuation oldCourseValuation,
			CourseValuation newCourseValuation) {
		for (ProfessorshipValuation oldProfessorshipValuation : oldCourseValuation.getProfessorshipValuations()) {
			ValuationTeacher newValuationTeacher = oldAndNewValuationTeacherMap.get(oldProfessorshipValuation.getValuationTeacher());

			if (newValuationTeacher != null) {
				ProfessorshipValuation.createAndCopy(newCourseValuation, newValuationTeacher, oldProfessorshipValuation);
			}
		}
	}
	...
	private List<CurricularCourseValuation> getNewAssociatedCurricularCourseValuation(
		...
		for (CurricularCourseValuation oldCurricularCourseValuation : oldCurricularCourseValuations) {
			CurricularCourseValuation newCurricularCourseValuation = oldAndNewCurricularCourseValuationMap.get(oldCurricularCourseValuation);
	...
	private Map<ValuationCompetenceCourse, ValuationCompetenceCourse> createAndCopyFromValuationCompetenceCourseList(
		...
		for (ValuationCompetenceCourse valuationCompetenceCourse : valuationCompetenceCourseList) {
			if (valuationCompetenceCourse.getIsRealCompetenceCourse()) {
				if(valuationCompetenceCourse.hasActiveValuationCurricularCoursesInExecutionPeriods(executionPeriodList)) {
					oldAndNewValuationCompetenceCourseMap.put(valuationCompetenceCourse, valuationCompetenceCourse);
				}
			} else {
				ValuationCompetenceCourse newValuationCompetenceCourse = new ValuationCompetenceCourse(
						valuationCompetenceCourse.getName());
				oldAndNewValuationCompetenceCourseMap.put(valuationCompetenceCourse, newValuationCompetenceCourse);
			}
		}
		...
		for (ValuationTeacher valuationTeacher : valuationTeacherList) {
			...
			if (valuationTeacher.getIsRealTeacher()
					&& thisDepartment == valuationTeacher.getTeacher().getLastWorkingDepartment(
							getTeacherServiceDistribution().getFirstExecutionPeriod().getBeginDateYearMonthDay(),
							getTeacherServiceDistribution().getFirstExecutionPeriod().getEndDateYearMonthDay())) {
				newValuationTeacher = new ValuationTeacher(valuationTeacher.getTeacher());

			} else if(!valuationTeacher.getIsRealTeacher()){
				newValuationTeacher = new ValuationTeacher(
						valuationTeacher.getCategory(),
						valuationTeacher.getName(),
						valuationTeacher.getRequiredHours());
			}
	...
	public static ValuationPhase createAndCopy(
			TeacherServiceDistribution newTeacherServiceDistribution,
			ValuationPhase oldValuationPhase,
			Map<ExecutionPeriod, ExecutionPeriod> oldAndNewExecutionPeriodMap,
...
//		 TODO Auto-generated method stub
	...
	@Override
	@Checked("ValuationPhasePredicates.writePredicate")
	public void setWeightFirstTimeEnrolledStudentsPerPratShift(Double weightFirstTimeEnrolledStudentsPerPratShift) {
		
		super.setWeightFirstTimeEnrolledStudentsPerPratShift(weightFirstTimeEnrolledStudentsPerPratShift);
	}

	@Override
	@Checked("ValuationPhasePredicates.writePredicate")
	public void setWeightFirstTimeEnrolledStudentsPerTheoPratShift(Double weightFirstTimeEnrolledStudentsPerTheoPratShift) {
		
		super.setWeightFirstTimeEnrolledStudentsPerTheoPratShift(weightFirstTimeEnrolledStudentsPerTheoPratShift);
	}
	...
	@Override
	@Checked("ValuationPhasePredicates.writePredicate")
	public void setWeightSecondTimeEnrolledStudentsPerPratShift(Double weightSecondTimeEnrolledStudentsPerPratShift) {
		
		super.setWeightSecondTimeEnrolledStudentsPerPratShift(weightSecondTimeEnrolledStudentsPerPratShift);
	}

	@Override
	@Checked("ValuationPhasePredicates.writePredicate")
	public void setWeightSecondTimeEnrolledStudentsPerTheoPratShift(Double weightSecondTimeEnrolledStudentsPerTheoPratShift) {
		
		super.setWeightSecondTimeEnrolledStudentsPerTheoPratShift(weightSecondTimeEnrolledStudentsPerTheoPratShift);
	}
