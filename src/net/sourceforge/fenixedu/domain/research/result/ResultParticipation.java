...
public class ResultParticipation extends ResultParticipation_Base {
	 ...
	 * personOrder.
	 */
	public static class OrderComparator implements
			Comparator<ResultParticipation> {
	 ...
	 */
	ResultParticipation(ResearchResult result, Person participator,
			ResultParticipationRole role, Integer order) {
		this();
	...
	public void setEditAll(ResearchResult result, Person participator,
			ResultParticipationRole role, Integer order) {
		fillAllAttributes(result, participator, role, order);
		setChangedBy();
	}
	...
	public final void movePersonToDesiredOrder(OrderChange orderChange) {
		if (orderChange == null) {
			throw new DomainException(
					"error.researcher.ResultParticipation.orderChange.null");
		}
		move(orderChange);
		setChangedBy();
	}
	...
	public void setRole(ResultParticipationRole role) {
		if (role == null) {
			throw new DomainException(
					"error.researcher.ResultEventAssociation.role.null");
		}
		if (!this.getRole().equals(role)) {
			if (!this.getResult().hasPersonParticipationWithRole(
					this.getPerson(), role)) {
				super.setRole(role);
				this.getResult().setModifiedByAndDate();
			} else {
				throw new DomainException(
						"error.researcher.ResultEventAssociation.association.exists",
	...
	public final static ResultParticipation readByOid(Integer oid) {
		...
		if (participation == null) {
			throw new DomainException(
					"error.researcher.ResultParticipation.null");
		}
	 ...
	 */
	private void checkParameters(ResearchResult result, Person participator,
			ResultParticipationRole role, Integer order) {
		if (result == null) {
			throw new DomainException(
					"error.researcher.ResultParticipation.result.null");
		}
		if (participator == null) {
			throw new DomainException(
					"error.researcher.ResultParticipation.person.null");
		}
		if (role == null) {
			throw new DomainException(
					"error.researcher.ResultParticipation.role.null");
		}
		...
		if (result.hasPersonParticipationWithRole(participator, role)) {
			throw new DomainException(
					"error.researcher.ResultParticipation.participation.exists");
		}
		if (order == 0 && result.getResultParticipationsCount() > 1) {
			throw new DomainException(
					"error.researcher.ResultParticipation.invalid.order");
		}
	}
	...
	private void move(OrderChange change) {
		final int newOrder = this.getPersonOrder()
				+ OrderChange.getOffset(change);
		final List<ResultParticipation> originalList = this.getResult()
				.getOrderedResultParticipations();
		...
		for (ResultParticipation participation : orderedParticipations) {
			originalList.get(originalList.indexOf(participation))
					.setPersonOrderAttribute(index++);
		}
	}

	private void fillAllAttributes(ResearchResult result, Person participator,
			ResultParticipationRole role, Integer order) {
	...
	public final void deleteIfNotLast() {
		if (getIsLastParticipation()) {
			throw new DomainException(
					"error.researcher.ResultParticipation.last.one");
		}
		delete();
	}
