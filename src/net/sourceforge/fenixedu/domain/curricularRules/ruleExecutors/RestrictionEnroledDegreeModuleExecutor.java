...
public class RestrictionEnroledDegreeModuleExecutor extends RuleExecutor {
	...
	if (moduleToEnrol == null) {
	    throw new DomainException(
		    "error.curricularRules.RestrictionEnroledDegreeModuleExecutor.cannot.find.degreeModuleToEnrol");
	}
	...
	result &= (enrolmentContext.getStudentCurricularPlan().isEnroledInExecutionPeriod(
		...
		enrolmentContext.getExecutionPeriod()) || enrolmentContext.getStudentCurricularPlan()
		.isEnroledInExecutionPeriod((CurricularCourse) rule.getPrecedenceDegreeModule(),
			enrolmentContext.getExecutionPeriod().getPreviousExecutionPeriod()));

	//return result ? RuleResult.createTrue() : RuleResult.createFalse(new LabelFormatter()
	//	.appendLabel("reason"));
	return null;
    }
