...
public class PreviousYearsEnrolmentExecutor extends CurricularRuleExecutor {
	...
	return result.getLeft() ? RuleResult.createTrue() : RuleResult.createFalse("curricularRules.rulesExecutor.PreviousYearsEnrolmentExecutor", String.valueOf(result.getRight()));
    }
	...
	final GenericPair<Boolean, Integer> result = isValidEnrolment(enrolmentInformation, enrolmentContext);
	if (!result.getLeft()) {
	    return RuleResult.createFalse("curricularRules.rulesExecutor.PreviousYearsEnrolmentExecutor", String.valueOf(result.getRight()));
	}
    ...
    private void addInformationToResult(final Map<Integer, GenericTrio<Integer, Integer, Integer>> result,
	    final int year, final int numberOfMissingCurricularCoursesToEnrol,
	    final int totalCurricularCourses,
	    final int numberOfEnroledCurricularCoursesInPreviousSemester) {
	
	result.put(year, new GenericTrio<Integer, Integer, Integer>(
		Integer.valueOf(numberOfMissingCurricularCoursesToEnrol),
		Integer.valueOf(totalCurricularCourses),
