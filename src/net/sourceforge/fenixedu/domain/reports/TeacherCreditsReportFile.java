...
public class TeacherCreditsReportFile extends TeacherCreditsReportFile_Base {
	...
	for (ExecutionSemester executionSemester : executionYear.getExecutionPeriods()) {
		...
		if (isContractedTeacher || teacherAuthorization != null) {
		    ...
		    row.setCell(teacher.getPerson().getEmployee() != null ? teacher.getPerson().getEmployee().getEmployeeNumber()
			    : null);
			...
			situation = teacher.getCurrentOrLastTeacherContractSituation(executionSemester.getBeginDateYearMonthDay()
		    ...
		    //O (desc)
		    Double reductionService = teacherService == null ? 0 : teacherService.getReductionService() == null ? 0
			    : teacherService.getReductionService().getCreditsReduction() == null ? 0 : teacherService
	...
	return teacherContractSituation
		.getGiafProfessionalData()
		.getPersonProfessionalData()
		.getDominantProfessionalRegime(teacherContractSituation.getGiafProfessionalData(), interval, CategoryType.TEACHER);
    }
	...
	if (employee != null) {
	    List<Contract> workingContracts = employee.getWorkingContracts(executionSemester.getBeginDateYearMonthDay(),
	...
	YearMonthDay begin = contract.getBeginDate().isBefore(executionSemester.getBeginDateYearMonthDay()) ? executionSemester
		.getBeginDateYearMonthDay() : contract.getBeginDate();
	YearMonthDay end = contract.getEndDate() == null
		|| contract.getEndDate().isAfter(executionSemester.getEndDateYearMonthDay()) ? executionSemester
