	...
	for (final StudentCurricularPlan studentCurricularPlan : getStudentCurricularPlansToProcess(executionYear)) {
		...
		for (final CycleType cycleType : registration.getDegreeType().getCycleTypes()) {
			...
			if (cycleCGroup.isConcluded()) {
			    conclusionDate = new RegistrationConclusionBean(registration, cycleCGroup).getConclusionDate();
			    if (conclusionDate.getYear() != civilYear && conclusionDate.getYear() != civilYear - 1) {
				continue;
			    }
			}

			if ((registration.isActive() || registration.isConcluded()) && conclusionDate != null
				&& (conclusionDate.getYear() == civilYear || conclusionDate.getYear() == civilYear - 1)) {
			    reportRaidesGraduate(spreadsheet, registration, executionYear, cycleType, true, conclusionDate);
		...
		count++; // if (count == this.iterationCount) { // break ; // }
		// }
