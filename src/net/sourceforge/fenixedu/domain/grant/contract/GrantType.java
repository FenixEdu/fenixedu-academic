...
public class GrantType extends GrantType_Base {
    ...
    public int countGrantContractsByActiveAndDate(final Boolean activeContracts,
            final Date beginContractDate, final Date endContractDate) {
        ...
        for (final GrantContract grantContract : this.getAssociatedGrantContractsSet()) {
            if (activeContracts != null && activeContracts) {
                if (!grantContract.hasActiveRegimes()
                        || grantContract.getEndContractMotive().length() != 0) {
                    continue;
                }
