	...
	public static Comparator<Space> SPACE_COMPARATOR_BY_CLASS = new Comparator<Space>() {
		...
		private int compareSpacesOfSameType(final Class<? extends Space> clazz, final Space space1, final Space space2) {
			if (clazz == Campus.class) {
				return Campus.COMPARATOR_BY_PRESENTATION_NAME.compare(space1, space2);
			} else if (clazz == Building.class) {
				return Building.COMPARATOR_BY_PRESENTATION_NAME.compare(space1, space2);
				...
				return Room.COMPARATOR_BY_PRESENTATION_NAME.compare(space1, space2);
			} else if (clazz == RoomSubdivision.class) {
				return RoomSubdivision.COMPARATOR_BY_PRESENTATION_NAME.compare(space1, space2);
			} else {
