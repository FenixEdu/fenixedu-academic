...
public class Vigilant extends Vigilant_Base {
	...
	public double getEstimatedPoints() {
		...
		for (Vigilancy vigilancy : getActiveVigilancies()) {
			int points = vigilancy.hasPointsAttributed() ? vigilancy.getPoints() : vigilancy
					.getEstimatedPoints();
	...
	public Boolean canBeConvokedForWrittenEvaluation(WrittenEvaluation writtenEvaluation) {
		...
		boolean isInExamPeriod = writtenEvaluation.getAssociatedExecutionCourses().get(0)
				.isInExamPeriod();
		return this.isAvailableOnDate(beginOfExam, endOfExam)
				...
				&& (isInExamPeriod || (!isInExamPeriod && ((this.getTeacher() != null && this
						.getTeacher().teachesAny(writtenEvaluation.getAssociatedExecutionCourses())) || !hasLessons(
