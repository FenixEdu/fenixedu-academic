...
public class Vigilant extends Vigilant_Base {
	...
	public static final Comparator<Vigilant> POINTS_COMPARATOR = new BeanComparator(
			"points");

	public static final Comparator<Vigilant> ESTIMATED_POINTS_COMPARATOR = new BeanComparator(
			"estimatedPoints");

	public static final Comparator<Vigilant> NAME_COMPARATOR = new BeanComparator(
			"person.name");

	public static final Comparator<Vigilant> USERNAME_COMPARATOR = new ReverseComparator(
			new BeanComparator("person.username"));
	...
	public double getEstimatedPoints() {
		...
		for (Vigilancy vigilancy : getActiveVigilancies()) {
			int points = vigilancy.hasPointsAttributed() ? vigilancy
					.getPoints() : vigilancy.getEstimatedPoints();
	...
	private List<VigilantGroup> getVigilantGroupsForGivenBoundValue(
			Boolean value) {
		List<VigilantGroup> groups = new ArrayList<VigilantGroup>();
		for (VigilantBound bound : this.getBounds()) {
		...
		if (employee != null) {
			...
			if (assiduousness != null) {
				return assiduousness.getCampusForInterval(year
						.getBeginDateYearMonthDay().toLocalDate(), year
	...
	public Boolean canBeConvokedForWrittenEvaluation(
		...
		boolean isInExamPeriod = writtenEvaluation
				.getAssociatedExecutionCourses().get(0).isInExamPeriod();
		return this.isAvailableOnDate(beginOfExam, endOfExam)
				&& this.hasNoEvaluationsOnDate(beginOfExam, endOfExam)
				&& (isInExamPeriod || (!isInExamPeriod && ((this.getTeacher() != null && this
						.getTeacher().teachesAny(
								writtenEvaluation
										.getAssociatedExecutionCourses())) || !hasLessons(
						beginOfExam, endOfExam))));
	}

	private boolean hasLessons(DateTime beginOfExam, DateTime endOfExam) {
		Teacher teacher = getTeacher();
		return teacher == null ? false : teacher.hasLessons(beginOfExam,
				endOfExam);
	}
	...
	public void delete() {
		...
		if (this.getActiveVigilanciesInList(this.getVigilancies()).size() == 0) {
			...
			for (; !this.getBounds().isEmpty(); this.getBounds().get(0)
					.delete())
				;
			for (; !this.getVigilancies().isEmpty(); this.getVigilancies().get(
					0).delete())
				;
			for (; !this.getVigilancies().isEmpty(); this.getVigilancies().get(
					0).delete())
	...
	private List<Vigilancy> getActiveVigilanciesInList(
	...
	public UnavailableTypes getWhyIsUnavailabeFor(
			WrittenEvaluation writtenEvaluation) {
		...
		if (teacher != null
			...
			for (TeacherServiceExemption situation : situations) {
				...
				Interval interval = new Interval(beginSituation
						.toDateTimeAtMidnight(), endSituation
	...
	public String getBoundsAsString() {
		...
		for (VigilantBound bound : this.getBounds()) {
			if (!bound.getConvokable()) {
				if (i > 0)
					result += ", ";
				result += bound.getVigilantGroup().getName() + ": "
						+ bound.getJustification();
				i++;
			}
		}
		return result;
	...
	public List<VigilantGroup> getVisibleVigilantGroups() {
		...
		if (employee != null) {
			...
			Department department = employee.getLastDepartmentWorkingPlace(
					executionYear.getBeginDateYearMonthDay(), executionYear
							.getEndDateYearMonthDay());
			groups.addAll(department
	...
	public void addVigilancies(Vigilancy vigilancy) {
		if (hasNoEvaluationsOnDate(vigilancy.getBeginDateTime(), vigilancy
				.getEndDateTime())) {
