...
public abstract class EmployeeJustificationFactory implements Serializable, FactoryExecutor {
            ...
            if (getCorrectionType() != null && getCorrectionType().equals(CorrectionType.JUSTIFICATION)) {
                if (getJustificationMotive() != null
                    ...
                    if (getJustificationMotive().getJustificationType().equals(JustificationType.TIME)) {
                        ...
                        if (isOverlapingOtherJustification(duration)) {
    ...
    protected ActionMessage canInsertTimeJustification() {
        ...
        if (clocks != null) {
            ...
            for (AssiduousnessRecord assiduousnessRecord : clocks) {
                if (assiduousnessRecord.getDate().toTimeOfDay().isBefore(getBeginTime())) {
                    before++;
                } else if (assiduousnessRecord.getDate().toTimeOfDay().isBefore(getEndTime())) {
                    middle++;
                } else {
                    break;
                }
    ...
    protected boolean isOverlapingOtherJustification(Duration duration) {
        return !getEmployee().getAssiduousness().getLeaves(getBeginDate(),
                getBeginDate().toDateTimeAtMidnight().plus(duration).toYearMonthDay()).isEmpty();
    }
