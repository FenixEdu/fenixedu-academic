...
public abstract class ClassUtils {
	...
	public static int getCurricularYearByClassName(String className) {
		...
		try {
			...
			try {
				...
				anoCurricular = 5;
				System.out.println(
					"NÃO FOI POSSÍVEL INFERIR O ANO CURRÍCULAR DA TURMA "
						+ className
	 ...
	 */
	public static ICursoExecucao getExecutionDegreeByClassName(String className, String [] executionDegreeInitials, HashMap executionDegreeHashMap) {
		...
		ICursoExecucao executionDegree =
			(ICursoExecucao) executionDegreeHashMap.get(degreeInitials);
		if (executionDegree == null)
			System.out.println(
				"Error execution degree not found" + degreeInitials);
	...
	public static List extractInitialsList(List classNameList) {
		...
		while (iterator.hasNext()) {
			...
			String degreeInitials = Constants.siglasLicenciaturas[Integer.parseInt(className.substring(0,2)) - 1];
