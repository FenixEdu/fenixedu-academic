...
public class TransformerMigrationLessonToIAula implements Transformer {
	...
	public Object transform(Object obj) {
		...
		if (obj instanceof MigrationLesson) {
			...
			if (migrationLesson.getLesson() == null) {
				...
				lesson.setInicio(
					convertHourToCalendar(
						migrationLesson.getStartIndex(),
						true));
				lesson.setFim(
					convertHourToCalendar(
						migrationLesson.getEndIndex(),
						false));
				lesson.setDiaSemana(
					conversorDiaSemana(migrationLesson.getDay()));
				lesson.setTipo(
					LessonTypeUtils.convertLessonType(
						migrationLesson.getLessonType()));
	...
	private Calendar convertHourToCalendar(int hora, boolean start) {
		...
		double aux = (double) 8.0 + ((double) (hora - 1) / 2);

		int hour = (int) Math.floor(aux);
		int minutes = (int) (((double) aux - (double) hour) * 60);
		if (!start) {
			if (minutes == 0){
				minutes = 30;
			}else{
