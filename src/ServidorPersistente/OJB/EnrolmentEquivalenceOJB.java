...
public class EnrolmentEquivalenceOJB extends ObjectFenixOJB implements IPersistentEnrolmentEquivalence {
	...
	public void lockWrite(IEnrolmentEquivalence equivalenceToWrite) throws ExcepcaoPersistencia, ExistingPersistentException {
		...
		try {
			...
			equivalenceFromDB = (IEnrolmentEquivalence) queryObject(EnrolmentEquivalence.class, criteria);
		...
		if (equivalenceFromDB == null) {
			...
			(equivalenceToWrite instanceof EnrolmentEquivalence)
				&& ((EnrolmentEquivalence) equivalenceFromDB).getIdInternal().equals(((EnrolmentEquivalence) equivalenceToWrite).getIdInternal())) {
			super.lockWrite(equivalenceToWrite);
