...
public class EnrolmentOJB extends ObjectFenixOJB implements IPersistentEnrolment {
	...
	public void lockWrite(IEnrolment enrolmentToWrite) throws ExcepcaoPersistencia, ExistingPersistentException {
		...
		enrolmentFromDB =
			(IEnrolment) this.readEnrolmentByStudentCurricularPlanAndCurricularCourseScopeAndExecutionPeriod(
		...
		if (enrolmentFromDB == null) {
			...
			(enrolmentToWrite instanceof Enrolment) && ((Enrolment) enrolmentFromDB).getIdInternal().equals(((Enrolment) enrolmentToWrite).getIdInternal())) {
			super.lockWrite(enrolmentToWrite);
	...
	public IEnrolment readEnrolmentByStudentCurricularPlanAndCurricularCourseScope(
		...
		try {
			...
			oqlQuery += " and curricularCourseScope.curricularCourse.degreeCurricularPlan.degree.nome = $7";
			oqlQuery += " and curricularCourseScope.curricularCourse.degreeCurricularPlan.degree.sigla = $8";
			...
			query.bind(curricularCourseScope.getCurricularCourse().getDegreeCurricularPlan().getDegree().getNome());
			query.bind(curricularCourseScope.getCurricularCourse().getDegreeCurricularPlan().getDegree().getSigla());
	...
	public List readEnrolmentsByStudentCurricularPlanAndEnrolmentState(IStudentCurricularPlan studentCurricularPlan, EnrolmentState enrolmentState)
		throws ExcepcaoPersistencia {
...
//		return enrolments;
	...
	public List readAllEnrolmentsByStudentCurricularPlanAndExecutionPeriod(IStudentCurricularPlan studentCurricularPlan, IExecutionPeriod executionPeriod)
		throws ExcepcaoPersistencia {
		try {
			String oqlQuery = "select all from " + Enrolment.class.getName();
			oqlQuery += " where studentCurricularPlan.student.number = $1";
	...
	public List readEnrolmentsByStudentCurricularPlanStateAndEnrolmentStateAndDegreeCurricularPlans(
		...
		criteria.addEqualTo("studentCurricularPlan.degreeCurricularPlanKey", degreeCurricularPlan.getIdInternal());
