...
public class StudentOJB extends ObjectFenixOJB implements IPersistentStudent
{
	...
	public void lockWrite(IStudent studentToWrite) throws ExcepcaoPersistencia, ExistingPersistentException
	{
		...
		if (studentFromDB == null)
		{
			...
			(studentToWrite instanceof Student) && ((Student) studentFromDB).getIdInternal().equals(((Student) studentToWrite).getIdInternal()))
		{
			super.lockWrite(studentToWrite);
	...
	public IStudent readByPersonAndDegreeType(IPessoa person, TipoCurso degreeType) throws ExcepcaoPersistencia
	{
		...
		{
			IStudent student = null;
			String oqlQuery = "select all from " + Student.class.getName() + " where person.idInternal = $1" + " and degreeType = $2";
			query.create(oqlQuery);
	...
	public Integer generateStudentNumber(TipoCurso degreeType) throws ExcepcaoPersistencia
	{
		try
		{
			...
			String oqlQuery = "select all from " + Student.class.getName() + " where degreeType = $1" + " order by number desc";
			query.create(oqlQuery);
