...
public class ServicoSeguroConsultarVerbete extends ServicoSeguro
{
    ...
    public void execute() throws NotExecuteException
    {
        ...
        if (!_mesInjustificado)
        {
            ...
            while (_dataConsulta.before(_dataFimEscolha))
            {
                ...
                //Se o status do funcionário é Pendente então não vale a pena continuar os cálculos
                ...
                //Se os dias do funcionário são injustificados então não vale a pena continuar os cálculos
                ...
                {
                    //Dias de Descanso com marcacoes e sem justificacoes, logo calcula o saldo
                    ...
                    if (_listaMarcacoesPonto.size() > 0)
                    {
                        ...
                        if (((Long) _listaSaldos.get(0)).longValue()
                            > horarioStrategy.duracaoDiaria(_horario))
                        {
                            _listaSaldos.set(0, new Long(horarioStrategy.duracaoDiaria(_horario)));
                        }

                        // para diferenciar entre feriados ao sabado e ao domingo
                        calendario.clear();
                        calendario.setTimeInMillis(_dataConsulta.getTime());

                        if (_horario.getSigla().equals(Constants.DSC)
                            || (_horario.getSigla().equals(Constants.FERIADO)
                                && calendario.get(Calendar.DAY_OF_MONTH) != Calendar.SUNDAY))
                        {
                            // saldo de sabado e feriado ao sabado ou dias de semana
                ...
                { //Dias Uteis
                    ...
                    if (_listaParamJustificacoes.size() > 0)
                    {
                        ...
                        while (iterJustificacoes.hasNext())
                        {
                            ...
                            paramJustificacao =
                                (ParamJustificacao) _listaParamJustificacoes.get(
                                    iterJustificacoes.previousIndex());
                    ...
                    if (_dataConsulta.before(agoraTimestamp))
                    {
                        ...
                        {
                            _listaVerbeteDiaria.set(
                                4,
                                new String("<b>" + Constants.INJUSTIFICADO + "</b>"));
                        }
    ...
    private void lerFimAssiduidade() throws NotExecuteException
    {
        ...
        ServicoSeguroLerFimAssiduidade servicoSeguroLerFimAssiduidade =
            new ServicoSeguroLerFimAssiduidade(
                servicoAutorizacao,
                _numMecanografico.intValue(),
                _dataInicioEscolha,
                _dataFimEscolha);
        ...
        {
            _dataFimEscolha =
                new Timestamp(servicoSeguroLerFimAssiduidade.getDataAssiduidade().getTime());
        }
    } /* lerFimAssiduidade */
    ...
    private void lerInicioAssiduidade() throws NotExecuteException
    {
        ...
        {
            _dataConsulta =
                new Timestamp(servicoSeguroLerInicioAssiduidade.getDataAssiduidade().getTime());
    ...
    private void verificarMesInjustificado() throws NotExecuteException
    {
            ...
            && _dataFimEscolha.before(agoraTimestamp))
        {
            ...
            if ((calendarioInicio.get(Calendar.DAY_OF_MONTH)
                == mes.getActualMinimum(Calendar.DAY_OF_MONTH))
                && (calendarioFim.get(Calendar.DAY_OF_MONTH)
                    == mes.getActualMaximum(Calendar.DAY_OF_MONTH)))
            {
    ...
    /*
     * Verifica se o status do funcionário é PENDENTE
     * Caso seja escreve no verbete até deixar de ser   
     */
            ...
            .contains(Constants.ASSIDUIDADE_PENDENTE))
        {
            ...
            construirListaDiasPendente(
                (StatusAssiduidade) servicoSeguroLerStatusAssiduidadeFuncionario
                    .getListaStatusAssiduidade()
                    .get(
                    0),
                new Timestamp(calendarioFim.getTimeInMillis()));

            calendarioFim.add(Calendar.DAY_OF_MONTH, 1);
            //se o ciclo ultrapassar a data fim de escolha já não nos interessa continuar
        ...
        if (calendario.before(agora))
        {
            ...
            if (_horario.getSigla() == Constants.DSC || _horario.getSigla() == Constants.FERIADO)
            {
                ...
                if (calendarioAnterior.before(agora))
                {
                    ...
                    // a data fim de dias injustificados nao pode ser posterior à data fim de consulta
                    if (dataFimConsulta.after(_dataFimEscolha))
                    {
                        dataFimConsulta = _dataFimEscolha;
                    }

                    // a data fim de dias injustificados nao pode ser posterior à data de hoje
    ...
    public void calcularIntervaloConsulta()
    {
        ...
        calendario.add(Calendar.DAY_OF_MONTH, 1);
        long margemExpediente =
            (calendario.getTimeInMillis() - _horario.getFimExpediente().getTime()) / 2;

        calendario.clear();
        calendario.setTimeInMillis(
            _calendarioConsulta.getTimeInMillis()
                + _horario.getInicioExpediente().getTime()
                - margemExpediente);
        _dataInicio = new Timestamp(calendario.getTimeInMillis());
        calendario.clear();
        calendario.setTimeInMillis(
            _calendarioConsulta.getTimeInMillis()
                + _horario.getFimExpediente().getTime()
                + margemExpediente
                - 60 * 1000);
    ...
    private String formataMarcacoes()
    {
        ...
        if (_listaMarcacoesPonto.size() > 0)
        {
            ...
            Comparador comparadorMarcacoes =
                new Comparador(new String("MarcacaoPonto"), new String("crescente"));
        ...
        { // nao existem marcacoes de ponto neste dia
            marcacoes = marcacoes.concat("&nbsp;");
        }
    ...
    private void consultarJustificacoesPorDia() throws NotExecuteException
    {
        ...
        ServicoSeguroConsultarJustificacoesPorDia servicoSeguro =
            new ServicoSeguroConsultarJustificacoesPorDia(
                servicoAutorizacao,
                _numMecanografico.intValue(),
                _dataInicio);
        servicoSeguro.execute();
        ...
        Comparador comparadorJustificacoes =
            new Comparador(new String("Justificacao"), new String("crescente"));
    ...
    private String formataJustificacoes()
    {
        ...
        if (_listaParamJustificacoes.size() > 0)
        {
            ...
            while (iterJustificacoes.hasNext())
            {
                ...
                paramJustificacao =
                    (ParamJustificacao) _listaParamJustificacoes.get(iterJustificacoes.previousIndex());
        ...
        { // nao existem justificacoes de ponto neste dia
            justificacoes = justificacoes.concat("&nbsp;");
        }
    ...
    private void completaListaMarcacoes()
    {
        ...
        while (iterador.hasNext())
        {
            ...
            ParamJustificacao paramJustificacao =
                (ParamJustificacao) _listaParamJustificacoes.get(iterador.previousIndex());

            IStrategyJustificacoes justificacaoStrategy =
                SuporteStrategyJustificacoes.getInstance().callStrategy(paramJustificacao.getTipo());

            justificacaoStrategy.completaListaMarcacoes(
                _dataConsulta,
                justificacao,
                _listaMarcacoesPonto);
        }
    ...
    private void calcularSaldoDiario()
    {
        ...
        CollectionUtils.transform(_listaMarcacoesPonto, new Transformer()
        {
            public Object transform(Object arg0)
            {
                ...
                Calendar calendario = Calendar.getInstance();
                calendario.setTimeInMillis(
                    marcacao.getData().getTime() - _calendarioConsulta.getTimeInMillis());
    ...
    private void calcularHorasEscalao()
    {
        ...
        if (saldo > 0)
        {
            ...
            _saldoNoctDepoisSegEscalao =
                _saldoNoctDepoisSegEscalao + ((Long) _listaSaldos.get(10)).longValue();
        } else
        {
    ...
    private void calcularTrabalhoNocturno(MarcacaoPonto entrada, MarcacaoPonto saida)
    {
        ...
        _saldoNocturno =
            _saldoNocturno + horarioStrategy.calcularTrabalhoNocturno(_horario, entrada, saida);
