...
public class ReadGratuitySituationListByExecutionDegreeAndSpecialization implements IService
{
            ...
            String gratuitySituationTypeName) throws FenixServiceException
    {
        ...
        if (executionDegreeId == null && executionYearName == null) { throw new FenixServiceException(
        ...
        try
        {
            ...
            {
                executionDegree.setIdInternal(executionDegreeId); //atention it
                // can be null
            }
            ...
            if (executionYearName != null)
            {
                ...
                executionDegree.setExecutionYear(executionYear); //atention it
                // can be null
            }
            ...
            gratuitySituationList = persistentGratuitySituation
                    .readGratuitySituationListByExecutionDegreeAndSpecialization(executionDegree,
                            specialization);
            ...
            if (gratuitySituationList != null && gratuitySituationList.size() > 0)
            {
                ...
                while (listIterator.hasNext())
                {
                    ...
                    infoGratuitySituation.getInfoStudentCurricularPlan().setInfoEnrolments(
                            clonerEnrolments(gratuitySituation.getStudentCurricularPlan()
                                    .getEnrolments()));
     ...
     * @return
     */
    private boolean verifySpecializationOfThisYear(InfoGratuitySituation infoGratuitySituation,
            IExecutionYear executionYear)
    {

        boolean result = true;

        if (infoGratuitySituation.getInfoStudentCurricularPlan().getSpecialization().equals(
                Specialization.ESPECIALIZACAO_TYPE))
        {
            List enrolmentsList = infoGratuitySituation.getInfoStudentCurricularPlan()
                    .getInfoEnrolments();
            InfoExecutionPeriod executionPeriodEnrollment = null;
            if (enrolmentsList != null && enrolmentsList.size() > 0)
            {
                executionPeriodEnrollment = ((InfoEnrolment) enrolmentsList.get(0))
    ...
    private void calculateGratuityPayedValue(ISuportePersistente sp,
            InfoGratuitySituation infoGratuitySituation) throws ExcepcaoPersistencia, Exception
    {

        //all guides of this student that aren't annulled but payed
        IPersistentGuide persistentGuide = sp.getIPersistentGuide();
        List guideList = persistentGuide.readNotAnnulledAndPayedByPersonAndExecutionDegree(
                infoGratuitySituation.getInfoStudentCurricularPlan().getInfoStudent().getInfoPerson()
                        .getIdInternal(), infoGratuitySituation.getInfoGratuityValues()
            ...
            InfoGratuitySituation infoGratuitySituation) throws Exception
    {
        ...
        if (infoGratuitySituation.getInfoStudentCurricularPlan().getSpecialization() != null
                && (infoGratuitySituation.getInfoStudentCurricularPlan().getSpecialization().equals(
                        ...
                        .getInfoStudentCurricularPlan().getSpecialization().equals(
                                Specialization.INTEGRADO_TYPE)))
        {
            if (infoGratuitySituation.getInfoGratuityValues().getAnualValue() != null
                    && infoGratuitySituation.getInfoGratuityValues().getAnualValue().doubleValue() > 0)
            {
                infoGratuitySituation.setRemainingValue(infoGratuitySituation.getInfoGratuityValues()
                        .getAnualValue());
            }
            else if (infoGratuitySituation.getInfoGratuityValues().getScholarShipValue() != null
                    && infoGratuitySituation.getInfoGratuityValues().getScholarShipValue().doubleValue() > 0
                    && infoGratuitySituation.getInfoGratuityValues().getFinalProofValue() != null
                    && infoGratuitySituation.getInfoGratuityValues().getFinalProofValue().doubleValue() > 0)
            {
                valueToPayedForEnrolment(infoGratuitySituation);

                if (infoGratuitySituation.getInfoStudentCurricularPlan().getCurrentState().equals(
                        StudentCurricularPlanState.SCHOOLPARTCONCLUDED_OBJ))
                {
                    double totalToPay = 0;
                    if (infoGratuitySituation.getRemainingValue() != null
                            && verifyIfAlreadyPayedEnrolments(sp, infoGratuitySituation) < infoGratuitySituation
                                    .getRemainingValue().doubleValue())
                    {
                        totalToPay = infoGratuitySituation.getRemainingValue().doubleValue();
                    }

                    totalToPay = totalToPay
                            + infoGratuitySituation.getInfoGratuityValues().getFinalProofValue()
                                    .doubleValue();
        ...
        {
            double exemptionDiscount = infoGratuitySituation.getRemainingValue().doubleValue()
                    * (infoGratuitySituation.getExemptionPercentage().doubleValue() / 100.0);
            double newValue = infoGratuitySituation.getRemainingValue().doubleValue()
                    - exemptionDiscount;
    ...
    private double verifyIfAlreadyPayedEnrolments(ISuportePersistente sp,
            InfoGratuitySituation infoGratuitySituation) throws ExcepcaoPersistencia
    {
        //all guides of this student that are payed
        IPersistentGuide persistentGuide = sp.getIPersistentGuide();
        List guideList = persistentGuide.readNotAnnulledAndPayedByPerson(infoGratuitySituation
                .getInfoStudentCurricularPlan().getInfoStudent().getInfoPerson().getIdInternal());
        List infoGuideList = (List) CollectionUtils.collect(guideList, new Transformer()
        {
    ...
    private void valueToPayedForEnrolment(InfoGratuitySituation infoGratuitySituation)
    {
                ...
                && infoGratuitySituation.getInfoStudentCurricularPlan().getInfoEnrolments().size() > 0)
        {
            ...
            {
                infoGratuitySituation.setRemainingValue(new Double(infoGratuitySituation
                        .getInfoGratuityValues().getCourseValue().doubleValue()
            ...
            {
                double totalToPay = 0;
                Iterator iterCourse = infoGratuitySituation.getInfoStudentCurricularPlan()
                        .getInfoEnrolments().iterator();
                while (iterCourse.hasNext())
                {
                    ...
                    totalToPay += infoEnrolment.getInfoCurricularCourse().getCredits().doubleValue()
                            * infoGratuitySituation.getInfoGratuityValues().getCreditValue()
                                    .doubleValue();
                }
                infoGratuitySituation.setRemainingValue(new Double(totalToPay));
            }
    ...
    { //infoGratuitySituation.getRemainingValue() contains the total value that
        // a student has to
