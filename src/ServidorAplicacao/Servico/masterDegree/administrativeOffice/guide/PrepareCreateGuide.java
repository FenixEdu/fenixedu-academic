...
public class PrepareCreateGuide implements IServico {
	...
	public InfoGuide run(
		...
		if ((contributor == null)
			&& ((contributorAddress == null) || (contributorAddress.length() == 0) || (contributorName.length() == 0) || (contributorName == null))) {
		...
		if (requesterType.equals(GuideRequester.CANDIDATE_STRING)) {
			...
			try {
				...
				masterDegreeCandidate =
					sp.getIPersistentMasterDegreeCandidate().readByNumberAndExecutionDegreeAndSpecialization(
						number,
						executionDegree,
