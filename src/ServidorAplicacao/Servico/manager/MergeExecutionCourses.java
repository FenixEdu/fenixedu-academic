...
public class MergeExecutionCourses implements IService {
    ...
    public void run(Integer executionCourseDestinationId,
            Integer executionCourseSourceId) throws FenixServiceException {
    ...
    private boolean isMergeAllowed(IExecutionCourse destination,
        ...
        try {
            ...
            List distributedTests = persistentDistributedTest
                    .readByTestScopeObject(source);
            distributedTestAuthorization = (metadatas == null || metadatas
                    .isEmpty())
     ...
     * @param ps
     */
    private void copyEvaluationMethod(IExecutionCourse destination,
            IExecutionCourse source, ISuportePersistente ps)
            throws ExcepcaoPersistencia {
        IPersistentEvaluationMethod persistentEvaluationMethod = ps
                .getIPersistentEvaluationMethod();
        IEvaluationMethod evaluationMethod = persistentEvaluationMethod
                .readByExecutionCourse(source);
     ...
     * @param ps
     */
    private void copySummaries(IExecutionCourse destination,
            IExecutionCourse source, ISuportePersistente ps)
            throws ExcepcaoPersistencia {
     ...
     * @param ps
     */
    private void copyCourseReport(IExecutionCourse destination,
            IExecutionCourse source, ISuportePersistente ps)
            throws ExcepcaoPersistencia {
        IPersistentCourseReport persistentCourseReport = ps
                .getIPersistentCourseReport();
        ICourseReport courseReport = persistentCourseReport
                .readCourseReportByExecutionCourse(source);
     ...
     * @param ps
     */
    private void copyGroups(IExecutionCourse destination,
            IExecutionCourse source, ISuportePersistente ps)
            throws ExcepcaoPersistencia {
     ...
     */
    private void copyEvaluations(IExecutionCourse destination,
            IExecutionCourse source, ISuportePersistente ps)
            throws ExcepcaoPersistencia, InvalidArgumentsServiceException {

        IPersistentEvaluationExecutionCourse persistentEvaluationExecutionCourse = ps
                .getIPersistentEvaluationExecutionCourse();
        List destinationEvaluations = persistentEvaluationExecutionCourse
                .readByExecutionCourse(destination);
        if (destinationEvaluations == null) {
            destinationEvaluations = new ArrayList();
        }
        List sourceEvaluations = persistentEvaluationExecutionCourse
                .readByExecutionCourse(source);
        if (sourceEvaluations != null && !sourceEvaluations.isEmpty()) {
            ...
            //                {
            //                    if
            // (evaluationExecutionCourse.getEvaluation().getAssociatedExecutionCourses().size()
            //                        == 1)
            //                    {
     ...
     * @param executionCourse2
     */
    private void copyBibliography(IExecutionCourse destination,
            IExecutionCourse source, ISuportePersistente ps)
            throws ExcepcaoPersistencia {
        ...
        if (sourceBibliographicReferences != null) {
            ...
            while (iter.hasNext()) {
                ...
                if (CollectionUtils.find(destinationBibliographicReferences,
                        new Predicate() {
                    ...
                    persistentBibliographicReference
                            .simpleLockWrite(bibliographicReference);
     ...
     * @param executionCourse2
     */
    private void copyShiftsAndLessons(IExecutionCourse destination,
            IExecutionCourse source, ISuportePersistente ps)
            throws ExcepcaoPersistencia {
...
//        IAulaPersistente persistentLesson = ps.getIAulaPersistente();
     ...
     * @param executionCourse2
     */
    private void copyAttends(IExecutionCourse destination,
            IExecutionCourse source, ISuportePersistente ps)
            throws ExcepcaoPersistencia {
        IFrequentaPersistente persistentAttend = ps.getIFrequentaPersistente();
        List destinationAttends = persistentAttend
                .readByExecutionCourse(destination);
     ...
     * @param executionCourse2
     */
    private void copySites(IExecutionCourse destination,
            IExecutionCourse source, ISuportePersistente ps)
            throws ExcepcaoPersistencia {
    ...
    private void copyProfessorshipsAndResponsibleFors(
            IExecutionCourse destination, IExecutionCourse source,
        ...
        IPersistentSupportLesson persistentSupportLesson = ps
                .getIPersistentSupportLesson();
        List destinationProfessorships = persistentProfessorship
                .readByExecutionCourse(destination);
        if (destinationProfessorships == null) {
            destinationProfessorships = new ArrayList();
        }
        List sourceProfessorships = persistentProfessorship
                .readByExecutionCourse(source);
        if (sourceProfessorships != null) {
            ...
            while (iter.hasNext()) {
                ...
                if (CollectionUtils.find(destinationProfessorships,
                        new Predicate() {
                    ...
                    Iterator iterator = associatedShiftsProfessorships
                            .iterator();

                    while (iterator.hasNext()) {
                        IShiftProfessorship shiftProfessorship = (IShiftProfessorship) iterator
                                .next();
                        persistentShiftProfessorship.delete(shiftProfessorship);
                    }

                    List supportLessons = persistentSupportLesson
                            .readByProfessorship(professorship);
                    iterator = supportLessons.iterator();
                    while (iterator.hasNext()) {
                        ISupportLesson supportLesson = (ISupportLesson) iterator
                                .next();
        ...
        List destinationResponsibleFor = persistentResponsibleFor
                .readByExecutionCourse(destination);
        if (destinationResponsibleFor == null) {
            destinationResponsibleFor = new ArrayList();
        }
        List sourceResponsibleFor = persistentResponsibleFor
                .readByExecutionCourse(source);
        if (sourceResponsibleFor != null) {
            ...
            while (iter.hasNext()) {
                ...
                if (CollectionUtils.find(destinationResponsibleFor,
                        new Predicate() {
