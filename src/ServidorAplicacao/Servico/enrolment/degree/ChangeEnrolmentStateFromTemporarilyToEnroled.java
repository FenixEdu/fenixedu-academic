...
/**
 * @author dcs-rjao 9/Abr/2003
 */
public class ChangeEnrolmentStateFromTemporarilyToEnroled implements IService
{
    ...
    //	private static IStudentCurricularPlanPersistente
    // persistentStudentCurricularPlan = null;
     ...
     */
    public ChangeEnrolmentStateFromTemporarilyToEnroled()
    {
    }
    ...
    public void run(String degreeCurricularPlanName) throws FenixServiceException
    {
        ...
        try
        {
            ...
            IDegreeCurricularPlan degreeCurricularPlan = (IDegreeCurricularPlan) persistentDegreeCurricularPlan
                    .readDomainObjectByCriteria(degreeCurricularPlanCriteria);

            //			IStudent student = studentDAO.readByUsername(((UserView)
            // userView).getUtilizador());
            //			IStudentCurricularPlan studentActiveCurricularPlan =
            // persistentStudentCurricularPlan.readActiveStudentCurricularPlan(student.getNumber(),
            // student.getDegreeType());
            //			List TemporarilyEnrolemts =
            // persistentEnrolment.readEnrolmentsByStudentCurricularPlanAndEnrolmentState(studentActiveCurricularPlan,
            // EnrolmentState.TEMPORARILY_ENROLED);

            //			List listActiveStudentCurricularPlans =
            //				persistentStudentCurricularPlan.readAllByDegreeCurricularPlanAndState(degreeCurricularPlan,
            ...
            //				List TemporarilyEnrolemts =
            //					persistentEnrolment.readEnrolmentsByStudentCurricularPlanAndEnrolmentState(studentCurricularPlan,
            // EnrolmentState.TEMPORARILY_ENROLED);
            //
            //				Iterator temporarilyEnrolemtsIterator =
            // TemporarilyEnrolemts.iterator();
            //				while (temporarilyEnrolemtsIterator.hasNext()) {
            //					IEnrolment enrolment = (IEnrolment)
            // temporarilyEnrolemtsIterator.next();
            ...
            //				}
            //			}
            List TemporarilyEnrolemts = persistentEnrolment
                    .readEnrolmentsByStudentCurricularPlanStateAndEnrolmentStateAndDegreeCurricularPlans(
                            StudentCurricularPlanState.ACTIVE_OBJ, EnrolmentState.TEMPORARILY_ENROLED,
            ...
            ExistingPersistentException
    {
        ...
        try
        {
            ...
            //TODO Este método vai passar a ser invocado da Classe que cria e
            // apaga enrolments (feita pelo David)
            IStudent student = enrolment.getStudentCurricularPlan().getStudent();
            IExecutionCourse executionCourse = (IExecutionCourse) enrolment.getCurricularCourse()
                    .getAssociatedExecutionCourses().get(0);

            IFrequenta frequenta = frequentaPersistente.readByAlunoAndDisciplinaExecucao(student,
                    executionCourse);
    ...
    public static void createEnrolmentEvaluation(IEnrolment enrolment) throws ExcepcaoPersistencia
    {
        ...
        try
        {
            IEnrolmentEvaluation enrolmentEvaluation = persistentEnrolmentEvaluation
                    .readEnrolmentEvaluationByEnrolmentAndEnrolmentEvaluationTypeAndGrade(enrolment,
                            enrolment.getEnrolmentEvaluationType(), null);
            if (enrolmentEvaluation == null)
            {
                ...
                // TODO [DAVID]: Quando o algoritmo do checksum estiver feito
                // tem de se actualizar este campo
