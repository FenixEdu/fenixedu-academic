...
public class ReadExamsWithRooms implements IServico {
	...
	public Object run(Integer studentId) throws FenixServiceException {
		...
		try {
			...
			List examsRoomDistribution =
				(List) persistentExamStudentRoom.readBy(student);
			...
			while (iter.hasNext()) {
				...
				if (currentPeriod != null
					...
					validDistributions.add(
						Cloner.copyIExamStudentRoom2InfoExamStudentRoom(
							examStudentRoom));
				}
			}
			ISiteComponent component =
				new InfoSiteStudentExamDistributions(validDistributions);
