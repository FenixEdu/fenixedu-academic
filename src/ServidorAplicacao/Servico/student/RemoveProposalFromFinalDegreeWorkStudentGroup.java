...
public class RemoveProposalFromFinalDegreeWorkStudentGroup implements IService
{
        ...
        if (groupProposal != null)
        {
            for (int i = 0; i < group.getGroupProposals().size(); i++)
            {
                ...
                if (!groupProposal.equals(otherGroupProposal)
                        && groupProposal.getOrderOfPreference().intValue() < otherGroupProposal.getOrderOfPreference()
                                .intValue())
                {
                    persistentFinalDegreeWork.simpleLockWrite(otherGroupProposal);
                    otherGroupProposal.setOrderOfPreference(new Integer(otherGroupProposal.getOrderOfPreference()
                            .intValue() - 1));
                }
            }
    ...
    private class PREDICATE_FIND_BY_ID implements Predicate
    {
        ...
        public boolean evaluate(Object arg0)
        {
            ...
            System.out.println("" + groupProposalID + " == " + groupProposal.getIdInternal() + "==>" + groupProposalID.equals(groupProposal.getIdInternal()));
