...
public class ReadDelegateCurricularCourses extends SearchService
{
    ...
    protected InfoObject cloneDomainObject(IDomainObject object)
    {
        ...
        List infoScopes = (List) CollectionUtils.collect(curricularCourse.getScopes(), new Transformer()
        {
        ...
        throws ExcepcaoPersistencia
    {
        ...
        // if he's a degree delegate then he can read all curricular courses report
        if (delegate.getType().booleanValue())
        {
            curricularCourses =
                persistentCurricularCourse.readExecutedCurricularCoursesByDegreeAndExecutionYear(
                    delegate.getDegree(),
                    executionYear);
        } else
        {
            ...
            curricularCourses =
                persistentCurricularCourse.readExecutedCurricularCoursesByDegreeAndYearAndExecutionYear(
                    delegate.getDegree(),
                    year,
                    executionYear);
        }
