...
public class ReadStudentShiftEnrolment implements IServico {
     ...
     *                if an error occurs
     */
    public InfoShiftStudentEnrolment run(
            InfoShiftStudentEnrolment infoShiftStudentEnrolment)
            throws FenixServiceException, ExcepcaoPersistencia {
        if (infoShiftStudentEnrolment == null) {
            throw new IllegalArgumentException(
                    "InfoShiftStudentEnrolment must be not null!");
        }

        IStudent student = new Student();
        IPessoa person = new Pessoa();
        person.setUsername(infoShiftStudentEnrolment.getInfoStudent()
        ...
        List courses = (List) ReadDisciplinesByStudent.getService().run(
        ...
        List allowedClasses = (List) CollectionUtils.collect(
                studentAllowedClasses, classTransformer);
        ...
        infoShiftStudentEnrolmentTmp = shiftStudentDAO
                .readByCriteria(shiftStudentExample);
        ...
        List infoShiftEnrolment = (List) CollectionUtils.collect(
                infoShiftStudentEnrolmentTmp, shiftStudentTransformer);
        ...
        List infoShiftEnrolmentTmp = (List) CollectionUtils.collect(
                currentEnrolment, shiftTransformer);
        ...
        CollectionUtils.select(infoShiftEnrolmentTmp, new Predicate() {
            public boolean evaluate(Object arg0) {
                ...
                return (shift.getAvailabilityFinal().intValue() > 0 && (shift
                        .getTipo().getTipo().intValue() == TipoAula.TEORICA
                        || shift.getTipo().getTipo().intValue() == TipoAula.TEORICO_PRATICA || shift
                        .getTipo().getTipo().intValue() == TipoAula.PRATICA));
            }
        }, availableShifts);

        // Remove the shifts in which the student is already enrolled
        List infoAvailableShifts = (List) CollectionUtils.subtract(
                availableShifts, infoShiftStudentEnrolment
                        .getCurrentEnrolment());
        ...
        for (int i = 0; i < infoAvailableShifts.size(); i++) {
            ...
            List classList = getClassesAssociatedWithShift(thisInfoShift,
                    currentEnrolment, sp);

            if (theClassIsAllowed(classList, infoShiftStudentEnrolment
                    .getAllowedClasses())) {
        ...
        infoShiftStudentEnrolment
                .setAvailableShift(infoAvailableShiftsFiltered);

        //adds the student lessons to the structure for timetable display
        infoShiftStudentEnrolment
                .setLessons(getStudentLessons(infoShiftStudentEnrolment));
        infoShiftStudentEnrolment
                .setCurrentEnrolment(setLessons2InfoShifts(infoShiftStudentEnrolment
                        .getCurrentEnrolment()));
     ...
     * @return
     */
    private List getStudentLessons(
            InfoShiftStudentEnrolment infoShiftStudentEnrolment)
     ...
     * @exception ExcepcaoPersistencia
     */
    private List getStudentsAllowedClasses(IStudent student,
            ISuportePersistente sp, List courses) throws ExcepcaoPersistencia {
        ...
        List classShifts = sp.getITurmaTurnoPersistente().readByCriteria(
                classShiftExample);
        ...
        while (iter.hasNext()) {
            executionCourses
                    .add(Cloner
                            .copyInfoExecutionCourse2ExecutionCourse((InfoExecutionCourse) iter
                                    .next()));
        }

        Comparator classesComparator = new ClassesComparatorByNumberOfCourses(
                executionCourses);
        ...
        while (iterator.hasNext()) {
            ...
            while (iterator2.hasNext()) {
                ...
                if ((element.getNome().equals(thisClass.getNome()))
                        && (element.getInfoExecutionDegree().equals(thisClass
     ...
     * @return
     */
    private List getClassesAssociatedWithShift(InfoShift thisInfoShift,
            List currentEnrolment, ISuportePersistente sp) {
        ...
        if (shift != null) {
            ...
            return (List) CollectionUtils.collect(classes,
     ...
     * @author <a href="mailto:edgar.gonçalves@tagus.ist.utl.pt">Edgar Gonçalves
     *         </a>
     * @version 1.0
     */
    ...
    private class ShiftStudentTransformer implements Transformer {
        ...
        public Object transform(Object arg0) {
            ...
            shift.setAvailabilityFinal(shiftStudent.getShift()
                    .getAvailabilityFinal());
            InfoExecutionCourse infoExecutionCourse;
            infoExecutionCourse = (InfoExecutionCourse) Cloner.get(shiftStudent
