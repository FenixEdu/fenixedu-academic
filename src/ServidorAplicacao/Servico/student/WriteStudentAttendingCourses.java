...
public class WriteStudentAttendingCourses implements IService
{
	 ...
	 *            a <code>List</code> with the wanted executionCourse.idInternal's of the ATTEND
	 *            table.
	 ...
	 *                if student is enrolled in any of the executionCourses groups.
...
//				Integer executionCourseId = (Integer) i.next();
			...
			//Delete all courses the student is currently attending to that he/she doesn't want to:
			//attendings to remove :
			List attendsToRemove =
				(List) CollectionUtils.subtract(attendingCourses, wantedAttendingCourses);
			List attendingCoursesToAdd =
				(List) CollectionUtils.subtract(wantedAttendingCourses, attendingCourses);
	...
	private void deleteAttends(
		List attendingCoursesToRemove,
		IStudent student,
