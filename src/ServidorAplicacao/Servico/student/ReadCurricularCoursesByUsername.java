 ...
 *
 * This service reads all the Curricular Courses for this student's curricular plans, and not only the
 * Curricular Courses currently in execution. For this case use ReadDisciplinesByStudent
 * Created at 3/Ago/2003, 21:37:27
 * 
 */
	...
	public List run(String username) throws BDException
	{
		...
		try
		{
			...
			for (Iterator iterator= curricularPlans.iterator(); iterator.hasNext();)
			{
				...
				for (Iterator curricularCoursesIterator=
					studentCurricularPlan.getDegreeCurricularPlan().getCurricularCourses().iterator();
					curricularCoursesIterator.hasNext();
					)
				{
					ICurricularCourse curricularCourse= (ICurricularCourse) curricularCoursesIterator.next();
                    curricularCourses.add(Cloner.copyCurricularCourse2InfoCurricularCourse(curricularCourse));
				}
			}
		}
		catch (ExcepcaoPersistencia ex)
		{
			throw new BDException("Got an error while trying to retrieve a student's curricular courses from the database", ex);
		}
