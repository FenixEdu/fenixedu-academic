...
//                            false);
                ...
                && exam.getAssociatedRoomOccupation().size() != 0)
            {
                ...
                {
                    period = (Period) sp.getIPersistentPeriod().readByCalendarAndNextPeriod(examDate, examDate,null);
                    if (period == null)
                    {
                        period = new Period();
            ...
            try
            {
                ...
                for (int i = 0; i < roomIDArray.length; i++)
                {
                    ...
                    RoomOccupation roomOccupation =
                        new RoomOccupation(room, examStartTime, examEndTime, weekday, RoomOccupation.DIARIA);
