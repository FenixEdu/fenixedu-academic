...
public class EditExamNew implements IServico {
    ...
    public Boolean run(Calendar examDate, Calendar examStartTime,
            Calendar examEndTime, Season season,
            String[] executionCourseIDArray, String[] scopeIDArray,
            String[] roomIDArray, Integer examID) throws FenixServiceException {
        ...
        try {
            ...
            IPersistentRoomOccupation roomOccupationDAO = sp
            ...
            for (int i = 0; i < executionCourseIDArray.length; i++) {
                IExecutionCourse executionCourse = findExecutionCourse(exam
                        .getAssociatedExecutionCourses(), new Integer(
                        executionCourseIDArray[i]));
                if (executionCourse == null) {
                    executionCourse = (IExecutionCourse) persistentExecutionCourse
                            .readByOID(ExecutionCourse.class, new Integer(
                                    executionCourseIDArray[i]));
                    newExecutionCourses.add(executionCourse);
                }

                for (int j = 0; j < executionCourse.getAssociatedExams().size(); j++) {
                    IEvaluation evaluation = (IEvaluation) executionCourse
                            .getAssociatedEvaluations().get(j);
                    if (evaluation instanceof IExam) {
                        ...
                        if (examAux.getSeason().equals(season)) {
                            ...
                            while (iterScopes.hasNext()) {
                                CurricularCourseScope scope = (CurricularCourseScope) iterScopes
                                        .next();
                                if (scopeIDs.contains(scope.getIdInternal()
                                        .toString())
                                        && !examAux.getIdInternal().equals(
                                                exam.getIdInternal())) {
                ...
                if (curricularCourseScope == null) {
                    ...
                    if (curricularCourseScope == null) {
                        throw new FenixServiceException(
                                "The curricular course scope doesnt exist");
                    }
                    exam.getAssociatedCurricularCourseScope().add(
            ...
            // Execution Courses
            for (int i = 0; i < executionCourseIDArray.length; i++) {
                Integer executionCourseID = new Integer(
                        executionCourseIDArray[i]);
                IExecutionCourse executionCourse = findExecutionCourse(exam
                        .getAssociatedExecutionCourses(), executionCourseID);
                if (executionCourse == null) {
                    executionCourse = findExecutionCourse(newExecutionCourses,
                            executionCourseID);
                    //                        (IExecutionCourse)
                    ...
                    if (executionCourse == null) {
                        throw new FenixServiceException(
                                "The execution course doesnt exist");
                    }
            ...
            for (int i = 0; i < exam.getAssociatedExecutionCourses().size(); i++) {
                ...
                if (!executionCourseIDs.contains(executionCourse
                        .getIdInternal().toString())) {
                    exam.getAssociatedExecutionCourses()
                            .remove(executionCourse);
                }
            }
            /** ********************* */
            ...
            if (exam.getAssociatedRoomOccupation() != null
                ...
                period = ((IRoomOccupation) exam.getAssociatedRoomOccupation()
                        .get(0)).getPeriod();
                ...
                if (rooms.size() == exam.getAssociatedRoomOccupation().size()) {
                    ...
                    period = (Period) sp.getIPersistentPeriod().readBy(
                            examDate, examDate);
                    if (period == null) {
                        period = new Period();
                ...
                period = (Period) sp.getIPersistentPeriod().readBy(examDate,
                        examDate);
                if (period == null) {
                    period = new Period();
                    period.setRoomOccupations(new ArrayList());
            ...
            try {
                List roomOccupationInDBList = sp.getIPersistentRoomOccupation()
                        .readAll();
                ...
                for (int i = 0; i < roomIDArray.length; i++) {
                    ISala room = (ISala) sp.getISalaPersistente().readByOID(
                            Sala.class, new Integer(roomIDArray[i]));
                    ...
                    roomsList.add(room);
                    DiaSemana weekday = new DiaSemana(examDate
                            .get(Calendar.DAY_OF_WEEK));

                    RoomOccupation roomOccupation = new RoomOccupation(room,
                            examStartTime, examEndTime, weekday);
                    roomOccupation.setPeriod(period);

                    Iterator iter = roomOccupationInDBList.iterator();
                    while (iter.hasNext()) {
                        IRoomOccupation roomOccupationInDB = (IRoomOccupation) iter
                                .next();

                        if (roomOccupation
                                .roomOccupationForDateAndTime(roomOccupationInDB)) {
                            throw new InterceptingRoomsServiceException(
                                    roomOccupation.getRoom().getNome());
                        }
                    }

                    if (!exam.getAssociatedRoomOccupation().contains(
                            roomOccupation)) {
                        try {
                ...
                Iterator iterator = exam.getAssociatedRoomOccupation()
                        .iterator();
                List roomOcupationToRemove = new ArrayList();
                while (iterator.hasNext()) {
                    RoomOccupation oldRoomOccupation = (RoomOccupation) iterator
                            .next();
                ...
                if (roomOcupationToRemove.size() > 0) {
                    exam.getAssociatedRoomOccupation().removeAll(
                            roomOcupationToRemove);
                }
    ...
    private ICurricularCourseScope findScopes(List scopes, final Integer scopeID) {
        return (ICurricularCourseScope) CollectionUtils.find(scopes,
                new Predicate() {
                    public boolean evaluate(Object arg0) {
                        if (arg0 != null
                                && arg0 instanceof ICurricularCourseScope) {
                            ICurricularCourseScope curricularCourseScope = (ICurricularCourseScope) arg0;
                            if (curricularCourseScope.getIdInternal().equals(
                                    scopeID)) {
                                return true;
                            }
     ...
     * @return
     */
    private IExecutionCourse findExecutionCourse(List executionCourses,
            final Integer executionCourseID) {
        return (IExecutionCourse) CollectionUtils.find(executionCourses,
                new Predicate() {
                    public boolean evaluate(Object arg0) {
                        if (arg0 != null && arg0 instanceof IExecutionCourse) {
                            IExecutionCourse executionCourse = (IExecutionCourse) arg0;
                            if (executionCourse.getIdInternal().equals(
                                    executionCourseID)) {
                                return true;
                            }
