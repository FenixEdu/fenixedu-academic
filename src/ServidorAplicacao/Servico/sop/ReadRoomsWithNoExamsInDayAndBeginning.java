...
public class ReadRoomsWithNoExamsInDayAndBeginning implements IServico {
	...
	private static ReadRoomsWithNoExamsInDayAndBeginning _servico =
		new ReadRoomsWithNoExamsInDayAndBeginning();
	/**
	...
	public List run(Calendar day, Calendar beginning) {
		...
		ArrayList availableInfoRooms = new ArrayList();
		...
		try {
			...
			for(int i = 0; i< exams.size(); i++) {
				List examRooms = (List) ((IExam)exams.get(i)).getAssociatedRooms();
				if(examRooms != null && examRooms.size()>0) {
					for(int r=0; r< examRooms.size();r++) {
						occupiedRooms.add(((ISala)examRooms.get(r)));
					}					
				}
			}
			System.out.println("## All Rooms = " +allRooms.size());
			System.out.println("## Occupied Rooms = " +occupiedRooms.size());
