			...
			for (int k = 0; k < executionDegrees.size(); k++) {
				...
				for (int curricularYear = 1;
					...
					for (int i = 0; i < executionCourses.size(); i++) {
						...
						for (int j = 0;
							j < executionCourse.getAssociatedExams().size();
							j++) {
							IExam exam =
								(IExam) executionCourse
									.getAssociatedExams()
									.get(
									j);
							if (exam.getSeason().getseason().intValue()
								== Season.SEASON1) {
								infoExecutionCourseAndExams.setInfoExam1(
									Cloner.copyIExam2InfoExam(exam));
							} else if (
								exam.getSeason().getseason().intValue()
									== Season.SEASON2) {
						...
						infoExecutionCourseAndExams
							.setNumberStudentesAttendingCourse(
							sp
								.getIFrequentaPersistente()
								.countStudentsAttendingExecutionCourse(
								executionCourse));

						infoExecutionCourseAndExamsList.add(
							infoExecutionCourseAndExams);
					}
