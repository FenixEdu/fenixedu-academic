...
public class ReadAvailableClassesForShift implements IServico {
    ...
    private static ReadAvailableClassesForShift service = new ReadAvailableClassesForShift();
    ...
    public List run(Integer shiftOID) throws FenixServiceException {
        ...
        try {
            ...
            List curricularCourses = shift.getDisciplinaExecucao()
            ...
            List classes = classDAO.readByExecutionPeriod(executionCourse
                    .getExecutionPeriod());
    ...
    private boolean containsScope(List scopes, ITurma classImpl) {
        for (int i = 0; i < scopes.size(); i++) {
            ICurricularCourseScope scope = (ICurricularCourseScope) scopes
                    .get(i);
            if (scope.getCurricularCourse().getDegreeCurricularPlan().equals(
                    classImpl.getExecutionDegree().getCurricularPlan())
                    && scope.getCurricularSemester().getCurricularYear()
                            .getYear().equals(classImpl.getAnoCurricular()))
                return true;
        }
