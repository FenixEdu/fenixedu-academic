...
public class ReadMetadatasByTest implements IService {
	...
	public SiteView run(Integer executionCourseId, Integer testId,
			String order, String asc) throws FenixServiceException {
		...
		try {
			...
			ITest test = (ITest) persistentSuport.getIPersistentTest()
					.readByOID(Test.class, testId);
			...
			List metadatasList = persistentSuport.getIPersistentMetadata()
					.readByExecutionCourseAndNotTest(executionCourse, test,
							order, asc);
