	...
	public SiteView run(Integer executionCourseId, Integer distributedTestId,
		...
		try {
			...
			while (it.hasNext()) {
				...
				int numOfStudentResponses = persistentStudentTestQuestion
				...
				if (infoStudentTestQuestion.getQuestion().getQuestionType()
					...
					for (int i = 1; i <= infoStudentTestQuestion.getQuestion()
							.getOptionNumber().intValue(); i++) {

						String response = new String("%<string>" + i
								+ "</string>%");

						int mark = persistentStudentTestQuestion
								.countResponses(studentTestQuestion
										.getTestQuestionOrder(), response,
										distributedTest);
						String percentage = new String("0%");
						if (mark != 0)
							percentage = df.format(mark
									* java.lang.Math.pow(numOfStudentResponses,
											-1));
						statistics.add(new LabelValueBean(new Integer(i)
								.toString(), percentage));
					}
				} else {
					List responses = persistentStudentTestQuestion
							.getResponses(studentTestQuestion
					...
					while (responseIt.hasNext()) {
						...
						if (r instanceof ResponseSTR)
							statistics.add(new LabelValueBean(percentage,
									((ResponseSTR) r).getResponse()));
						else
							statistics.add(new LabelValueBean(percentage,
									((ResponseNUM) r).getResponse()));

					}
				}
				statistics.add(new LabelValueBean(new String(
						"Número de alunos que responderam"), new Integer(
						numOfStudentResponses).toString()));
				infoInquiryStatistics
						.setInfoStudentTestQuestion(infoStudentTestQuestion);
		...
		infoSiteInquiryStatistics
				.setInfoInquiryStatistics(infoInquiryStatisticsList);
		SiteView siteView = new ExecutionCourseSiteView(
				infoSiteInquiryStatistics, infoSiteInquiryStatistics);
		return siteView;
	}
