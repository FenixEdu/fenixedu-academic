...
public class ChangeStudentTestQuestion implements IService {
	...
	public List run(Integer executionCourseId, Integer distributedTestId,
		...
		try {
			...
			IQuestion oldQuestion = (IQuestion) persistentQuestion.readByOID(
					Question.class, oldQuestionId, true);
			if (oldQuestion == null)
				throw new InvalidArgumentsServiceException();
			...
			if (newMetadataId != null) {
				metadata = (IMetadata) persistentMetadata.readByOID(
						Metadata.class, newMetadataId);
				if (metadata == null)
					throw new InvalidArgumentsServiceException();
			...
			if (delete.booleanValue()) {
				metadata = (IMetadata) persistentMetadata.readByOID(
						Metadata.class, oldQuestion.getKeyMetadata(), true);
				if (metadata == null)
					throw new InvalidArgumentsServiceException();
