 ...
 */
public class ReadAuthorPublicationsToInsert implements IServico {
	...
	public String getNome() {
		...
		return "ReadAuthorPublicationsToInsert";
	}
	/**
	...
	private List getInfoPublications(ISuportePersistente sp, ITeacher teacher)
		...
		List infoTeacherPublications = new ArrayList();
		...
		if (authorPublications != null || authorPublications.size() != PublicationConstants.ZERO_VALUE) {
			if ((publicationsInTeacherSList != null)
				|| (publicationsInTeacherSList.size() != 0)) {

				newAuthorPublications =
					deletePublicationsTeacherInAuthorPublications(
						authorPublications,
						publicationsInTeacherSList);
				infoTeacherPublications =
					(
					...
					public Object transform(Object object) {
						IPublication publicationTeacher = (IPublication) object;
						IPublication publicationTeacher2 = publicationTeacher;
						publicationTeacher2.setPublicationString(
							publicationTeacher.toString());
			...
			infoAuthorPublications =
				(
					List) CollectionUtils
						.collect(newAuthorPublications, new Transformer() {
	...
	public List deletePublicationsTeacherInAuthorPublications(
		List publicationsAuthor,
		...
		while (iteratorAuthors.hasNext()) {
			IPublication publicationAuthor =
				(IPublication) iteratorAuthors.next();

			contains = Boolean.FALSE;
			iteratorTeachers = publicationsTeacher.iterator();
			while (iteratorTeachers.hasNext()) {
				IPublication publicationTeacher =
					(IPublication) iteratorTeachers.next();

				if (publicationTeacher.getIdInternal().intValue()
					== publicationAuthor.getIdInternal().intValue()) {
