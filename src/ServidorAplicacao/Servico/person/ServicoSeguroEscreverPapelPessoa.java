...
public class ServicoSeguroEscreverPapelPessoa extends ServicoSeguro {
	...
	public void execute() throws NotExecuteException {
		...
		if (!iPessoaPersistente.escreverPapelPessoa(pessoa, 1)) { // buscar estas chaves da base de dados
		...
		if (iFuncionarioPersistente.lerFuncionarioPorPessoa(pessoa.getCodigoInterno().intValue()) != null) {
		...
		if ((listaCargos = iPessoaPersistente.lerCargos(pessoa.getCodigoInterno().intValue())) != null) {
			if (listaCargos.contains(Constants.GESTAO_ASSIDUIDADE)) {
				if (!iPessoaPersistente.escreverPapelPessoa(pessoa, 10)) { // buscar estas chaves da base de dados
					throw new NotExecuteException();
				}
