	...
	public InfoEquivalenceContext run(InfoStudent infoStudent, IUserView responsible, InfoExecutionPeriod infoExecutionPeriod) throws FenixServiceException {
		...
		try {
			...
			while(iterator.hasNext()) {
				...
				List curricularCoursesFromCurrentDegreeCurricularPlanForStudent = degreeCurricularPlan.getCurricularCourses();
				...
				if(studentCurricularPlan.getCurrentState().equals(StudentCurricularPlanState.ACTIVE_OBJ)) {
					...
					while(iterator1.hasNext()) {
						...
						while(iterator2.hasNext()) {
							...
							if	(
									(curricularCourseScope.getBranch().equals(studentCurricularPlan.getBranch()) ||
					...
					studentAprovedEnrolmentsWithDiferentDegreeCurricularPlanAndWithNoEquivalences.addAll(GetListsOfCurricularCoursesForEquivalence.getEnrolmentsWithNoEquivalences(studentAprovedEnrolmentsWithDiferentDegreeCurricularPlan, persistentSupport));
				}
			}
