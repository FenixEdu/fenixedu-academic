...
public class DeleteChosenEnrollmentEquivalences extends EnrollmentEquivalenceServiceUtils implements IService
{
	public DeleteChosenEnrollmentEquivalences()
	{
	}

	public InfoEquivalenceContext run(Integer studentNumber, TipoCurso degreeType, List idsOfChosenEnrollments)
							...
							&& (!equivalentEnrollmentsForEnrollmentEquivalence.isEmpty()))
						{
							for (int j = 0; j < equivalentEnrollmentsForEnrollmentEquivalence.size(); j++)
							{
								...
								equivalentEnrollmentForEnrollmentEquivalenceDAO.deleteByOID(
									EquivalentEnrolmentForEnrolmentEquivalence.class, equivalentEnrolmentForEnrolmentEquivalence
										.getIdInternal());
							}
						}
						
						enrollmentEquivalenceDAO.deleteByOID(EnrolmentEquivalence.class, enrollmentEquivalence.getIdInternal());
					}
					
					for (int k = 0; k < enrollment.getEvaluations().size(); k++)
					{
						IEnrolmentEvaluation enrollmentEvaluation = (IEnrolmentEvaluation) enrollment.getEvaluations().get(k);
						enrolmentEvaluationDAO.deleteByOID(EnrolmentEvaluation.class, enrollmentEvaluation.getIdInternal());
					}
