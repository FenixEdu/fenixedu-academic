...
public class ReadDepartmentTeachersCreditsDetailsService implements IService {
    ...
    public List run(HashMap searchParameters) throws FenixServiceException {
        ...
        try {
            ...
            final IExecutionPeriod executionPeriod = readExecutionPeriod(
                    searchParameters, executionPeriodDAO);

            List teachersIds = TransformationUtils.transformToIds(teachers);

            final List creditsList = creditsDAO
                    .readByTeachersAndExecutionPeriod(teachersIds,
                            executionPeriod);

            List teachersCreditsDetailsList = (List) CollectionUtils.collect(
                    teachers, new Transformer() {
                        public Object transform(Object input) {
                            ITeacher teacher = (ITeacher) input;
                            InfoTeacher infoTeacher = Cloner
                                    .copyITeacher2InfoTeacher(teacher);
                            TeacherCreditsDetailsDTO teacherCreditsDetailsDTO = new TeacherCreditsDetailsDTO(
                                    infoTeacher);
                            ICredits credits = getCredits(teacher);
                            if (credits != null) {
                                InfoCredits infoCredits = Cloner
                                        .copyICredits2InfoCredits(credits);
                                teacherCreditsDetailsDTO
                                        .setInfoCredits(infoCredits);
                            }
                        ...
                        private ICredits getCredits(ITeacher teacher) {
                            ...
                            return (ICredits) (indeOf == -1 ? null
                                    : creditsList.get(indeOf));
                        }
    ...
    private IExecutionPeriod readExecutionPeriod(HashMap searchParameters,
        ...
        try {
            executionPeriodId = Integer.valueOf((String) searchParameters
    ...
    protected List doSearch(HashMap searchParameters, ISuportePersistente sp)
            throws ExcepcaoPersistencia {
        Integer departmentId = Integer.valueOf((String) searchParameters
                .get("idInternal"));
        IPersistentDepartment departmentDAO = sp.getIDepartamentoPersistente();
        IDepartment department = (IDepartment) departmentDAO.readByOID(
