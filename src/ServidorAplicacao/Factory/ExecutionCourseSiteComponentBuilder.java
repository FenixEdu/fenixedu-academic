...
public class ExecutionCourseSiteComponentBuilder
{
	...
	public ISiteComponent getComponent(ISiteComponent component, ISite site, ISiteComponent commonComponent,
		Integer sectionIndex, Integer curricularCourseId) throws FenixServiceException
	{
		...
		} else if (component instanceof InfoSiteCurricularCoursesAndAssociatedShiftsAndClasses)
		{
			return getInfoSiteCurricularCoursesAndAssociatedShiftsAndClasses(
				(InfoSiteCurricularCoursesAndAssociatedShiftsAndClasses) component, site);
	...
	private ISiteComponent getInfoSiteSummaries(InfoSiteSummaries component, ISite site) throws FenixServiceException
	{
		try
		{
			...
			{
				List summariesBySummaryType = persistentSummary.readByExecutionCourseShiftsAndTypeLesson(executionCourse,
					new TipoAula(component.getLessonType()));

				//read summary also by execution course key
				//and add to the last list
				List summariesByExecutionCourseBySummaryType = persistentSummary.readByExecutionCourseAndType(executionCourse,
					new TipoAula(component.getLessonType()));
			...
			if (component.getShiftId() != null && component.getShiftId().intValue() > 0)
			{
				...
				shiftSelected = (ITurno) persistentShift.readByOId(shiftSelected, false);
				...
				List summariesByExecutionCourseByShift = findLesson(persistentSummary, executionCourse, shiftSelected);

				if (summaries != null)
				{
					summaries = (List) CollectionUtils.intersection(summaries, allSummaries(summariesByShift,
			...
			if (component.getTeacherId() != null && component.getTeacherId().intValue() > 0)
			{
				...
				professorshipSelected = (IProfessorship) persistentProfessorship.readByOId(professorshipSelected, false);
				...
				List summariesByProfessorship = persistentSummary.readByTeacher(executionCourse, professorshipSelected
					.getTeacher());
			...
			component.setExecutionCourse((InfoExecutionCourse) Cloner.get(executionCourse));
	...
	private List findLessonTypesExecutionCourse(IExecutionCourse executionCourse)
	{
		...
		if (executionCourse.getTheoreticalHours() != null && executionCourse.getTheoreticalHours().intValue() > 0)
		{
			lessonTypes.add(new TipoAula(1));
		}
		if (executionCourse.getTheoPratHours() != null && executionCourse.getTheoPratHours().intValue() > 0)
		{
			lessonTypes.add(new TipoAula(3));
		}
		if (executionCourse.getPraticalHours() != null && executionCourse.getPraticalHours().intValue() > 0)
		{
	...
	private List findLesson(IPersistentSummary persistentSummary, IExecutionCourse executionCourse, ITurno shift)
		throws ExcepcaoPersistencia
	{
		...
		if (summariesByExecutionCourse != null && summariesByExecutionCourse.size() > 0)
		{
			...
			while (iterator.hasNext())
			{
				...
				dateAndHourSummary.set(Calendar.DAY_OF_MONTH, summary.getSummaryDate().get(Calendar.DAY_OF_MONTH));
				...
				dateAndHourSummary.set(Calendar.HOUR_OF_DAY, summary.getSummaryHour().get(Calendar.HOUR_OF_DAY));
	 ...
	 * @return
	 */
	private ISiteComponent getInfoSiteSection(InfoSiteSection component, ISite site, InfoSiteCommon commonComponent,
		Integer sectionIndex) throws FenixServiceException
	{
	...
	private InfoSiteCurricularCoursesAndAssociatedShiftsAndClasses getInfoSiteCurricularCoursesAndAssociatedShiftsAndClasses(
		InfoSiteCurricularCoursesAndAssociatedShiftsAndClasses component, ISite site) throws FenixServiceException
	{
		...
		try
		{
			...
			} else
			{
				...
				{
					ITurno shift = (ITurno) shifts.get(i);
					shiftWithAssociatedClassesAndLessons = new InfoShiftWithAssociatedInfoClassesAndInfoLessons(Cloner
						.copyShift2InfoShift(shift), null, null);
