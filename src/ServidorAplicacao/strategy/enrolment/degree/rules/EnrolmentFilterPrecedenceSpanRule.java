...
/**
 * @see ServidorAplicacao.strategy.enrolment.degree.rules.PrecedenceRule
 * @author jpvl
 */
public class EnrolmentFilterPrecedenceSpanRule extends PrecedenceRule implements IEnrolmentRule {
	...
	protected void doApply(EnrolmentContext enrolmentContext, List precedenceList, List curricularCourseToApply) {
			...
			if (!precedence.evaluate(enrolmentContext)) {
				...
				for (int scopeIndex = 0;
					...
					ICurricularCourseScope curricularCourseScope =
						(ICurricularCourseScope) scopes.get(scopeIndex);
					if (!curricularCourseScopesNotToStay
						.contains(curricularCourseScope)) {
						curricularCourseScopesNotToStay.add(
		...
		enrolmentContext
			.getCurricularCoursesScopesAutomaticalyEnroled()
			.removeAll(
