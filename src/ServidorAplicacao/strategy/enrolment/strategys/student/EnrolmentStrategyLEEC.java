...
public class EnrolmentStrategyLEEC extends EnrolmentStrategy implements
        IEnrolmentStrategy {
    ...
    public StudentEnrolmentContext getAvailableCurricularCourses()
     ...
     * @throws ExcepcaoPersistencia
     */
    private void computeAvailableCurricularCourses()
            throws ExcepcaoPersistencia {
        if (this.studentHasSpecializationArea && this.studentHasSecundaryArea) {
            List specializationAndSecundaryAreaCurricularCourses = leecAlgorithm(
                    studentCurricularPlan, this.getStudentEnrolmentContext()
                            .getStudentApprovedEnrollments(), this
                            .getStudentEnrolmentContext()
                            .getStudentCurrentSemesterEnrollments(), this
                            .getStudentEnrolmentContext().getExecutionPeriod());

            List finalCurricularCoursesWhereStudentCanBeEnrolled = studentEnrolmentContext
                    .getFinalCurricularCoursesWhereStudentCanBeEnrolled();

            finalCurricularCoursesWhereStudentCanBeEnrolled
                    .addAll(specializationAndSecundaryAreaCurricularCourses);

            finalCurricularCoursesWhereStudentCanBeEnrolled
                    .addAll(getOptionalAndTFCCurricularCourses(studentEnrolmentContext));

            studentEnrolmentContext
                    .setFinalCurricularCoursesWhereStudentCanBeEnrolled(finalCurricularCoursesWhereStudentCanBeEnrolled);
        }

        IEnrolmentRule enrolmentRule = new EnrolmentApplyPrecedencesRule();
        this.setStudentEnrolmentContext(enrolmentRule
                .apply(getStudentEnrolmentContext()));
    ...
    private void prepareCurricularCoursesForEnrollment()
        ...
        List baseAreasCurricularCourses = getBaseAreasCurricularCourses(
                enrollmentsWithAprovedState, enrollmentsWithEnrolledState,
                executionPeriod);
        ...
        this.studentEnrolmentContext
                .setStudentCurricularPlan(this.studentCurricularPlan);
        this.studentEnrolmentContext.setExecutionPeriod(executionPeriod);
        this.studentEnrolmentContext
                .setStudentApprovedEnrollments(enrollmentsWithAprovedState);
        this.studentEnrolmentContext
                .setStudentCurrentSemesterEnrollments(enrollmentsWithEnrolledState);
        this.studentEnrolmentContext
                .setFinalCurricularCoursesWhereStudentCanBeEnrolled(baseAreasCurricularCourses);

        List acumulatedCurricularCourses = getStudentEverEnrolledCurricularCourses();
        this.studentEnrolmentContext
                .setAcumulatedEnrolments(acumulatedCurricularCourses);
    }

    /**
     * @return studentEverEnrolledCurricularCourses
     */
    private List getStudentEverEnrolledCurricularCourses()
            throws ExcepcaoPersistencia {
        ISuportePersistente persistentSuport = SuportePersistenteOJB
                .getInstance();
        IPersistentEnrolment enrolmentDAO = persistentSuport
        ...
        List enrollments = enrolmentDAO
                .readAllByStudentCurricularPlan(studentEnrolmentContext
        ...
        if (enrollments != null && !enrollments.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                studentEverEnrolledCurricularCourses.add(enrolment
                        .getCurricularCourse());
            }
        }
    ...
    private List getBaseAreasCurricularCourses(
            ...
            List enrollmentsWithEnrolledState, IExecutionPeriod executionPeriod)
            throws ExcepcaoPersistencia {
        ISuportePersistente persistentSuport = SuportePersistenteOJB
                .getInstance();
        ...
        List baseBranches = branchDAO
                .readAllByDegreeCurricularPlanAndBranchType(
        ...
        while (iterator.hasNext()) {
            ...
            List groups = curricularCourseGroupDAO.readByBranchAndAreaType(
                    baseArea, AreaType.BASE_OBJ);
            Iterator iterator2 = groups.iterator();
            while (iterator2.hasNext()) {
                ICurricularCourseGroup curricularCourseGroup = (ICurricularCourseGroup) iterator2
                        .next();
                baseAreasCurricularCourses.addAll(curricularCourseGroup
                        .getCurricularCourses());
            }
        }

        selectDesiredCurricularCourses(enrollmentsWithAprovedState,
                baseAreasCurricularCourses);
        selectDesiredCurricularCourses(enrollmentsWithEnrolledState,
                baseAreasCurricularCourses);
        selectDesiredCurricularCourses(baseAreasCurricularCourses,
     ...
     * @throws ExcepcaoPersistencia
     */
    private List leecAlgorithm(IStudentCurricularPlan studentCurricularPlan,
        ...
        List specializationAreaCurricularCourses = selectAllCurricularCoursesFromGivenArea(
                studentCurricularPlan.getBranch(), AreaType.SPECIALIZATION_OBJ);
        List secundaryAreaCurricularCourses = selectAllCurricularCoursesFromGivenArea(
        ...
        List studentApprovedEnrollmentsFromSpecializationAndSecundaryAreas = selectEnrollmentsInSpecializationAndSecundaryAreasCurricularCourses(
                studentApprovedEnrollments, allCurricularCourses);

        List studentCurrentSemesterEnrollmentsFromSpecializationAndSecundaryAreas = selectEnrollmentsInSpecializationAndSecundaryAreasCurricularCourses(
                studentCurrentSemesterEnrollments, allCurricularCourses);
        ...
        Integer creditsInSecundaryArea = calculateCreditsInSecundaryArea(
                studentCurricularPlan, creditsInSecundaryAreaGroups,
                creditsInAnySecundaryArea);
        Integer creditsInSpecializationArea = calculateCreditsInSpecializationArea(
                studentCurricularPlan, creditsInSpecializationAreaGroups);

        this.studentEnrolmentContext
                .setCreditsInSecundaryArea(creditsInSecundaryArea);
        this.studentEnrolmentContext
                .setCreditsInSpecializationArea(creditsInSpecializationArea);
    ...
    private List selectCurricularCourses(
            ...
            int creditsInAnySecundaryArea, List enrollmentsWithAprovedState,
            List enrollmentsWithEnrolledState, Integer semester)
            throws ExcepcaoPersistencia {
    ...
    private List selectSecundaryAreaCurricularCoursesFromGroupsNotFull(
        ...
        while (iterator.hasNext()) {
            ICurricularCourseGroup group = (ICurricularCourseGroup) iterator
                    .next();
     ...
     */
    private void selectDesiredCurricularCourses(
            List enrollmentsInCurricularCourseToRemove,
            List curricularCoursesToRemoveFrom) {
        List curricularCoursesToRemove = new ArrayList();
        Iterator iterator = enrollmentsInCurricularCourseToRemove.iterator();
        while (iterator.hasNext()) {
            IEnrollment enrolment = (IEnrollment) iterator.next();
            if (curricularCoursesToRemoveFrom.contains(enrolment
                    .getCurricularCourse())) {
     ...
     */
    private void selectDesiredCurricularCourses(
            List curricularCoursesToRemoveFrom, Integer semester) {
        List curricularCoursesToRemove = new ArrayList();
        ...
        while (iterator.hasNext()) {
            ...
            while (iteratorScopes.hasNext()) {
                ICurricularCourseScope curricularCourseScope = (ICurricularCourseScope) iteratorScopes
                        .next();
                if (curricularCourseScope.getCurricularSemester().getSemester()
                        .equals(semester)
    ...
    private void calculateGroupsCreditsFromEnrollments(
        ...
        while (iterator.hasNext()) {
            IEnrollment enrolment = (IEnrollment) iterator.next();
            ICurricularCourse curricularCourse = enrolment
                    .getCurricularCourse();
    ...
    private void calculateClashingScientificAreas(
        ...
        if (!clashingGroups.entrySet().isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                //					(Integer)
                // creditsInSpecializationAreaGroups.get(specializationGroup.getIdInternal());
                //				Integer creditsInSecundaryGroup = (Integer)
                // creditsInSecundaryAreaGroups.get(secundaryGroup.getIdInternal());
                //				Integer creditsInScientificArea = (Integer)
                // creditsInScientificAreas.get(scientificAreaID);
                //				
                ...
                //					specializationGroup.getMinimumCredits().intValue() +
                // secundaryGroup.getMinimumCredits().intValue();
                //
                ...
                //				} else // if (sumOfGroupsCredits < sumOfMaximumGroupsCredits)
                // AND if (sumOfGroupsCredits >=
    ...
    private void distributeCreditsInScientificAreaByTheTwoGroups(
        ...
        for (int i = 0; i < x && alreadyGivenCredits < creditsInScientificArea; i++) {
            ...
            if (creditsInSpecializationGroup < maximumLevelToConsiderForSpecializationGroup
                    .intValue()
            ...
            if (creditsInSecundaryGroup < maximumLevelToConsiderForSecundaryGroup
                    .intValue()
                    && alreadyGivenCredits < creditsInScientificArea) {
                if (dux >= maximumLevelToConsiderForSpecializationGroup
                        .intValue()) {
        ...
        //		if (creditsInSpecializationGroup >
        // maximumLevelToConsiderForSpecializationGroup.intValue())
        //		{
        //			creditsInSpecializationGroup =
        // maximumLevelToConsiderForSpecializationGroup.intValue();
        //		}
        //
        //		if (creditsInSecundaryGroup >
        // maximumLevelToConsiderForSecundaryGroup.intValue())
        //		{
    ...
    private int getGivenCreditsInAnySecundaryArea(
        ...
        if (givenCreditsInAnySecundaryAreas != null
            ...
            while (iterator.hasNext()) {
                CreditsInAnySecundaryArea creditsInAnySecundaryArea = (CreditsInAnySecundaryArea) iterator
                        .next();
    ...
    private void getGivenCreditsInScientificArea(
        ...
        if (givenCreditsInScientificAreas != null
            ...
            while (iterator.hasNext()) {
                CreditsInScientificArea creditsInScientificArea = (CreditsInScientificArea) iterator
                        .next();
    ...
    private List selectEnrollmentsInSpecializationAndSecundaryAreasCurricularCourses(
            List enrollments, List curricularCourses) {
     ...
     * @throws ExcepcaoPersistencia
     */
    private List selectAllCurricularCoursesFromGivenArea(IBranch area,
            AreaType areaType) throws ExcepcaoPersistencia {
        ISuportePersistente persistentSuport = SuportePersistenteOJB
                .getInstance();
        IPersistentCurricularCourseGroup curricularCourseGroupDAO = persistentSuport
                .getIPersistentCurricularCourseGroup();
        List groups = curricularCourseGroupDAO.readByBranchAndAreaType(area,
                areaType);
     ...
     * @throws ExcepcaoPersistencia
     */
    private List getOptionalAndTFCCurricularCourses(
        ...
        selectDesiredCurricularCourses(studentEnrolmentContext
                .getStudentApprovedEnrollments(),
                optionalAndTFCCurricularCourses);
        selectDesiredCurricularCourses(studentEnrolmentContext
                .getStudentCurrentSemesterEnrollments(),
                optionalAndTFCCurricularCourses);
        selectDesiredCurricularCourses(optionalAndTFCCurricularCourses,
                studentEnrolmentContext.getExecutionPeriod().getSemester());
     ...
     * @throws ExcepcaoPersistencia
     */
    private void automaticalyEnrollInMandatoryCurricularCourses()
            throws ExcepcaoPersistencia {
        HashMap enrollmentsCreated = new HashMap();

        // FIXME [DAVID]: There is a bug here!
        // Try this: Enroll in all mandatory curricular courses and some others
        // more untill you get a total of maxCourses
        // enrolled. Now try to unenroll a mandatory curricular course.
        // It was allowed because totalSize >= maxCourses so it will not
        // automaticaly enroll in mandatory curricular courses
        // that were unenrolled before.
        List curricularCourses = getMandatoryCurricularCourses(this.studentEnrolmentContext
                .getFinalCurricularCoursesWhereStudentCanBeEnrolled());

        int maxCourses = this.studentEnrolmentContext
                .getStudentCurricularPlan().getStudent().getStudentKind()
                .getMaxCoursesToEnrol().intValue();

        int currentSemesterEnrollmentsSize = this.studentEnrolmentContext
                .getStudentCurrentSemesterEnrollments().size();

        int totalSize = curricularCourses.size()
                + currentSemesterEnrollmentsSize;

        if (curricularCourses != null && !curricularCourses.isEmpty()
                && totalSize <= maxCourses) {
            IStudentCurricularPlan studentCurricularPlan = this.studentEnrolmentContext
                    .getStudentCurricularPlan();
            IExecutionPeriod executionPeriod = this.studentEnrolmentContext
                    .getExecutionPeriod();
            ...
            while (iterator.hasNext()) {
                ICurricularCourse curricularCourse = (ICurricularCourse) iterator
                        .next();
                IEnrollment enrolment = writeEnrollment(studentCurricularPlan,
                        executionPeriod, curricularCourse, enrollmentsCreated);
                if (enrolment != null) {
                    enrollmentsToAdd.add(enrolment);
                    enrollmentsCreated.put(curricularCourse.getIdInternal(),
                            enrolment);
                }
            }
            ...
            this.studentEnrolmentContext.getStudentCurrentSemesterEnrollments()
                    .addAll(enrollmentsToAdd);
            this.studentEnrolmentContext
                    .getFinalCurricularCoursesWhereStudentCanBeEnrolled()
                    .removeAll(curricularCourses);
            this.studentEnrolmentContext
                    .setAcumulatedEnrolments(studentEverEnrolledCurricularCourses);
            computeAvailableCurricularCourses();
        }
    }
    ...
    private List getMandatoryCurricularCourses(List curricularCourses) {
        ...
        if (curricularCourses != null && !curricularCourses.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                if (curricularCourse.getMandatoryEnrollment() != null
                        && curricularCourse.getMandatoryEnrollment()
                                .booleanValue()) {
    ...
    private IEnrollment writeEnrollment(
            ...
            ICurricularCourse curricularCourse, HashMap enrollmentsCreated)
            throws ExcepcaoPersistencia {
        ISuportePersistente persistentSuport = SuportePersistenteOJB
                .getInstance();
        IPersistentEnrolment enrolmentDAO = persistentSuport
        ...
        if (enrolment == null) {
            enrolment = (IEnrollment) enrollmentsCreated.get(curricularCourse
                    .getIdInternal());
            if (enrolment == null) {
