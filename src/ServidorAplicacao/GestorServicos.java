 ...
 * a service. It receives the service to execute, its arguments
 * and an identificator of the entity that wants to run the service.
 *
 * The static method init must be executed before accessing the
...
public class GestorServicos {
  ...
  public class AssociacaoServico {
    ...
    public final boolean equals(Object another) {
      return (another instanceof AssociacaoServico) &&
              servico.getNome().equals(
                  ((AssociacaoServico) another).servico.getNome());
    }
   ...
   *
   * @throws InvalidServiceException if the service specification is
   * not valid
   **/
  private GestorServicos() {
    ...
    try {
      ...
      throw new InvalidServiceException(
        "Service specification not found: " + ex);
    } catch (IOException ex) {
      throw new InvalidServiceException(
        "Error reading service specification file: " + ex);
    }
    ...
    while (enum.hasMoreElements()) {
      ...
      try {
        invocationType = type != null ? Integer.parseInt(type)
                                      : InvocadorServicos.NONE;
      } catch (NumberFormatException e) {
        throw new InvalidServiceException(
            "Bad format of property " + serviceName + "." + type + " (" 
            + type + ") specified in the service configuration file: " + e);
      }
   ...
   *
   * @throws InvalidServiceException if the service is not a valid
   * service, namely, the service class does not exist, the service
   * class does not specify the public static getSingleton method, or
   * the execution of this method throws an exception
   **/
  private IServico createService(String serviceName)
    ...
    try {
      ...
      throw new InvalidServiceException(
        "The class corresponding to service "
          + serviceName
          + " does not exist: "
          + e);
   ...
   **/
  private void registerService(IServico servico, GestorFiltros gestor,
			       InvocadorServicos invocador) {
  ...
  /**
   * Returns the service and corresponding filter manager given the
   * name of the service.
   **/
