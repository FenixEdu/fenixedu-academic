...
public class XMLQuestion extends FenixUtil {
    ...
    public String getXmlQuestion(String questionText,
            String secondQuestionText, QuestionType questionType,
            String[] options, String[] shuffle,
            List responseProcessingInstructionsList,
        ...
        String result = start
                + getPresentation(questionText, secondQuestionText,
                        breakLineBeforeResponseBox, breakLineAfterResponseBox,
                        questionType, options, shuffle);
        result = result.concat(getResponseProcessingInstructions(
                responseProcessingInstructionsList, correctFeedbackText,
                wrongFeedbackText, questionType));
        return result + end;
    }

    private String getPresentation(String questionText,
            String secondQuestionText, Boolean breakLineBeforeResponseBox,
            Boolean breakLineAfterResponseBox, QuestionType questionType,
            String[] options, String[] shuffle) {
        String result = new String(getStartElementTag(presentation)
                + getStartElementTag(flow));
        ...
        result = result
                .concat(getResponseLabels(questionType, options, shuffle));
        ...
        if (breakLineAfterResponseBox.booleanValue()
                || breakLineBeforeResponseBox.booleanValue())
                result = result.concat(getEndElementTag(flow));
        result = result.concat(getEndElementTag(flow)
                + getEndElementTag(presentation));

        return result;
    }

    private String getResponseLabels(QuestionType questionType,
            String[] options, String[] shuffle) {
        String result = new String();
        if (questionType.getType().intValue() == QuestionType.LID) {
            for (int i = 0; i < options.length; i++) {
                allOptions.add(new Integer(i + 1).toString());
                result = result.concat(getResponseLabelLID(options[i], i + 1,
                        contains(shuffle, new Integer(i + 1).toString())));
            }
        } else
    ...
    private String getResponseLabelLID(String text, int ident, boolean shuffle) {
        String result = new String("<response_label ident=\"" + ident
                + "\" rshuffle=\"" + RenderChoise.getShuffleString(shuffle)
                + "\">");
    ...
    private String getResponseProcessingInstructions(List rpList,
            String correctFeedbackText, String wrongFeedbackText,
            QuestionType questionType) {
        if (rpList == null || rpList.size() == 0) return "";
        String result = new String(
        ...
        if (correctFeedbackText != null && !correctFeedbackText.equals("")) {
            ...
            feedback = feedback.concat(new String(
                    "<itemfeedback ident = \"Correct\" view = \"All\">\n"
                            + getMattextElement(correctFeedbackText)
                            + getEndElementTag("itemfeedback")));
        }
        if (wrongFeedbackText != null && !wrongFeedbackText.equals("")) {
            if (questionType.getType().intValue() == QuestionType.LID)
                rpList
                        .addAll(getIncorrectResponseProcessingLID(rpList,
                                questionType.getCardinalityType().getType()
                                        .intValue()));
            else
            ...
            feedback = feedback.concat(new String(
                    "<itemfeedback ident = \"Incorrect\" view = \"All\">\n"
                            + getMattextElement(wrongFeedbackText)
                            + getEndElementTag("itemfeedback")));
        }

        for (int i = 0; i < rpList.size(); i++) {
            ResponseProcessing responseProcessing = (ResponseProcessing) rpList
                    .get(i);
            if (responseProcessing.isFenixCorrectResponse())
                result = result.concat(responseProcessing
                        .toXML(displayfeedbackCorrect));
            else
                result = result.concat(responseProcessing
                        .toXML(displayfeedbackIncorrect));
        }
    ...
    private List getIncorrectResponseProcessingLID(List rpList, int cardinality) {
        ...
        for (int i = 0; i < rpList.size(); i++) {
            ResponseProcessing responseProcessing = (ResponseProcessing) rpList
                    .get(i);
            if (responseProcessing.isFenixCorrectResponse()) {
                ResponseProcessing newResponseProcessing = new ResponseProcessing(
                        new ArrayList(), new Double(0), new Integer(
                                ResponseProcessing.SET), null, false);
                if (cardinality == CardinalityType.MULTIPLE)
                    newRpList = getAllMultipleIncorrectResponseProcessingList(
                            newResponseProcessing, responseProcessing
                                    .getResponseConditions());
                else {
                    newResponseProcessing
                            .setResponseConditions(responseProcessing
                                    .getResponseConditions());
    ...
    private List getAllSingleIncorrectResponseProcessingList(
            ResponseProcessing rp) {

        List all = new ArrayList();
        for (int i = 0; i < allOptions.size(); i++) {
            String thisResponse = (String) allOptions.get(i);
            ResponseProcessing newRp = new ResponseProcessing(new ArrayList(),
                    rp.getResponseValue(), rp.getAction(), null, false);
            if (!(thisResponse.equals(((ResponseCondition) rp
                    .getResponseConditions().get(0)).getResponse()))) {
                newRp.getResponseConditions().add(
                        new ResponseCondition(
                                ResponseCondition.VAREQUAL_XML_STRING,
                                thisResponse, "1"));
                all.add(newRp);
            }
        }
    ...
    private List getAllMultipleIncorrectResponseProcessingList(
            ResponseProcessing rp, List correctResponseCondition) {
        List all = new ArrayList();
        all.add(rp);
        for (int i = 0; i < allOptions.size(); i++) {
    ...
    private List addNextIncorrectResponse(List rpList, String value,
            String ident) {
        List newRpList = new ArrayList();
        for (int i = 0; i < rpList.size(); i++) {
            ResponseProcessing rp2Copy = (ResponseProcessing) rpList.get(i);

            ResponseProcessing rp = new ResponseProcessing(new ArrayList(),
                    rp2Copy.getResponseValue(), rp2Copy.getAction(), null,
                    false);
            ResponseProcessing rpNot = new ResponseProcessing(new ArrayList(),
                    rp2Copy.getResponseValue(), rp2Copy.getAction(), null,
                    false);

            for (int j = 0; j < rp2Copy.getResponseConditions().size(); j++) {
                ResponseCondition rc = (ResponseCondition) rp2Copy
                        .getResponseConditions().get(j);
                rp.getResponseConditions().add(rc);
                rpNot.getResponseConditions().add(rc);
            }
            rp.getResponseConditions()
                    .add(
                            new ResponseCondition(
                                    ResponseCondition.VAREQUAL_XML_STRING,
                                    value, ident));
            rpNot.getResponseConditions().add(
                    new ResponseCondition(
                            ResponseCondition.NOTVAREQUAL_XML_STRING, value,
                            ident));
            newRpList.add(rp);
            newRpList.add(rpNot);
        }
    ...
    private ResponseProcessing getIncorrectResponseProcessing(List rpList) {
        ...
        for (int i = 0; i < rpList.size(); i++) {
            ResponseProcessing responseProcessing = (ResponseProcessing) rpList
                    .get(i);
            if (responseProcessing.isFenixCorrectResponse()) {
                newRp = new ResponseProcessing(
                        getIncorrectResponseConditionsList(responseProcessing
                                .getResponseConditions()), new Double(0),
                        new Integer(ResponseProcessing.SET), null, false);
            }
        }
        return newRp;
    }
    ...
    private List getIncorrectResponseConditionsList(List correctRcList) {
        ...
        for (int i = 0; i < correctRcList.size(); i++) {
            ResponseCondition responseCondition = (ResponseCondition) correctRcList
                    .get(i);
            ResponseCondition newRc = new ResponseCondition(
                    ResponseCondition.getConditionString(responseCondition
                            .getReverseCondition()), responseCondition
                            .getResponse(), responseCondition
                            .getResponseLabelId());
            newRcList.add(newRc);
        }
        return newRcList;
    ...
    private String getMattextElement(String text) {
        return new String(getStartElementTag(material)
                + getStartElementTag(mattext) + convertCharacters(text)
