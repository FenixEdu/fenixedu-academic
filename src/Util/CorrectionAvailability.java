...
public class CorrectionAvailability implements Serializable {
	...
	public List getAllAvailabilities() {
		...
		result.add(
			new LabelValueBean(NEVER_STRING, new Integer(NEVER).toString()));
		result.add(
			new LabelValueBean(ALWAYS_STRING, new Integer(ALWAYS).toString()));
		result.add(
			new LabelValueBean(
				AFTER_CLOSING_STRING,
				new Integer(AFTER_CLOSING).toString()));
		return result;
	}
